...
### Deferred (fresh sweep, loop iteration 87 — not yet fixed, ranked by severity)

- **`backends/facts-db/procedures/crud.ts`'s `upsertProcedure` (line ~527) checks for an existing row via `getProcedureById(db, id)` with no scope filter**, even though its caller (`services/procedure-feedback-tool.ts`, wired to the `memory_procedure_feedback` MCP tool) already did a correctly-scoped existence check moments earlier. The tool explicitly instructs agents to invent a human-readable `procedureId` slug when calling with `registerIfMissing: true` — so two different tenants independently choosing a similar slug (e.g. "fix-flaky-test") lets one tenant's call silently overwrite the other's procedure row: `task_pattern`/`recipe_json`/`procedure_type`/`confidence`/`ttl_days`/`scope`/`scope_target` all get overwritten and success/failure counts reset to 0. Live, MCP-tool-reachable cross-tenant write — highest priority in this list.
- `services/procedure-extractor.ts`'s merge-candidate lookup (`factsDb.findProcedureByTaskPattern`, same `procedures/crud.ts` file) runs a global FTS5 match with no scope/vault/agent filter either, unlike sibling `searchProcedures`/`searchProceduresRanked` (`procedures/search.ts`) which both thread a scope filter through. The matched id flows into `recordProcedureSuccess`/`recordProcedureFailure` (`backends/facts-db/procedures/promotion.ts`), also scope-blind — a cross-tenant write, but lower severity since it's only reachable via the offline `extract-procedures` CLI batch job, not a live per-tenant MCP call.
