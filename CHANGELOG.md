...
### Deferred (fresh sweep, loop iterations 87-90 — not yet fixed, ranked by severity)

- `lifecycle/stage-injection.ts:131` and `lifecycle/stage-recall.ts:50` have a timeout-fallback path that can leave an unhandled promise rejection / hang risk — the timeout race's losing branch isn't guarded the way `lifecycle/stage-recall/degraded-recall.ts`'s `buildNarrativePart` already is; sibling blocks in the same file (`buildHotPart`/`runFts`) lack the equivalent try/catch.
