...
### Deferred (fresh sweep, loop iterations 87-91 — not yet fixed, ranked by severity)

- `lifecycle/stage-injection.ts:131`'s injection-stage timeout fallback (`void runInjection(recallResult, api, ctx, event, { skipLlmSummarize: true, emitGate }).then(resolve);`) has the same unguarded-`.then()`-with-no-`.catch()` shape as the now-fixed `stage-recall.ts:50` — if the fallback `runInjection` call rejects, it's an unhandled rejection, and (unlike the recall stage) `resolve` is never called for that race branch, so if the primary `runInjection` also never settles, `runInjectionStage` hangs forever instead of falling back. HIGH confidence, not yet fixed.
- `lifecycle/stage-active-task.ts:77`, `lifecycle/stage-cleanup.ts:409,533,589`, and `lifecycle/stage-goal-subagent.ts:29` call `loadTaskLedgerFromFacts`/`loadTaskLedgerFromFactsWithMetrics` with no `scopeFilter`, unlike sibling call sites in `services/goal-context-injection.ts:119` and `services/active-task-tools-loader.ts:31`, which carry an explicit `// SECURITY:` comment documenting this exact contract. Not currently exploitable — every existing write path uses `scope: "global"` — but an undocumented divergence that becomes an immediate cross-tenant leak the moment any future write path stops being global-only.
- `services/lifecycle/github-adapter.ts:196-202,244` mutates facts with no scope filter across all scopes — lower confidence, may be intentional design for GitHub PR/issue metadata rather than a bug.
- `services/maintenance-job-run/reindex-bridge.ts`'s `importLegacyReindexCheckpoint` unconditionally overwrites a fresh job-run's checkpoint with whatever is in the legacy `.reindex_checkpoint.json` file, with no check that the fresh checkpoint already holds valid/newer state and no fingerprint/total compatibility check (`isOffsetResumeCompatible` exists for this but is never called in production). A `storage re-index` run that aborted partway can have its stale offset silently imported into a *later, non-`--resume`* run, causing `migrateEmbeddings` to skip re-embedding facts `0..N` into the new shadow table — if the remaining fraction still clears `--min-fraction-success`, the incomplete shadow table gets swapped into production with no warning. Contrast with `batch-job-run-bridge.ts`'s `loadBatchJobResume`, which only migrates legacy state after confirming the fresh store has none, and always validates compatibility.
- `cli/commands/manage/register-storage-entities-decay.ts`'s `search` command builds a `ScopeFilter` for `user`/`agent`/`session` scopes but has no `global` branch, so `--scope global` produces an empty filter `{}` — and both `filterByScope()`/`scopeFilterClausePositional()` treat an empty filter as "no restriction," per their own doc comments. `hybrid-mem search "x" --scope global` therefore returns every user's/agent's/session's facts instead of only global-scoped ones. The sibling `scope prune` command in `register-credentials-scope.ts` correctly sets `scopeFilter.global = true` for the same case.
- `services/maintenance-job-run/run-catalog.ts`'s job-run classifiers (`isJobRunSummaryPath`/`isOrchestratorSummaryPath`) hardcode `/`-separator checks (`.includes("/job-runs/")`, a `/^(.+)\/\d{8}\/([^/]+)\.summary\.json$/` regex) against paths built via `node:path`'s `join()`, which uses `\` on Windows — job-run summaries would match neither classifier there, silently dropping them from `maintenance run list/explain/resume`.
- (Minor, cosmetic) `cli/commands/manage/register-storage-entities-decay.ts`'s `entities clean --stopwords` flag is a plain (non-negated) boolean, so `opts.stopwords` is only ever `true`/`undefined`, never `false` — but the handler's stopword-skip check only triggers on `=== false`, making the flag permanently inert (dry-run by default, so limited impact).
- (Minor, dormant) `services/maintenance-job-run/batch-job-run-bridge.ts`'s `finishBatchJobRun` re-derives its own default checkpoint-clear rule instead of delegating to `MaintenanceJobRun.finish()`'s default, and the two disagree for `monitoring`/`success_with_review` outcomes (batch-bridge would discard resumable progress that the base default preserves) — currently dormant since every call site passes an explicit `clearCheckpoint`, but a latent trap for the next one that doesn't.
- (Lower confidence, likely unreachable) `backends/facts-db/procedures/crud.ts`'s `getDuplicateIdByNormalizedHash(db, text)` has no scope filter, unlike scoped hash lookups in `services/dedupe-policy.ts` — no live caller found besides its public `FactsDB` re-export, so may be dead code.
