...
- ~~Lost-update race between the heartbeat/cron `reconcileActiveTaskInProgressSessions` and `active_task_checkpoint` on `ACTIVE-TASKS.md` (plain read-then-write, no optimistic-concurrency check unlike sibling writers).~~ — **`reconcileActiveTaskInProgressSessions`'s half fixed in loop iteration 66** (see below). `services/active-task-checkpoint.ts`'s `syncMarkdownLedgerFromCheckpoint` (lines ~344,372) is a separate call site with the same plain read-then-write shape and remains open.
