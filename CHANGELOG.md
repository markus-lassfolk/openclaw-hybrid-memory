...
### Deferred (fresh sweep, loop iterations 87-88 — not yet fixed, ranked by severity)

- `services/procedure-extractor.ts`'s merge-candidate lookup (`factsDb.findProcedureByTaskPattern`, `backends/facts-db/procedures/crud.ts`) runs a global FTS5 match with no scope/vault/agent filter, unlike sibling `searchProcedures`/`searchProceduresRanked` (`procedures/search.ts`) which both thread a scope filter through. The matched id flows into `recordProcedureSuccess`/`recordProcedureFailure` (`backends/facts-db/procedures/promotion.ts`), also scope-blind — a cross-tenant write, but lower severity since it's only reachable via the offline `extract-procedures` CLI batch job, not a live per-tenant MCP call.
- `setup/register-hooks.ts`'s `before_compaction` hook builds its scope filter directly from `api.context` (the codebase's own docs call this the weakest/last-resort identity source) instead of the canonical `resolveRecallScopeFilter(lifecycleContext)` the sibling `after_compaction` hook (~140 lines below, same file) correctly uses. When `api.context` is unset/stale, the scope filter clause becomes empty and `getHotFacts(4000, {})` returns hot facts across every agent/user/session, injected verbatim into the compaction prompt — a cross-tenant memory leak in multi-agent deployments.
