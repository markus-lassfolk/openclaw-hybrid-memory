...
- Serendipity promotion + idle hand-off (#2119): findings can be promoted into durable work — a goal (goal stewardship then drives it), an active task, or a Skill Workshop proposal — via `serendipity_promote` / `hybrid-mem serendipity promote`. When the Level-4 cron sweep runs with `sweep.dispatch` enabled it auto-promotes the top in-bounds backlog findings (up to `sweep.maxDispatch`, `sweep.target` goal|task), and the pending-review digest gains a Serendipity backlog section.
