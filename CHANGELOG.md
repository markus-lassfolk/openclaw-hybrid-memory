...
- **Hot-reload database reuse under `reuse-databases` policy**: compare parse-time config snapshots instead of bootstrap-mutated runtime `cfg` (e.g. auto-injected empty `llm` tiers), so re-register can reuse handles when plugin config is unchanged.
