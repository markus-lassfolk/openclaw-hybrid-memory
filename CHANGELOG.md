...
### Added
- Serendipity Protocol (#2119): bounded proactive findings with a dedicated `SerendipityStore`, a configurable engagement level (0–4, default 2.5) resolved per user/agent/session/repo/channel, and seven agent tools (`serendipity_record`/`_list`/`_decide`/`_resolve`/`_promote`/`_digest`/`_set_level`). Includes a deterministic decision policy, a TTL-bounded deferred backlog with heartbeat resurfacing and an opt-in Level-4 cron sweep (off by default, surface-only), a compact policy-summary injection, an `openclaw hybrid-mem serendipity …` CLI, and docs at `docs/SERENDIPITY-PROTOCOL.md`. External/destructive/privacy-sensitive actions always require approval; the plugin surfaces findings but never edits code. Disabled by default (`serendipityProtocol.enabled: false`).
- Serendipity promotion + idle hand-off (#2119): findings can be promoted into durable work — a goal (goal stewardship then drives it), an active task, or a Skill Workshop proposal — via `serendipity_promote` / `hybrid-mem serendipity promote`. When the Level-4 cron sweep runs with `sweep.dispatch` enabled it auto-promotes the top in-bounds backlog findings (up to `sweep.maxDispatch`, `sweep.target` goal|task); with `resurface.promote` enabled, heartbeat turns promote the resurfaced finding to a goal too. The pending-review digest gains a Serendipity backlog section.
...
- Bumps `openclaw-hybrid-memory` and the lockstep standalone installer to `2026.8.3`. `package.json` remains the source of truth; derived plugin and installer version metadata are synchronized by `scripts/sync-plugin-version.cjs`.
...
A full code review of this release's diff found several residual edge cases in the fixes above, all closed before merge with regression tests:

- `FactsDB.getEdgesForFactIds()` (#2126) applied its SQL `LIMIT` before the in-scope id filter, so a hub fact's links to facts *outside* the requested set could still exhaust the row budget before genuinely in-scope edges were read — the SQL-level cap is now a generous defensive bound, decoupled from the actual result limit.
- `collectGraphPayload`'s connected-mode neighbor expansion (#2126) could produce edges referencing a node id that was skipped from `nodes` (e.g. a fact deleted mid-request), and didn't filter `expires_at` like every other "active fact" query in the same payload — both fixed, and the same dangling-edge fix applied to `collectGraphRecallPayload`.
- The same connected-mode expansion defaulted `hubDegreeCap` to a hardcoded `50`, 10x stricter than the codebase-wide default of `500` and never configurable via `/api/graph` — now defaults to `500` and threads the operator's configured cap through, matching the sibling `/api/graph/recall` endpoint.
- `graph-link-enrichment`'s orphan scan (#2127) didn't filter `expires_at`, so a bounded `--limit` run could spend its whole budget on already-expired pinned/verified orphans instead of active ones.
- `memory_health`'s `lastReflectionAt` (#2129) discarded the corrupt-value diagnostic that `lastPruneAt` already had, so a corrupt (not merely absent) reflection timestamp silently read as "never" — now reports `lastReflectionInvalidRaw` symmetrically.
- `validate-cron-exit --output-json`'s write (#2133) had no error handling, so an I/O failure silently dropped the already-computed validation result, console output, and telemetry; the cron harness's `HM_VALIDATION_JSON` copy also didn't handle `validate-cron-exit` never producing output at all — both now degrade to a still-valid, parseable fallback instead of failing silently.
- `maintenance-log-analyzer`'s new `orchestration-bootstrap-only-exit` classification (#2131) had no dedicated human-readable `suggestedAction`, falling through to the generic empty-ledger message.
- `verify`'s new stale-install-root check (#2125) could throw uncaught if a same-run dual-install fix had just quarantined the tree the running CLI process loads from, aborting the rest of `verify` — now wrapped in try/catch like the equivalent `doctor` check.
- `upgrade-plugin-manual.sh`'s non-atomic staging-dir swap (#2130) had no rollback if the second step failed, potentially leaving the plugin directory missing and the gateway stopped indefinitely — now rolls back to the previous version and restarts the gateway on failure, and cleans up its own staging/download directories on any exit path (leftover `.staging-$$` dirs no longer accumulate across failed runs).
- `post-upgrade.sh` (#2130) left the gateway stopped indefinitely if `npm install` failed after the gateway was already stopped — now restarts the gateway before exiting on failure.
...
Follow-up QA batch on `claude/memory-hybrid-qa-followup`, restarted from `main` after the previous round merged (#2065) — a full-codebase sweep (not diff-scoped, since the branch had no diff at start) across `cli/`, `services/`, `backends/`, `routes/`, `tools/`, `utils/`, `config/`, `setup/`, `lifecycle/`, `scripts/`, `benchmark/`, split across four parallel review passes:

- **`backends/facts-db/links.ts`, `backends/facts-db/facts-db-layer1.ts`, `tools/memory/register-recall-tools.ts`** (security): the shared `getConnectedFactIds()` BFS primitive — used by `memory_recall`'s legacy (non-`expandGraph`) graph-traversal path, which is the default recall mode — had no scope awareness at all, only excluding superseded facts from each hop's `JOIN facts f`. A hidden (out-of-scope) fact could still serve as a stepping-stone: visible fact A linked to hidden fact B linked to visible fact C traversed straight through B to reach C, and the caller only scope-checked the *final* result ids via `getById()`, never the intermediate hop — so C surfaced in recall results even though no edge on the path was ever visible, indirectly confirming B exists and links to the caller's own fact. This is the same leak class already fixed for the GraphQL `graph`/`relatedFacts` traversal and `memory_graph`'s connected-facts count in earlier rounds, but those were both fixed as one-off JS-side wrapper functions per call site — this round found a second live call site had the same bug, so instead of adding a third wrapper, the fix went into the shared primitive itself: `getConnectedFactIds()` now accepts an optional `scopeFilter` and applies it (via `scopeFilterClauseForAlias`) to the same `JOIN facts f` clause already used to exclude superseded facts, so a hidden fact is excluded from traversal entirely — not just from the final result set. `register-recall-tools.ts`'s legacy graph-traversal branch now threads its already-in-scope `scopeFilter` variable through to this call.
- **`services/per-folder-context.ts`** (security): `regeneratePerFolderContext()`'s SQL query backing CONTEXT.md generation had no scope filter at all, so a user/agent/session-scoped decision or preference fact was written verbatim into `memory/projects/<path>/CONTEXT.md` — a file readable by any other agent/session with folder access. The sibling export services (`wiki-workspace-export.ts`, `public-artifacts-provider.ts`) both apply `globalOnlyScopeFilter()` before writing facts to a file for exactly this reason; this function now does too, via the existing `scopeFilterClausePositional` SQL helper.
- **`cli/commands/manage/register-storage-maintenance.ts`, `cli/commands/manage/register-credentials-scope.ts`** (correctness): six failure branches (`storage re-index`'s shadow-table-creation/aborted-migration/insufficient-migrated/swap-failure paths, and `scope promote`'s `--threshold-days`/`--min-importance` validation) called the raw global `process.exit(1)` directly instead of setting `process.exitCode = 1` and returning. A raw `process.exit()` bypasses `withExit`'s `isStandaloneCliProcess()` guard entirely and terminates the *whole host process* — fatal when these `hybrid-mem` subcommands are invoked in-process (e.g. from the gateway), not just the intended CLI subcommand. All six now set `process.exitCode` and return, matching the established convention used everywhere else in the CLI.
- **`backends/facts-db/fact-read-queries.ts`** (correctness): `getByCategory()` had no `superseded_at`/`expires_at` filter at all, unlike its sibling `listFactsByCategory()` and `getProjectFacts()`. `services/reflection.ts` and `services/procedure-skill-generator.ts` both call it for `pattern`/`rule`/`preference` facts feeding rule-generation and skill-generation logic — a corrected/replaced rule or an expired preference could resurface and keep influencing those pipelines after being superseded or expired. Now filters both, matching its siblings.
- **`backends/facts-db/row-mapper.ts`** (correctness, latent): `rowToMemoryEntry()` mapped `confidence: (row.confidence as number) || 1.0` — the classic JS falsy-zero pitfall every other nullable field in this same mapper already avoids via `??`. `facts.confidence` is `NOT NULL DEFAULT 1.0`, but `StoreFactInput.confidence` is a public, unclamped field at insert time (unlike the `updateConfidence`/`setConfidenceTo` mutation paths, which floor at 0.1), so a caller explicitly storing a fact with `confidence: 0` had that value silently coerced back to full confidence (`1.0`) on every subsequent read. Switched to `??`.

Each fix verified via a stash-based regression proof. tsc clean, biome clean, full test suite passing (9202 passed; the 4 pre-existing failures in `cli-deprecated-aliases.test.ts`, `crystallization-proposer.test.ts`, `implicit-feedback-routing.test.ts`, and `reflection.test.ts` were confirmed present on a clean baseline with none of this round's changes applied, and are unrelated to any file touched in this round).

## [2026.7.208] - 2026-07-10

### Fixed

Follow-up QA batch on `claude/memory-hybrid-qa-followup`, part 2 of the tenth sweep round — a fresh security-focused pass over `routes/` (HTTP/GraphQL/dashboard surface) and `tools/` (the MCP tool-call surface directly exposed to an LLM agent), the two highest-value attack surfaces in the codebase:

- **`routes/graphql-resolvers.ts`** (availability/DoS): `getAllLinks()` loaded the entire `memory_links` table with no SQL-level `LIMIT` at all, used by every top-level link-touching resolver (`link`, `links`, `graph`, `stats`, and the scope-checked `relatedFacts`/`graph` traversal). Since `node:sqlite` calls are synchronous, a single request against a large table could stall the whole GraphQL/dashboard HTTP process for every tenant sharing it, not just the caller who issued the expensive query. Added a bounded `LIMIT` (20,000 rows) and a fast indexed `WHERE id = ?` lookup (`getLinkById`) for the single-link call sites (`link`, `createLink`, `deleteLink`) that previously scanned the whole table just to find one row. Also added a `limit` argument to `relatedFacts` (previously unbounded, unlike its `facts`/`search`/`links`/`entityFacts` siblings), clamped to `[1, 500]` matching `links`' own convention.
- **`tools/graph-tools.ts`** (security): `memory_graph`'s connected-facts count used the raw, unscoped `FactsDB.getConnectedFactIds()` and only filtered the result set by scope *after* the BFS completed — the same "hidden fact as stepping-stone" leak already fixed for the equivalent GraphQL traversal in an earlier round. Visible fact A linked to a hidden (out-of-scope) fact B linked to visible fact C would traverse through B to reach C, and C still surfaced in the connected-facts count even though no edge on that path was ever visible to the caller, indirectly confirming B exists. Migrated to a local `scopedConnectedFactIds()` BFS that scope-checks every traversed edge, mirroring `routes/graphql-resolvers.ts`'s existing fix.
- **`services/goal-registry.ts`** (security): `readGoal()` built its canonical file path via `join(goalsDir, `${id}.json`)` with no validation of the caller-supplied `id` at all — reachable from the `goal_get`/`goal_update`/`goal_complete`/`goal_assess` MCP tools. A `goal_id` containing `../` segments escaped `goalsDir` entirely, giving an arbitrary-file-read primitive: a resolved path that parsed as JSON and satisfied the minimal Goal shape check returned its full contents to the caller, and even a non-matching path still acted as a file-existence oracle via the differing not-found/corrupt error responses. Goal ids are normally UUIDs, but legacy label-derived ids (#1999, e.g. `"legacy-label-0001"`) are also still supported and aren't UUID-shaped, so the fix rejects only the specific characters that could escape `goalsDir` (path separators, `..`, null bytes, empty string) rather than requiring UUID format.
- **`tools/memory/register-directory-tools.ts`** (security): `memory_directory`'s `org_view` operation kept an out-of-scope linked fact's raw id in its response (`{ id, text: "(missing)", category: "?" }`) instead of omitting the entry entirely — an id in the response still discloses that id's existence and its org-linkage to a caller who cannot otherwise see it, contradicting the established convention the sibling `memory_graph` tool already follows (`if (!t) continue`). Now filtered out entirely.
- **`tools/utility-tools.ts`, `backends/facts-db/maintenance.ts`** (security): `memory_checkpoint`'s `saveCheckpoint()`/`restoreCheckpoint()` stored and read the checkpoint fact with no `scope`/`scopeTarget` at all — every checkpoint landed in (and was read back from) a single implicit global slot regardless of who saved it, so any caller's `restore` returned whichever tenant's checkpoint was saved most recently, potentially leaking another tenant's `intent`/`state`/`workingFiles` verbatim. The sibling `active_task_checkpoint` tool already threads a scope filter through for exactly this reason; `memory_checkpoint` now does too, via the same `buildToolScopeFilter`/`scopeFieldsFromFilter` pattern used throughout `tools/`.

One high-severity finding from this sweep was investigated and deliberately **not** auto-fixed: `tools/credential-tools.ts`'s `credential_get`/`credential_list`/`credential_delete`/`credential_store` have no tenant/scope isolation at all, and `backends/credentials-db.ts` has no `scope`/`scope_target` column in its schema — any caller can read, list, or delete any other tenant's stored secret by service name. Unlike every other finding in this sweep, this isn't a missing scope-filter call on an already-scoped resource; the credentials vault is architecturally a single vault per plugin installation (its DB path is derived from the single `resolvedSqlitePath`, with no per-tenant dimension anywhere in the storage layer), consistent with README.md describing it as "an optional credential vault" alongside the durable-memory feature set, not a per-conversation-tenant secret store. Whether this vault is meant to be a single-operator resource (intentional, matching the dashboard's documented "local, no-auth, single-operator" design) or should be tenant-isolated (given the rest of the codebase's extensive `scope`/`scopeTarget` machinery exists specifically to prevent this exact class of cross-tenant leak) is a product/security decision this sweep should not make unilaterally — it would require a schema migration (adding scope columns), a decision on default behavior for existing un-scoped rows, and updating every read/write/list/delete call site. Flagged here for explicit follow-up rather than silently fixed or silently ignored.

Each fix verified via a stash-based regression proof. tsc clean, biome clean (identical rule/file/count distribution before vs after — zero new findings across all 8 changed source files), all affected + wider ripple test suites passing (17 files, 282 tests).
...
Follow-up QA batch (8 fixes) on `claude/memory-hybrid-qa-followup`, from a ninth sweep round — 3 parallel agents covering `cli/commands/manage/`, `cli/verify/`, and `cli/install/`, directories already swept once in earlier batches but revisited for a deeper pass:

- **`cli/install/run-install.ts`** (security): `install --dry-run`'s JSON preview echoed the fully-merged config object verbatim, including any literal secret string a user had pasted directly into `embedding.apiKey`/similar fields (as opposed to a `SecretRef` pointer like `{source:"env",...}`) — a `--dry-run` invocation logged or piped to a file leaked the raw key. The real (non-preview) config write was already correct; only the printed preview leaked. Added a new `redactCredentialStringsOnly()` helper (`services/pending-autopilot/redaction.ts`) that redacts literal string secrets under credential-keyed fields while leaving `SecretRef` pointer objects (which carry no secret material) untouched, and applied it to the dry-run preview only.
- **`cli/commands/manage/register-reflection-pipeline.ts`**: `reflect --window`, `reflect-identity --window`, `classify --limit`, and `backfill --limit` all parsed their flag with raw `Number.parseInt` and never checked `Number.isFinite`. A non-numeric value (e.g. `"abc"`) parses to `NaN`, which for `reflect`/`reflect-identity`/`classify` propagated through downstream clamps to silently match zero rows (reporting `"0 facts"`/`"0 insights"` with exit code 0 instead of rejecting the bad flag) and for `backfill` did the opposite — its cap check is `limit > 0 && stored >= limit`, and `NaN > 0` is `false`, so the intended cap silently never applied and the backfill ran **unbounded** instead of erroring. All four now validate the same way the codebase's other numeric CLI flags do.
- **`cli/commands/manage/register-procedure-lifecycle.ts`**: `procedure list --limit` was typed as `opts.limit?: number`, but Commander always passes raw option strings — `(opts.limit ?? 20) * 3` worked "by accident" via JS implicit string-to-number coercion for a valid numeric string, but a non-numeric `--limit` coerced to `NaN`, which threw inside `listProcedures()` and was silently swallowed, printing `"Procedures (showing 0 of 0)"` with exit code 0. Corrected the type and added the same `Number.isFinite` validation.
- **`cli/commands/manage/register-reflection-pipeline.ts`**: the `entity-mentions audit`/`entity-mentions cleanup` subcommands were registered inside `if (registerReflect) { if (registerIngest) { ... } }`. Every real caller of `registerManageReflectionPipeline()` passes exactly one of `onlyReflect`/`onlyQuality`/`onlyIngest`, which makes `registerReflect` and `registerIngest` mutually exclusive in practice — so the nested condition was never satisfied and both subcommands were unreachable from any real CLI invocation (the automated maintenance orchestrator step calls `factsDb.cleanupEntityMentions()` directly and was unaffected; only the manual troubleshooting commands were dead). Moved into the top-level `if (registerIngest)` block alongside `backfill`/`ingest-files`/`export`, where it's reachable whenever ingest commands are registered.
- **`cli/verify/sections/infrastructure.ts`, `cli/verify/sections/reconcile.ts`**: both `verify --fix` code paths wrote `openclaw.json` via a plain `writeFileSync`, unlike every other config writer in the codebase (`cli/install/`, `cli/proposals.ts`, several `services/*`), which use the shared `atomicWriteFile()` helper (write-to-temp-then-rename). A process crash or kill mid-write during a `verify --fix` run could leave the operator's config file truncated or empty. Both now use `atomicWriteFile()`.
- **`cli/install/workspace.ts`, `cli/install/run-install.ts`, `cli/install/upgrade-config-preflight.ts`**: three `npm install`/`npx` child-process calls in the upgrade path had no `timeout`, unlike the sibling `npm pack`/`tar` calls in `upgrade-config-preflight.ts` which already bound themselves to `NPM_PACK_PREFLIGHT_TIMEOUT_MS`. A stalled npm registry or network partition could hang the install/upgrade CLI indefinitely instead of failing with a clear, actionable error. Added a shared `NPM_INSTALL_TIMEOUT_MS` (5 min) constant and applied it to all three.
- **`cli/commands/manage/maintenance-step-runners.ts`**: the `scope-promote` orchestrator step queried `findSessionFactsForPromotion()` with no `LIMIT` and looped over every candidate synchronously with no orchestrator-wide wall-clock deadline check — the same class of bug already fixed for `repair-vectors` (#2041 review finding) and `implicit-feedback-collapse`. A large session-scope promotion backlog could run past `--max-runtime-min`, blocking every step queued behind it with no graceful stop. Added an optional `limit` parameter to `findSessionFactsForPromotion()` (SQL-level `LIMIT`, mirroring `listVectorlessActiveFacts`'s convention), capped the orchestrator step at 500 candidates, and wrapped the loop in `runUntilMaintenanceDeadline()`. The manual `hybrid-mem credentials scope promote` CLI command (deliberate, operator-invoked, already has its own progress heartbeat) is intentionally left unbounded.

One lower-severity finding from this sweep was investigated and deferred rather than fixed: the `decay-reclassify` orchestrator step's `reclassifyDecayClasses()` query also has no default `LIMIT`, but unlike `scope-promote`/`repair-vectors` its per-row work is a synchronous, in-memory decay-class classification with no network/LLM call — a full-table scan completes in a small fraction of the orchestrator's deadline budget even at large row counts, and wrapping a tight synchronous loop in the async `runUntilMaintenanceDeadline()` would add real per-row overhead for no corresponding safety benefit.

Each fix verified via a stash-based regression proof (new/updated test fails without the fix, passes with it) plus, for the two test-flaw catches during this batch, a second stash-verify after correcting the test itself: the first `entity-mentions` regression test initially registered the pipeline with no `opts` at all, which made `registerReflect`/`registerIngest` both `true` regardless of which code block the commands lived in and so passed even against the buggy nesting — rewritten to call with `{ onlyIngest: true }`, mirroring the one real production call site; and the `install-npm-spawn-timeout` test initially asserted `opts.timeout === NPM_INSTALL_TIMEOUT_MS` re-imported from the same module being stashed, which made the assertion `undefined === undefined` (vacuously true) under a stash-based revert instead of catching the missing option — rewritten to assert against a hardcoded numeric floor. tsc clean, biome clean (identical rule/file/count distribution before and after — zero new findings, only line-number shifts from added code), all affected + wider ripple test suites passing.

## [2026.7.205] - 2026-07-10

### Fixed

Follow-up QA batch (5 fixes) on `claude/memory-hybrid-qa-followup`, from an eighth sweep round — a smaller 3-agent pass covering ground genuinely untouched by any of batches 1-7: `benchmark/offline-qa/` (the offline QA/benchmark harness), `src/`+`contracts/`+`scripts/` (build/publish/dev tooling), and `cli/`'s 58 top-level `cmd-*.ts` files (only its `verify/`, `install/`, and `commands/` subdirectories had been explicitly covered before):

- **`cli/cmd-store.ts`** (security): `hybrid-mem store --supersedes <id>` called `factsDb.supersede(supersedesId, entry.id)` (and deleted its alias records) directly on the caller-supplied fact id with no check that the target belongs to the same scope as the write — `factsDb.supersede()` applies no scope filter itself. Any CLI caller who knew (or guessed) another tenant's fact id could pass it as `--supersedes` to silently soft-delete (hide from recall) a fact outside their own scope. The equivalent MCP tool path (`tools/memory/register-store-tools.ts`'s `memory_store` `supersedes` param) was already guarded; the CLI path now mirrors it — validates the target's scope with `matchesExactScope()` before superseding, and warns instead of applying the operation when it doesn't match.
- **`scripts/split-register-plugin.mjs`, `split-large-files.mjs`, `split-remaining.mjs`, `split-storage-stats.mjs`**: these one-shot historical migration scripts (each already run in its own past commit, confirmed unreferenced by any `package.json` script, CI workflow, or doc) unconditionally sliced their source files at hardcoded line offsets and overwrote the split targets — but every source file is now a thin post-split re-export shim, far shorter than the offsets assume. Re-running any of them today would silently truncate real production source (e.g. `setup/register-plugin.ts`, `backends/vector-db/vector-db-class.ts`) to near-empty stubs with no error, prompt, or backup. Each script now validates its source file(s) are still large enough for a pre-split monolith before writing anything, and refuses with a clear message otherwise.
- **`scripts/lint-arch.sh`**: the CI-enforced ".test.ts not .spec.ts" naming check (and the softer console.* check) scanned a hardcoded directory allowlist that had drifted stale — missing `benchmark/`, `contracts/`, `scripts/`, `skills/`, `src/`, and `workspace-snippets/`. A `.spec.ts` file added under any of those directories violated the convention but the check never saw it, printed "OK", and exited 0 — letting a real CI-gated violation ship silently. Both checks now scan the whole tree (minus `node_modules`/`.git`) instead of an allowlist that will otherwise keep going stale.
- **`scripts/markitdown-worker.py`**: the persistent JSON-RPC worker's `main()` only validated that each stdin line was parseable JSON, not that it was a JSON *object* — `handle()`'s `request.get("id")` then crashed with `AttributeError` on any bare number/string/array/null input, killing the whole long-lived worker process (driven by `services/python-bridge.ts`) and silently dropping every subsequent queued request. Now returns a scoped JSON-RPC `-32600 Invalid Request` error and keeps serving.
- **`benchmark/offline-qa/analyze-quality.ts`**: `sqlQuery()` treated "the `sqlite3` CLI binary couldn't even be launched" (`spawnSync`'s `error` field, typically `ENOENT`) the same as "the query ran and matched zero rows," silently returning `[]` either way with no warning. This disabled every DB-based privacy/quality check that reads through it — including the private-path/email leak scan — so a report could claim a task was "good" purely because the leak check never ran, not because no leak existed. Now throws a clear, actionable error when the binary is genuinely unavailable, while a real zero-row query result still degrades to `[]` as before.

Two candidates from the `benchmark/offline-qa/` sweep were investigated and deferred rather than fixed, since both need a QA-harness state-machine redesign (not a bounded fix) and are dev-only tooling never reachable by end users or agents: `run-maintenance-qa.ts`'s `--redo <task>` wipes and re-clones the sandbox (per its own documented behavior) while `qa-state.json` keeps marking earlier tasks "passed" against data that no longer exists, so the regenerated report can claim success for evidence it just deleted; and `analyze.ts`'s provider-leak classification can be silently reclassified from `needs-fix` to `data-gap` (clearing the leak error message) when an unrelated embedding failure co-occurs on the same task, so the report's "Provider guard" section and its "Go/no-go" verdict can contradict each other for the same run.

Each TypeScript/Python fix verified via a stash-based regression proof (new test fails without the fix, passes with it, including a live before/after repro for the two non-TypeScript scripts); the four `split-*.mjs` guards verified by running each script directly against the current repo state and confirming it now refuses instead of writing. tsc clean, biome clean (zero new findings), all affected + wider ripple test suites passing.

## [2026.7.204] - 2026-07-10

### Fixed

Follow-up QA batch (10 fixes) on `claude/memory-hybrid-qa-followup`, from a seventh sweep round — a second pass over directories that had each had only one prior QA pass across batches 1-6 (`api/`, `routes/`, `types/`, `config/`, `setup/`, `tools/`, `utils/`):

- **`routes/graphql-resolvers.ts`** (security): `Query.relatedFacts`'s default (no `linkTypes` filter) traversal branch called `FactsDB.getConnectedFactIds()` — an unscoped multi-hop BFS — while the sibling `linkTypes` branch had already been hardened to a scope-checked BFS (every traversed edge must be `isLinkVisible`). A visible fact linked through another tenant's hidden fact to a third, otherwise-visible fact surfaced that third fact, revealing that a path exists through private data that should never have been traversable. Both branches now share one `scopedConnectedFactIds()` helper that enforces the per-edge visibility check regardless of whether `linkTypes` is given.
- **`routes/graphql-resolvers.ts`**: `Query.graph` silently ignored the `linkTypes`/`maxDepth`/`startNodeIds` fields `GraphFilterInput` declares in the schema, always returning the same top-N-by-recency node set instead of a seed-rooted subgraph. Now implemented via the same `scopedConnectedFactIds()` helper, so a `startNodeIds`-rooted traversal gets the identical scope guarantee as `relatedFacts`.
- **`routes/graphql-resolvers.ts`**: `Fact.links`/`Fact.linkedFrom` loaded the entire `memory_links` table via `getAllLinks()` on every parent fact resolved, an O(N × table-size) scan for a `facts { links { ... } } }` query over N facts, unlike every other query in the file which bounds its rows. Now query with a `WHERE source_fact_id = ?` / `WHERE target_fact_id = ?` clause scoped to the one parent fact.
- **`utils/scope-filter.ts`** (security): `readGatewayIdentity()`'s `sessionId` derivation fell through to `sessionKey` only via `??`, missing the `|| null` normalization applied to `userId`/`agentId` on the lines above it — an empty-string `sessionId` (falsy but not `null`) short-circuited the fallback instead of reaching `sessionKey`, silently widening a session-scoped gateway RPC/corpus read to the caller's entire user-level history.
- **`utils/yaml-parser.ts`**: `parseSeqMapItem()` hardcoded a sequence item's sibling-key indent to `seqIndent + 2`, assuming exactly one space after `- `. A hand-aligned sequence item with more than one space after the dash (e.g. `-   name: foo`) computed the wrong expected indent for its sibling keys, silently dropping every key past the first. Now derives the real column the first key starts at and uses that for both the nested-value base indent and the sibling-key check.
- **`utils/dates.ts`**: `parseTimestamp`'s `isoDateOnly` branch, `parseSourceDate`'s ISO branch, and their shared `parseCompactDateDigits()` helper built a UTC timestamp via `Date.UTC(year, month-1, day)` with no validation that month/day were in range — `Date.UTC` silently normalizes an out-of-range component (e.g. `2026-02-30` rolls into March, `2026-00-10` rolls back into the *previous year*) instead of erroring. Both functions are fed LLM tool-call input (`memory_recall`'s `asOf` param) and CLI flags (`--source-date`), so a malformed-but-date-shaped string was silently accepted as a different, valid-looking date. Now validated the same way `date-detector.ts` already guards its own date parsing.
- **`config/index.ts`**: `hasKey()` — used by `getDefaultCronModelLegacy`/`getDefaultPreferredModelList`/`getProvidersWithKeys`, none of which resolve `distill.apiKey`/`claude.apiKey`'s `env:`/`file:` SecretRef prefix before calling it — applied a `>= 10` char floor to the raw, *unresolved* SecretRef string. A short reference like `"env:GK"` (a 2-char env var name) is well under that floor even though it resolves to a full-length key at runtime, so `getProvidersWithKeys()` (the `verify` CLI's provider-configuration report) silently omitted that provider, and the cron-model tier fallbacks skipped straight to the hard-coded OpenAI default instead of the operator's configured provider.
- **`setup/register-plugin.ts`**: `variantQueue` was unconditionally rebuilt on every `register()` call instead of being carried over from `donorRuntime`, unlike every sibling optional service (`eventBus`, `pythonBridge`, `learningsDb`, `auditStore`, `agentHealthStore`). `VariantGenerationQueue` has no dispose/drain/shutdown method, so any hot-reload — even one triggered by an unrelated config change on the `reuse-databases` policy — silently discarded whatever contextual-variant-generation work was still queued on the old instance.
- **`tools/issue-tools.ts`**: `memory_issue_list`'s `limit` had no validation, and `IssueStore.list()` only appends a SQL `LIMIT` clause when `limit > 0` — omitting the param (the documented "default: 50" path) or passing `0`/negative returned the entire issues table unbounded. Now clamped, with a non-positive value treated the same as "not provided" rather than clamped up to a near-useless `1`.
- **`tools/public-api-routes.ts`** (security): the `/fact/mutate` HTTP `create` action skipped `isValidCategory()` validation entirely — unlike the equivalent `memory_store`/`memory_edit` tool paths — and its own fallback default (`"general"`) isn't even a valid category itself (the real default is `"other"`), so an unvalidated or omitted category silently produced a fact invisible to every category-filtered query. Now validated with the same `isValidCategory()` gate and `"other"` default used elsewhere.

One candidate from the same sweep round was investigated and deferred rather than fixed: `tools/memory/register-directory-tools.ts`'s `memory_directory` (`list_contacts`/`org_view`) exposes contact rows with no scope filtering, and the `contacts`/`organizations` tables have no `scope`/`scope_target` columns at all — a private, user-scoped fact's `PERSON` mention still creates a globally-visible contact row (only the *enrichment* fields were gated to global-scope facts by the earlier `#2067` fix, not the row's existence). Fixing this properly needs a schema migration plus a design decision on contact-scope-assignment semantics (what scope a contact gets when enriched from facts across multiple different scopes over time), not a bounded fix.

Each fix verified via a stash-based regression proof (new test fails without the fix, passes with it), tsc clean, biome clean (zero findings on every changed file), and passing related + wider ripple test suites (reregister-policy, register-plugin failed-registration-cleanup, register-plugin reload-teardown-drain, issue-tools-scope-security, comprehensive e2e) with zero regressions.
...
Follow-up QA batch (10 fixes) on `claude/memory-hybrid-qa-followup`, from a fifth flat sweep round (`services/` subdirectories, `cli/verify`+`cli/install`+`cli/commands`, a deeper `backends/` second pass, `lifecycle/`+`types/`):

- **`cli/commands/manage/register-reflection-pipeline.ts`** (security): `find-duplicates`/`consolidate --threshold`/`--limit` had zero validation. A malformed `--threshold` (negative or >1) makes almost every fact pair register as "similar," and `consolidate` merges each resulting cluster via LLM and **deletes the source facts** with no confirmation flag; a negative `--limit` also flowed into a raw SQL `LIMIT` clause, and SQLite treats a negative `LIMIT` as unbounded. Both flags are now validated the same way the sibling `reflect-meta --collapse-implicit-feedback --threshold` and `register-storage-graph-audit.ts` commands already do.
- **`cli/commands/manage/register-self-correction-feedback.ts`** and **`cli/commands/manage/register-backfill-maintenance.ts`**: 12 `--days` flag sites across `self-correction-extract`/`self-correction-run`/`extract-implicit`/`cost-report`/`analyze-feedback-phrases`/`workflow-trace-quality`/`backfill-*`/`maintenance-coverage` did `Number.parseInt(opts.days, 10)` with no validation — a non-numeric or negative value flowed into a `cutoff = Date.now() - days * 86400000` computation whose `mtimeMs > cutoff` comparison is always false for NaN/a future cutoff, silently scanning zero session files and reporting success (exit code 0) instead of rejecting the flag. All 12 now share the same `parseDaysFlag()` helper already used by `cli/distill.ts` (now exported for reuse).
- **`backends/facts-db/procedures/promotion.ts`** and **`services/procedure-skill-generator.ts`** (security): `getProceduresReadyForSkill()` and `generateAutoSkillForProcedure()`'s procedure lookup had no scope gate, unlike every other procedure accessor in the module (`getProcedureById`, `findProcedureByTaskPattern`, `recordProcedureSuccess`/`Failure`, all of which take an explicit `scopeFilter`). Auto skill-generation writes a `SKILL.md` into the shared, globally-installed skills directory, so a user/agent/session-scoped procedure (e.g. via `memory_procedure_feedback`'s `registerIfMissing`) could be drafted into a skill visible to every tenant. Both now restrict to `scope = 'global'` procedures only.
- **`lifecycle/session-state.ts`**: `clearSessionState()` — which runs at the end of *every* agent turn, not once per chat session — was still deleting `sessionStartSeen`, `ambientSeenFactsMap`, `ambientLastEmbeddingMap`, and `authFailureRecallsThisSession` every turn, the same bug class already fixed for `frustrationStateMap`/`changeNotifyStateMap`/etc. This made the one-time `autoRecall.retrievalDirectives.sessionStart` directive re-fire every turn instead of once per session, defeated `SessionSeenFacts`' cross-turn ambient-fact dedup after a topic shift, and reset the `autoRecall.authFailure.maxRecallsPerTarget` cap every turn so the same credential hint kept re-injecting. Now excluded from the per-turn clear, matching the established pattern; `pruneSessionMaps`' existing recency-based eviction already bounds all four.
- **`services/embeddings/onnx-provider.ts`** (security): the ONNX model cache path sanitized `repo` before joining it into the cache directory, but not `revision` (both derived from the operator-supplied `embedding.model` config string, split at the last `@`) — a value like `"custom-model@../../../../tmp/evil"` resolved the cache directory outside the intended cache root, so the downloaded model/vocab files would be written there. `revision` is now sanitized the same way `repo` is.
- **`services/transcript-importers/index.ts`** (security): `redactSecretsInText()` (the only redaction pass applied before raw ChatGPT/Claude.ai/Slack transcript content is stored as `facts.text`) only matched plain `sk-<alnum>` keys, missing the `sk-ant-`/`sk-proj-` prefixed formats and any keyword-based (`password=`/`secret=`/`token=`) or private-key-block secrets. Pattern set now kept in sync with `services/error-reporter/sanitize.ts`'s more complete `scrubString`.

Two candidates from the same sweep round were investigated and deferred rather than fixed, since both require a genuine design decision rather than a bounded bug fix: `backends/facts-db/maintenance.ts`'s `trimToBudget()` apply-mode path bypasses the superseded-texts cache invalidation contract documented on `FactsDB.supersede()` (currently dormant — its only caller always passes `simulate: true`); and `lifecycle/stage-change-notify.ts`'s change-feed notification cursor can advance past notifiable events that were dropped by `trimEventsToBudget()` for token-budget reasons rather than muted, permanently losing them instead of deferring to a later turn.

Each fix verified via a stash-based regression proof (new test fails without the fix, passes with it), tsc clean, biome clean (zero new findings beyond pre-existing baseline via structured JSON diagnostic diff, using a path-preserving baseline mirror after an earlier flattened-basename comparison was found to produce false positives from directory-scoped biome config overrides), and passing related test suites with zero regressions.

## [2026.7.201] - 2026-07-10

### Fixed

Follow-up QA batch (7 fixes) on `claude/memory-hybrid-qa-followup`, from a fourth flat sweep round (`tools/`+`api/`, `setup/`+`config/`, `backends/facts-db/`, `routes/`+GraphQL) — the first sweep since the 10 open security-fix PRs were consolidated into this branch, with extra scrutiny on that freshly-merged code:

- **`backends/facts-db/entity-layer.ts`** (security): `replaceFactEntityMentions()`'s person↔org co-mention heuristic set `contacts.primary_org_id` unconditionally, bypassing the scope gate just added in the #2067 merge (`applyContactProfileEnrichmentForFact` now only enriches from global-scope facts). A private/scoped note mentioning both a person and an org (e.g. a session-scoped note about a therapist and a clinic) still created a globally-visible person→org affiliation link, re-leaking the scoped fact's content via the unscoped `memory_directory` tool. Now gated to global-scope facts only, matching the sibling fix.
- **`backends/facts-db/entity-layer.ts`**: `upsertContact()`'s exact-normalized-key-match branch wrote `display_name` unconditionally on every call, ignoring the `CONTACT_SOURCE_PRIORITY` arbitration (`manual > agent/import > ner`) already enforced for every other profile field. A manually curated or roster-imported contact name could be silently overwritten by the next lower-priority NER mention's surface-text variant (e.g. a lowercase/nickname form). Now routes through the same priority check.
- **`tools/memory/register-episode-tools.ts`**: `memory_search_episodes`'s `limit` clamp was `Math.min(limit, 200)` — upper-bound only. A negative `limit` passed straight into `episodes.ts`'s raw SQL `LIMIT` clause, and SQLite treats a negative `LIMIT` as "no limit," returning every matching row instead of the documented 200-row cap. Now clamped to `[1, 200]`, matching the fix already applied to `fact-mutation-gateway.ts`.
- **`setup/reregister-policy.ts`** (security): `canReuseDatabasesOnReregister()` compared embedding/LLM/credentials/health config but never compared the flags that gate `bootstrap-optional.ts`'s conditionally-constructed stores (`wal`, `personaProposals`, `identityReflection`, `identityPromotion`, `nightlyCycle`, `graph.autoSupersede`, `passiveObserver`, `aliases`, `verification`, `provenance`). Under `OPENCLAW_HYBRID_MEM_REREGISTER_POLICY=reuse-databases`, flipping one of these on/off in config had no effect on a hot reload — the donor's store handle (often still `null`) was carried over unchanged, so the feature silently stayed off (or, for `wal.enabled`, writes silently kept bypassing WAL) until some unrelated field forced a full teardown. Now every such flag is compared, forcing a full teardown whenever any of them differ.
- **`routes/dashboard/collectors.ts`**: `collectStatus()`'s `Promise.all` included `collectMemoryStats()` unwrapped, unlike its four sibling collectors (`collectCronJobs`/`collectTaskQueue`/`collectForgeState`/`collectGitActivity`), each of which already degrades to a safe empty value on error. A synchronous throw from `ctx.factsDb.count()`/`countExpired()` (e.g. `SQLITE_BUSY` while a maintenance job holds a lock) rejected the whole `Promise.all`, and `/api/status` returned a bare `{error:"InternalServerError"}`, discarding the other collectors' already-succeeded results. Now wrapped with the same degrade-to-empty pattern.
- **`routes/dashboard/collectors.ts`** (security): `performFactAction()`'s `verify` action cast `body.verifiedBy` — untrusted client JSON from `POST /api/viewer/facts/:id/verify` — straight into `VerificationStore`'s trusted `"agent"|"user"|"system"` enum with a bare `as` assertion and no runtime check, allowing an arbitrary string to be persisted verbatim into the `verified_facts.verified_by` provenance column. Now validated against the literal set, falling back to `"agent"` for anything else.
- **`routes/graphql-resolvers.ts`**: the `search` query's `SearchInput.decayClasses` and `SearchInput.scopeTarget` fields were declared in the GraphQL schema and accepted from clients but never read by `searchFacts()`, so a caller narrowing a search to a specific decay class or scope target silently got back unfiltered results instead. Now both are applied as additional filters, mirroring the handling already present for the fields `search` and `graph` do support.

One candidate from the same sweep round was investigated and deferred rather than fixed: `routes/graphql-resolvers.ts`'s `graph` query declares `GraphFilterInput.linkTypes`/`maxDepth`/`startNodeIds` in its schema but the resolver never reads them — a caller expecting a subgraph BFS rooted at `startNodeIds` instead gets the same unfiltered global top-`maxNodes` graph as an unfiltered call. The `relatedFacts` resolver already implements the equivalent scoped BFS, so the fix is reachable, but wiring it into `graph` (root-set BFS bounded by `maxDepth`/`linkTypes`, combined with the existing category/decayClass/scope filters) is a feature-completion change rather than a bounded regression fix, so it's deferred pending an explicit decision on whether to implement the traversal or drop the unused schema fields.

Each fix verified via a stash-based regression proof (new test fails without the fix, passes with it), tsc clean, biome clean (zero new findings beyond pre-existing baseline via structured JSON diagnostic diff on every touched file), and passing related test suites with zero regressions.
