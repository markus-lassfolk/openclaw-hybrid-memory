...
- **Directive extraction quality gate:** `extract-directives` now rejects untrusted metadata envelopes (`Conversation info (untrusted metadata)`, `Sender (untrusted metadata)`), JSON/chat-fragment candidates, and one-off issue/PR commands before storing durable rules. If any candidates are rejected, the run is marked partial and the directive scan watermark is not advanced.
...
- **Forced QA reruns:** when you manually bypass wrapper guard checks (`HYBRID_MEM_CLI_JOB_GUARD_WINDOW_MS=0` or `QA_FORCE=1` / `HYBRID_MEM_QA_FORCE=1`), guarded scan-style commands automatically receive `--force` (same effect as legacy `--full`) so their internal 23h scan cooldown and incremental watermarks are bypassed. Manual reruns: `openclaw hybrid-mem <cmd> --force` or `run-all --force`.
