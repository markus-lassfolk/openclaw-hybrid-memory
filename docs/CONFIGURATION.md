...
| `contacts.importPath` | `null` | Default file for `openclaw hybrid-mem contacts sync` (falls back to `./workspace/CONTACTS.md`). |
...
CLI: `contacts import --from …`, `contacts sync`, `contacts merge <canonical> <duplicate>`. See [GRAPH-MEMORY.md § Structured contact profiles](GRAPH-MEMORY.md#structured-contact-profiles-2014).
