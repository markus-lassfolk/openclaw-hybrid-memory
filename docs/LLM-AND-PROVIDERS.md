...
**Maintenance fallback policy (`llm.maintenanceFallbackPolicy`, Issue #1226):** When maintenance work uses the same ordered list as `llm.default` (because `llm.maintenance` is unset), the plugin used to retry through every default-tier model — including expensive ones (`o3`, `gpt-5.4`, `gpt-4o`, large MiniMax, etc.) if the primary cheap model failed. **By default** (`cheap-only`, applied when the key is omitted), automatic maintenance fallbacks **strip** those expensive IDs unless you list them explicitly in `llm.maintenance`. Set `llm.maintenance` to a full ordered chain (e.g. `["azure-foundry/gpt-4.1-mini", "azure-foundry/gpt-4.1-nano", "openai/gpt-4o-mini"]`) for a cheap-safe failover path. Use `explicit-only` when `llm.maintenance` is the **complete** chain and you do not want `llm.fallbackModel` / `distill.fallbackModels` merged in. Use `default` or `allow-heavy` to restore the previous “try the whole default list + global fallbacks” behaviour. `openclaw hybrid-mem -v config` and verbose distill print the effective policy and fallback chain.
