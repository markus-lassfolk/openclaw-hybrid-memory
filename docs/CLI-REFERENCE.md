...
| `enrich-entities [--limit N] [--all] [--dry-run] [--model M]` | Backfill **PERSON**/**ORG** extraction for facts that have no stored entity-mention rows yet (same franc + LLM pipeline as store-time enrichment when `graph.enabled`). Queue priority is `hot → warm → structural → cold`, then recent access, recall/access counts, importance, and created-at. Use `--limit` for bounded catch-up batches or `--all` for one-shot exhaustive backfill. |
