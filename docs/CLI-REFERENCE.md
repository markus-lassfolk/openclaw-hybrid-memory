...
| `contacts import --from <path> [--dry-run] [--no-part-of] [--json]` | Import a markdown **CONTACTS.md** roster into `organizations`, `contacts`, roster facts, and `org_fact_links` (#2014). |
| `contacts sync [--from <path>] [--force] [--dry-run] [--json]` | Re-import when the file mtime changed (tracks state in `maintenance_state`). |
| `contacts merge <canonical> <duplicate> [--json]` | Merge duplicate contact rows; reassign NER mentions and `entity_contact_id` FKs. |
