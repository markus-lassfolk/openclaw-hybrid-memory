...
| `enrich-entities [--limit N] [--all] [--dry-run] [--model M] [--adaptive-catch-up] [--batch-size N] [--batch-delay-ms N]` | Backfill **PERSON**/**ORG** extraction for facts that have no stored entity-mention rows yet (same franc + LLM pipeline as store-time enrichment when `graph.enabled`). Queue priority is `hot → warm → structural → cold`, then recent access, recall/access counts, importance, and created-at. Use `--limit` for bounded catch-up batches or `--all` for one-shot exhaustive backfill. Add `--adaptive-catch-up` to automatically ramp throughput after consecutive successful batches and back off on pressure/rate-limit signals (429/retry hints/transient failures), starting from the provided `--batch-size`/`--batch-delay-ms` baseline. |
