...
3. **Tactical tasks** live in **`ACTIVE-TASK.md`** (see **`activeTask.*`** in config). Optional **task hygiene** on heartbeat — stale nudges, optional long-running hints, and tool **`active_task_propose_goal`** — is documented in **[TASK-HYGIENE.md](TASK-HYGIENE.md)**.
4. Optional toggles:
   ...
   - **`heartbeatRefreshActiveTask`** — on each heartbeat match, rewrite **`ACTIVE-TASK.md`** with an **## Active Goals** mirror section (requires **`activeTask.enabled`**). Do not edit that section by hand.
...
For promoting work from **`ACTIVE-TASK.md`** into a registered goal, use **`active_task_propose_goal`** (draft fields for **`goal_register`**; requires **`activeTask.enabled`**). Details: **[TASK-HYGIENE.md](TASK-HYGIENE.md)**.
