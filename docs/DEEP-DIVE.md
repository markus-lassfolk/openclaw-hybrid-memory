...
The vector is generated by sending the fact text to the configured embedding provider (default: OpenAI `text-embedding-3-small`, 1536 dimensions). Alternative providers — Ollama (local), ONNX (local), or Google (Gemini API) — are also supported; see [LLM-AND-PROVIDERS.md](LLM-AND-PROVIDERS.md#embedding-providers). LanceDB uses this vector for **approximate nearest neighbor** search — finding facts that are semantically similar to a query even when the exact words don't match.
