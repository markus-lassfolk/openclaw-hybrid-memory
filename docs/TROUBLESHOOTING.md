...
| "duplicate plugin id detected" / "plugin entry path escapes plugin root" on extensions-canonical hosts (gateway loads `~/.openclaw/extensions/…` but a copy also sits in `~/.openclaw/npm/projects/openclaw-hybrid-memory`) | `openclaw plugins install` installed into `npm/projects/` (not the extensions tree the gateway loads), leaving a duplicate | Run `openclaw hybrid-mem verify --fix` — it removes the redundant `npm/projects` tree and reconciles the install-index sidecar. **Do not** run `openclaw plugins install` here (it recreates the duplicate); use `openclaw hybrid-mem upgrade` for upgrades. See [UPGRADE-PLUGIN.md](UPGRADE-PLUGIN.md#extensions-canonical-hosts-maevedoris-layout). |
| "duplicate plugin id detected; global plugin will be overridden by … `npm/projects/…`" where the **managed npm copy is newer** and a stale `~/.openclaw/extensions/openclaw-hybrid-memory` copy remains (#2117) | A managed npm install (`openclaw plugins install`) is canonical, but an older legacy extensions copy was left behind | Run `openclaw hybrid-mem doctor` to see both paths, both versions, and the winning source, then `openclaw hybrid-mem doctor --fix` (or `openclaw hybrid-mem verify --fix`) — it quarantines the stale extensions copy to `~/.openclaw/.cache/openclaw-hybrid-memory.removed-duplicate-<ts>`. Restart the gateway afterward. To remove it by hand: `mv ~/.openclaw/extensions/openclaw-hybrid-memory ~/.openclaw/.cache/openclaw-hybrid-memory.removed-duplicate-$(date -u +%Y%m%dT%H%M%SZ)`. |
| Fresh install fails to load with `Cannot find module 'apache-arrow'` (from `@lancedb/lancedb/dist/arrow.js`) (#2116) | Older plugin versions (≤ `2026.7.217`) did not declare `apache-arrow`, a peer dependency of `@lancedb/lancedb`, so npm never installed it | Upgrade to `2026.7.218` or later (which declares `apache-arrow` directly). As an immediate fix on an affected install: `cd ~/.openclaw/extensions/openclaw-hybrid-memory && npm install apache-arrow@18.1.0 --no-audit --no-fund`, then restart the gateway. |
| `plugins inspect` shows a different version than `openclaw hybrid-mem version` (#2115) | On plugin versions ≤ `2026.7.217`, `openclaw.plugin.json.version` (read by `plugins inspect`) could drift from `package.json.version` (read by the CLI/gateway) | Upgrade to `2026.7.218` or later, where the two are synced at publish time and a CI gate prevents drift. The gateway-registration log line (`memory-hybrid: registered (v…​)`) reflects the truly-loaded version. |
