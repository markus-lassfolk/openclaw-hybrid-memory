...
const OUTCOME_PATTERNS: Array<{
	...
	},
	{
		...
		outcome: "success",
		eventTemplate: (_p, ctx) =>
			`Deployment succeeded: ${ctx.slice(0, 120).trim()}`,
	},
	{
	...
	},
	{
		...
		outcome: "failure",
		eventTemplate: (_p, ctx) =>
			`Error encountered: ${ctx.slice(0, 120).trim()}`,
	},
	// Partial patterns
	{
 ...
 */
async function extractTextFromSessionFile(
	sessionFile: string,
): Promise<string> {
	try {
...
async function autoCaptureEpisodes(
	...
	for (const { pattern, outcome, eventTemplate } of OUTCOME_PATTERNS) {
		...
		try {
			const contextSnippet = text.slice(
				Math.max(0, text.indexOf(phrase) - 20),
				text.indexOf(phrase) + 100,
			);
			...
			created++;
			logger.debug?.(
				`memory-hybrid: auto-captured episode [${outcome}]: ${event.slice(0, 80)}`,
			);
		} catch (err) {
...
export class HybridMemoryContextEngine implements MinimalContextEngine {
	...
	async assemble(params: {
		...
		try {
			...
			logger.warn?.(
				`memory-hybrid: context-engine assemble failed (non-fatal): ${err}`,
			);
			return { messages: params.messages, estimatedTokens: 0 };
		}
	}
	...
	async compact(_params: {
		...
		try {
			...
			const { committed: walCommitted, skipped: walSkipped } =
			...
			try {
				...
				if (episodesCreated > 0) {
					logger.info?.(
						`memory-hybrid: auto-captured ${episodesCreated} episode(s) from session compaction`,
					);
				}
			} catch (err) {
				logger.warn?.(
					`memory-hybrid: episode auto-capture failed (non-fatal): ${err}`,
				);
			}
	...
	async prepareSubagentSpawn(params: {
		...
		try {
			...
			if (topFacts.length === 0) {
				logger.debug?.(
					`memory-hybrid: prepareSubagentSpawn — no facts to inject for child=${params.childSessionKey}`,
				);
			...
			logger.warn?.(
				`memory-hybrid: prepareSubagentSpawn failed (non-fatal): ${err}`,
			);
			return undefined;
		}
	}
	...
	async onSubagentEnded(params: {
		...
		try {
			...
			logger.warn?.(
				`memory-hybrid: onSubagentEnded failed (non-fatal): ${err}`,
			);
		}
	}
 ...
 */
export async function registerHybridContextEngine(
	...
	try {
		...
		if (!sdk) {
			opts.logger.debug?.(
				"memory-hybrid: openclaw/plugin-sdk not available; skipping ContextEngine registration",
			);
			return false;
		}
		...
		opts.logger.info?.(
			"memory-hybrid: ContextEngine registered (id=hybrid-memory)",
		);
		return true;
	} catch (err) {
		capturePluginError(err instanceof Error ? err : new Error(String(err)), {
		...
		opts.logger.warn?.(
			`memory-hybrid: ContextEngine registration failed (non-fatal): ${err}`,
		);
		return false;
	}
}
