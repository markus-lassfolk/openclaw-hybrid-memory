...
export function mergeResults(
	...
	for (const r of byId.values()) {
		...
		let rrfScore = 0;
		const sqliteRank =
			sqliteRankById.get(r.entry.id) ?? sqliteRankByText.get(norm);
		const lanceRank =
			lanceRankById.get(r.entry.id) ?? lanceRankByText.get(norm);
	...
	withRrf.sort((a, b) => {
		...
		return (
			(a.r.backend === "sqlite" ? 0 : 1) - (b.r.backend === "sqlite" ? 0 : 1)
		);
	});
	...
	return withRrf
		.slice(0, limit)
		.map(({ r, rrfScore }) => ({ ...r, score: rrfScore }));
}
