...
export class MonthlyReviewService {
	...
	async runReview(): Promise<MonthlyReviewReport> {
		...
		const totalFacts = Object.values(factsByCategory).reduce(
			(sum, n) => sum + n,
			0,
		);
		...
		const avgRow = rawDb
			.prepare(
				"SELECT AVG(confidence) as avg FROM facts WHERE superseded_at IS NULL",
			)
		...
		const averageConfidencePreviousMonth = normalizeAverage(
			prevAvgRow?.avg ?? 0,
		);
...
export async function runMonthlyReview(opts: {
	...
	const service = new MonthlyReviewService(
		opts.factsDb,
		opts.openai,
		opts.model,
	);
