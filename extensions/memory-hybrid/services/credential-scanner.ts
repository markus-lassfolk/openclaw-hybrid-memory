...
const TOOL_CALL_CREDENTIAL_PATTERNS: Array<{
	...
	extract: (
		...
		fullText: string,
	) => ToolCallCredential | null;
}> = [
	// sshpass -p <password> ssh [options] <user>@<host>
	{
		regex:
			/sshpass[ \t]+-p[ \t]+(\S+)[ \t]+ssh(?:[ \t]+\S+)*[ \t]+([\w.-]+)@([\w.-]+)/i,
		extract(m) {
			const [, pass, user, host] = m;
			if (!pass || pass.length < 4) return null;
			return {
				service: `ssh://${user}@${host}`,
				type: "password",
				value: pass,
				notes: "auto-captured from sshpass tool call",
			};
		},
	},
	...
	{
		regex:
			/curl\b[\s\S]{0,500}?-H\s+["']Authorization:\s+Bearer\s+([A-Za-z0-9_.~+/=-]{8,})/i,
		extract(m, fullText) {
			...
			return {
				...
				value: m[1],
				url: urlMatch?.[0],
				notes: "auto-captured from curl Authorization Bearer tool call",
			};
		},
	},
	...
	// Fix #13: Handle quoted values properly
	{
		...
		extract(m, fullText) {
			...
			if (!pass || pass.length < 4) return null;
			const urlMatch = fullText.match(/https?:\/\/[^\s'"]+/);
			const service = urlMatch
				? extractHostFromUrl(urlMatch[0])
				: slugify(user);
			return {
				service,
				type: "password",
				value: pass,
				url: urlMatch?.[0],
				notes: `auto-captured from curl -u ${user} tool call`,
			};
		},
	},
	...
	// -H "X-API-Key: <key>" (standalone or in curl)
	{
		...
		extract(m, fullText) {
			...
			return {
				...
				value: m[1],
				url: urlMatch?.[0],
				notes: "auto-captured from X-API-Key header tool call",
			};
		},
	},
	...
	{
		regex:
			/(postgres(?:ql)?|mysql|mongodb(?:\+srv)?|redis|mssql):\/\/([\w.-]+):([\S]+?)@([\w.-]+(?::\d+)?)\/([\w-]*)/i,
		extract(m) {
			const [, proto, user, pass, host, db] = m;
			if (!pass || pass.length < 4) return null;
			const service = db
				? `${proto.toLowerCase()}://${host}/${db}`
				: `${proto.toLowerCase()}://${host}`;
			...
			return {
				...
				value: pass,
				url,
				notes: `auto-captured connection string (user: ${user})`,
			};
		},
	},
	...
	{
		regex:
			/\bexport\s+([A-Z][A-Z0-9_]*_(?:KEY|TOKEN|PASSWORD|SECRET))\s*=\s*(?:"([^"]{8,})"|'([^']{8,})'|([^\s"';\n]{8,}))/i,
		extract(m) {
			...
			const service = slugify(
				varName
					.replace(/_(?:KEY|TOKEN|PASSWORD|SECRET)$/i, "")
					.replace(/_/g, "-"),
			);
			return {
				service,
				type,
				value: val,
				notes: `auto-captured from export ${varName} tool call`,
			};
		},
	},
	...
	// Instead, filter out lines that start with 'export ' in post-processing
	{
		...
		extract(m, fullText) {
			...
			return {
				...
				type,
				value: val,
				notes: `auto-captured from env assignment ${varName}`,
			};
		},
	},
];
 ...
 */
export function extractCredentialsFromToolCalls(
	text: string,
): ToolCallCredential[] {
		...
		try {
			...
			for (const match of text.matchAll(globalRegex)) {
				try {
					...
					const validatedService = validateAndNormalizeServiceName(
						cred.service,
					);
