...
export function parseSessionJsonl(
				...
				if (looksLikeFailure(toolContent)) {
					lastFailure =
						typeof toolContent === "string"
							? toolContent.slice(0, 200)
							: JSON.stringify(toolContent).slice(0, 200);
				} else {
	...
	if (!taskIntent || steps.length < 2) {
		if (opts?.includeSkipReason) {
			return {
				skipReason: !taskIntent ? "no_task_intent" : "fewer_than_2_steps",
			};
		}
		return null;
	}
...
export async function extractProceduresFromSessions(
	...
	for (const filePath of filePaths) {
		...
		if (parsed && "skipReason" in parsed) {
			if (verbose)
				logger.info(
					`procedure-extractor: skip ${sessionId}.jsonl — ${parsed.skipReason}`,
				);
			continue;
		}
		...
		const existing = factsDb.findProcedureByTaskPattern(
			parsed.taskIntent,
			1,
		)[0];
		if (
			existing &&
			taskSimilarity(existing.taskPattern, parsed.taskIntent) >= 0.5
		) {
			let recorded = false;
			if (parsed.success) {
				recorded = factsDb.recordProcedureSuccess(
					existing.id,
					recipeJson,
					sessionId,
				);
			} else {
				recorded = factsDb.recordProcedureFailure(
					existing.id,
					recipeJson,
					sessionId,
				);
