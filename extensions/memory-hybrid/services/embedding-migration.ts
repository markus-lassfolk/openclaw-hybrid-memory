...
export async function migrateEmbeddings(
	...
	const total = useBatched
		? (
				factsDb as {
					getCount: (opts: { includeSuperseded: boolean }) => number;
				}
			).getCount({ includeSuperseded: false })
	...
	while (offset < total) {
		...
		try {
			...
			const batchAsErr =
				batchErr instanceof Error ? batchErr : new Error(String(batchErr));
			const isRateLimit =
				is429OrWrapped(batchAsErr) || is403QuotaOrRateLimitLike(batchErr);

			if (isRateLimit) {
				// Back off before per-fact fallback to avoid amplifying RPM pressure (#940 §3).
				const backoffMs =
					delayMsBetweenBatches > 0 ? delayMsBetweenBatches * 3 : 5_000;
				log.warn(
			...
			for (const fact of batch) {
				try {
					...
					if (is429OrWrapped(singleAsErr) || is403QuotaOrRateLimitLike(err)) {
						const singleBackoff =
							delayMsBetweenBatches > 0 ? delayMsBetweenBatches * 2 : 3_000;
						log.warn(
...
export async function runEmbeddingMaintenance(
	...
	try {
		...
		log.warn(
			`memory-hybrid: embedding-migration: maintenance run failed: ${err}`,
		);
		return { changed: true, migrated: false };
	}
}
