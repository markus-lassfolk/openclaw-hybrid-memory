...
import type {
	GoalStewardshipAttentionWeights,
...
export const DEFAULT_HEARTBEAT_PATTERN_SOURCES = [
	"heartbeat",
	"scheduled ping",
	"cron heartbeat",
];
...
export function compileHeartbeatMatchers(patterns: string[]): RegExp[] {
	const src =
		patterns.length > 0 ? patterns : DEFAULT_HEARTBEAT_PATTERN_SOURCES;
	const out: RegExp[] = [];
	for (const s of src) {
		try {
...
export function matchesHeartbeat(
	userText: string,
...
function priorityWeight(
	p: Goal["priority"],
	w: GoalStewardshipAttentionWeights,
): number {
	switch (p) {
		case "critical":
			return w.critical;
...
async function readRoundRobin(
	goalsDir: string,
): Promise<StewardshipRoundRobinState> {
	try {
		const raw = await readFile(join(goalsDir, RR_FILE), "utf-8");
		const j = JSON.parse(raw) as { offset?: number };
		return {
			offset:
				typeof j.offset === "number" && j.offset >= 0
					? Math.floor(j.offset)
					: 0,
		};
	} catch {
...
async function writeRoundRobin(
	goalsDir: string,
	state: StewardshipRoundRobinState,
): Promise<void> {
	await writeFile(
		join(goalsDir, RR_FILE),
		JSON.stringify(
			{ offset: state.offset, updatedAt: new Date().toISOString() },
			null,
			2,
		),
		"utf-8",
...
function sortGoalsForRoundRobin(
	goals: Goal[],
	weights: GoalStewardshipAttentionWeights,
): Goal[] {
	return [...goals].sort((a, b) => {
		const wd =
			priorityWeight(b.priority, weights) - priorityWeight(a.priority, weights);
		if (wd !== 0) return wd;
...
export function buildStewardshipBlockFull(
	goal: Goal,
	allActive: number,
): string {
	...
	const linked =
		goal.linkedTasks.length > 0
			? goal.linkedTasks.map((t) => `  - ${t.label}: ${t.status}`).join("\n")
			: "  (none)";
	let directive =
		"Assess progress toward acceptance criteria; dispatch work or call goal_assess.";
...
export function buildStewardshipBlockCompact(
	goal: Goal,
	maxChars: number,
	allActive: number,
): string {
...
export async function buildMultiGoalStewardshipPrepend(
	...
	const now = Date.now();
	const candidates = sortGoalsForRoundRobin(
		candidateGoals(allGoals, now),
		cfg.attentionWeights,
	);
	...
	const weights = selected.map((g) =>
		priorityWeight(g.priority, cfg.attentionWeights),
	);
	const sumW = weights.reduce((a, b) => a + b, 0) || 1;
	const cap = cfg.multiGoalMaxChars;
	const budgets = weights.map((w) =>
		Math.max(400, Math.floor((w / sumW) * cap)),
	);
	...
	if (opts.suggestHeavyDirective && suggestHeavy) {
		header +=
			"<!-- triage: prefer heavy-tier model or deliberate tool use for substantive dispatch this turn -->\n";
	}
