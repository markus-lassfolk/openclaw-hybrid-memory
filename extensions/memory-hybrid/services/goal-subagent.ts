...
export async function resolveGoalForSpawn(
	event: GoalSubagentSpawnEvent,
	goalsDir: string,
): Promise<string | null> {
	const metaGoal =
...
export async function updateGoalOnSubagentEnd(
	...
	for (const g of goals) {
		const sessionMatch = info.sessionKey
			? g.linkedTasks.find(
					(t) => t.sessionKey && t.sessionKey === info.sessionKey,
				)
			: undefined;
		const match =
			sessionMatch ?? g.linkedTasks.find((t) => t.label === info.label);
		if (!match) continue;
		...
		const linkedTasks = g.linkedTasks.map((t) =>
			t.label === match.label
				? {
						...
						status: newStatus,
						updatedAt: ts,
						sessionKey: info.sessionKey ?? t.sessionKey,
					}
				: t,
		);
		...
		if (info.success && allLinkedTasksTerminal({ ...g, linkedTasks })) {
			await updateGoal(
				...
				g.id,
				{
					...
					status: "verifying",
					lastOutcome:
						lastOutcome ?? "All linked tasks completed — verify goal",
				},
				{
					timestamp: ts,
