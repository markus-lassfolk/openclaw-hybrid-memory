 ...
 */
export function detectTrajectoryBoundaries(
	...
	for (let i = 0; i < turns.length; i++) {
		...
		if (turn.role === "user") {
			...
			const isGrateful =
				/\b(thanks|thank you|cheers|perfect|great|awesome)\b/i.test(
					turn.content,
...
export function classifyTrajectoryOutcome(trajectoryTurns: TrajectoryTurn[]): {
	...
	const SELF_SERVICE_RE =
		/\b(i'?ll (do|handle|fix) it (myself)?|never mind|forget it|i'?ll figure it out)\b/i;
	const ESCALATION_RE =
		/\b(i'?ll ask (someone|my|the|a)|going to ask|ask (someone else|my colleague|gpt|chatgpt))\b/i;

	if (
		SELF_SERVICE_RE.test(lastUserTurn.summary) ||
		ESCALATION_RE.test(lastUserTurn.summary)
	) {
	...
	if (lastUserTurn.sentiment === "positive") {
		if (hasCorrections || hasRephrases) {
			...
			for (let i = trajectoryTurns.length - 1; i >= 0; i--) {
				if (
					trajectoryTurns[i].wasCorrection ||
					trajectoryTurns[i].wasRephrase
				) {
 ...
 */
export function extractTrajectoryLessons(
	...
	if (outcome === "success") {
		if (turnCount <= 3) {
			const tools =
				toolsUsed.length > 0 ? ` using ${toolsUsed.join(", ")}` : "";
			lessons.push(
				`Direct approach${tools} worked immediately${topicStr} — no corrections needed.`,
			);
		} else {
		...
		if (keyPivot !== undefined) {
			...
			const before = turns
				.slice(0, keyPivot)
				.filter((t) => t.wasCorrection).length;
			const pivotSummary = pivotTurn
				? pivotTurn.summary.slice(0, 100)
				: "unknown";
			lessons.push(
			...
			lessons.push(
				`User had corrections then eventual success${topicStr} after ${turnCount} turns.`,
			);
		}
	} else {
		// failure
		...
		if (corrections > 0 || rephrases > 0) {
			...
			lessons.push(
				`Interaction did not reach resolution${topicStr} after ${turnCount} turns.`,
			);
		}
	}
...
export async function analyzeTrajectoriesWithLLM(
	...
	try {
		...
		const filledPrompt = prompt.replace(
			"{{trajectory_json}}",
			JSON.stringify(turnsPayload, null, 2),
		);
