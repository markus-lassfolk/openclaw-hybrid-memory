...
export function buildHybridMemCronBashBody(
  ...
  return [
    ...
    '    case "${cmd[2]:-}" in',
    "      distill|extract-procedures|extract-directives|extract-reinforcement|self-correction-run)",
    "        cmd+=(--full)",
    "        ;;",
    "    esac",
    "  fi",
    ...
    "  esac",
    '  if [ -z "$maintenance_status" ]; then',
    '    if [ "$final_ec" -eq 0 ]; then',
    '      final_label="SUCCESS"',
    "    fi",
    "  fi",
    '  echo "$(date -u +%Y-%m-%dT%H:%M:%SZ) validate-cron-exit exit=${validation_ec}${maintenance_status:+ status=${maintenance_status}}" >>"$HM_EXIT"',
