...
}: BuildExplicitSemanticQueryVectorDeps): Promise<{
	...
	try {
		...
		if (policy.allowHyde && cfg.queryExpansion.enabled) {
			textToEmbed = await expandQueryWithHyde({
				query,
				rawCfg: cfg as Parameters<
					typeof import("../config/index.js").getCronModelConfig
				>[0],
		...
		return {
			queryVector: null,
			warning:
				"Semantic search unavailable due to embedding failure; results may be incomplete.",
		};
	}
}
...
export function serializeFactForContext(
	entry: MemoryEntry,
	options?: { isContradicted?: boolean },
): string {
	const parts: string[] = [];
...
async function runMultiModelSemanticStrategies(
	...
	for (const s of settled) {
		...
		const candidates = factsDbWithEmbeddings.getEmbeddingsByModel(
			name,
...
function runGraphStrategy(
	...
	if (maxDepth <= 0 || seeds.length === 0 || !hasGraphLookup(factsDb))
		return [];
...
function hasClusterLookup(
	factsDb: FactLookup,
): factsDb is FactLookup & ClusterFactLookup {
	return (
		typeof factsDb.getAllLinkedFactIds === "function" &&
		typeof factsDb.getAllLinks === "function"
	);
}

function hasGraphLookup(
	factsDb: FactLookup,
): factsDb is FactLookup & GraphFactLookup {
	return (
...
class ClusterCache {
	...
	getClusterMap(
		...
		const now = Date.now();
		const linkCount =
			typeof factsDb.linksCount === "function" ? factsDb.linksCount() : null;
...
function describeEmbeddingRegistry(
	registry: EmbeddingRegistry | null | undefined,
): unknown {
	if (!registry) return null;

	const primaryModel =
		typeof registry.getPrimaryModel === "function"
			? registry.getPrimaryModel()
			: null;
	...
	return {
		...
		additionalModels,
		multiModel:
			typeof registry.isMultiModel === "function"
				? registry.isMultiModel()
...
function buildSemanticCacheFilterKey(
	...
	const expanderMode =
		...
		typeof (options.queryExpander as QueryExpander).getMode === "function"
	...
	const expanderThreshold =
		options.queryExpander &&
		typeof (options.queryExpander as QueryExpander).getThreshold === "function"
	...
	return stableStringify({
		...
		rerankingAvailable: Boolean(
			options.rerankingConfig?.enabled && options.rerankingOpenai,
		),
		rerankingConfig: options.rerankingConfig ?? null,
		documentGradingAvailable: Boolean(
			options.documentGrader ||
				(options.documentGradingConfig?.enabled && options.adaptiveOpenai),
		),
		documentGradingConfig: options.documentGradingConfig ?? null,
		documentGraderOverride: options.documentGrader
			? (options.documentGrader.constructor?.name ?? "custom")
			: null,
...
function shouldPreferResult(
	...
	return (
		(candidate.result.fused[0]?.finalScore ?? 0) >
		(incumbent.fused[0]?.finalScore ?? 0)
	);
}
...
function buildOrchestratorResult(
	...
	const packedFactIds = orderedEntries
		.slice(0, packed.length)
...
function buildCachedResult(
	...
	const getByIdOpts =
		options.scopeFilter || options.asOf != null
			? { scopeFilter: options.scopeFilter, asOf: options.asOf }
			: undefined;
...
export interface RetrievalPipelineOptions {
	...
	queryValidator?:
		| ((
				query: string,
		  ) => QueryValidationResult | Promise<QueryValidationResult>)
		| null;
...
export async function runExplicitDeepRetrieval(
	...
	const applyConditionalReranking = async (
		...
		if (
			!(policy as ExplicitDeepRetrievalPolicy).allowReranking ||
		...
		try {
			const rrfScoreMap = new Map(
				initial.fused.map((result) => [result.factId, result.finalScore]),
			);
			const fusedEntryMap = new Map<string, MemoryEntry>(
				initial.fused
					.map(
						(result, index) =>
							[result.factId, initial.entries[index]] as [string, MemoryEntry],
					)
			...
			const orderedEntriesReranked = reranked
				.map((fact) => ({
					factId: fact.factId,
					entry: fusedEntryMap.get(fact.factId)!,
				}))
				.filter((entry) => entry.entry);
			const rerankedOrder = new Map(
				reranked.map((fact, index) => [fact.factId, index]),
			);
	...
	const runBasePipeline = async (
		...
		if (
			currentQueryVector &&
			typeof vectorDbWithCache.getSemanticQueryCacheMatch === "function"
		) {
			const cached = await vectorDbWithCache.getSemanticQueryCacheMatch(
				currentQueryVector,
				{
					ttlMs: semanticCacheTtlMs ?? DEFAULT_SEMANTIC_CACHE_TTL_MS,
					minSimilarity:
						semanticCacheMinSimilarity ?? DEFAULT_SEMANTIC_CACHE_MIN_SIMILARITY,
		...
		if (strategies.includes("semantic") && currentQueryVector) {
			strategyPromises.push(
				safeStrategy("semantic", () =>
					runSemanticStrategy(vectorDb, currentQueryVector, semanticTopK),
				),
			);
		}

		if (
			(policy as ExplicitDeepRetrievalPolicy).allowAliasExpansion &&
			aliasDb &&
			currentQueryVector
		) {
			strategyPromises.push(
				safeStrategy("aliases", () =>
					searchAliasStrategy(aliasDb, currentQueryVector, semanticTopK),
				),
			);
		}
		...
		if (
			...
			try {
				...
				if (expansion.useLlm) {
					const variants = await queryExpander.expandQuery(
						queryText,
						queryExpansionContext,
					);
		...
		if (
			(policy as ExplicitDeepRetrievalPolicy).allowGraphExpansion &&
			strategies.includes("graph")
		) {
			const seedScores = new Map<string, number>();
			const seedEntries = new Map<string, MemoryEntry>();
			const getByIdOpts =
				scopeFilter || asOf != null ? { scopeFilter, asOf } : undefined;
			...
			for (const results of strategyMap.values()) {
				for (const result of results) {
					...
					if (!seedEntries.has(result.factId))
						seedEntries.set(result.factId, entry);
				}
			}
			...
			for (const res of allResults) {
				if (!deduped.has(res.factId)) {
					deduped.set(res.factId, {
						...
						sources: [
							{
								strategy: (res as any).strategyName || "unknown",
								rank: res.rank,
							},
						],
					});
		...
		for (const result of fused) {
			...
			if (!includeSuperseded) {
				if (entry.supersededAt != null) continue;
				if (entry.expiresAt != null && entry.expiresAt <= effectiveNow)
					continue;
			}
				...
				if (clusterByFact.size > 0) {
					...
					for (const indices of clusterToIndices.values()) {
						...
						for (const index of indices) {
							if (
								scopedFused[index].finalScore >
								scopedFused[bestIndex].finalScore
							) {
								bestIndex = index;
							}
						}
		...
		const finalOrder = new Map<string, number>(
			scopedFused.map((result, index) => [result.factId, index]),
		);
		orderedEntries.sort(
			(a, b) =>
				(finalOrder.get(a.factId) ?? 0) - (finalOrder.get(b.factId) ?? 0),
		);

		if (activeDocumentGrader && orderedEntries.length > 0) {
			const grades = await activeDocumentGrader.gradeDocuments(
				queryText,
				orderedEntries.map(({ factId, entry }) => ({
					factId,
					text: entry.text,
				})),
			);
			if (grades.length > 0) {
				...
				const relevantFactIds = new Set(
					grades.filter((grade) => grade.relevant).map((grade) => grade.factId),
				);
				orderedEntries = orderedEntries.filter(({ factId }) =>
					relevantFactIds.has(factId),
				);
				scopedFused = scopedFused.filter((result) =>
					relevantFactIds.has(result.factId),
				);
			}
		}
		...
		if (
			rerankingConfig?.enabled &&
			rerankingOpenai &&
			!expansion.skipReranking
		) {
			try {
				const rrfScoreMap = new Map<string, number>(
					scopedFused.map((result) => [result.factId, result.finalScore]),
				);
				const scoredFacts: ScoredFact[] = orderedEntries.map(
					({ factId, entry }) => {
						const storedSec = entry.sourceDate ?? entry.createdAt;
						return {
							factId,
	...
	const executePipelineQuery = async (
		...
		const expanderMode =
			queryExpander &&
			typeof (queryExpander as QueryExpander).getMode === "function"
		...
		if (expanderMode === "conditional") {
			const alias =
				queryExpander &&
				typeof (queryExpander as QueryExpander).getRuleBasedAlias === "function"
			...
			return {
				...initial,
				result: await applyConditionalReranking(queryText, initial.result),
			};
		}
	...
	for (let iteration = 0; iteration <= MAX_REWRITE_ITERATIONS; iteration++) {
		...
		const rewritten = await activeDocumentGrader.rewriteQuery(
			query,
			attemptedQueries,
		);
		if (!rewritten) {
