...
} from "./openclaw-session-artifact.js";
...
async function tryDeleteStaleCorruptSignalFile(
	filePath: string,
): Promise<void> {
	try {
...
function parseTaskBlock(
	...
	const entry: Partial<ActiveTaskEntry> & {
		label: string;
		description: string;
	} = {
		label,
...
function extractGoalsMirrorSection(content: string): string | undefined {
	...
	while (
		goalLines.length > 0 &&
		goalLines[goalLines.length - 1].trim() === ""
	) {
		goalLines.pop();
	}
...
export function serializeTaskEntry(entry: ActiveTaskEntry): string {
	...
	if (entry.relatedGoal?.trim())
		lines.push(`- **Related goal:** ${entry.relatedGoal.trim()}`);
	if (entry.handoff)
		lines.push(`- **Handoff:** ${stableStringify(entry.handoff)}`);
...
export function detectStaleTasks(
	...
	return tasks.map((t) => {
		...
		const hasValidUpdated =
			t.updated !== UNKNOWN_ACTIVE_TASK_TIME &&
...
export function upsertTask(
	...
	const updatedTimestamp = preserveUpdated
		? entry.updated
...
export function buildActiveTaskInjection(
	...
	const lines: string[] = [
		"<active-tasks>",
		"In-progress tasks from ACTIVE-TASKS.md:",
	];
...
export function buildStaleWarningInjection(
	...
	if (staleTasks.length > 0) {
		...
		for (const task of staleTasks) {
			...
			const hoursAgo = Number.isNaN(updatedMs)
				? "?"
	...
	if (inProgressWithSubagent.length > 0) {
		...
		const header =
			"💡 In-progress tasks with subagents — verify they are still running:";
		...
		const footer =
			"Hint: use `subagents list` to check if these subagents are still active.";
		if (maxChars == null || usedChars + footer.length <= maxChars) {
			lines.push(footer);
		}
	}
...
export async function flushCompletedTaskToMemory(
	...
	const summary = [
		...
		...(task.handoff
			? [`- **Handoff:** ${stableStringify(task.handoff)}`]
			: []),
		"",
	].join("\n");
...
function toHandoffRefFromArtifact(
	artifact: OctaveTaskHandoffArtifact,
): ActiveTaskHandoffRef {
	return {
...
export function createOctaveTaskHandoffArtifact(
	signal: TaskSignal,
): OctaveTaskHandoffArtifact {
...
export function validateOctaveTaskHandoffArtifact(
	...
	| { valid: true; artifact: OctaveTaskHandoffArtifact }
	| { valid: false; reason: string } {
	if (!value || typeof value !== "object")
		return { valid: false, reason: "artifact is not an object" };
	const artifact = value as Partial<OctaveTaskHandoffArtifact>;
	if (artifact.schema !== OCTAVE_TASK_HANDOFF_SCHEMA)
		return { valid: false, reason: "schema mismatch" };
	if (artifact.artifactType !== "task_handoff")
		return { valid: false, reason: "artifact type mismatch" };
	...
	if (
		typeof artifact.canonical !== "string" ||
		artifact.canonical.length === 0
	) {
	...
	if (!Array.isArray(artifact.auditTrail))
		return { valid: false, reason: "missing audit trail" };
	if (!isTaskSignal(artifact.payload))
		return { valid: false, reason: "invalid payload" };
	const expectedCanonical = buildCanonicalPayload(artifact.payload);
	if (artifact.canonical !== expectedCanonical)
		return { valid: false, reason: "canonical mismatch" };
 ...
 * @param memoryDir Absolute path to the memory directory
 */
export async function readPendingSignals(
	memoryDir: string,
): Promise<PendingTaskSignal[]> {
	const signalsDir = join(memoryDir, "task-signals");
...
export async function writeActiveTaskFileGuarded(
	...
	if (isSubagentSession(sessionKey)) {
		return {
			skipped: true,
			reason:
				"sub-agent sessions are read-only for ACTIVE-TASKS.md; use writeTaskSignal instead",
		};
	}
