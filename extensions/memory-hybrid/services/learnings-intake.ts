...
export function evaluatePromotion(entry: LearningEntry): PromotionEvaluation {
	if (entry.status !== "open") {
		return {
			...
			reason: `entry already ${entry.status}`,
		};
	}
	...
	switch (entry.type) {
		...
		case "learning":
			...
			return {
				...
				reason:
					"stable lesson — flag for human review before writing to SKILL.md",
			};
		...
		case "feature_request":
			return {
				...
				reason:
					"feature request — file as GitHub issue when ready, not a memory fact",
			};
	}
}
