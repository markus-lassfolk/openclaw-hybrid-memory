...
function countFeedbackInWindow(
  ...
  try {
    // Corrections from facts table (self-correction sources)
    const correctionsQ = topic
      ? `SELECT COUNT(*) as cnt FROM facts WHERE source IN ('self-correction', 'self-correction-analysis') AND created_at >= ? AND created_at <= ? AND (text LIKE ? OR summary LIKE ?)`
      : `SELECT COUNT(*) as cnt FROM facts WHERE source IN ('self-correction', 'self-correction-analysis') AND created_at >= ? AND created_at <= ?`;
