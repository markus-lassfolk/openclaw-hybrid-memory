...
function countFeedbackInWindow(
	...
	try {
		...
		const correctionsRow = topic
			? (db
					.prepare(correctionsQ)
					.get(windowStart, windowEnd, `%${topic}%`, `%${topic}%`) as {
					cnt: number;
				})
			: (db.prepare(correctionsQ).get(windowStart, windowEnd) as {
					cnt: number;
				});
	...
	try {
		...
		const praiseRow = topic
			? (db.prepare(praiseQ).get(windowStart, windowEnd, `%${topic}%`) as {
					cnt: number;
				})
	...
	try {
		...
		const implRows = topic
			? (db
					.prepare(implQ)
					.all(windowStart, windowEnd, `%${topic}%`, `%${topic}%`) as Array<{
					polarity: string;
					cnt: number;
				}>)
			: (db.prepare(implQ).all(windowStart, windowEnd) as Array<{
					polarity: string;
					cnt: number;
				}>);
...
export function measureRuleEffectiveness(
	...
	try {
		...
		const topic = topicTags.find(
			(t) =>
				!["reinforcement", "behavioral", "trajectory", "feedback"].includes(t),
		);

		const before = countFeedbackInWindow(
			factsDb,
			windowStart,
			ruleCreatedAt,
			topic,
		);
		const after = countFeedbackInWindow(
			factsDb,
			ruleCreatedAt + 1,
			windowEnd,
			topic,
		...
		const beforeTotal =
			before.corrections +
			before.praise +
			before.implicitPositive +
			before.implicitNegative;
		const afterTotal =
			after.corrections +
			after.praise +
			after.implicitPositive +
			after.implicitNegative;
		...
		const effectScore =
			(afterPositive - beforePositive) / denom -
			(afterNegative - beforeNegative) / denom;
		const clampedScore = Math.max(-1, Math.min(1, effectScore));

		// Confidence scales with sample size
		const confidence = Math.min(
			1.0,
			(sampleSize / Math.max(config.minSampleSize ?? 5, 1)) * 0.5,
		);
		...
		for (const row of rows) {
			...
			if (m.effectScore < deprecateThreshold) {
				try {
					...
					db.prepare("UPDATE facts SET importance = 0.1 WHERE id = ?").run(
						row.id,
					);
					report.deprecated++;
				} catch (err) {
			...
			if (m.effectScore > boostThreshold) {
				try {
					db.prepare(
						"UPDATE facts SET importance = MIN(1.0, importance + 0.2) WHERE id = ?",
					).run(row.id);
					report.boosted++;
				} catch (err) {
					capturePluginError(
...
export function getEffectivenessReport(factsDb: FactsDB): string {
	try {
		...
		if (rows.length === 0)
			return "No feedback effectiveness data available yet.";

		const lines: string[] = ["# Feedback Effectiveness Report", ""];
		for (const row of rows) {
			const scoreStr = row.effect_score.toFixed(2);
			const icon =
				row.effect_score > 0.3 ? "✓" : row.effect_score < -0.2 ? "✗" : "~";
			const rulePreview = row.rule_text.slice(0, 80);
			lines.push(
				`${icon} [${scoreStr}] "${rulePreview}" (n=${row.sample_size}, conf=${row.confidence.toFixed(2)})`,
			);
		}
