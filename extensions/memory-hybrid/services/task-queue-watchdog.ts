 ...
 */
export function taskQueueItemMatchesStale(
	...
	return (
		current.issue === stale.issue &&
		current.dispatchToken === stale.dispatchToken &&
 ...
 */
export async function getActiveWorktreeBranches(
	repoDir: string,
): Promise<Set<string>> {
	try {
 ...
 */
export function taskQueueItemHasRecognizedSemantics(
	item: TaskQueueItem,
): boolean {
	...
	if (
		...
		Number.isFinite(item.issue) &&
		item.issue > 0
	)
		return true;
	if (typeof item.pid === "number" && item.pid > 0) {
		if (typeof item.started === "string" && item.started.length > 0)
			return true;
	}
	if (
		item.dispatchToken != null &&
		String(item.dispatchToken).trim().length > 0
	)
		return true;
	const st = item.status != null ? String(item.status).toLowerCase() : "";
	if (st && TASK_QUEUE_STATUS_WITH_PAYLOAD.has(st)) {
		if (item.producer != null && String(item.producer).trim().length > 0)
			return true;
	}
...
export async function writeCanonicalIdleTaskQueueFile(
	...
	logger?.info?.(
		`memory-hybrid: wrote canonical idle task-queue file at ${currentPath}`,
	);
}
...
export async function ensureTaskQueueIdlePlaceholder(
	...
	logger?.info?.(
		`memory-hybrid: wrote task-queue idle placeholder at ${currentPath}`,
	);
	return true;
}
...
async function countMatchingHistory(
	...
	try {
		const files = (await readdir(historyDir)).filter((f) =>
			f.endsWith(".json"),
		);
		let count = 0;
...
export async function runTaskQueueWatchdog(
	...
	if (!taskQueueItemHasRecognizedSemantics(item)) {
		...
		if (
			recheckBefore != null &&
			taskQueueItemHasRecognizedSemantics(recheckBefore)
		) {
		...
		if (
			recheckBefore == null ||
			stableStringify(recheckBefore) !== stableStringify(item)
		) {
		...
		try {
			...
			const stillSameSnapshot =
				recheckAfter != null &&
				stableStringify(recheckAfter) === stableStringify(item);
	...
	const historyCount = await countMatchingHistory(
		historyDir,
		item.issue,
		item.branch,
	);
	const isExhausted =
		previousRetries >= maxRetries || historyCount >= maxRetries;
	...
	try {
		...
		const identityMatches =
			recheck != null && taskQueueItemMatchesStale(recheck, item);
		if (identityMatches) {
			await unlink(currentPath);
		}
