...
export async function runFindDuplicates(
	...
	if (candidateFacts.length < 2) {
		logger.info(
			"memory-hybrid: find-duplicates -- fewer than 2 candidate facts",
		);
		return {
			pairs: [],
			candidatesCount: candidateFacts.length,
			skippedStructured,
		};
	}
	...
	logger.info(
		`memory-hybrid: find-duplicates -- embedding ${ids.length} facts...`,
	);
	...
	for (let i = 0; i < ids.length; i += BATCH_SIZE) {
		...
		const vecs = await safeEmbedBatch(embeddings, batchTexts, (msg) =>
			logger.warn(msg),
		);
		...
		for (let j = 0; j < batchFacts.length; j++) {
			...
			if (!vec || vec.length === 0) {
				logger.warn(
					`memory-hybrid: find-duplicates -- skipping fact ${item.id} due to embedding failure`,
				);
				skippedEmbeddings++;
				continue;
			}
	...
	if (skippedEmbeddings > 0) {
		logger.info(
			`memory-hybrid: find-duplicates -- skipped ${skippedEmbeddings} facts due to embedding failures`,
		);
	}
	...
	logger.info(
		`memory-hybrid: find-duplicates -- ${pairs.length} pairs >= ${opts.threshold}`,
	);
