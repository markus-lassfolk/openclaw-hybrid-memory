...
export async function runMemoryDiagnostics(opts: {
	...
	try {
		...
		if (!semanticOk && semanticFailReason) {
			...
			capturePluginError(
				new Error(
					`Semantic search diagnostic failed: ${semanticFailReason}${dimInfo}`,
				),
				{
					subsystem: "diagnostics",
		...
		const autoRecallResults = mergeResults(
			structuredResults,
			semanticResults,
			autoRecallLimit,
			factsDb,
		...
		return {
			...
			markerText,
			structured: {
				ok: structuredResults.some((r) => r.entry.id === entry.id),
			...
			hybrid: {
				ok: hybridResults.some((r) => r.entry.id === entry.id),
				count: hybridResults.length,
			},
			autoRecall: {
				ok: autoRecallResults.some((r) => r.entry.id === entry.id),
				count: autoRecallResults.length,
			},
		};
	} finally {
		try {
