...
export function extractEventText(event: EventLogEntry): string {
	...
	if (typeof c.text === "string" && c.text.trim().length > 0)
		return c.text.trim();
	if (typeof c.decision === "string" && c.decision.trim().length > 0)
		return c.decision.trim();
	if (typeof c.summary === "string" && c.summary.trim().length > 0)
		return c.summary.trim();
	if (typeof c.action === "string" && c.action.trim().length > 0)
...
export function buildDigestSummary(counts: {
	...
	if (counts.factsDecayed > 0)
		parts.push(`${counts.factsDecayed} facts decayed`);
	if (counts.linksPruned && counts.linksPruned > 0)
		parts.push(`${counts.linksPruned} orphaned links removed`);
	if (counts.eventsConsolidated > 0) {
		parts.push(
			`${counts.eventsConsolidated} events consolidated into ${counts.factsCreated} facts`,
		);
	}
	if (counts.patternsFound > 0)
		parts.push(`${counts.patternsFound} patterns extracted`);
	if (counts.rulesGenerated > 0)
		parts.push(`${counts.rulesGenerated} rules generated`);
	if (counts.logRowsPruned && counts.logRowsPruned > 0)
		parts.push(`${counts.logRowsPruned} log rows pruned`);
	...
	for (const [entity, groupEvents] of groups) {
		...
		if (eventTexts.length === 0) {
			...
			try {
				...
				logger.warn(
					`memory-hybrid: dream-cycle — failed to mark no-text events as consolidated: ${err}`,
				);
				capturePluginError(
		...
		try {
			...
			logger.warn(
				`memory-hybrid: dream-cycle — failed to store consolidated fact for entity "${entity}": ${err}`,
			);
		...
		for (const event of groupEvents) {
			...
			try {
				...
				logger.warn(
					`memory-hybrid: dream-cycle — failed to create source fact for event ${event.id}: ${err}`,
				);
				capturePluginError(
					err instanceof Error ? err : new Error(String(err)),
					{
		...
		try {
			...
			logger.warn(
				`memory-hybrid: dream-cycle — failed to mark events as consolidated: ${err}`,
			);
			capturePluginError(err instanceof Error ? err : new Error(String(err)), {
...
export async function runDreamCycle(
	...
	if (config.pruneMode === "expired" || config.pruneMode === "both") {
		try {
			...
			logger.info(
				`memory-hybrid: dream-cycle — pruned ${factsPruned} expired facts`,
			);
		} catch (err) {
	...
	if (config.pruneMode === "decay" || config.pruneMode === "both") {
		try {
			...
			logger.warn(
				`memory-hybrid: dream-cycle — decayConfidence failed: ${err}`,
			);
			capturePluginError(err instanceof Error ? err : new Error(String(err)), {
	...
	try {
		...
		if (linksPruned > 0) {
			logger.info(
				`memory-hybrid: dream-cycle — pruned ${linksPruned} orphaned link(s)`,
			);
		}
	} catch (err) {
		logger.warn(
			`memory-hybrid: dream-cycle — pruneOrphanedLinks failed: ${err}`,
		);
		capturePluginError(err instanceof Error ? err : new Error(String(err)), {
	...
	if (eventLog) {
		try {
			...
			logger.warn(
				`memory-hybrid: dream-cycle — consolidation step failed: ${err}`,
			);
			capturePluginError(err instanceof Error ? err : new Error(String(err)), {
	...
	if (eventLog && config.eventLogArchivalDays > 0) {
		try {
			...
			logger.warn(
				`memory-hybrid: dream-cycle — archiveConsolidated failed: ${err}`,
			);
			capturePluginError(err instanceof Error ? err : new Error(String(err)), {
	...
	if (eventLog && config.maxUnconsolidatedAgeDays > 0) {
		try {
			const deleted = eventLog.archiveOld(
				config.maxUnconsolidatedAgeDays,
				true,
			);
			if (deleted > 0) {
				logger.info(
	...
	try {
		...
		logger.info(
			`memory-hybrid: dream-cycle — reflection complete: ${patternsFound} patterns stored`,
		);
	} catch (err) {
	...
	if (patternsFound >= MIN_PATTERNS_FOR_RULES) {
		try {
			...
			logger.info(
				`memory-hybrid: dream-cycle — reflect-rules complete: ${rulesGenerated} rules stored`,
			);
		} catch (err) {
			logger.warn(
				`memory-hybrid: dream-cycle — reflect-rules step failed: ${err}`,
			);
			capturePluginError(err instanceof Error ? err : new Error(String(err)), {
	...
	try {
		...
		logger.warn(
			`memory-hybrid: dream-cycle — memory index update failed: ${err}`,
		);
		capturePluginError(err instanceof Error ? err : new Error(String(err)), {
	...
	if (config.vacuumOnCycle) {
		try {
			...
			logger.warn(
				`memory-hybrid: dream-cycle — vacuumAndCheckpoint failed: ${err}`,
			);
			capturePluginError(err instanceof Error ? err : new Error(String(err)), {
