...
export function parseGithubPrTarget(
	target: string,
...
async function verifyPrMergedApi(
	target: string,
	...
	if (!parsed) {
		return {
			ok: false,
			detail:
				"pr_merged: target must be owner/repo#N or a https://github.com/owner/repo/pull/N URL",
		};
	}
	...
	if (!token) {
		return {
			ok: false,
			detail: "pr_merged: set GITHUB_TOKEN or GH_TOKEN for GitHub API access",
		};
	}
	...
	try {
		...
		if (res.status === 404) {
			return {
				ok: false,
				detail: `pr_merged: PR not found (${parsed.owner}/${parsed.repo}#${parsed.number})`,
			};
		}
		if (!res.ok) {
		...
		if (body.merged === true) {
			return {
				ok: true,
				detail: `pr_merged: ${parsed.owner}/${parsed.repo}#${parsed.number} merged`,
			};
		}
		return {
			ok: false,
			detail: `pr_merged: ${parsed.owner}/${parsed.repo}#${parsed.number} not merged yet`,
		};
	} catch (e) {
...
async function runMechanicalVerification(
	...
	if (v.type === "pr_merged") {
		if (!cfg.allowPrVerification) {
			return {
				ok: false,
				detail: "pr_merged: skipped (allowPrVerification is false)",
			};
		}
	...
	if (v.type === "command_exit_zero") {
		...
		if (SHELL_DENY_RE.test(v.target)) {
			return {
				ok: false,
				detail:
					"command_exit_zero target contains disallowed shell metacharacters",
			};
		}
		try {
...
export async function runGoalHealthCheck(
	opts: GoalHealthCheckOptions,
): Promise<GoalHealthCheckResult> {
	const { goalsDir, cfg, workspaceRoot, logger, eventLog } = opts;
	const result: GoalHealthCheckResult = {
		goalsChecked: 0,
		goalsUpdated: 0,
		actions: [],
	};
	...
	for (const goal of goals) {
		...
		if (
			...
			goal.assessmentCount >= goal.maxAssessments
		) {
		...
		if (
			...
			await updateGoal(
				...
				goal.id,
				{
					status: "blocked",
					currentBlockers: [
						`Escalated after ${goal.consecutiveFailures} consecutive failures`,
					],
				},
				{
					timestamp: nowIso(),
			...
			if (pidMatch) {
				...
				if (!Number.isNaN(pid) && !isPidAlive(pid)) {
					const updatedTasks = g.linkedTasks.map((t) =>
						t.label === lt.label
							? { ...t, status: "failed", updatedAt: nowIso() }
							: t,
					);
					await updateGoal(
						goalsDir,
		...
		if (
			!reread3 ||
			isTerminalStatus(reread3.status) ||
			reread3.status === "blocked"
		)
			continue;
		g = reread3;
