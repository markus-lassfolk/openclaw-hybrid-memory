...
function formatAcquireBlockReason(
	issue: number,
	lease: DispatchLeaseRecord,
): string {
	if (isActiveState(lease.state)) {
...
function normalizeRegistry(raw: unknown): DispatchLeaseRegistry {
	...
	const leases: Record<string, DispatchLeaseRecord> = Object.create(
		null,
	) as Record<string, DispatchLeaseRecord>;
	...
	if (rawLeases && typeof rawLeases === "object" && !Array.isArray(rawLeases)) {
		for (const [key, value] of Object.entries(rawLeases)) {
			...
			if (typeof lease.issue !== "number" || !Number.isInteger(lease.issue))
				continue;
			...
			leases[key] = {
				...
				expiresAt:
					typeof lease.expiresAt === "string" ? lease.expiresAt : undefined,
				completedAt:
					typeof lease.completedAt === "string" ? lease.completedAt : undefined,
...
async function writeRegistry(
	filePath: string,
	registry: DispatchLeaseRegistry,
): Promise<void> {
...
export async function acquireDispatchLease(
	input: AcquireDispatchLeaseInput,
): Promise<AcquireDispatchLeaseResult> {
...
export async function transitionDispatchLease(
	input: TransitionDispatchLeaseInput,
): Promise<boolean> {
	const now = input.now ?? new Date();
	const nowIso = now.toISOString();
	...
	return withRegistryLock(input.stateDir, async (registryPath) => {
		...
		if (input.toState === "running") {
			...
			lease.expiresAt = new Date(
				now.getTime() + DEFAULT_COMPLETED_VISIBILITY_WINDOW_MS,
			).toISOString();
		} else {
...
export async function expireDispatchLeases(
	stateDir: string,
	now?: Date,
): Promise<number> {
	return withRegistryLock(stateDir, async (registryPath) => {
		const registry = await loadRegistry(registryPath);
		const before = Object.values(registry.leases).filter(
			(l) => l.state === "lease-expired",
		).length;
		const changed = expireActiveLeases(registry, now ?? new Date());
		if (!changed) return 0;
		const after = Object.values(registry.leases).filter(
			(l) => l.state === "lease-expired",
		).length;
		await writeRegistry(registryPath, registry);
...
export async function readDispatchLeaseRegistry(
	stateDir: string,
): Promise<DispatchLeaseRegistry> {
	const registryPath = join(stateDir, LEASES_FILE);
