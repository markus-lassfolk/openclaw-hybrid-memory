...
export interface RecallTimingLogger {
  ...
  phaseStarted: (phase: string, fields?: Record<string, RecallTimingValue>) => number;
  phaseCompleted: (phase: string, startedAtMs: number, fields?: Record<string, RecallTimingValue>) => number;
  event: (phase: string, event: "started" | "completed", fields?: Record<string, RecallTimingValue>) => void;
}
...
function toFieldParts(fields: Record<string, RecallTimingValue> | undefined): string[] {
  if (!fields) return [];
  const parts: string[] = [];
