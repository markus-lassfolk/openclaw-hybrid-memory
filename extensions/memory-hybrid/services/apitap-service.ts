...
export function validateUrl(url: string, cfg: ApiTapConfig): string | null {
	...
	if (cfg.allowedPatterns.length > 0) {
		const allowed = cfg.allowedPatterns.some((p) =>
			matchesPattern(normalizedUrl, p),
		);
		if (!allowed) {
...
export class ApitapService {
	...
	async capture(
		...
		return new Promise((resolve) => {
			...
			proc.on("close", (code) => {
				...
				if (code !== 0) {
					const errMsg =
						stderr.trim() || `apitap capture exited with code ${code}`;
					resolve({
						sessionId: "",
						siteUrl,
						endpoints: [],
	...
	async peek(siteUrl: string): Promise<ApitapCaptureResult> {
		...
		return new Promise((resolve) => {
			...
			proc.on("close", (code) => {
				...
				if (code !== 0) {
					const errMsg =
						stderr.trim() || `apitap peek exited with code ${code}`;
					resolve({
						sessionId: "",
						siteUrl,
						endpoints: [],
	...
	buildSkillScaffold(
		...
		const endpointSlug = endpoint
			...
			.replace(/^_|_$/g, "");
		const skillName =
			`${host.replace(/\./g, "_")}_${method.toLowerCase()}_${endpointSlug}`.slice(
				0,
				60,
			);
	...
	private parseCaptureOutput(
		...
		try {
			...
			const rawEndpoints = Array.isArray(parsed.endpoints)
				? (parsed.endpoints as unknown[]).slice(
						0,
						this.cfg.maxEndpointsPerSession,
					)
				: [];
