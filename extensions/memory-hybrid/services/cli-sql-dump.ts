...
function tableColumns(db: DatabaseSync, table: string): Set<string> {
  ...
  const rows = db.prepare(`PRAGMA table_info(${safe})`).all() as Array<{ name: string }>;
...
export type SqlDumpResult = { ok: true; table: string; rows: Record<string, unknown>[] } | { ok: false; error: string };

function truncateValue(key: string, value: unknown, table: string, maxLen: number): unknown {
...
export function runSqliteTableDump(
  ...
  const exists = db.prepare("SELECT 1 FROM sqlite_master WHERE type = 'table' AND name = ?").get(table) as
    | { 1?: number }
    | undefined;
  if (!exists) {
    return { ok: false, error: `Table "${table}" does not exist in this database (feature may be unused yet).` };
  }
  ...
  const rows = db.prepare(`SELECT * FROM ${qTable} ORDER BY ${qOrder} ${dir} LIMIT ?`).all(limit) as Record<
    string,
    unknown
  >[];
