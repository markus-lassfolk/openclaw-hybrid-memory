...
export function formatNarrativeRange(
	periodStart: number,
	periodEnd: number,
): string {
...
export function recallNarrativeSummaries(
	options: RecallNarrativeSummariesOptions,
): NarrativeSummaryMatch[] {
	const nowSec = options.nowSec ?? Math.floor(Date.now() / 1000);
	const sinceSec = options.sessionId
		? undefined
		: (options.sinceSec ?? nowSec - DEFAULT_LOOKBACK_DAYS * 86_400);
	const limit = Math.max(1, Math.floor(options.limit ?? 2));
	const scanLimit = Math.max(
		limit,
		Math.floor(options.scanLimit ?? DEFAULT_SCAN_LIMIT),
	);
	...
	const eventMatches = collectEventMatches(options.eventLog ?? null, {
		...
		maxEventsPerSession: Math.max(
			1,
			Math.floor(options.maxEventsPerSession ?? DEFAULT_EVENT_LIMIT),
		),
	});
...
function collectNarrativeMatches(
	...
	return rows
		.filter(
			(row) => options.sinceSec == null || row.periodEnd >= options.sinceSec,
		)
		.map((row) => ({
			id: row.id,
...
function summarizeEvents(events: EventLogEntry[], maxEvents: number): string {
	...
	return ordered.length > maxEvents
		? `${summary} Then ${ordered.length - maxEvents} more event(s) followed.`
		: summary;
}
...
function scoreCandidate(
	...
	const overlap =
		queryTerms.length > 0
			? queryTerms.filter((term) => normalizedText.includes(term)).length /
				queryTerms.length
			: 0;
...
function compareMatches(
	a: NarrativeSummaryMatch,
	b: NarrativeSummaryMatch,
): number {
