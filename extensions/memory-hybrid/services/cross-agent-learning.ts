	...
	try {
		const existing = db
         ...
         LIMIT 1`,
			...
			.get(CROSS_AGENT_SOURCE, `%${normalised.slice(0, 60)}%`) as
			| { id: string }
			| undefined;
		return !!existing;
	} catch {
...
function agentFactAlreadyGeneralised(
	factsDb: FactsDB,
	agentFactId: string,
): boolean {
	const db = factsDb.getRawDb();
...
async function callLLMForGeneralisation(
	...
	try {
		...
		logger.warn?.(
			`cross-agent-learning: failed to parse LLM response as JSON: ${text.slice(0, 200)}`,
		);
		return [];
	}
}
...
export async function runCrossAgentLearning(
	...
	try {
		const allLessons = collectAgentLessons(
			factsDb,
			cfg.windowDays,
			cfg.minSourceConfidence,
		);
		...
		const candidates = allLessons.filter(
			(l) => !agentFactAlreadyGeneralised(factsDb, l.factId),
		);
		...
		for (const batch of batches) {
			try {
				...
				const generalised = await callLLMForGeneralisation(
					openai,
					model,
					prompt,
					fallbackModels,
				...
				for (const lesson of generalised) {
					...
					const sourceAgentIds =
						lesson.sourceAgents ?? batch.map((b) => b.agentId);
					const sourceFacts = batch.filter((b) =>
						sourceAgentIds.includes(b.agentId),
					);

					const importance = Math.min(
						0.95,
						Math.max(0.5, lesson.importance ?? 0.7) + 0.1,
					);
 ...
 */
export function getCrossAgentFacts(
	factsDb: FactsDB,
	limit = 100,
): MemoryEntry[] {
	const db = factsDb.getRawDb();
		...
		if (candidates.length === 0) {
			...
			const generalRows = rows.filter((row) => {
				...
				const agentSpecificTags = tags.filter(
					(t) =>
						t !== CROSS_AGENT_TAG && t !== "*" && !t.startsWith("verified-by:"),
				);
...
export function formatBriefInjection(lessons: MemoryEntry[]): string {
	...
	for (const lesson of lessons) {
		...
		lines.push(
			`- ${lesson.text} (learned by ${sourceAgent}, confidence: ${confidence})`,
		);
	}
...
export async function verifyLessonForAgent(
	...
	try {
		const row = db
			.prepare(
				"SELECT id, confidence, tags FROM facts WHERE id = ? AND source = ? AND superseded_at IS NULL",
			)
			.get(lessonId, CROSS_AGENT_SOURCE) as
			| { id: string; confidence: number; tags: string | null }
			| undefined;
