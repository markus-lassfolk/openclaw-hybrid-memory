...
export const optionalBootstrapInstaller: OptionalBootstrapInstaller = {
	...
	install({ cfg, api, factsDb, resolvedSqlitePath }) {
		...
		if (
			cfg.identityReflection.enabled ||
			cfg.personaProposals.enabled ||
			cfg.identityPromotion.enabled
		) {
			const identityReflectionsPath = join(baseDir, "identity-reflections.db");
			identityReflectionStore = new IdentityReflectionStore(
				identityReflectionsPath,
			);
			api.logger.info(
				`memory-hybrid: identity reflections enabled (${identityReflectionsPath})`,
			);
		}
		...
		if (cfg.identityPromotion.enabled || cfg.personaProposals.enabled) {
			...
			api.logger.info(
				`memory-hybrid: persona state store enabled (${personaStatePath})`,
			);
		}
		...
		api.logger.info(
			`memory-hybrid: issue store initialized (${issueStorePath})`,
		);
		...
		api.logger.info(
			`memory-hybrid: apitap store initialized (${apitapStorePath})`,
		);
...
export function installOptionalBootstrapServices(
	context: OptionalBootstrapContext,
): OptionalBootstrapServices {
