...
class AliasVectorIndex {
	...
	private async doInitialize(): Promise<void> {
		...
		try {
			...
			pluginLogger.warn(
				`memory-hybrid: failed to delete alias schema seed row (non-fatal): ${deleteErr}`,
			);
		}
	}
	...
	private async validateSchema(): Promise<void> {
		try {
			...
			pluginLogger.warn(
				`memory-hybrid: alias LanceDB schema validation failed (non-fatal): ${err}`,
			);
		}
	}
	...
	private getTable(): lancedb.Table {
		if (!this.table) {
			throw new Error(
				"AliasVectorIndex not initialized. Call ensureInitialized() first.",
			);
		}
		return this.table;
	}
	...
	async store(entry: {
		...
		try {
			...
			pluginLogger.warn(
				`memory-hybrid: alias LanceDB store failed (non-fatal): ${err}`,
			);
		}
	}
	...
	async search(
		...
		try {
			...
			if (
				...
				err.message.includes(LANCE_NO_VECTOR_COL_MSG)
			) {
				this.schemaValid = false;
			} else {
				capturePluginError(err as Error, {
			...
			pluginLogger.warn(
				`memory-hybrid: alias LanceDB search failed (non-fatal): ${err}`,
			);
			return [];
		}
	}
	...
	async deleteByFactId(factId: string): Promise<void> {
		try {
			...
			if (!UUID_REGEX.test(factId)) {
				pluginLogger.warn(
					`memory-hybrid: skipping alias LanceDB delete for non-UUID factId: ${factId}`,
				);
				return;
			}
			...
			pluginLogger.warn(
				`memory-hybrid: alias LanceDB delete failed (non-fatal): ${err}`,
			);
		}
	}
...
export class AliasDB {
	...
	private deleteByFactIdInternal(factId: string): void {
		...
		const normalizedFactId = factId.toLowerCase();
		const res = this.db
			.prepare("DELETE FROM fact_aliases WHERE factId = ?")
			.run(normalizedFactId);
		if (this.aliasCountCache != null) {
			this.aliasCountCache = Math.max(
				0,
				this.aliasCountCache - Number(res.changes ?? 0),
			);
		}
	...
	async search(
		...
		try {
			...
			if (aliasCount >= 1000) {
				...
				try {
					const results = await this.aliasIndex.search(
						queryVector,
						limit,
						minScore,
					);
					return this.aliasIndex.isSchemaValid()
						? results
						: this.linearSearchInternal(queryVector, limit, minScore);
				} catch {
