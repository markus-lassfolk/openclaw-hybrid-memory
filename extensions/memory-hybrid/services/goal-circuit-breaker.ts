...
export function evaluateCircuitBreakerTrip(
	...
	if (
		cfg.sameBlockerRepeatLimit > 0 &&
		state.sameBlockerStreak >= cfg.sameBlockerRepeatLimit
	) {
	...
	if (cfg.maxAssessmentsWithoutProgress > 0) {
		const since =
			newAssessmentCount - state.circuitBreakerLastProgressAssessmentCount;
...
export function composeCircuitBreakerHumanSummary(
	...
	const lines: string[] = [
		...
		...(goal.linkedTasks.length > 0
			? goal.linkedTasks.map(
					(t) => `- ${t.label}: ${t.status} (${t.sessionKey ?? "no session"})`,
				)
			: ["- (none)"]),
		"",
	...
	lines.push(
		"",
		"— Automated triage: stop autonomous retries; a human should unblock or abandon this goal.",
	);

	return lines.join("\n");
}

export function circuitBreakerShortBlocker(
	reason: CircuitBreakerTripReason,
): string {
