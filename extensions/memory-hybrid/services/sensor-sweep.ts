...
async function sweepGarmin(
	...
	const result: SensorSweepResult = {
		sensor: "garmin",
		eventsWritten: 0,
		eventsSkipped: 0,
	};
	try {
		...
		const fp = computeFingerprint(
			`sensor.garmin:${prefix}:${stableStringify(fingerprintPayload)}`,
		);
...
function getSessionDir(): string {
	return (
		getEnv("OPENCLAW_SESSION_DIR") ?? join(homedir(), ".openclaw", "sessions")
	);
}
...
export async function sweepSessionHistory(
	...
	try {
		...
		const fp = computeFingerprint(
			`sensor.session-history:${limit}:${stableStringify(payload)}`,
		);
		if (bus.dedup(fp, cooldownHours)) {
			result.eventsSkipped++;
			return result;
...
export async function sweepMemoryPatterns(
	...
	try {
		...
		const staleCutoffSec = Math.floor(
			(Date.now() - staleAfterDays * 24 * 3600 * 1000) / 1000,
		);
		...
		const fp = computeFingerprint(
			`sensor.memory-patterns:${stableStringify(payload)}`,
		);
		if (bus.dedup(fp, cooldownHours)) {
			result.eventsSkipped++;
			return result;
...
export async function sweepGitHub(
	...
	try {
		...
		for (const pr of openPrs.slice(0, 5)) {
			...
			if (ciOutput) {
				try {
					const checks = JSON.parse(ciOutput) as Array<{
						name: string;
						bucket: string;
					}>;
		...
		const staleCutoff = new Date(
			Date.now() - staleIssueDays * 24 * 3600 * 1000,
		).toISOString();
		...
		const fp = computeFingerprint(
			`sensor.github:${repo}:${stableStringify(payload)}`,
		);
		if (bus.dedup(fp, cooldownHours)) {
			result.eventsSkipped++;
			return result;
...
export async function sweepSystemHealth(
	...
	try {
		...
		const fp = computeFingerprint(
			`sensor.system-health:${process.version}:${process.platform}`,
		);
		if (bus.dedup(fp, cooldownHours)) {
			result.eventsSkipped++;
			return result;
	...
	try {
		...
		try {
			...
			if (res.ok) {
				...
				const current = (json.current_condition as unknown[])?.[0] as
		...
		const fp = computeFingerprint(
			`sensor.weather:${location}:${stableStringify(weatherData)}`,
		);
		if (bus.dedup(fp, cooldownHours)) {
			result.eventsSkipped++;
			return result;
...
async function sweepYarbo(
	...
	try {
		...
		if (
			...
			entities.every((e) => e.state === "idle" || e.state === "off")
		) {
			return result;
		}
		...
		const fp = computeFingerprint(
			`sensor.yarbo:${prefix}:${stableStringify(fingerprintPayload)}`,
		);
		if (bus.dedup(fp, cooldownHours)) {
			result.eventsSkipped++;
			return result;
...
export async function sweepAll(
	...
	function shouldRun(name: string): boolean {
		if (normalizedSources !== null && !normalizedSources.includes(name))
			return false;
		return true;
	}
	...
	if (tier === 1 || tier === "all") {
		if (shouldRun("garmin") && cfg.garmin?.enabled && ha) {
			if (!opts.dryRun) {
				results.push(
					await sweepGarmin(bus, cfg.garmin, ha, cooldown, cachedHaStates),
				);
			} else {
		...
		if (shouldRun("session-history") && cfg.sessionHistory?.enabled) {
			if (!opts.dryRun) {
				results.push(
					await sweepSessionHistory(bus, cfg.sessionHistory, cooldown),
				);
			} else {
				results.push({
					sensor: "session-history",
		...
		if (shouldRun("memory-patterns") && cfg.memoryPatterns?.enabled) {
			if (!opts.dryRun) {
				results.push(
					await sweepMemoryPatterns(bus, cfg.memoryPatterns, factsDb, cooldown),
				);
			} else {
				results.push({
					sensor: "memory-patterns",
	...
	if (tier === 2 || tier === "all") {
		if (shouldRun("ha-anomaly") && cfg.homeAssistantAnomaly?.enabled && ha) {
			if (!opts.dryRun) {
				results.push(
					await sweepHomeAssistantAnomaly(
						bus,
						cfg.homeAssistantAnomaly,
						ha,
						cooldown,
					),
				);
		...
		if (shouldRun("system-health") && cfg.systemHealth?.enabled) {
			if (!opts.dryRun) {
				results.push(
					await sweepSystemHealth(
						bus,
						cfg.systemHealth,
						opts.resolvedSqlitePath ?? "",
						cooldown,
					),
				);
			} else {
		...
		if (shouldRun("yarbo") && cfg.yarbo?.enabled && ha) {
			if (!opts.dryRun) {
				results.push(
					await sweepYarbo(bus, cfg.yarbo, ha, cooldown, cachedHaStates),
				);
			} else {
