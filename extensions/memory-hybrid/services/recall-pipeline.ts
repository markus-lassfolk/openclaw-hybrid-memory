	...
	try {
		return await Promise.race([
			...
			new Promise<number[]>((_, reject) => {
				if (signal.aborted) {
					reject(
						Object.assign(new Error(abortMessage), { name: "AbortError" }),
					);
					return;
				}
				onAbort = () =>
...
export interface RecallPipelineDeps {
	factsDb: Pick<
		FactsDB,
		"search" | "getById" | "lookup" | "getSupersededTexts"
	>;
...
export async function runRecallPipelineQuery(
	...
	const runFtsSearchSync = (): {
		...
		if (opts?.entity) {
			const entityResults = factsDb
				.lookup(opts.entity, undefined, undefined, {
					scopeFilter: recallOpts.scopeFilter,
				})
				.slice(0, limitNum);
	...
	if (!useSemantic) {
		const ftsStartedAt = recallTiming.phaseStarted("fts_search", {
			limit: limitNum,
		});
		const t0 = Date.now();
		...
		const usePrecomputedVector =
			Boolean(_precomputedVector) && textToEmbed === trimmed;
		...
		const timeoutPromise = new Promise<never>((_, reject) => {
			timeoutId = setTimeout(() => {
				...
				reject(
					new Error(
						`${policy.mode} timed out after ${policy.vectorStepTimeoutMs}ms`,
					),
				);
		...
		const vectorRacePromise = Promise.race([vectorStepPromise, timeoutPromise])
			.catch((err: unknown) => {
				...
				if (isTimeout)
					logger.warn(`memory-hybrid: ${String(err)}, using FTS-only recall`);
				else {
					...
					logger.warn(
						`memory-hybrid: ${opts?.errorPrefix ?? ""}vector recall failed: ${err}`,
					);
				}
