...
export const CAPTURE_FILTER_PATTERNS = [
	...SENSITIVE_PATTERNS.slice(0, 3),
	/token/i,
	...SENSITIVE_PATTERNS.slice(4),
];
...
const CREDENTIAL_PATTERNS: Array<{
...
}> = [
	{
		...
		type: "bearer",
		hint: "Bearer/JWT token",
	},
	{
		regex: /sk-[A-Za-z0-9]{20,}/,
		type: "api_key",
		hint: "OpenAI-style API key (sk-...)",
	},
	{
...
export function detectCredentialPatterns(
	text: string,
): Array<{ type: string; hint: string }> {
...
export function extractCredentialMatch(
	text: string,
): { type: string; secretValue: string } | null {
...
export function isCredentialLike(
	...
	if (
		["api_key", "password", "token", "secret", "bearer"].some(
	...
	if (
		value &&
		value.length >= 8 &&
		/^(eyJ|sk-|ghp_|gho_|xox[baprs]-)/i.test(value)
	)
		return true;
	return (
		CREDENTIAL_PATTERNS.some((p) => p.regex.test(text)) ||
		CAPTURE_FILTER_PATTERNS.some((r) => r.test(text))
	);
}
...
export function tryParseCredentialForVault(
	...
	const typeFromPattern = (match?.type ?? "other") as
		| "token"
		| "password"
		| "api_key"
		| "ssh"
	...
	const valueValidation = validateCredentialValue(
		secretValue,
		typeFromPattern,
		hasPatternMatch,
	);
	...
	return {
		...
		secretValue,
		notes:
			text.length <= CREDENTIAL_NOTES_MAX_CHARS
				? text
				: truncateText(text, CREDENTIAL_NOTES_MAX_CHARS - 3, "..."),
	};
}
