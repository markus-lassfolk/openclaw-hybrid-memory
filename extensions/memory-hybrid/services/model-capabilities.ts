...
const CAPABILITIES: Array<{ match: Matcher; cap: ModelCapabilities }> = [
	...
	// ——— GPT-4o / GPT-4 Turbo (128k, 16k out) ———
	{
		match: (n) =>
			n.startsWith("gpt-4o") ||
			n.startsWith("gpt-4o-mini") ||
			(n.startsWith("gpt-4") && n.includes("turbo")),
		cap: {
			contextWindow: 128_000,
			maxOutputTokens: 16_384,
			batchTokenLimitForDistill: 80_000,
	...
	{
		match: (n) =>
			(n.includes("claude-opus-4-6") || n.includes("claude-sonnet-4-6")) &&
			!n.includes("4-5") &&
			!n.includes("4-1"),
		cap: {
	...
	// ——— DeepSeek (128k–163k) ———
	{
		match: (n) =>
			...
			n.includes("deepseek-v3.2") ||
			n.includes("deepseek-v3.1"),
		cap: {
...
export function getDistillBatchTokenLimit(model: string): number {
	...
	return (
		cap?.batchTokenLimitForDistill ??
		DEFAULT_CAPABILITIES.batchTokenLimitForDistill
	);
}
...
export function isReasoningModel(model: string): boolean {
	return modelPathSegments(model).some((seg) =>
		/^o[0-9]+(?:-[a-z]+)?$/.test(seg),
	);
}
 ...
 */
export function resolveWireApi(
	model: string,
