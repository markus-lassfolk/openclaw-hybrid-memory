...
function buildSkillContent(
  ...
  const primaryGoal =
    card.provenance.example_goals
      .filter((g) => g.trim().length > 0 && !isSystemWorkflowGoal(g))
...
> Review and refine before relying on this skill in production workflows.
...
export function buildExamplesText(exampleGoals: string[], skillName: string, toolSequence: string[]): string {
  const cleanedGoals = exampleGoals
    .map((g) => g.replace(/\n/g, " ").replace(/\s+/g, " ").trim())
    .filter((g) => g.length > 0 && !isSystemWorkflowGoal(g));
...
function buildProposalCard(
  ...
  const goals = filterUserFacingGoals(pattern.exampleGoals, cfg).slice(0, 5);
...
export function crystallizeSkill(input: CrystallizationInput, cfg: CrystallizationConfig): CrystallizationResult {
  ...
  // Exec-only scaffold scripts are omitted until concrete commands can be extracted safely from traces.
...
/** Build on-disk skill files for install (SKILL.md only until trace-derived commands exist). */
export function buildCrystallizationInstallFiles(
  _skillName: string,
  skillContent: string,
  _pattern: WorkflowPattern | undefined,
