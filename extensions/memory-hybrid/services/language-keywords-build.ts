 ...
 */
async function detectTopLanguages(
	samples: string[],
	openai: OpenAI,
	model: string,
): Promise<string[]> {
...
function normalizeExtraction(raw: unknown): LanguageExtractionTemplate | null {
	...
	if (o.decision && typeof o.decision === "object") {
		...
		const verbs = Array.isArray(d.verbs)
			? d.verbs.filter((x): x is string => typeof x === "string")
			: [];
	...
	if (o.choiceOver && typeof o.choiceOver === "object") {
		...
		const verbs = Array.isArray(c.verbs)
			? c.verbs.filter((x): x is string => typeof x === "string")
			: [];
		const rejectors = Array.isArray(c.rejectors)
			? c.rejectors.filter((x): x is string => typeof x === "string")
			: [];
		const connectors = Array.isArray(c.connectors)
			? c.connectors.filter((x): x is string => typeof x === "string")
			: [];
		if (verbs.length > 0 && rejectors.length > 0)
			out.choiceOver = { verbs, rejectors, connectors };
	}
	if (o.convention && typeof o.convention === "object") {
		...
		const always = Array.isArray(c.always)
			? c.always.filter((x): x is string => typeof x === "string")
			: [];
		const never = Array.isArray(c.never)
			? c.never.filter((x): x is string => typeof x === "string")
			: [];
		if (always.length > 0 || never.length > 0)
			out.convention = { always, never };
	}
	if (o.possessive && typeof o.possessive === "object") {
		...
		const possessiveWords = Array.isArray(p.possessiveWords)
			...
			: [];
		const isWords = Array.isArray(p.isWords)
			? p.isWords.filter((x): x is string => typeof x === "string")
			: [];
		if (possessiveWords.length > 0 && isWords.length > 0)
			out.possessive = { possessiveWords, isWords };
	}
	if (o.preference && typeof o.preference === "object") {
		const p = o.preference as Record<string, unknown>;
		const subject = Array.isArray(p.subject)
			? p.subject.filter((x): x is string => typeof x === "string")
			: [];
		const verbs = Array.isArray(p.verbs)
			? p.verbs.filter((x): x is string => typeof x === "string")
			: [];
		if (subject.length > 0 && verbs.length > 0)
			out.preference = { subject, verbs };
	}
	if (o.nameIntro && typeof o.nameIntro === "object") {
		const n = o.nameIntro as Record<string, unknown>;
		const verbs = Array.isArray(n.verbs)
			? n.verbs.filter((x): x is string => typeof x === "string")
			: [];
	...
	try {
		...
		for (const lang of toTranslate) {
			...
			if (Object.keys(out).length > 0)
				translations[lang] = out as Record<KeywordGroup, string[]>;
...
{"<langCode>": {"triggers": ["...", ...], "categoryDecision": [...], ..., ${KEYWORD_GROUPS.slice(
		-1,
	)
		.map((g) => `"${g}": [...]`)
		.join("")}}, ...}
	...
	try {
		...
		for (const lang of toTranslate) {
			...
			for (const g of KEYWORD_GROUPS) {
				const arr = data[g];
				if (Array.isArray(arr))
					out[g] = arr.filter((x): x is string => typeof x === "string");
			}
			if (Object.keys(out).length > 0)
				result[lang] = out as Record<KeywordGroup, string[]>;
		}
		return result;
	} catch (err) {
		capturePluginError(err instanceof Error ? err : new Error(String(err)), {
...
export async function runBuildLanguageKeywords(
	...
	if (toTranslate.length === 0 && samples.length > 0) {
		return {
			ok: true,
			path: join(sqliteDir, LANG_FILE_NAME),
			topLanguages: ["en"],
			languagesAdded: 0,
		};
	}

	const { translations, triggerStructures, extraction } =
		await generateIntentBasedLanguages(
	...
	for (const [fileKey, mergedKey] of Object.entries({
		...
		if (Array.isArray(list) && list.length > 0)
			directiveSignalsByCategory[fileKey] = list;
	}
	...
	for (const [fileKey, mergedKey] of Object.entries({
		...
		const list = merged[mergedKey as KeywordGroup];
		if (Array.isArray(list) && list.length > 0)
			reinforcementCategories[fileKey] = list;
	}
	reinforcementCategories.genericPoliteness = [
		"thanks",
		"thank you",
		"ok",
		"okay",
	...
	const data: LanguageKeywordsFile = {
		...
		translations,
		triggerStructures:
			Object.keys(triggerStructures).length > 0 ? triggerStructures : undefined,
		extraction: Object.keys(extraction).length > 0 ? extraction : undefined,
		directiveSignalsByCategory:
			Object.keys(directiveSignalsByCategory).length > 0
				? directiveSignalsByCategory
				: undefined,
		reinforcementCategories:
			Object.keys(reinforcementCategories).length > 0
				? reinforcementCategories
				: undefined,
	};
