 ...
 *
 * The storage layer (`backends/facts-db/generated-skills.ts`) is pure CRUD over
 * telemetry rows and procedure-row state; it no longer carries policy knowledge.
 * This service is the single place that evaluates thresholds, decides transitions,
 * and calls into the storage layer.
 *
...
export class GeneratedSkillLifecycleService {
  ...
  constructor(
    ...
    this.policy = { ...DEFAULT_GENERATED_SKILL_LIFECYCLE_POLICY, ...(policy ?? {}) };
  }
   ...
   */
  recordTelemetry(input: GeneratedSkillTelemetryRecordInput): GeneratedSkillTelemetryEntry {
  ...
  markFalsePositive(
    ...
    return markGeneratedSkillTelemetryFalsePositive(this.db, activationId, correctionReason, this.policy);
  }

  /**
   * List recent telemetry entries for a skill (or all skills).
   */
  listTelemetry(skillName?: string, limit = 50): GeneratedSkillTelemetryEntry[] {
  ...
  setState(
    ...
    return setGeneratedSkillLifecycleState(this.db, skillName, state, reason, at);
  }
  ...
  refresh(skillName: string, now?: number): ProcedureEntry | null {
    return refreshGeneratedSkillLifecycleState(this.db, skillName, this.policy, now);
  }
