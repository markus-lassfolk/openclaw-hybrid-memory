...
function generateParameterSchema(gap: DetectedGap): string {
  ...
  return JSON.stringify({ type: "object", properties: params, required: ["goal"] }, null, 2);
}
...
export class ToolProposer {
  ...
  runCycle(opts?: Partial<GapDetectorOptions>): ProposeCycleResult {
    ...
    if (!this.cfg.enabled) {
      result.reasons.push("Self-extension is disabled (selfExtension.enabled = false).");
      return result;
    }
    ...
    for (const gap of gaps) {
      const remaining = this.cfg.maxProposals - this.proposalStore.count("proposed");
      if (remaining <= 0) {
        result.skipped++;
        result.reasons.push(`Skipped gap "${gap.suggestedToolName}" — max proposals reached.`);
        continue;
      }
    ...
    if (gaps.length === 0) {
      result.reasons.push("No actionable gaps detected in current workflow traces.");
    }
   ...
   */
  approveProposal(id: string): { success: boolean; message: string; proposal?: ToolProposal } {
