...
} from "../backends/identity-reflection-store.js";
import type {
	PersonaStateEntry,
	PersonaStateStore,
} from "../backends/persona-state-store.js";
...
export function calculatePersonaInsightSimilarity(
	left: string,
	right: string,
): number {
	...
	if (leftTokens.size === 0 || rightTokens.size === 0) {
		return normalizePersonaInsight(left) === normalizePersonaInsight(right)
			? 1
			: 0;
	}
	let intersection = 0;
	...
	const bigramSimilarity = diceCoefficient(
		buildCharacterBigrams(left),
		buildCharacterBigrams(right),
	);
	return Math.max(tokenSimilarity, bigramSimilarity);
}

function inferPersonaTargetFile(
	questionKey: string,
	insight: string,
): IdentityFileType {
	const combined = `${questionKey} ${insight}`.toLowerCase();
	if (
		/\b(name|identity|avatar|creature|role|vibe|species|origin)\b/.test(
			combined,
		)
	) {
		return "IDENTITY.md";
...
function buildStateKey(questionKey: string, insight: string): string {
	const tokenSignature = uniqueStrings(tokenizeInsight(insight))
		.sort()
		.slice(0, 12)
		.join("-");
	const stablePart =
		tokenSignature || normalizePersonaInsight(insight).slice(0, 160);
...
export function collectPersonaPromotionCandidates(
	...
	const lookbackCutoff =
		Math.floor(Date.now() / 1000) - config.lookbackDays * 24 * 3600;
	const durable = reflections
		.filter(
			(entry) =>
				entry.durability === "durable" && entry.createdAt >= lookbackCutoff,
		)
	...
	for (const reflection of durable) {
		...
		if (!match) {
			clusters.push({
				...
				targetFile: inferPersonaTargetFile(
					reflection.questionKey,
					reflection.insight,
				),
		...
		if (
			...
			match.targetFile = inferPersonaTargetFile(
				reflection.questionKey,
				reflection.insight,
			);
		}
	...
	return clusters
		.map((cluster) => ({
			...
			averageConfidence:
				cluster.confidenceSum / Math.max(1, cluster.durableCount),
		...
		.filter(
			(candidate) =>
				candidate.durableCount >= config.minDurableReflections &&
				candidate.averageConfidence >= config.minConfidence,
		)
		.sort(
			(left, right) =>
				right.durableCount - left.durableCount ||
				right.averageConfidence - left.averageConfidence,
		)
		.slice(0, config.maxPromotionsPerRun);
}

export function buildPersonaStateInsightsBlock(
	entries: PersonaStateEntry[],
): string {
...
export function promotePersonaStateFromReflections(
	...
	const durableReflections = reflections.filter(
	...
	for (const candidate of candidates) {
		const matchingEntry = existingEntries.find(
			(entry) =>
				...
				calculatePersonaInsightSimilarity(entry.insight, candidate.insight) >=
					config.similarityThreshold,
		);
		if (matchingEntry) {
