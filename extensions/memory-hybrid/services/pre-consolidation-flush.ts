...
export async function runPreConsolidationFlush(
	...
	try {
		const result = await replayWalEntries(
			deps.wal,
			deps.factsDb,
			deps.vectorDb,
			deps.embeddings,
		...
		if (result.committed > 0 || result.skipped > 0) {
			logger.info?.(
				`memory-hybrid: ${phase} — WAL replay: ${result.committed} committed, ${result.skipped} skipped`,
			);
		}
		return result;
	} catch (err) {
		logger.warn?.(
			`memory-hybrid: ${phase} — WAL replay failed (non-fatal): ${String(err)}`,
		);
