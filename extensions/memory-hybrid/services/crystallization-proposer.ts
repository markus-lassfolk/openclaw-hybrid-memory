...
export class CrystallizationProposer {
	...
	runCycle(): ProposeResult {
		...
		if (approvedCount >= this.cfg.maxCrystallized) {
			return {
				...
				skipped: 0,
				reasons: [
					`maxCrystallized limit reached (${this.cfg.maxCrystallized})`,
				],
			};
		}
			...
			try {
				...
				if (this.cfg.autoApprove) {
					...
					const currentApprovedCount =
						this.crystallizationStore.count("approved");
					if (currentApprovedCount >= this.cfg.maxCrystallized) {
						skipped++;
						reasons.push(
							`Skipped '${result.skillName}': maxCrystallized limit reached (${this.cfg.maxCrystallized})`,
						);
						continue;
					}
					...
					this.crystallizationStore.approve(
						proposal.id,
						result.proposedOutputPath,
					);
				...
				reasons.push(
					`Error processing candidate ${candidate.patternId}: ${msg}`,
				);
	...
	approveProposal(proposalId: string): ApproveResult {
		...
		const outputDir = this.cfg.outputDir.replace(
			/^~/,
			getEnv("HOME") || homedir(),
		);
		const safeName = proposal.skillName
			.replace(/[^a-z0-9_-]/gi, "-")
			.replace(/^\.+/, "");
