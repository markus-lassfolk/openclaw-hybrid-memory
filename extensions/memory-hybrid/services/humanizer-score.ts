	...
	try {
		...
		if (
			...
			for (const [k, v] of Object.entries(
				obj.category_breakdown as Record<string, unknown>,
			)) {
				if (typeof v === "number") {
		...
		if (typeof obj.uniformity_score === "number")
			rawOutput.uniformity_score = obj.uniformity_score;
		if (typeof obj.pattern_score === "number")
			rawOutput.pattern_score = obj.pattern_score;
...
export async function runHumanizerScore(
	...
	const truncated =
		trimmed.length > cfg.maxTextLength
			...
			child.stdin.on("error", (e) => {
				...
				if ((e as NodeJS.ErrnoException).code !== "EPIPE") {
					capturePluginError(e, {
						operation: "humanizer-stdin-write",
						subsystem: "humanizer",
						severity: "info",
					});
				}
