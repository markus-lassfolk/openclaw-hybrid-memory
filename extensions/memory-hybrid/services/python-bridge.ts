...
export class PythonBridge {
	...
	private spawnProcess(): void {
		...
		if (
			...
			typeof (
				stdin as {
					on?: (ev: string, fn: (err: NodeJS.ErrnoException) => void) => void;
				}
			).on === "function"
		) {
			(
		...
		this.proc.stderr.on("data", (data: Buffer) => {
			if (this.pending.size > 0) {
				...
				if (msg.length > 0) {
					capturePluginError(
						new Error(`[python-bridge] stderr: ${msg.slice(0, 200)}`),
						{
							subsystem: "python-bridge",
							operation: "stderr",
						},
	...
	private handleLine(line: string): void {
		...
		if (resp.error) {
			req.reject(
				new Error(`Python RPC error ${resp.error.code}: ${resp.error.message}`),
			);
		} else {
	...
	private async ensureStarted(): Promise<void> {
		if (this.startupFailed) {
			throw new Error(
				"Python bridge startup previously failed and will not retry",
			);
		}
		...
		while (true) {
			try {
				...
				if (this.restartCount > PYTHON_BRIDGE_MAX_RETRIES) {
					...
					throw new Error(
						`Python bridge failed to start after ${PYTHON_BRIDGE_MAX_RETRIES} retries: ${err}`,
					);
				}
	...
	private send<T>(
		...
		return new Promise<T>((resolve, reject) => {
			const timer = setTimeout(() => {
				...
				reject(
					new Error(
						`Python RPC timeout after ${timeoutMs}ms (method=${method})`,
					),
				);
			}, timeoutMs);
			this.pending.set(id, {
	...
	async shutdown(): Promise<void> {
		...
		try {
			await Promise.race([
				this.send<{ ok: boolean }>(
					"shutdown",
					{},
					PYTHON_BRIDGE_SHUTDOWN_WAIT_MS,
				),
				new Promise<void>((resolve) =>
					setTimeout(resolve, PYTHON_BRIDGE_SHUTDOWN_WAIT_MS),
				),
			]);
		} catch {
