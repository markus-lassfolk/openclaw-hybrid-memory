...
export function formatGoalsMirrorSection(goals: Goal[]): string {
	...
	for (const g of goals) {
		lines.push(
			`### [${g.label}]: ${g.description.slice(0, 200)}${g.description.length > 200 ? "…" : ""}`,
		);
...
export async function refreshActiveTaskMirrorWithGoals(opts: {
	...
	try {
		const parsed = await readActiveTaskFile(
			opts.activeTaskPath,
			opts.staleMinutes,
		);
		...
		opts.logger?.warn?.(
			`memory-hybrid: ACTIVE-TASKS.md mirror refresh failed: ${msg}`,
		);
