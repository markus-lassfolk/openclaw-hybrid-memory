...
function labelFromEntry(entry: MemoryEntry): string {
	...
	if (summary && summary !== entry.text.trim())
...
export function buildMemoryIndexSnapshot(
	...
	const recentWindowDays = Math.min(
		90,
		Math.max(1, options.recentWindowDays ?? 30),
	);
	const allFacts = factsDb.getAll();
	const recentCutoffSec =
		Math.floor(Date.now() / 1000) - recentWindowDays * 86400;
	const recentFacts = allFacts.filter(
		(entry) => (entry.sourceDate ?? entry.createdAt) >= recentCutoffSec,
	);
	const liveFactIds = new Set(allFacts.map((f) => f.id));
	const filteredFactsDb = {
		getAllLinkedFactIds: () =>
			factsDb.getAllLinkedFactIds().filter((id) => liveFactIds.has(id)),
		getAllLinks: () =>
			factsDb
				.getAllLinks()
				.filter(
					(l) =>
						liveFactIds.has(l.sourceFactId) && liveFactIds.has(l.targetFactId),
				),
	...
	const clusters = clusterResult.clusters
		...
		.map((cluster) => {
			...
			const lastActiveSec = entries.reduce(
				(maxSec, entry) =>
					Math.max(maxSec, entry.sourceDate ?? entry.createdAt),
				0,
			);
			const entityCount = new Set(
				entries.map((entry) => entry.entity?.trim()).filter(Boolean),
			).size;
			return {
				label: cluster.label,
	...
	const recentDecisions = takeLiveRecent(
		factsDb.listFactsByCategory("decision", MAX_DECISIONS * 3),
		MAX_DECISIONS,
	).map((entry) => ({
	...
	const recentPatterns = [
		...takeLiveRecent(
			factsDb.listFactsByCategory("pattern", MAX_PATTERNS * 2),
			MAX_PATTERNS,
		),
		...takeLiveRecent(
			factsDb.listFactsByCategory("rule", MAX_PATTERNS * 2),
			MAX_PATTERNS,
		),
	]
		.sort(
			(a, b) => (b.sourceDate ?? b.createdAt) - (a.sourceDate ?? a.createdAt),
		)
	...
	const keyEntities = [...entityStats.entries()]
		...
		.map(([entity, stats]) => ({
			...
			importance: importanceBand(
				stats.importanceTotal / Math.max(1, stats.mentions),
			),
		}));
...
export function renderMemoryIndexMarkdown(
	...
	const decisionLines =
		snapshot.recentDecisions.length > 0
			? snapshot.recentDecisions.map(
					(decision) =>
						`- ${decision.date}: ${decision.label} [${decision.ref}]`,
				)
			: ["- none"];
	...
	const patternLines =
		snapshot.recentPatterns.length > 0
			? snapshot.recentPatterns.map(
					(pattern) => `- ${pattern.date}: ${pattern.label} [${pattern.ref}]`,
				)
			: ["- none"];
...
function sanitizeIndexMarkdown(content: string): string {
	...
	if (
		!trimmed.includes("## Active Clusters") ||
		!trimmed.includes("## Recent Decisions")
	)
		return "";
	return trimmed.length > MAX_OUTPUT_CHARS
		? `${trimmed.slice(0, MAX_OUTPUT_CHARS).trimEnd()}\n`
		: `${trimmed}\n`;
}
...
async function synthesizeMemoryIndex(
	...
	try {
		...
		logger.warn(
			`memory-hybrid: memory-index — synthesis failed, using fallback: ${err}`,
		);
...
function resolveOutputPath(
	options: Pick<MemoryIndexOptions, "workspaceRoot" | "outputPath">,
): string {
	if (options.outputPath) return options.outputPath;
	const workspaceRoot =
		options.workspaceRoot ?? getEnv("OPENCLAW_WORKSPACE") ?? process.cwd();
...
export async function writeMemoryIndex(
	...
	const llmMarkdown = await synthesizeMemoryIndex(
		snapshot,
		openai,
		options,
		logger,
