...
export type ActiveTaskCanonicalBackfillResult = {
  scannedFacts: number;
...
function mergeCanonicalLabelProvenance(fact: MemoryEntry, canonical: string): string | null {
  let base: Record<string, unknown> = {};
  if (fact.provenanceJson?.trim()) {
    try {
      const parsed = JSON.parse(fact.provenanceJson) as unknown;
      if (parsed && typeof parsed === "object" && !Array.isArray(parsed)) {
        base = { ...(parsed as Record<string, unknown>) };
      }
    } catch {
      // Ignore malformed provenance and rebuild canonical fields.
    }
  }
  const activeTask =
    base.activeTask && typeof base.activeTask === "object" && !Array.isArray(base.activeTask)
      ? { ...(base.activeTask as Record<string, unknown>) }
      : {};
  if (activeTask.canonicalLabel === canonical && base.canonical_label === canonical) {
    return null;
  }
  activeTask.canonicalLabel = canonical;
  base.activeTask = activeTask;
  base.canonical_label = canonical;
  return JSON.stringify(base);
}

function chooseCanonicalKeeper(key: string, rows: MemoryEntry[]): MemoryEntry {
  if (key === "status") {
    const ranked = [...rows].sort((a, b) => {
      const aTerminal = isTerminalFactStatus(a.value ?? "");
      const bTerminal = isTerminalFactStatus(b.value ?? "");
...
export function backfillActiveTaskCanonicalLabels(
  ...
  const limit = Number.isFinite(opts.limit) && (opts.limit ?? 0) > 0 ? Math.trunc(opts.limit ?? 0) : 50_000;
  ...
  for (const fact of activeFacts) {
    ...
    if (!canonical) continue;
    const merged = mergeCanonicalLabelProvenance(fact, canonical);
    const needsSourceBackfill = fact.source !== "active-task";
    if (merged) {
      canonicalLabelsUpdated++;
      if (!dryRun) {
        updateProvenanceAndSource.run(merged, fact.id);
      }
      continue;
    }
    if (!dryRun && needsSourceBackfill) {
      updateSourceOnly.run(fact.id);
    }
  }

  const byCanonical = new Map<string, MemoryEntry[]>();
  for (const fact of activeFacts) {
    const canonical = canonicalLabel(fact.entity ?? "");
    if (!canonical) continue;
    ...
    if (rows) {
      ...
      byCanonical.set(canonical, [fact]);
    }
  }
  ...
  for (const [canonical, rows] of byCanonical) {
    ...
    supersededFacts += groupSuperseded;
