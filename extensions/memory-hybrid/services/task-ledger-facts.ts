...
export function groupProjectFactsByEntity(
	facts: MemoryEntry[],
): Map<string, Map<string, MemoryEntry>> {
...
function createdBoundsFromKeyMap(
	keyMap: Map<string, MemoryEntry>,
...
function resolveTaskStarted(
	f: Record<string, string>,
	bounds: ReturnType<typeof createdBoundsFromKeyMap>,
): string {
	return (
 ...
 */
export function buildTaskEntriesFromGroupedFacts(
	byEntity: Map<string, Map<string, MemoryEntry>>,
): {
	...
	const sorted = [...byEntity.entries()].sort((a, b) =>
...
export function applyActiveTaskProjectionFilters(
	...
	if (config.titleMinChars > 0) {
		out = out.filter(
			(e) => e.description.trim().length >= config.titleMinChars,
		);
	}
	...
	for (const e of out) {
		const key =
			config.dedupeBy === "label"
				? e.label.trim().toLowerCase()
				: normalizeActiveTaskTitleForDedupe(e.description);
...
function capRows<T extends ActiveTaskEntry>(
	rows: T[],
	max?: number,
...
export async function upsertProjectTaskKey(
	...
	try {
		...
		log?.warn?.(
			`memory-hybrid: active-task ledger vector store failed: ${err}`,
		);
	}
}
...
export async function syncActiveTaskEntryToFacts(
	...
	await upsertProjectTaskKey(
		...
		entity,
		"status",
		displayStatusToFact(entry.status),
		log,
	);
	await upsertProjectTaskKey(
		factsDb,
...
export async function renderActiveTaskMarkdownFile(
	...
	if (projection.sectioned) {
		...
		const cappedCombined = capRows(
			combinedActive,
			projection.maxRowsPerSection,
		);
...
export async function consumePendingTaskSignalsFacts(
	...
	try {
		...
		logger?.warn?.(
			`memory-hybrid: failed to read pending task signals: ${err}`,
		);
		return;
	}
	...
	const { active: rawActive, completed: rawCompleted } =
		loadTaskLedgerFromFacts(factsDb);
	...
	const findMatchingTask = (
		...
		const byDescription = activeEntries.filter(
			(t) => t.description === signal.taskRef,
		);
	...
	for (const signal of signals) {
		try {
			...
			if (signal.signal === "completed") {
				const { updated, completed } = completeTask(
					updatedActive,
					existing.label,
				);
				if (completed) {
			...
			logger?.warn?.(
				`memory-hybrid: failed to process signal from ${signal._filePath}: ${err}`,
			);
		}
	}
	...
	if (processedSignals.length === 0) {
		if (expiredSignals.length > 0) {
			...
			logger?.info?.(
				`memory-hybrid: pruned ${expiredSignals.length} expired task signal(s) (facts ledger)`,
			);
		}
		return;
	}
	...
	try {
		...
		logger?.warn?.(
			`memory-hybrid: failed to persist facts after task signals: ${err}`,
		);
		return;
	}
	...
	logger?.info?.(
		`memory-hybrid: consumed ${processedSignals.length} pending task signal(s) into facts ledger`,
	);
}
