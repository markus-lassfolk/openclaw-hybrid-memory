...
export function expandGraph(
	...
	const seedScoreMap = new Map<string, number>(
		seedResults.map((s) => [s.factId, s.score]),
	);
	...
	if (factsDb.expandGraphWithCTE) {
		const expansionRows = factsDb.expandGraphWithCTE(
			seedIds,
			maxDepth,
			getByIdOpts,
		);
		...
		for (const row of expansionRows) {
			...
			const decay =
				HOP_SCORE_DECAY[row.hopCount] ??
				HOP_SCORE_DECAY[HOP_SCORE_DECAY.length - 1];
		...
		function tryAddNode(
			...
			if (
				hop === existing.hopCount &&
				newSeedScore > (seedScoreMap.get(existing.seedId) ?? 0)
			) {
				nodeMeta.set(candidateId, { hopCount: hop, steps, seedId });
			}
		}
		...
		for (let hop = 1; hop <= maxDepth && frontier.length > 0; hop++) {
			...
			const candidateIds = Array.from(
				new Set(candidateMoves.map((m) => m.candidateId)),
			);
...
export function formatLinkPath(linkPath: LinkPathStep[]): string {
	...
	return linkPath
		.map(
			(step) =>
				`via ${step.linkType} from ${step.fromFactId.slice(0, 8)}\u2026`,
		)
		.join(" \u2192 ");
}
