...
export class UnconfiguredProviderError extends Error {
	...
	constructor(provider: string, model: string, detail?: string) {
		...
		const hint =
			detail ?? `Set llm.providers.${provider}.apiKey in plugin config.`;
		super(`${base} ${hint}`);
...
export function is404Like(err: unknown): boolean {
	...
	if (err instanceof Error) {
		...
		return (
			/\bHTTP\s+404\b|\b404\b.*not\s+found|model.*not\s+found|not\s+found.*model/i.test(
				err.message,
			) ||
...
export function is403Like(err: unknown): boolean {
	...
	if (err instanceof Error) {
		...
		if (
			...
			/\bHTTP\s+403\b|\bError\s+code:\s*403\b|\b403\s+[A-Za-z]/i.test(
				err.message,
			)
		) {
			return true;
		...
		if (
			/\bcountry,\s*region,\s*or\s*territory\s+not\s+supported\b/i.test(
				err.message,
			)
		)
			return true;
		...
		if (
			/\baccess\s+(denied|forbidden)\b/i.test(err.message) &&
			!/\b(file|directory|path|disk)\b/i.test(err.message)
		)
			return true;
	}
	return false;
...
function is401Like(err: unknown): boolean {
	...
	if (err instanceof Error) {
		...
		if (
			/incorrect api key|invalid api key|authentication failed/i.test(
				err.message,
			)
		)
			return true;
...
function is429Like(err: unknown): boolean {
	...
	if (err instanceof Error) {
		return (
			...
			/\bHTTP\s+429\b|\bError\s+code:\s*429\b|\b429\s+[A-Za-z]/i.test(
				err.message,
			) ||
...
export function is500Like(err: unknown): boolean {
	...
	if (err instanceof Error) {
		...
		if (
			/\bHTTP\s+5\d{2}\b|\b5\d{2}\s+(internal\s+)?(error|status)|status\s+5\d{2}|internal\s+server\s+error/i.test(
				m,
			)
		) {
			return true;
...
export function isConnectionErrorLike(err: unknown): boolean {
	...
	for (const candidate of candidates) {
		...
		if (
			typeof code === "string" &&
			/ECONNREFUSED|ECONNRESET|ETIMEDOUT|ENOTFOUND|EHOSTUNREACH|UND_ERR_CONNECT_TIMEOUT/i.test(
				code,
			)
		) {
			return true;
	...
	if (err instanceof Error) {
		...
		return (
			((msg.includes("400") || msg.includes("bad request")) &&
				(msg.includes("context length") ||
					msg.includes("maximum context") ||
					/max.*token.*(length|limit)|token limit|context.length/i.test(
						err.message,
					))) ||
 ...
 */
function enrichLlmErrorMessage(
	err: Error,
	ctx?: { model?: string; operation?: string },
): Error {
...
export function isAbortOrTransientLlmError(err: unknown): boolean {
	...
	if (err && typeof err === "object" && "cause" in err) {
		...
		if (c !== undefined && c !== null && isAbortOrTransientLlmError(c))
			return true;
	}
	if (!(err instanceof Error)) {
	...
	if (
		/request was aborted|Request was aborted|The operation was aborted|operation was aborted/i.test(
			msg,
		)
	)
		return true;
	if (
		/gateway client stopped|gateway not reachable|not reachable\.|is it running/i.test(
			msg,
		)
	)
		return true;
...
export async function withLLMRetry<T>(
	...
	for (let attempt = 0; attempt <= maxRetries; attempt++) {
		try {
			...
			const isReasoningSequenceError =
				isResponsesReasoningSequenceError(lastError);
			...
			const isTimeout =
				/timed out|llm request timeout|request was aborted|Request was aborted/i.test(
					lastError.message,
			...
			if (attempt === maxRetries || opts?.signal?.aborted) {
				...
				if (!isTransient) {
					capturePluginError(retryError, {
						...
						...(opts?.llmContext?.model
							? { configShape: { model: opts.llmContext.model } }
							: {}),
					});
				}
				throw retryError;
...
export async function chatCompleteWithRetry(opts: {
	...
	for (let i = 0; i < modelsToTry.length; i++) {
		...
		const effectiveMaxTokens =
			maxTokens ?? distillMaxOutputTokens(currentModel);
		...
		try {
			...
			const isUnconfigured =
				...
				(lastError instanceof LLMRetryError &&
					lastError.cause instanceof UnconfiguredProviderError);
	...
	const finalIsReasoningSequence =
		isResponsesReasoningSequenceError(finalError); // #1034
	...
	if (unconfiguredCount > 0 && unconfiguredCount === modelsToTry.length) {
		...
		const finalIsUnconfigured =
			...
			(finalError instanceof LLMRetryError &&
				finalError.cause instanceof UnconfiguredProviderError);
		if (
			!finalIs500 &&
			!finalIsOOM &&
