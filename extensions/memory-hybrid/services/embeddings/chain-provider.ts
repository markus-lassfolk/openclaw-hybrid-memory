...
export class ChainEmbeddingProvider implements EmbeddingProvider {
	...
	get activeProvider(): string {
		...
		return (
			this.providers[this.activeIndex]?.activeProvider ??
	...
	constructor(providers: EmbeddingProvider[], labels: string[]) {
		if (providers.length === 0 || providers.length !== labels.length) {
			throw new Error(
				"ChainEmbeddingProvider requires non-empty providers and same-length labels",
			);
		}
		const dim = providers[0].dimensions;
		if (providers.some((p) => p.dimensions !== dim)) {
			throw new Error(
				"ChainEmbeddingProvider: all providers must have the same dimensions",
			);
		}
		...
		while (currentIndex < this.providers.length) {
			...
			try {
				...
				if (isConfigError(asErr)) {
					this.failedUntil.set(currentIndex, {
						expiry: Date.now() + ChainEmbeddingProvider.COOLDOWN_MS,
						error: asErr,
					});
				}
	...
	async embedBatch(texts: string[]): Promise<number[][]> {
		return this.tryProviders(
			(provider) => provider.embedBatch(texts),
			"embedBatch",
		);
	}
}
