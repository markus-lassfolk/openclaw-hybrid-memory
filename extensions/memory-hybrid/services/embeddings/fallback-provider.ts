...
export class FallbackEmbeddingProvider implements EmbeddingProvider {
	...
	constructor(
		...
		if (!Number.isFinite(retryIntervalMs) || retryIntervalMs <= 0) {
			throw new Error(
				`FallbackEmbeddingProvider: retryIntervalMs must be a finite number > 0, got ${retryIntervalMs}`,
			);
		}
	...
	async embed(text: string): Promise<number[]> {
		...
		if (
			this.switched &&
			Date.now() - this.lastRetryAttempt >= this.retryIntervalMs
		) {
			const recovered = await this.tryReturnToPrimary(
				(p) => p.embed(text),
				"embed",
			);
