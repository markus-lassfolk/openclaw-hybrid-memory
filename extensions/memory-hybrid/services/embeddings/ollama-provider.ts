 ...
 */
const _ollamaCircuitByEndpoint = new Map<
	string,
	{ failCount: number; disabledUntil: number }
>();

function _getOllamaCircuit(endpoint: string): {
	failCount: number;
	...
	async embedBatch(texts: string[]): Promise<number[][]> {
		...
		for (let i = 0; i < texts.length; i += this.batchSize) {
			...
			if (data.embeddings.length === 0) {
				throw new Error(
					`Ollama embed returned empty 'embeddings' array (expected ${batch.length})`,
				);
			}
			if (data.embeddings.length !== batch.length) {
				throw new Error(
					`Ollama embed returned ${data.embeddings.length} embeddings for ${batch.length} inputs`,
				);
			}
