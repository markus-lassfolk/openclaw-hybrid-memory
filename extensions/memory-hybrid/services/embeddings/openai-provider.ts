...
function vectorFromEmbeddingResponse(
	...
	const row0 = Array.isArray(data)
		? (data[0] as { embedding?: number[] } | undefined)
		: undefined;
...
export class Embeddings implements EmbeddingProvider {
	...
	constructor(
		...
		this.endpoint =
			maybeClient.baseURL ?? maybeClient._options?.baseURL ?? "default";
		this.models = Array.isArray(modelOrModels)
			? modelOrModels
			: [modelOrModels];
		if (this.models.length === 0)
			throw new Error("Embeddings requires at least one model");
		...
		for (const model of this.models) {
			...
			if (maxDim !== undefined && this.dimensions > maxDim) {
				throw new Error(
					`Dimensions ${this.dimensions} exceed maximum ${maxDim} for model ${effective}`,
				);
			}
		...
		try {
			...
			for (const model of this.models) {
				...
				try {
					...
					const includeDims =
						supportsDimensions && !this.omitDimensionsInRequest;
					...
					const vector = vectorFromEmbeddingResponse(
						resp,
						this.providerName,
						this.endpoint,
						model,
	...
	async embedBatch(texts: string[]): Promise<number[][]> {
		...
		try {
			for (let i = 0; i < uncachedTexts.length; i += this.batchSize) {
				...
				let lastErr: Error | undefined;
				let resp:
					| Awaited<ReturnType<typeof this.client.embeddings.create>>
					| undefined;
				...
				for (const model of this.models) {
					try {
						...
						const supportsDimensions =
							effective.startsWith("text-embedding-3-");
				...
				if (resp !== undefined && succeededModel !== undefined) {
					...
					for (let j = 0; j < batch.length; j++) {
						...
						this.cache.set(
							makeCacheKey(succeededModel, uncachedTexts[i + j]),
							sorted[j],
						);
					}
