...
const DEFAULT_ONNX_CACHE_DIR = join(
	homedir(),
	".cache",
	"openclaw",
	"onnx-embeddings",
...
const ONNX_MODEL_SPECS: Record<
	string,
	{ repo: string; modelFile: string; vocabFileCandidates: string[] }
> = {
	"all-MiniLM-L6-v2": {
...
export function isOnnxRuntimeMissingError(
	err: unknown,
): err is OnnxRuntimeMissingError {
	return (
		err instanceof OnnxRuntimeMissingError ||
		(err instanceof Error &&
			(err as Error & { code?: string }).code === "ONNX_RUNTIME_MISSING")
	);
}
...
export function __setOnnxRuntimeLoaderForTests(
	loader: OnnxRuntimeLoader | null,
): void {
...
function splitRepoAndRevision(input: string): {
	repo: string;
	revision: string;
} {
...
async function downloadFile(url: string, destPath: string): Promise<void> {
	...
	if (contentLength) {
		...
		if (!Number.isNaN(len) && len > ONNX_DOWNLOAD_MAX_BYTES) {
			throw new Error(
				`Refusing to download ${url}: size ${len} exceeds limit ${ONNX_DOWNLOAD_MAX_BYTES}`,
			);
		}
	}
		...
		const countStream = new Transform({
			transform(chunk: Buffer, _enc, cb) {
				...
				if (bytesWritten > ONNX_DOWNLOAD_MAX_BYTES) {
					cb(
						new Error(`Download size exceeds limit ${ONNX_DOWNLOAD_MAX_BYTES}`),
					);
					return;
				}
				cb(null, chunk);
...
async function resolveOnnxModelFiles(
	...
	const vocabCandidates = spec?.vocabFileCandidates ?? [
		"vocab.txt",
		"tokenizer/vocab.txt",
	];
	...
	if (!vocabPath) {
		throw new Error(
			`Unable to locate or download vocab.txt for model '${model}'.`,
		);
	}
	return { modelPath, vocabPath };
}
...
class WordPieceTokenizer {
	...
	encode(text: string, maxLen: number): TokenizedInput {
		const tokens = basicTokenize(text).flatMap((t) =>
			wordPieceTokenize(t, this.vocab, this.unkToken),
		);
		const maxTokens = Math.max(0, maxLen - 2);
		const trimmed =
			tokens.length > maxTokens ? tokens.slice(0, maxTokens) : tokens;
		const finalTokens = [this.clsToken, ...trimmed, this.sepToken];
		const inputIds = finalTokens.map(
			(t) => this.vocab.get(t) ?? this.vocab.get(this.unkToken)!,
		);
...
export class OnnxEmbeddingProvider implements EmbeddingProvider {
		...
		if (!this.ready) {
			this.ready = (async () => {
				try {
					...
					const { modelPath, vocabPath } = await resolveOnnxModelFiles(
						this.modelName,
						{
	...
	async embedBatch(texts: string[]): Promise<number[][]> {
		...
		for (let i = 0; i < texts.length; i += this.batchSize) {
			...
			feeds.input_ids = new this.ort.Tensor("int64", inputIds, [
				batchSize,
				maxLen,
			]);
			if (inputNames.includes("attention_mask")) {
				feeds.attention_mask = new this.ort.Tensor("int64", attentionMask, [
					batchSize,
					maxLen,
				]);
			}
			if (inputNames.includes("token_type_ids")) {
				feeds.token_type_ids = new this.ort.Tensor("int64", tokenTypeIds, [
					batchSize,
					maxLen,
				]);
			}
			...
			if (!outputTensor) {
				throw new Error(
					"ONNX output missing sentence_embedding/pooler_output/last_hidden_state",
				);
			}
			...
			if (outputTensor === output.last_hidden_state) {
				const [b, s, h] = dims;
				if (!b || !s || !h)
					throw new Error("ONNX last_hidden_state has invalid dims");
			...
			if (vectors.length !== batchSize) {
				throw new Error(
					`ONNX embed returned ${vectors.length} embeddings for ${batchSize} inputs`,
				);
			}
			for (const vec of vectors) {
				...
				if (normalized.length !== this.dimensions) {
					throw new Error(
						`ONNX embedding dimension mismatch: expected ${this.dimensions}, got ${normalized.length}`,
					);
				}
