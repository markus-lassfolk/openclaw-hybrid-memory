...
export const coreBootstrapInstaller: CoreBootstrapInstaller = {
	...
	install({ cfg, api, resolvedSqlitePath, resolvedLancePath }) {
		...
		try {
			...
			api.logger.error(
				`memory-hybrid: core bootstrap failed: ${err instanceof Error ? err.message : String(err)}`,
			);
			throw err;
		}
...
export function installCoreBootstrapServices(
	context: CoreBootstrapContext,
): CoreBootstrapServices {
