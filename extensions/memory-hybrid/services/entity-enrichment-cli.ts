...
export type EntityEnrichmentMentionSummary = {
	label: string;
	surfaceText: string;
};
...
export async function runEntityEnrichmentForCli(
	...
	const model =
		opts.model ?? getDefaultCronModel(getCronModelConfig(cfg), "nano");
	let factsEnriched = 0;
	let processed = 0;
	...
	for (const id of ids) {
		...
		if (!f?.text) continue;
		const { mentions, detectedLang } = await extractEntityMentionsWithLlm(
			f.text,
			openai,
			model,
		);
	...
	return {
		...
		factsEnriched,
		enrichedFacts:
			verbose && enrichedFacts.length > 0 ? enrichedFacts : undefined,
	};
}
