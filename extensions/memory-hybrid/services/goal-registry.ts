...
export function resolveGoalsDir(
...
export function validateGoalLabel(
	label: string,
	...
	if (!LABEL_RE.test(t))
		return {
			ok: false,
			error: "label must be alphanumeric, underscore, or hyphen only",
		};
	return { ok: true };
}
...
function normalizeGoalJson(g: Goal): Goal {
	return {
		...
		circuitBreakerLastProgressAssessmentCount:
			g.circuitBreakerLastProgressAssessmentCount ?? 0,
...
export async function readGoalByLabel(
	goalsDir: string,
	label: string,
): Promise<Goal | null> {
...
export async function createGoal(
	...
	if (existing && !isTerminalStatus(existing.status)) {
		throw new Error(
			`A goal with label "${input.label}" already exists (status: ${existing.status})`,
		);
	}
	...
	const goal: Goal = {
		...
		escalateAfterFailures:
			input.escalateAfterFailures ?? defaults.escalateAfterFailures,
		...
		history: [
			{
				timestamp: ts,
				action: "created",
				detail: input.description.slice(0, 500),
				actor: "user",
			},
		],
	};
...
export async function terminateGoal(
	...
	const next: Goal = {
		...
		history: [
			...(g.history ?? []),
			{ timestamp: ts, action: status, detail: reason, actor },
		],
	};
	await writeGoal(goalsDir, next);
