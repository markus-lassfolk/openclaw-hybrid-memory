...
import {
  flushActiveTaskCoherenceRepairs,
    ...
    runOptionalBeforeAgentStartStage(
      ...
      async () => {
        try {
          ...
          let factsSelectionMetrics:
            | import("../services/task-ledger-facts.js").ActiveTaskLedgerSelectionMetrics
            | null = null;
            ...
            if (!alreadyActive && shouldAutoRegisterLongRunningTask(longRunningMode, sessionKey)) {
              if (ctx.cfg.activeTask.ledger === "facts") {
                ...
                if (writeResult.skipped) {
                  api.logger?.debug?.(
                    `memory-hybrid: skipped long-running task auto-registration: ${writeResult.reason}`,
                  );
                } else {
