...
import {
	...
	runHumanizerScore,
} from "../services/humanizer-score.js";
...
export async function runCaptureStage(
	...
	await withTimeout(CAPTURE_STAGE_TIMEOUT_MS, () =>
		runCapture(event, api, ctx, sessionState),
	);
}
...
async function runCapture(
	...
	const { resolveSessionKey, clearSessionState, frustrationStateMap } =
		sessionState;
	const sessionKey =
		resolveSessionKey(event, api) ?? ctx.currentAgentIdRef.value ?? "default";
	const ev = event as { success?: boolean; messages?: unknown[] };
	const messages = ev?.messages ?? [];

	const assistantText =
		messages.length > 0
			? extractLastAssistantText(messages as unknown[])
			: undefined;
	...
	if (assistantText?.trim()) {
		try {
			...
			if (state) {
				...
				if (state.turns.length > 20)
					state.turns.splice(0, state.turns.length - 20);
	...
	if (ctx.cfg.humanizer?.enabled && assistantText?.trim()) {
		try {
			...
			if (result !== null) {
				...
				api.logger.debug?.(
					`memory-hybrid: humanizer_score=${result.score.toFixed(2)} stored`,
				);
			}
		} catch (err) {
			capturePluginError(err instanceof Error ? err : new Error(String(err)), {
			...
			api.logger.debug?.(
				`memory-hybrid: humanizer scoring skipped: ${String(err)}`,
			);
		}
	}
	...
	if (ctx.eventLog) {
		try {
			ctx.eventLog.append({
				...
				content: {
					action: "session_end",
					agentId: ctx.currentAgentIdRef.value,
				},
			});
		} catch {
			// Non-fatal
	...
	api.logger.debug?.(
		`memory-hybrid: cleared all session state for ${sessionKey}`,
	);

	// 5. Compaction on session end
	if (
		ctx.cfg.memoryTiering.enabled &&
		ctx.cfg.memoryTiering.compactionOnSessionEnd
	) {
		try {
	...
	if (ctx.cfg.autoCapture && ev.success && messages.length > 0) {
		try {
			const captureProvenance = resolveCaptureProvenance(
				event,
				api,
				sessionKey,
			);
			...
			for (
				let turnIndex = 0;
				turnIndex < (messages as unknown[]).length;
				turnIndex++
			) {
			...
			const toCapture = captureProvenance.shouldAutoCapture
				? candidates.filter(
						(candidate) => candidate.text && ctx.shouldCapture(candidate.text),
					)
				: [];
			if (toCapture.length > 0) {
				let stored = 0;
				const classifyModel =
					ctx.cfg.store.classifyModel ??
					getDefaultCronModel(getCronModelConfig(ctx.cfg), "nano");
				const classifyMicroBatch = Math.max(
					1,
					Math.min(10, ctx.cfg.autoClassify?.batchSize ?? 10),
				);
				...
				for (const candidate of toCapture.slice(0, 3)) {
					...
					textToStore = truncateForStorage(
						textToStore,
						ctx.cfg.captureMaxChars,
					);
					...
					if (ctx.cfg.retrieval.strategies.includes("semantic")) {
						try {
							...
							api.logger.warn(
								`memory-hybrid: auto-capture embedding failed: ${err}`,
							);
						}
					}
					...
					if (ctx.cfg.store.classifyBeforeWrite) {
						similarFacts = vector
							? await ctx.findSimilarByEmbedding(
									ctx.vectorDb,
									ctx.factsDb,
									vector,
									3,
				...
				if (ctx.cfg.store.classifyBeforeWrite) {
					...
					for (let s = 0; s < withSimilar.length; s += classifyMicroBatch) {
						...
						const outs = await classifyMemoryOperationsBatch(
							inputs,
							ctx.openai,
							classifyModel,
							api.logger,
				...
				for (let pi = 0; pi < prepared.length; pi++) {
						...
						if (similarFacts.length > 0) {
							try {
								...
								if (
									...
									api.logger.info?.(
										`memory-hybrid: auto-capture DELETE — retracted ${classification.targetId}`,
									);
									continue;
								}
								if (
									...
									if (oldFact) {
										...
										const walEntryId = await ctx.walWrite(
											"update",
											{
												...
												extractionMethod: getAutoCaptureExtractionMethod(
													candidate.role,
													captureProvenance,
												),
												extractionConfidence:
													getAutoCaptureExtractionConfidence(candidate.role),
												vector,
											},
											api.logger,
											classification.targetId,
										...
										const newEntry = ctx.factsDb.store({
											...
											extractionConfidence: getAutoCaptureExtractionConfidence(
												candidate.role,
											),
										});
								...
								api.logger.warn(
									`memory-hybrid: auto-capture classification failed: ${err}`,
								);
							}
						}
					}
					...
					try {
						...
						capturePluginError(
							vecErr instanceof Error ? vecErr : new Error(String(vecErr)),
							{
	...
	// capturing outcomes is low-cost and high-value for the episodic history.
	{
		...
		for (const { text } of sessionTexts) {
			...
			for (const pattern of SUCCESS_PATTERNS) {
				...
				if (match) {
					const eventText = pattern.label
						? `Agent reported: ${pattern.label}`
						: match[0];
					...
					if (existing.length === 0) {
						try {
							...
							api.logger.debug?.(
								`memory-hybrid: auto-captured success episode: ${episode.id}`,
							);
						} catch (err) {
							capturePluginError(
								err instanceof Error ? err : new Error(String(err)),
				...
				if (match) {
					...
					if (existing.length === 0) {
						try {
							...
							api.logger.debug?.(
								`memory-hybrid: auto-captured failure episode: ${episode.id}`,
							);
						} catch (err) {
							capturePluginError(
								err instanceof Error ? err : new Error(String(err)),
	...
	if (
		ctx.cfg.credentials.enabled &&
		ctx.cfg.credentials.autoCapture?.toolCalls &&
		messages.length > 0
	) {
		...
		try {
			...
			api.logger.warn(
				`memory-hybrid: tool-call credential auto-capture failed: ${errMsg}`,
			);
		}
	}
}
