...
import { dispatchRequestFromToolParams, evaluateGoalDispatch, recordGoalDispatchPreflight } from "../services/goal-dispatch-authorization.js";
import type { LifecycleContext } from "./types.js";

export function registerGoalDispatchAuthorization(api: ClawdbotPluginApi, ctx: LifecycleContext, goalsDir: string): void {
  ...
  api.on("before_tool_call", async (event: unknown) => {
    ...
    const denied = async (reason: string) => {
      const result = { allowed: false, reason, at: new Date().toISOString(), request: request ?? { taskClass: "implementation", requestedAgent: "", actualAgent: "" } };
