...
import {
	createRecallSpan,
	createRecallTimingLogger,
} from "../services/recall-timing.js";
import {
...
async function runRecall(
	...
	const recallStageStartedAt = recallTiming.phaseStarted("recall_stage_run", {
		prompt_chars: e.prompt.length,
	});
	...
	const completeStage = (result: RecallStageResult): RecallStageResult => {
		recallStageFields = {
			...
			candidate_count:
				result.kind === "full" ? result.result.candidates.length : 0,
			degraded: result.kind === "degraded",
		};
		recallTiming.phaseCompleted(
			"recall_stage_run",
			recallStageStartedAt,
			recallStageFields,
		);
	...
	try {
		...
		api.logger.debug?.(
			`memory-hybrid: auto-recall start (prompt length ${e.prompt.length})`,
		);
		...
		const searchLimit = isProgressive
			? (ctx.cfg.autoRecall.progressiveMaxCandidates ??
				Math.max(ctx.cfg.autoRecall.limit, 15))
			: ctx.cfg.autoRecall.limit;
		const { minScore } = ctx.cfg.autoRecall;
		const limit = searchLimit;
		const tierFilter: "warm" | "all" = ctx.cfg.memoryTiering.enabled
			? "warm"
			: "all";
		...
		const { degradationQueueDepth, degradationMaxLatencyMs } =
			interactivePolicy;
		const forceDegraded =
			degradationQueueDepth > 0 &&
			ctx.recallInFlightRef.value > degradationQueueDepth;

		if (forceDegraded) {
			const recallOpts = {
				tierFilter: ctx.cfg.memoryTiering.enabled
					? ("warm" as const)
					: ("all" as const),
				scopeFilter,
			...
			if (
				...
				ctx.cfg.memoryTiering.hotMaxTokens > 0
			) {
				const hotResults = ctx.factsDb.getHotFacts(
					ctx.cfg.memoryTiering.hotMaxTokens,
					scopeFilter,
				);
			...
			if (block)
				return completeStage({
					kind: "degraded",
					prependContext: `${degradedMarker + block}\n\n`,
				});
			return completeStage({
				kind: "degraded",
				prependContext: `${degradedMarker}\n\n`,
			});
		}
		...
		if (ctx.cfg.procedures.enabled && procMaxTokens > 0) {
			...
			const negativeUnfiltered = rankedProcs.filter(
				(p) => p.procedureType === "negative",
			);
			...
			if (procLines.length > 0) {
				...
				while (lines.length > 0 && estimateTokens(block) > maxTokens) {
					lines.pop();
					block =
						lines.length > 0 ? wrapper + lines.join("\n") + wrapperEnd : "";
				}
				procedureBlock = block;
			}
		}
		recallTiming.phaseCompleted("procedures_block", proceduresStartedAt, {
			injected: procedureBlock.length > 0,
		});
		await yieldEventLoop();
		const withProcedures = (s: string) =>
		...
		recallTiming.phaseCompleted("hot_facts_block", hotFactsStartedAt, {
			injected: hotBlock.length > 0,
		});
		...
		if (!ambientSeenFactsMap.has(sessionScopeKey)) {
			...
			const lastEmbedding =
				ambientLastEmbeddingMap.get(sessionScopeKey) ?? null;
		...
		let candidates = await runRecallPipelineQuery(
			e.prompt,
			limit,
			pipelineDeps,
			hydeUsedRef,
		...
		recallTiming.phaseCompleted("main_pipeline", mainPipelineStartedAt, {
			...
			try {
				const isTopicShift =
					...
					detectTopicShift(
						...
						promptEmbedding,
						ambientCfg.topicShiftThreshold ?? 0.15,
					);
				if (isTopicShift)
					api.logger.info?.(
						"memory-hybrid: topic shift detected — re-running ambient retrieval",
					);
				if (promptEmbedding !== null)
					ambientLastEmbeddingMap.set(sessionScopeKey, promptEmbedding);
				const knownEntities = ctx.factsDb.getKnownEntities
					? ctx.factsDb.getKnownEntities()
					: [];
				...
				if (extraQueries.length > 0) {
					...
					for (const q of extraQueries) {
						...
						await yieldEventLoop();
						try {
							const qResults = await runRecallPipelineQuery(
								q.text,
								Math.ceil(limit / 2),
								pipelineDeps,
								hydeUsedRef,
					...
					const merged = deduplicateResultsById(
						extraResultSets,
						(r) => r.entry.id,
					);
				...
				api.logger.warn?.(
					`memory-hybrid: ambient multi-query failed, continuing with main recall: ${err}`,
				);
				recallTiming.phaseCompleted("ambient_multi_query", ambientStartedAt, {
					status: "error",
					queries_run: ambientQueriesRun,
		...
		if (ambientCfg.enabled && ctx.issueStore) {
			try {
				...
				if (
					issueResults.openIssues.length > 0 ||
					issueResults.resolvedIssues.length > 0
				) {
					...
					if (issueResults.openIssues.length > 0) {
						...
						for (const issue of issueResults.openIssues) {
							issueLines.push(
								`- [${issue.severity}] ${issue.title} (status: ${issue.status})`,
							);
						}
						issueLines.push("</known-issues>");
					}
					if (issueResults.resolvedIssues.length > 0) {
						...
						for (const issue of issueResults.resolvedIssues) {
							const resolution = issue.fix
								? ` — Fix: ${issue.fix.slice(0, 100)}`
								: "";
							issueLines.push(
								`- [${issue.severity}] ${issue.title}${resolution}`,
							);
						}
						issueLines.push("</resolved-issues>");
					}
		...
		recallTiming.phaseCompleted("issues_block", issuesStartedAt, {
			injected: issueBlock.length > 0,
		});
		...
		recallTiming.phaseCompleted("narrative_block", narrativeStartedAt, {
			injected: narrativeBlock.length > 0,
		});
		...
		if (entityLookup.enabled) {
			const entityLookupNames = resolveEntityLookupNames(
				entityLookup,
				ctx.factsDb,
			);
		...
		function canRunDirective(): boolean {
			return (
				directiveCalls < maxDirectiveCalls &&
				candidates.length < maxDirectiveCandidates
			);
		}
		...
		if (directivesCfg.enabled) {
			try {
				...
				for (const [taskType, triggers] of Object.entries(
					directivesCfg.taskTypes,
				)) {
					if (recallAborted(signal)) {
						return abortDirectives();
					}
				...
				if (directivesCfg.sessionStart) {
					...
					const sessionKey =
						resolveSessionKey(e, api) ?? currentAgentIdRef.value ?? "default";
		...
		if (directiveMatches.length > 0) {
			candidates = candidates.map((r) =>
				directivePriorityIds.has(r.entry.id)
					? { ...r, score: r.score * 1.25 }
					: r,
			);
			candidates.sort((a, b) => {
				const s = b.score - a.score;
		...
		const boosted = candidates.map((r) => {
			let s = r.score;
			if (
				ctx.cfg.autoRecall.preferLongTerm &&
				!isConsolidatedDerivedFact(r.entry)
			) {
				s *=
		...
		const indexCap = Math.min(
			progressiveIndexMaxTokens ?? maxTokens,
			maxTokens,
		);
		...
		const result: RecallResult = {
			...
			ambientCfg: {
				...
				multiQuery: ambientCfg.multiQuery,
			},
			ambientSeenFacts:
				ambientCfg.enabled && ambientCfg.multiQuery ? ambientSeenFacts : null,
		};
