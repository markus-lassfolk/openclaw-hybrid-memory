...
function getToolEffectivenessStore(
	resolvedSqlitePath: string,
): ToolEffectivenessStore {
	const effectivenessDbPath = resolvedSqlitePath.replace(
		/(\.[^.]+)?$/,
	...
	api.on("before_agent_start", async (event: unknown, hookCtx: unknown) => {
		...
		try {
			...
			if (typeof e.prompt === "string" && e.prompt.trim().length > 0) {
				...
				const userMsgs = e.messages.filter(
          ...
          `);
					...
					capturePluginError(
						fsErr instanceof Error ? fsErr : new Error(String(fsErr)),
						{
			...
			let toolHintText = "";
			if (
				ctx.cfg.toolEffectiveness?.enabled !== false &&
				ctx.cfg.toolEffectiveness?.injectHints !== false
			) {
				try {
					...
					capturePluginError(
						thErr instanceof Error ? thErr : new Error(String(thErr)),
						{
