	...
	api.on("before_agent_start", async (event: unknown) => {
		try {
			...
			if (isGlobalRateLimited(gs.globalLimits.maxDispatchesPerHour)) {
				api.logger?.warn?.(
					"memory-hybrid: goal stewardship skipped — global dispatch rate limit",
				);
				return {
					prependContext:
			...
			if (gs.llmTriageOnHeartbeat && ctx.openai) {
				const summary = goals
					.map(
						(g) =>
							`${g.label} [${g.priority}]: ${g.status} blockers=${g.currentBlockers.length}`,
					)
					.join("\n");
			...
			const built = await buildMultiGoalStewardshipPrepend(
				goalsDir,
				gs,
				goals,
				{
			...
			api.logger?.warn?.(
				`memory-hybrid: goal stewardship injection error: ${String(err)}`,
			);
			return undefined;
		}
	});
