...
export async function runInjectionStage(
	...
	return withTimeout(
		...
		() => runInjection(recallResult, api, ctx),
		undefined,
	);
}
...
async function runInjection(
	...
	const markDegradedLatency = (content: string): string => {
		if (
			r.degradationMaxLatencyMs > 0 &&
			Date.now() - r.recallStartMs > r.degradationMaxLatencyMs
		) {
			api.logger.debug?.(
	...
	function buildProgressiveIndex(
		...
		const totalTokens = list.reduce(
			(sum, x) =>
				sum + estimateTokensForDisplay(x.entry.summary || x.entry.text),
			0,
		);
		...
		for (let i = 0; i < list.length; i++) {
			...
			const title = x.entry.key
				...
				: x.entry.summary ||
					x.entry.text.slice(0, 60).trim() +
						(x.entry.text.length > 60 ? "…" : "");
			const tokenCost = estimateTokensForDisplay(
	...
	if (injectionFormat === "progressive_hybrid") {
		...
		for (const x of pinned) {
			let text =
				useSummaryInInjection && x.entry.summary
					? x.entry.summary
					: x.entry.text;
		...
		if (indexBudget <= 0) {
			...
			if (pinnedPart.length > 0) {
				...
				if (
					...
					ctx.cfg.graph.strengthenOnRecall &&
					pinnedIds.length >= 2
				) {
				...
				return {
					prependContext: markDegradedLatency(
						wrapRecalledContext(baseContext + withProcedures(fullContent)),
					),
				};
			}
			if (procedureBlock) {
				return {
					prependContext: markDegradedLatency(
						wrapRecalledContext(baseContext + procedureBlock),
					),
				};
			}
			...
			return combinedContext
				? {
						prependContext: markDegradedLatency(
							wrapRecalledContext(combinedContext),
						),
					}
				: undefined;
		}
	...
	if (injectionFormat === "progressive") {
		...
		if (ambientSeenFacts && indexIds.length > 0)
			ambientSeenFacts.markSeen(indexIds);
		if (
			ctx.cfg.graph.enabled &&
			ctx.cfg.graph.strengthenOnRecall &&
			indexIds.length >= 2
		...
		return {
			prependContext: markDegradedLatency(
				wrapRecalledContext(
					baseContext +
						withProcedures(`${indexHeader}${indexContent}${indexFooter}`),
				),
			),
		};
	}
	...
	if (summarizeWhenOverBudget && lines.length < candidates.length) {
		const summarizeStartedAt = recallTiming.phaseStarted(
			"injection_summarize",
			{
		...
		try {
			...
			api.logger.warn(
				`memory-hybrid: summarize-when-over-budget failed: ${err}`,
			);
			recallTiming.phaseCompleted("injection_summarize", summarizeStartedAt, {
				status: "error",
			});
	...
	return {
		prependContext: markDegradedLatency(
			wrapRecalledContext(
				baseContext + withProcedures(`${header}${memoryContext}${footer}`),
			),
		),
	};
}
