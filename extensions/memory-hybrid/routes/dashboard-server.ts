...
import {
	collectGraphPayload,
	collectGraphRecallPayload,
	getGraphExplorerHtml,
		...
		return (
			await Promise.all(
				withMtime.slice(0, 20).map(async (e) => {
					const item = await readJsonFile<ForgeTaskItem>(
						join(forgeDir, e.name),
					);
					if (item) {
...
function collectCostStats(ctx: DashboardContext): CostStats {
	...
	try {
		...
		return {
			...
			totalEstimatedCostUsd: allFeatures.reduce(
...
async function collectAgentHealth(
	...
	try {
		...
		for (const a of agents) {
			if (a.status === "stale")
				alerts.push(`${a.agentId}: no activity > 4h — check dispatch`);
			if (a.status === "degraded")
				alerts.push(`${a.agentId}: score ${a.score.toFixed(0)} (degraded)`);
		}
...
function collectAuditSummary(ctx: DashboardContext): AuditSummaryPayload {
	...
	try {
		...
		const failed = ctx.auditStore.query({
...
function openFactsDbReadonly(
	path: string,
): import("node:sqlite").DatabaseSync | null {
	try {
...
async function collectMemoryViewerStats(
	ctx: DashboardContext,
): Promise<MemoryViewerStats> {
	...
	if (roDb) {
		try {
			const sr = roDb
				.prepare(
					"SELECT COUNT(*) as cnt FROM facts WHERE superseded_at IS NOT NULL",
				)
		...
		try {
			const vr = roDb
				.prepare("SELECT COUNT(*) as cnt FROM verified_facts")
				.get() as { cnt: number } | undefined;
		...
		try {
			const er = roDb.prepare("SELECT COUNT(*) as cnt FROM edicts").get() as
				| { cnt: number }
				| undefined;
		...
		try {
			const ar = roDb.prepare("SELECT COUNT(*) as cnt FROM episodes").get() as
				| { cnt: number }
				| undefined;
...
function collectSessionTimeline(
	...
	if (ctx.eventLog) {
		try {
			const aggregates = ctx.eventLog.getSessionAggregates(
		...
		try {
			...
			for (const ev of auditEvents) {
				...
				if (ev.action === "memory_recall" && ev.outcome === "success") {
					injectedSessions.set(
						ev.sessionId,
						(injectedSessions.get(ev.sessionId) ?? 0) + 1,
					);
				}
			}
		} catch {
	...
	if (ctx.narrativesDb) {
		try {
			...
			for (const n of narratives) {
				...
				narrativeSessions.set(
					n.sessionId,
					(narrativeSessions.get(n.sessionId) ?? 0) + 1,
				);
			}
		} catch {
			/* non-fatal */
	...
	if (ctx.workflowStore) {
		try {
			...
			for (const t of traces) {
				...
				workflowSessions.set(
					t.sessionId,
					(workflowSessions.get(t.sessionId) ?? 0) + 1,
				);
			}
		} catch {
			/* non-fatal */
 ...
 */
function collectSessionTimelineDetail(
	ctx: DashboardContext,
	sessionId: string,
): SessionTimelineDetail | null {
...
function collectMemoryViewerEpisodes(
	ctx: DashboardContext,
	limit = 50,
): MemoryViewerEpisode[] {
	try {
...
function collectMemoryViewerNarratives(
	ctx: DashboardContext,
	limit = 20,
): MemoryViewerNarrative[] {
	try {
...
function collectMemoryViewerWorkflows(
	ctx: DashboardContext,
	limit = 100,
): MemoryViewerWorkflow[] {
	try {
		...
		const result: MemoryViewerWorkflow[] = traces.map((t) => ({
			...
			successRate:
				patterns.find(
					(p) =>
						JSON.stringify(p.toolSequence) === JSON.stringify(t.toolSequence),
				)?.successRate ?? 0,
...
function collectMemoryViewerVerified(
	ctx: DashboardContext,
	limit = 100,
): MemoryViewerVerification[] {
	try {
...
function collectMemoryViewerEntities(
	ctx: DashboardContext,
	limit = 50,
): MemoryViewerEntity[] {
	try {
...
function collectMemoryViewerProvenance(
	ctx: DashboardContext,
	factId: string,
): MemoryViewerProvenance | null {
	try {
...
function collectMemoryViewerLinks(
	ctx: DashboardContext,
	limit = 5000,
): MemoryViewerLinks[] {
	try {
		...
		try {
			const rows = roDb
				.prepare("SELECT * FROM memory_links LIMIT ?")
				.all(limit) as Array<Record<string, unknown>>;
			return rows.map((r) => ({
	...
	try {
		...
		if (action === "verify") {
			if (!ctx.verificationStore)
				return { ok: false, message: "Verification store not available" };
			const verifiedBy =
				(body.verifiedBy as "agent" | "user" | "system") ?? "agent";
		...
		try {
			...
			if (!ok)
				return { ok: false, message: `Could not supersede fact ${factId}` };
		} catch (err) {
			return {
				ok: false,
				message: `Could not forget fact ${factId}: ${String(err)}`,
			};
		}
	...
	const server = createServer((req, res) => {
		...
		if (pathname === "/api/graph") {
			try {
				const days = Math.min(
					...
					Math.max(
						1,
						Number.parseInt(searchParams.get("days") ?? "30", 10) || 30,
					),
				);
				const maxNodes = Math.min(
					...
					Math.max(
						20,
						Number.parseInt(searchParams.get("maxNodes") ?? "400", 10) || 400,
					),
				);
				const body = JSON.stringify(
					collectGraphPayload(ctx.factsDb, days, maxNodes),
				);
				res.writeHead(200, {
		...
		if (pathname === "/api/graph/recall") {
			try {
				...
				res.writeHead(500, { "Content-Type": "application/json" });
				pluginLogger.error(
					`[dashboard-server] /api/graph/recall: ${
						err instanceof Error ? err.message : String(err)
					}`,
				);
				res.end(JSON.stringify({ error: "InternalServerError" }));
		...
		if (pathname === "/api/audit/summary") {
			try {
				...
				res.writeHead(500, { "Content-Type": "application/json" });
				pluginLogger.error(
					`[dashboard-server] /api/audit/summary: ${
						err instanceof Error ? err.message : String(err)
					}`,
				);
				res.end(JSON.stringify({ error: "InternalServerError" }));
		...
		if (pathname === "/api/audit/events") {
			...
			try {
				...
				const targetContains =
					searchParams.get("targetContains") ??
				...
				res.writeHead(500, { "Content-Type": "application/json" });
				pluginLogger.error(
					`[dashboard-server] /api/audit/events: ${
						err instanceof Error ? err.message : String(err)
					}`,
				);
				res.end(JSON.stringify({ error: "InternalServerError" }));
		...
		if (pathname === "/api/viewer/timeline/sessions") {
			try {
				...
				const timeline = collectSessionTimeline(ctx, {
					days,
					sessionLimit: limit,
				});
				res.writeHead(200, {
		...
		if (
			req.method === "GET" &&
			pathname.startsWith("/api/viewer/timeline/sessions/")
		) {
			const sessionId = decodeURIComponent(
				pathname.replace("/api/viewer/timeline/sessions/", ""),
			);
			if (!sessionId) {
			...
			try {
				...
				if (!detail) {
					...
					res.end(
						JSON.stringify({
							error: "Session not found or no activity recorded",
						}),
					);
					return;
				}
		...
		if (pathname === "/api/viewer/timeline/stats") {
			try {
				...
				const fullTimeline = collectSessionTimeline(ctx, {
					days: 30,
					sessionLimit: 500,
				});
				res.writeHead(200, {
				...
				res.end(
					JSON.stringify({
						totals: fullTimeline.totals,
						allEventTypes: fullTimeline.allEventTypes,
					}),
				);
			} catch (err: unknown) {
				res.writeHead(500, { "Content-Type": "application/json" });
		...
		if (pathname === "/api/viewer/facts") {
			try {
				...
				const allFacts = ctx.factsDb.list(limit, {
				...
				const facts: MemoryViewerFact[] = allFacts.map((f) => {
					...
					return {
						...
						expiresAt:
							record.expires_at != null ? Number(record.expires_at) : null,
						...
						provenanceSession:
							(record.provenance_session as string | null) ?? null,
						reinforcedCount:
							record.reinforced_count != null
								? Number(record.reinforced_count)
								: undefined,
					};
				});
				res.writeHead(200, {
		...
		if (pathname === "/api/viewer/links") {
			try {
				const limit = Math.min(
					10000,
					Math.max(1, Number.parseInt(searchParams.get("limit") ?? "5000", 10)),
				);
		...
		if (
			req.method === "POST" &&
			pathname.match(/^\/api\/viewer\/facts\/[^/]+\/verify$/)
		) {
		...
		if (
			req.method === "POST" &&
			pathname.match(/^\/api\/viewer\/facts\/[^/]+\/forget$/)
		) {
			const factId = pathname.split("/")[4];
			let body = "";
			req.on("data", (chunk: Buffer) => {
