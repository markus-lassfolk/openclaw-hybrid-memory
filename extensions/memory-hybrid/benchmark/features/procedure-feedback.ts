	...
	const procedures = [
		{
			...
			recipeJson: JSON.stringify([
				{
					tool: "exec",
					args: { command: "curl -s http://nibe.local/status" },
					summary: "fetch status",
				},
				{
					tool: "exec",
					args: { command: "jq '.status'" },
					summary: "parse JSON",
				},
			]),
		...
		},
		{
			...
			recipeJson: JSON.stringify([
				{
					tool: "exec",
					args: { command: "curl http://nibe.local/api/status" },
					summary: "fetch status v2",
				},
				{
					tool: "exec",
					args: { command: "cat status.json" },
					summary: "read file",
				},
			]),
			procedureType: "positive" as const,
		...
		},
		{
			...
			recipeJson: JSON.stringify([
				{
					tool: "exec",
					args: { command: "git tag v$(date +%Y%m%d%H%M)" },
					summary: "tag",
				},
				{
					tool: "exec",
					args: { command: "git push --tags" },
					summary: "push tags",
				},
				{
					tool: "exec",
					args: { command: "./deploy.sh --staged" },
					summary: "deploy staged",
				},
			]),
			procedureType: "positive" as const,
			successCount: 7,
...
export function benchmark(
	...
	const recallFn = () =>
		recallBestProcedureVersioned(fixture.procedures, query);
...
export async function testAccuracy(
	...
	const fixture = createFixture();
	const prompt =
		'Task: "check nibe system status". Which procedure should be used?';

	const onResult = recallBestProcedureVersioned(
		fixture.procedures,
		"nibe system status",
	);
	const offResult = recallBestProcedureFlat(
		fixture.procedures,
		"nibe system status",
	);
