...
function createFixture(): FreqFixture {
	...
	const entities = [
		{
			text: "Nibe is a Swedish HVAC system brand",
			entity: "Nibe",
			key: "type",
			value: "HVAC brand",
		},
		{
			text: "OpenClaw is the agent framework being used",
			entity: "OpenClaw",
			key: "type",
			value: "agent framework",
		},
		{
			text: "TypeScript is the language for this project",
			entity: "TypeScript",
			key: "language",
			value: "TypeScript",
		},
		{
			text: "Vitest is the testing framework",
			entity: "Vitest",
			key: "framework",
			value: "testing",
		},
		{
			text: "Markus is the user's name",
			entity: "Markus",
			key: "name",
			value: "user",
		},
...
export function benchmark(
	...
	const trackFn = () => {
		...
		for (const text of fixture.sampleTexts) {
			const mentions = fixture.db.extractEntitiesFromText(
				text,
...
export function shadowBenchmark(
	...
	const withTracking = () => {
		let total = 0;
		for (const text of fixture.sampleTexts) {
			total += fixture.db.extractEntitiesFromText(
				text,
				fixture.knownEntities,
			).length;
		}
...
export async function testAccuracy(
	...
	const prompt =
		"User mentions Nibe, TypeScript, and Markus in conversation. Which entities were mentioned?";
