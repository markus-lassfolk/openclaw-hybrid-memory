...
describe("ToolProposer", () => {
	it("returns skipped when selfExtension is disabled", () => {
		...
		const proposer = new ToolProposer(
			detector,
			proposalStore,
			DISABLED_CFG.selfExtension,
		);
		const result = proposer.runCycle();
		expect(result.proposed).toBe(0);
		expect(result.reasons.some((r: string) => r.includes("disabled"))).toBe(
			true,
		);
	});
	...
	it("skips duplicate tool names", () => {
		...
		expect(
			result.reasons.some((r: string) => r.includes("already exists")),
		).toBe(true);
	});
	...
	it("respects maxProposals cap", () => {
		...
		expect(result.proposed).toBe(0);
		expect(
			result.reasons.some((r: string) => r.includes("Max pending proposals")),
		).toBe(true);
	});
	...
	it("reports no gaps when traces are insufficient", () => {
		...
		expect(result.proposed).toBe(0);
		expect(
			result.reasons.some((r: string) => r.includes("No actionable gaps")),
		).toBe(true);
	});
});
