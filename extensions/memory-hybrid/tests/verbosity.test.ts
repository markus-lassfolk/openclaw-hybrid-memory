...
describe("runVerifyForCli — quiet-mode sink filtering", () => {
	...
	it("suppresses ✅ OK lines in quiet mode", async () => {
		...
		const tableSectionHeaders = headerLines.filter(
			(l) =>
				l.includes("Embeddings Tests") ||
				l.includes("Embedding ↔ vector store") ||
				l.includes("LLM / Models"),
		);
...
describe("memory_prune — verbosity output", () => {
	...
	it("quiet: single-line count only, but includes all details fields", async () => {
		const result = (await testPruneVerbosity("quiet")) as {
			content: { text: string }[];
			details: any;
		};
		expect(result.content[0].text).toMatch(
			/^Pruned: \d+ \(\d+ expired, \d+ low-confidence\)\./,
		);
	...
	it("normal: full breakdown text and all details fields", async () => {
		const result = (await testPruneVerbosity("normal")) as {
			content: { text: string }[];
			details: any;
		};
	...
	it("verbose: includes mode information and all details fields", async () => {
		const result = (await testPruneVerbosity("verbose")) as {
			content: { text: string }[];
			details: any;
		};
...
describe("memory_reflect — verbosity output", () => {
	...
	it("quiet: only stored count", async () => {
		const result = (await testReflectVerbosity("quiet")) as {
			content: { text: string }[];
		};
	...
	it("normal: full summary without model", async () => {
		const result = (await testReflectVerbosity("normal")) as {
			content: { text: string }[];
		};
	...
	it("verbose: includes model info", async () => {
		const result = (await testReflectVerbosity("verbose")) as {
			content: { text: string }[];
		};
...
describe("memory_store — verbosity output", () => {
	...
	async function testStoreVerbosity(
		...
		return await storeTool.execute("call-1", {
			text,
			importance: 0.7,
			category: "fact",
			...extraParams,
	...
	it("normal: shows text preview and decay class", async () => {
		const result = (await testStoreVerbosity(
			"normal",
	...
	it("verbose: shows scope even when global", async () => {
		const result = (await testStoreVerbosity("verbose", "Global fact", {
			scope: "global",
		})) as {
			content: { text: string }[];
		};
	...
	it("truncates long text at 100 chars for normal/verbose", async () => {
		...
		const result = (await testStoreVerbosity("normal", longText)) as {
			content: { text: string }[];
		};
...
function makeMinimalLifecycleContext(
	...
	return {
		...
		pendingLLMWarnings: {
			drain: () => [],
		} as unknown as LifecycleContext["pendingLLMWarnings"],
		walWrite: vi
			.fn()
			.mockResolvedValue("wal-id") as unknown as LifecycleContext["walWrite"],
		walRemove: vi
			.fn()
			.mockResolvedValue(undefined) as unknown as LifecycleContext["walRemove"],
		findSimilarByEmbedding:
			vi.fn() as unknown as LifecycleContext["findSimilarByEmbedding"],
		shouldCapture: () => false,
...
describe("silent mode — hook suppression", () => {
	it("registers fewer before_agent_start handlers in silent mode than in normal mode", () => {
		...
		const silentHooks = createLifecycleHooks(
			makeMinimalLifecycleContext("silent"),
		);
		silentHooks.onAgentStart(silentApi as never);

		const normalHooks = createLifecycleHooks(
			makeMinimalLifecycleContext("normal"),
		);
