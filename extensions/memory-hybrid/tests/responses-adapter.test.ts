...
describe("extractResponsesUsage", () => {
	...
	it("returns undefined when usage is absent", () => {
		expect(
			extractResponsesUsage({ id: "resp_123", output: [] }),
		).toBeUndefined();
	});
...
describe("callResponsesApi", () => {
	...
	it("throws when client.responses is not available", async () => {
		...
		await expect(
			callResponsesApi(mockClient, { model: "o3-pro", content: "test" }),
		).rejects.toThrow("does not expose responses.create()");
	});
...
describe("chatMessagesToResponsesInput", () => {
	...
	it("folds tool role into user content", () => {
		expect(
			chatMessagesToResponsesInput([{ role: "tool", content: "result" }]),
		).toEqual([{ role: "user", content: "[tool result]\nresult" }]);
	});
});
