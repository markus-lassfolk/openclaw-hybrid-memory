...
describe("task-hygiene", () => {
	it("buildHeartbeatTaskHygieneBlock includes stale labels", () => {
		const tasks = [
			baseTask({ label: "a", stale: true }),
			baseTask({ label: "b", stale: false }),
		];
	...
	it("buildGoalEscalationHeartbeatBlock returns empty when no blocked/stalled", () => {
		expect(
			buildGoalEscalationHeartbeatBlock([{ label: "a", status: "active" }], {
				maxChars: 500,
			}),
		).toBe("");
	});
	...
	it("buildProposeGoalDraftFromTask maps row to draft", () => {
		...
		expect(draft.suggestedCriteria.some((c) => c.includes("Run tests"))).toBe(
			true,
		);
	});
});
