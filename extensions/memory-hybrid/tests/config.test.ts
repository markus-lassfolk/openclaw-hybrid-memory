...
describe("hybridConfigSchema.parse", () => {
	...
	it("parses retrieval directives config", () => {
		...
		expect(result.autoRecall.retrievalDirectives.taskTypes.debug).toEqual([
			"bug",
			"fix",
		]);
	...
	it("throws on missing embedding (model required when provider defaults to ollama)", () => {
		expect(() => hybridConfigSchema.parse({})).toThrow(
			/embedding\.model|embedding\.apiKey/,
		);
	});
	...
	it("throws when embedding.apiKey env: SecretRef references an unset env var", () => {
		...
		expect(() =>
			hybridConfigSchema.parse({
				embedding: {
					provider: "openai",
					apiKey: "env:TEST_EMBED_KEY_UNSET_333",
					model: "text-embedding-3-small",
				},
			}),
	...
	it("resolves embedding.apiKey env: SecretRef for non-openai provider fallback (ollama)", () => {
		vi.stubEnv(
			"TEST_EMBED_FALLBACK_KEY_333",
			"sk-fallback-key-that-is-long-enough",
		);
		try {
			const result = hybridConfigSchema.parse({
				embedding: {
					...
					apiKey: "env:TEST_EMBED_FALLBACK_KEY_333",
				},
			});
			expect(result.embedding.apiKey).toBe(
				"sk-fallback-key-that-is-long-enough",
		...
		try {
			expect(() =>
				hybridConfigSchema.parse({
					embedding: {
						provider: "openai",
						apiKey: "env:TEST_EMBED_PLACEHOLDER_333",
						model: "text-embedding-3-small",
					},
				}),
			).toThrow(/missing or a placeholder/);
		...
		try {
			const result = hybridConfigSchema.parse({
				embedding: {
					provider: "ollama",
					model: "nomic-embed-text",
					apiKey: "env:TEST_EMBED_FALLBACK_UNSET_333",
				},
			});
	...
	it("warns when embedding section present but provider is not set", () => {
		...
		try {
			...
			expect(warnSpy).toHaveBeenCalledWith(
				expect.stringMatching(/embedding\.provider not set/),
			);
		} finally {
			warnSpy.mockRestore();
		}
		...
		try {
			...
			const warnCalls = warnSpy.mock.calls.filter(
				(args) =>
					...
					args[0].includes("embedding.provider not set"),
			);
	...
	it("errorReporting defaults to opt-out config (enabled+consent=true) when not provided", () => {
		...
		expect(result.errorReporting.dsn).toBe(
	...
	it("parses errorReporting in self-hosted mode with DSN", () => {
		...
		expect(result.errorReporting?.dsn).toBe(
			"https://abc123@glitchtip.example.com/1",
		);
	});
	...
	it("resolves embedding.googleApiKey when distill.apiKey is env:VAR SecretRef (Issue #344)", () => {
		vi.stubEnv(
			"TEST_GEMINI_API_KEY_344",
			"test-google-key-resolved-long-enough-00000001",
		);
		const result = hybridConfigSchema.parse({
			embedding: {
				provider: "google",
	...
	it("resolves embedding.googleApiKey when llm.providers.google.apiKey is env:VAR SecretRef (Issue #344)", () => {
		vi.stubEnv(
			"TEST_GEMINI_PROVIDER_KEY_344",
			"test-google-key-provider-long-enough-000000002",
		);
		const result = hybridConfigSchema.parse({
			embedding: {
				provider: "google",
	...
	it("resolves short SecretRef like env:GKEY (9 chars) for google embedding (Issue #344 edge case)", () => {
		...
		expect(result.embedding.googleApiKey).toBe(
			"test-google-key-short-ref-resolved-long-enough",
		);
		expect(result.embedding.googleApiKey).not.toMatch(/^env:/);
	});
	...
	it("resolves embedding.googleApiKey when distill.apiKey uses ${VAR} template syntax (Issue #373)", () => {
		vi.stubEnv(
			"TEST_GEMINI_TMPL_KEY_373",
			"AIzaSy-template-key-that-is-long-enough-to-pass",
		);
		const result = hybridConfigSchema.parse({
			embedding: {
				provider: "google",
			...
			distill: {
				apiKey: "${TEST_GEMINI_TMPL_KEY_373}",
				defaultModel: "gemini-2.0-flash",
			},
		});
		expect(result.embedding.googleApiKey).toBe(
	...
	it("resolves embedding.googleApiKey when distill.apiKey is a file: SecretRef (Issue #373)", () => {
		...
		require("node:fs").writeFileSync(
			tmpFile,
			"AIzaSy-file-key-that-is-long-enough-to-pass\n",
		);
		try {
			const result = hybridConfigSchema.parse({
				embedding: {
	...
	it("hasGoogleKey: short ${VAR} template (< 10 chars) is recognised and key is resolved (Issue #2921626583)", () => {
		...
		expect(result.embedding.googleApiKey).toBe(
			"AIzaSy-short-template-key-long-enough-to-pass",
		);
	});
	...
	it("falls back to llm.providers.google.apiKey when distill.apiKey is short/invalid (Issue #2921626579)", () => {
		vi.stubEnv(
			"LLM_GOOGLE_KEY_373",
			"AIzaSy-llm-google-key-long-enough-to-pass",
		);
		const result = hybridConfigSchema.parse({
			embedding: {
				provider: "google",
	...
	it("malformed template distill.apiKey (no closing brace) falls back to llm.providers.google.apiKey (Issue #2921658704)", () => {
		vi.stubEnv(
			"LLM_GOOGLE_FALLBACK_373",
			"AIzaSy-llm-fallback-key-long-enough-to-pass",
		);
		const result = hybridConfigSchema.parse({
			embedding: {
				provider: "google",
	...
	it("resolveSecretRef returns env var value even when it contains a literal ${ sequence (Issue #2921445142)", () => {
		...
		vi.stubEnv(
			"GEMINI_KEY_WITH_DOLLAR_BRACE",
			"AIzaSy-value-containing-${literal}-suffix-123456",
		);
		const result = hybridConfigSchema.parse({
			embedding: {
				provider: "google",
			...
			distill: {
				apiKey: "${GEMINI_KEY_WITH_DOLLAR_BRACE}",
				defaultModel: "gemini-2.0-flash",
			},
		});
		expect(result.embedding.googleApiKey).toBe(
	...
	it("parses distill.extractionModelTier (nano | default | heavy)", () => {
		expect(
			...
			}).distill?.extractionModelTier,
		).toBe("nano");
		expect(
			hybridConfigSchema.parse({
				...validBase,
				distill: { extractionModelTier: "default" },
			}).distill?.extractionModelTier,
		).toBe("default");
		expect(
			hybridConfigSchema.parse({
				...validBase,
				distill: { extractionModelTier: "heavy" },
			}).distill?.extractionModelTier,
		).toBe("heavy");
		expect(
			hybridConfigSchema.parse({
				...validBase,
				distill: { extractionModelTier: "other" },
			}).distill?.extractionModelTier,
		).toBeUndefined();
	});
	...
	it("getLLMModelPreference does not append fallback when fallbackModel is unset", () => {
		...
		expect(getLLMModelPreference(cronCfg, "default")).toEqual([
			"gemini-2.0-flash",
		]);
	...
	it("getLLMModelPreference and getProvidersWithKeys exclude llm.disabledProviders", () => {
		...
		expect(getLLMModelPreference(cronCfg, "heavy")).toEqual([]);
		expect(getProvidersWithKeys(cronCfg)).toEqual(
			expect.arrayContaining(["google", "openai"]),
		);
	...
	it("getLLMModelPreference legacy path: Gemini first then OpenAI when both have keys (failover list)", () => {
		const cronCfg = {
			...
			distill: {
				apiKey: "GEMINI_API_KEY_LONG_ENOUGH_12345",
	...
	it("getLLMModelPreference legacy path: Claude second (claude.apiKey set, no distill)", () => {
		const cronCfg = {
			claude: {
				apiKey: "sk-claude-key-that-is-long-enough",
	...
	describe("resolveReflectionModelAndFallbacks", () => {
		...
		it("returns heavy tier from llm.heavy with fallbacks when multiple models", () => {
			...
			const { defaultModel, fallbackModels } =
				resolveReflectionModelAndFallbacks(cfg, "heavy");
	...
	describe("config mode presets", () => {
		...
		it("mode minimal: enables autoClassify, graph, procedures, ingest paths; disables reflection; distill extraction uses nano tier", () => {
			...
			expect(result.ingest?.paths).toEqual([
				"skills/**/*.md",
				"TOOLS.md",
				"AGENTS.md",
			]);
