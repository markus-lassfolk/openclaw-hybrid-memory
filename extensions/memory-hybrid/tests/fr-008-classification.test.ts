...
describe("parseClassificationResponse", () => {
	const existingFacts: MemoryEntry[] = [
		mockFact(
			...
			"User prefers VS Code with dark mode",
		),
	...
	it("parses ADD with reason", () => {
		...
		expect(r.reason).toBe(
			"this is new information about the user's work setup",
		);
	});
	...
	it("falls back to ADD when UPDATE has missing target id", () => {
		const r = parseClassificationResponse(
			"UPDATE  | user changed preference",
			existingFacts,
		);
		expect(r.action).toBe("ADD");
	...
	it("falls back to ADD when UPDATE references unknown id", () => {
		const r = parseClassificationResponse(
			"UPDATE 00000000-0000-0000-0000-000000000000 | user changed",
			existingFacts,
		);
		expect(r.action).toBe("ADD");
	...
	it("falls back to ADD when DELETE references unknown id", () => {
		const r = parseClassificationResponse(
			"DELETE 00000000-0000-0000-0000-000000000000 | retracted",
			existingFacts,
		);
		expect(r.action).toBe("ADD");
		expect(r.reason).toContain("unknown id");
...
describe("findSimilarByEmbedding", () => {
	...
	it("returns facts resolved from vector search ids and filters superseded", async () => {
		...
		const afterSupersede = await findSimilarByEmbedding(
			vectorDb,
			factsDb,
			vector,
			5,
