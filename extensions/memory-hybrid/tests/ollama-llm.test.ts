...
describe("Ollama cost tracking — $0 for local models", () => {
	...
	it("getModelPricing returns $0 for any ollama/* model", () => {
		expect(getModelPricing("ollama/llama3:8b")).toEqual({
			inputPer1M: 0,
			outputPer1M: 0,
		});
		expect(getModelPricing("ollama/mistral:7b")).toEqual({
			inputPer1M: 0,
			outputPer1M: 0,
		});
		expect(getModelPricing("ollama/phi4:14b")).toEqual({
			inputPer1M: 0,
			outputPer1M: 0,
		});
	});
	...
	it("getModelPricing is case-insensitive for ollama prefix", () => {
		expect(getModelPricing("OLLAMA/Qwen3:8b")).toEqual({
			inputPer1M: 0,
			outputPer1M: 0,
		});
		expect(getModelPricing("Ollama/llama3")).toEqual({
			inputPer1M: 0,
			outputPer1M: 0,
		});
	});
...
describe("Ollama graceful fallback — ECONNREFUSED", () => {
	...
	it("falls back to cloud model when primary ollama model returns ECONNREFUSED", async () => {
		const ollamaErr = Object.assign(
			new Error(
				"Ollama not available at http://127.0.0.1:11434 (ECONNREFUSED) — try next model",
			),
			{ code: "ECONNREFUSED" },
		);
		const mockOpenai = {
...
describe("parseLLMConfig — provider baseURL / baseUrl", () => {
	it("accepts camelCase `baseUrl` in provider config (OpenClaw convention)", () => {
		...
		const result = parseLLMConfig(cfg);
		expect(result?.providers?.["azure-foundry"]?.baseURL).toBe(
