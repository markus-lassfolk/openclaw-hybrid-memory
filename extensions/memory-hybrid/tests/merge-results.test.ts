...
describe("mergeResults — single-list passthrough", () => {
  ...
  it("returns lancedb-only results when sqlite is empty", () => {
    const lance = [makeResult("fact C", 0.8, "lancedb"), makeResult("fact D", 0.6, "lancedb")];
...
describe("mergeResults — limit enforcement", () => {
  it("returns exactly limit results from large inputs", () => {
    const sqlite = Array.from({ length: 20 }, (_, i) => makeResult(`sqlite ${i}`, 1 - i * 0.01));
    const lance = Array.from({ length: 20 }, (_, i) => makeResult(`lance ${i}`, 0.9 - i * 0.01, "lancedb"));
...
describe("mergeResults — superseded filtering", () => {
  ...
  it("removes all lance results when all are superseded", () => {
    ...
    const provider = {
      getSupersededTexts: () => new Set(["superseded fact one", "superseded fact two"]),
    };
...
describe("mergeResults — RRF score correctness", () => {
  it("result in both lists has higher score than result in only one list", () => {
    ...
    const sqlite = [makeResult("only in sqlite", 0.9), makeResult("in both", 0.5, "sqlite", { id: sharedId })];
    const lance = [
    ...
    const onlyInSqlite = merged.find((r) => r.entry.text === "only in sqlite")?.score ?? 0;
...
describe("filterByScope", () => {
  ...
  it("passes scopeFilter to getById correctly", () => {
    ...
    const getById = (id: string, opts?: { scopeFilter?: typeof scopeFilter | null }) => {
      if (id === outOfScope.entry.id && opts?.scopeFilter?.userId === "alice") return null;
      return { id };
    };
