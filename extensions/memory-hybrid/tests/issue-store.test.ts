...
describe("IssueStore.update", () => {
	...
	it("updates rollback information", () => {
		const issue = store.create({
			title: "Deployment broke auth",
			symptoms: ["403 on all endpoints"],
		});
		const updated = store.update(issue.id, {
	...
	it("throws when updating nonexistent issue", () => {
		expect(() => store.update("fake-id", { rootCause: "test" })).toThrow(
			"Issue not found",
		);
	});
});
...
describe("IssueStore.transition — valid transitions", () => {
	it("transitions open → diagnosed", () => {
		...
		const t = store.transition(issue.id, "diagnosed", {
			rootCause: "Infinite loop in scheduler",
		});
	...
	it("transitions diagnosed → fix-attempted", () => {
		...
		const t = store.transition(issue.id, "fix-attempted", {
			fix: "Increased heap limit",
		});
	...
	it("full lifecycle: open → diagnosed → fix-attempted → resolved → verified", () => {
		...
		expect(issue.status).toBe("open");
		const d = store.transition(issue.id, "diagnosed", {
			rootCause: "Root cause found",
		});
		expect(d.status).toBe("diagnosed");
		const fa = store.transition(issue.id, "fix-attempted", {
			fix: "Applied patch",
		});
	...
	it("transitions resolved → open (regression after resolution)", () => {
		const issue = store.create({
			title: "Regression from resolved",
			symptoms: ["symptom"],
		});
		store.transition(issue.id, "fix-attempted");
		store.transition(issue.id, "resolved");
...
describe("IssueStore.transition — invalid transitions", () => {
	it("throws on verified → diagnosed (terminal state)", () => {
		...
		expect(() => store.transition(issue.id, "diagnosed")).toThrow(
			"Invalid transition",
		);
	});
	...
	it("throws on verified → open", () => {
		...
		store.transition(issue.id, "resolved");
		store.transition(issue.id, "verified");
		expect(() => store.transition(issue.id, "open")).toThrow(
			"Invalid transition",
		);
	});
	...
	it("throws on open → verified (skip steps)", () => {
		...
		expect(() => store.transition(issue.id, "verified")).toThrow(
			"Invalid transition",
		);
	});
	...
	it("throws on open → resolved (skip steps)", () => {
		...
		expect(() => store.transition(issue.id, "resolved")).toThrow(
			"Invalid transition",
		);
	});
	...
	it("throws transition on nonexistent id", () => {
		expect(() => store.transition("fake-id", "diagnosed")).toThrow(
			"Issue not found",
		);
	});
});
...
describe("IssueStore.search", () => {
	...
	it("finds issues by symptom keyword", () => {
		store.create({
			title: "Unknown error",
			symptoms: ["JWT signature invalid", "401 response"],
		});
...
describe("IssueStore.linkFact", () => {
	...
	it("throws when linking to nonexistent issue", () => {
		expect(() => store.linkFact("nonexistent", "fact-001")).toThrow(
			"Issue not found",
		);
	});
});
