...
import {
	findOpenClawSessionJsonlForKey,
	looksLikeOpenClawSessionRef,
} from "../services/openclaw-session-artifact.js";
...
describe("looksLikeOpenClawSessionRef", () => {
	...
	it("accepts bare UUIDs", () => {
		expect(
			looksLikeOpenClawSessionRef("f3d14066-09ea-492f-a3f3-7ae2fe6c9b0a"),
		).toBe(true);
	});
...
describe("findOpenClawSessionJsonlForKey", () => {
	...
	beforeEach(async () => {
		...
		await mkdir(join(openclawHome, "agents", "forge", "sessions"), {
			recursive: true,
		});
	});
