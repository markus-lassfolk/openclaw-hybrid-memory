...
describe("FactsDB procedures table", () => {
	it("upsertProcedure inserts new procedure and returns entry", () => {
		const proc = db.upsertProcedure({
			...
			recipeJson: JSON.stringify([
				{ tool: "web_fetch", args: { url: "https://api.example.com" } },
			]),
	...
	it("searchProcedures finds by task pattern words", () => {
		...
		expect(results.some((p) => p.taskPattern.includes("Home Assistant"))).toBe(
			true,
		);
	});
	...
	it("markProcedurePromoted sets promoted_to_skill and skill_path", () => {
		...
		const ok = db.markProcedurePromoted(
			created.id,
			"skills/auto/check-moltbook",
		);
		expect(ok).toBe(true);
		const after = db.getProcedureById(created.id);
...
describe("searchProceduresRanked (confidence-weighted ranking)", () => {
	...
	it("applies recency decay (30-day window, min 0.3)", () => {
		...
		expect(recent?.relevanceScore ?? 0).toBeGreaterThan(
			old?.relevanceScore ?? 0,
		);
	...
	it("penalizes never-validated procedures (30% penalty)", () => {
		...
		expect(val?.relevanceScore ?? 0).toBeGreaterThan(
			never?.relevanceScore ?? 0,
		);
		// Never-validated should have ~70% of validated score (30% penalty)
		expect(never?.relevanceScore ?? 0).toBeLessThan(
			(val?.relevanceScore ?? 0) * 0.75,
		);
	});
	...
	it("returns procedures matching FTS query", () => {
		...
		expect(
			results.every(
				(r) =>
					r.taskPattern.toLowerCase().includes("api") ||
					r.taskPattern.toLowerCase().includes("check"),
			),
		).toBe(true);
	});
	...
	it("returns positive procedures before negative", () => {
		...
		if (results.length >= 2) {
			const positiveIdx = results.findIndex(
				(r) => r.procedureType === "positive",
			);
			const negativeIdx = results.findIndex(
				(r) => r.procedureType === "negative",
			);
	...
	it("upsertProcedure stores procedure with agent scope", () => {
		const proc = db.upsertProcedure({
			...
			recipeJson: JSON.stringify([
				{ tool: "exec", args: { command: "git commit" } },
			]),
			procedureType: "positive",
			scope: "agent",
			scopeTarget: "forge",
	...
	it("searchProcedures with scopeFilter returns only matching scoped procedures", () => {
		...
		expect(forgeResults.some((p) => p.taskPattern.includes("Global"))).toBe(
			true,
		);
		expect(forgeResults.some((p) => p.taskPattern.includes("Forge"))).toBe(
			true,
		);
		expect(forgeResults.some((p) => p.taskPattern.includes("Hearth"))).toBe(
			false,
		);
		...
		expect(hearthResults.some((p) => p.taskPattern.includes("Global"))).toBe(
			true,
		);
		expect(hearthResults.some((p) => p.taskPattern.includes("Hearth"))).toBe(
			true,
		);
		expect(hearthResults.some((p) => p.taskPattern.includes("Forge"))).toBe(
			false,
		);
	});
...
describe("FactsDB procedureFeedback", () => {
	it("procedureFeedback on success increments success count and updates last_validated", () => {
		const proc = db.upsertProcedure({
			...
			recipeJson: JSON.stringify([
				{ tool: "exec", args: { command: "deploy.sh" } },
			]),
			procedureType: "positive",
			successCount: 1,
		});
	...
	it("procedureFeedback on failure creates version bump and failure record", () => {
		...
		const result = db.procedureFeedback({
			...
			success: false,
			context:
				"SSH key rejected — Doris rebooted and dropped the authorized_keys entry",
			failedAtStep: 2,
		});
	...
	it("procedureFeedback records avoidance notes on failure", () => {
		...
		expect(
			result?.avoidanceNotes?.some((n) => n.includes("SSH key may be dropped")),
		).toBe(true);
	});
