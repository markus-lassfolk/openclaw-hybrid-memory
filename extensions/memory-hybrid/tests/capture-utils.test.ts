...
describe("shouldCapture", () => {
	...
	it("rejects text containing <relevant-memories>", () => {
		const text =
			"please remember this <relevant-memories>something</relevant-memories>";
	...
	it("rejects XML-like text (starts with < and contains </)", () => {
		expect(
			shouldCapture(
				"<tool_result>I prefer dark mode</tool_result>",
				MAX_CHARS,
				TRIGGERS,
			),
		).toBe(false);
	...
	it("accepts text that matches a trigger and passes all filters", () => {
		expect(
			shouldCapture("I prefer dark mode when coding", MAX_CHARS, TRIGGERS),
		).toBe(true);
	});
	...
	it("accepts text matching 'my name is' trigger", () => {
		expect(
			shouldCapture(
				"my name is Claude and I like TypeScript",
				MAX_CHARS,
				TRIGGERS,
			),
		).toBe(true);
	...
	it("is case-insensitive for trigger match", () => {
		expect(
			shouldCapture("I PREFER uppercase sometimes", MAX_CHARS, TRIGGERS),
		).toBe(true);
	});
});
...
describe("detectCategory", () => {
	...
	it("returns 'decision' for 'went with' keyword", () => {
		expect(
			detectCategory(
				"we went with Postgres for the database",
				decisionRegex,
				preferenceRegex,
				entityRegex,
				factRegex,
	...
	it("returns 'decision' for 'always use' keyword", () => {
		expect(
			detectCategory(
				"I always use two spaces for indentation",
				decisionRegex,
				preferenceRegex,
				entityRegex,
				factRegex,
	...
	it("returns 'entity' for text matching entity regex", () => {
		expect(
			detectCategory(
				"the project is called Acme Corp",
				decisionRegex,
				preferenceRegex,
				entityRegex,
				factRegex,
	...
	it("decision takes priority over preference when both match", () => {
		expect(
			detectCategory(
				"I decided I prefer tabs over spaces",
				decisionRegex,
				preferenceRegex,
				entityRegex,
				factRegex,
