...
vi.mock("node:fs/promises", async (importOriginal) => {
	...
	return {
		...
		open: vi.fn((...args: Parameters<typeof actual.open>) =>
			actual.open(...args),
		),
	};
});
...
const makeEnoent = () =>
	Object.assign(
		new Error("ENOENT: no such file or directory, open '/fake/path'"),
		{ code: "ENOENT" },
	);

const makeEperm = () =>
	Object.assign(
		new Error("EPERM: operation not permitted, open '/fake/path'"),
		{ code: "EPERM" },
	);
...
describe("passive-observer ENOENT handling (Issue #485)", () => {
	...
	it("stat ENOENT — other sessions in same dir are still processed", async () => {
		...
		const goodContent =
			'{"message":{"role":"user","content":"We use TypeScript"}}\n';
	...
	it("stat non-ENOENT error (EPERM) — is still counted as error and reported", async () => {
		...
		expect(
			logger.warn.mock.calls.some((c: unknown[]) =>
				String(c[0]).includes("failed to stat"),
			),
		).toBe(true);
	});
	...
	it("open ENOENT — skips file without incrementing errors", async () => {
		...
		expect(
			logger.info.mock.calls.some((c: unknown[]) =>
				String(c[0]).includes("pruned between scan and read"),
			),
		).toBe(true);
		expect(
			logger.warn.mock.calls.some((c: unknown[]) =>
	...
	it("open ENOENT — other sessions still processed after the pruned one", async () => {
		...
		writeFileSync(
			goodFile,
			'{"message":{"role":"user","content":"The team uses Rust"}}\n',
		);
		...
		const chatSpy = vi
			...
			.mockResolvedValue(
				JSON.stringify([
					{
						text: "The team uses Rust for CLI tools",
						category: "fact",
						importance: 0.8,
					},
				]),
	...
	it("multiple pruned files — all skipped, zero errors total", async () => {
		for (let i = 0; i < 3; i++) {
			writeFileSync(
				join(sessionsDir, `pruned-${i}.jsonl`),
