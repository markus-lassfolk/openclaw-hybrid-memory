...
describe("FactsDB satisfies FactsDbWithEmbeddings", () => {
  ...
  it("getEmbeddingsByModel returns typed records", () => {
    ...
    factsDb.storeEmbedding(factId, "domain-model", "canonical", new Float32Array([1, 0, 0, 0]), 4);
...
describe("Multi-model semantic search via fact_embeddings", () => {
  it("finds facts when query vector matches stored embedding", async () => {
    ...
    registry.register({ name: "nomic-embed-text", provider: "ollama", dimensions: 4, role: "domain" });
  ...
  it("excludes facts with low cosine similarity (below 0.3 threshold)", async () => {
    ...
    factsDb.storeEmbedding(factId, "nomic-embed-text", "canonical", new Float32Array([0, 1, 0, 0]), 4);
  ...
  it("merges results from multiple models via RRF", async () => {
    ...
    factsDb.storeEmbedding(factA, "model-1", "canonical", new Float32Array([1, 0, 0, 0]), 4);
    factsDb.storeEmbedding(factB, "model-2", "canonical", new Float32Array([0, 1, 0, 0]), 4);
