...
function makeMockProvider(modelName: string, dims: number): EmbeddingProvider {
  return {
    modelName,
    dimensions: dims,
    embed: vi.fn().mockResolvedValue(Array.from({ length: dims }, (_, i) => i / dims)),
    embedBatch: vi.fn().mockResolvedValue([Array.from({ length: dims }, (_, i) => i / dims)]),
  };
}

describe("EmbeddingRegistry — ONNX integration", () => {
  it("creates ONNX providers for onnx models", async () => {
    const { buildEmbeddingRegistry } = await import("../services/embedding-registry.js");
    const primary = makeMockProvider("text-embedding-3-small", 1536);
    const multiModels: EmbeddingModelConfig[] = [
      { name: "all-MiniLM-L6-v2", provider: "onnx", dimensions: 384, role: "domain" },
    ];
