...
describe("resolveCaptureProvenance", () => {
	...
	it("blocks system sessions even when no cron prompt is present", () => {
		const result = resolveCaptureProvenance(
			{
				messages: [
					{ role: "user", content: "Internal automation status report" },
				],
			},
...
describe("auto-capture provenance metadata helpers", () => {
	it("encodes extraction method with role and origin", () => {
		expect(
			getAutoCaptureExtractionMethod("user", { origin: "interactive" }),
		).toBe("auto-capture:user:interactive");
		expect(
			getAutoCaptureExtractionMethod("assistant", { origin: "cron" }),
		).toBe("auto-capture:assistant:cron");
	});
