...
function makeEntry(overrides: Partial<ActiveTaskEntry> = {}): ActiveTaskEntry {
	const now = Date.now();
	const defaultStarted = new Date(now - 60 * 60 * 1000).toISOString();
	const defaultUpdated = new Date(now - 5 * 60 * 1000).toISOString();
	return {
		label: "test-task",
...
describe("detectStaleTasks", () => {
	...
	it("treats UNKNOWN_ACTIVE_TASK_TIME as stale", async () => {
		const { UNKNOWN_ACTIVE_TASK_TIME } = await import(
			"../services/active-task.js"
		);
	...
	it("caps injection to budget (approximate)", () => {
		...
		const tasks = Array.from({ length: 20 }, (_, i) =>
			makeEntry({
				label: `task-${i}`,
				description:
					"A very long description that takes up lots of space and tokens in the injection block",
...
describe("upsertTask", () => {
	...
	it("updates existing task when label matches", () => {
		const existing = [
			makeEntry({ label: "task", status: "In progress", next: "original" }),
		];
		const result = upsertTask(
			existing,
			makeEntry({ label: "task", status: "Waiting", next: "updated" }),
		);
	...
	it("updates the 'updated' timestamp on upsert", () => {
		...
		const existing = [
			makeEntry({ label: "task", updated: "2020-01-01T00:00:00.000Z" }),
		];
	...
	it("preserves order for non-matching tasks", () => {
		...
		const result = upsertTask(
			existing,
			makeEntry({ label: "b", status: "Waiting" }),
		);
...
describe("completeTask", () => {
	it("removes task from active and returns completed entry", () => {
		const active = [
			makeEntry({ label: "task-a" }),
			makeEntry({ label: "task-b" }),
		];
...
describe("readActiveTaskFile / writeActiveTaskFile", () => {
	...
	it("returns null when file does not exist", async () => {
		const result = await readActiveTaskFile(
			join(tmpDir, "ACTIVE-TASKS.md"),
			1440,
		);
...
describe("flushCompletedTaskToMemory", () => {
	...
	it("appends to existing memory log file", async () => {
		...
		// First flush
		await flushCompletedTaskToMemory(
			makeEntry({ label: "task-a", status: "Done" }),
			tmpDir,
		);
		// Second flush
		const filePath = await flushCompletedTaskToMemory(task, tmpDir);
...
describe("writeTaskSignal / readPendingSignals / deleteSignal", () => {
	...
	it("sanitises label to be filesystem-safe", async () => {
		const signal = makeSignal();
		const filePath = await writeTaskSignal(
			"label with spaces/and:colons",
			signal,
			tmpDir,
		);
	...
	it("removes corrupt JSON signal files older than the stale threshold", async () => {
		...
		const signals = await readPendingSignals(tmpDir);
		expect(signals).toHaveLength(1);
		await expect(readFile(badPath, "utf-8")).rejects.toMatchObject({
			code: "ENOENT",
		});
	});
	...
	it("removes stale abandoned atomic-write temp files under task-signals", async () => {
		...
		await expect(readFile(tmpPath, "utf-8")).rejects.toMatchObject({
			code: "ENOENT",
		});
	});
	...
	it("preserves optional fields in signal round-trip", async () => {
		...
		const signals = await readPendingSignals(tmpDir);
		expect(signals[0].statusChange).toEqual({
			from: "in-progress",
			to: "blocked",
		});
	...
	it("creates and validates OCTAVE handoff artifacts", () => {
		const artifact = createOctaveTaskHandoffArtifact(
			makeSignal({ signal: "update" }),
		);
	...
	it("calls merge when file was modified concurrently", async () => {
		...
		await writeActiveTaskFileOptimistic(
			...
			async (fresh) => {
				...
				return [
					[...fresh.active, makeEntry({ label: "mine" })],
					fresh.completed,
				];
			},
		);
...
`,
	...
	it("does not reconcile when session jsonl exists", async () => {
		...
		await mkdir(join(openclawHome, "agents", "x", "sessions"), {
			recursive: true,
		});
		await writeFile(
			join(openclawHome, "agents", "x", "sessions", `${key}.jsonl`),
			"{}\n",
			"utf-8",
		);
		await writeFile(
...
describe("registerActiveTaskCommands", () => {
	...
	it("accepts bare 'active-tasks' and 'active-tasks list'", async () => {
		const { registerActiveTaskCommands } = await import(
			"../cli/active-tasks.js"
		);
		...
		try {
			...
			await program
				.parseAsync(["active-tasks"], { from: "user" })
				.catch(() => {});
			...
			await program
				.parseAsync(["active-tasks", "list"], { from: "user" })
				.catch(() => {});
		} finally {
			console.log = origLog;
	...
	it("active-tasks list subcommand is registered", async () => {
		...
		const activeTasksCmd = program.commands.find(
			(c) => c.name() === "active-tasks",
		);
