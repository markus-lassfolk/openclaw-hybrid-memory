...
import {
	type MigrateCredentialsOptions,
...
function makeFactsDB(overrides: Partial<MockFactsDB> = {}): MockFactsDB {
	return {
		...
		store: vi
			.fn()
			.mockReturnValue({ id: "pointer-id-1", text: "", category: "technical" }),
...
function makeCredentialsDB(
	overrides: Partial<MockCredentialsDB> = {},
): MockCredentialsDB {
	return {
		store: vi.fn(),
		get: vi
...
function makeEmbeddings(
	overrides: Partial<MockEmbeddings> = {},
): MockEmbeddings {
	return {
...
function makeOpts(
	...
	return {
		factsDb: (overrides.factsDb ??
			makeFactsDB()) as unknown as MigrateCredentialsOptions["factsDb"],
		edictStore: null as any,
		vectorDb: (overrides.vectorDb ??
			makeVectorDB()) as unknown as MigrateCredentialsOptions["vectorDb"],
		embeddings: (overrides.embeddings ??
			makeEmbeddings()) as unknown as MigrateCredentialsOptions["embeddings"],
...
describe("migrateCredentialsToVault", () => {
	describe("happy path — fact migrated to vault pointer", () => {
		it("migrates a credential fact, deletes original, stores pointer, deletes vector", async () => {
			...
			const result = await migrateCredentialsToVault(
				makeOpts({ factsDb, vectorDb, credentialsDb, embeddings }),
			);
		...
		it("stores pointer text containing service name and retrieve instructions", async () => {
			...
			const storeCall = (factsDb.store as ReturnType<typeof vi.fn>).mock
				.calls[0][0];
	...
	describe("idempotency — running twice does not create duplicate vault entries", () => {
		it("skips facts whose text already contains 'stored in secure vault'", async () => {
			...
			const factsDb = makeFactsDB({
				lookup: vi.fn().mockReturnValue([pointerFact]),
			});
			const credentialsDb = makeCredentialsDB();

			const result = await migrateCredentialsToVault(
				makeOpts({ factsDb, credentialsDb }),
			);
	...
	describe("unparseable fact — skipped counter incremented", () => {
		it("skips fact with no credential pattern and no usable secret value", async () => {
			...
			const factsDb = makeFactsDB({
				lookup: vi.fn().mockReturnValue([unparseable]),
			});
			const credentialsDb = makeCredentialsDB();
		...
		it("handles mix of parseable and unparseable facts", async () => {
			...
			const factsDb = makeFactsDB({
				lookup: vi.fn().mockReturnValue([credFact, unparseable]),
			});
			const credentialsDb = makeCredentialsDB();
	...
	describe("markDone=true — flag file written after successful migration", () => {
		it("calls writeFn with the migration flag path when markDone=true and no errors", async () => {
			...
			await migrateCredentialsToVault(
				makeOpts({
					...
					credentialsDb,
					markDone: true,
					migrationFlagPath: "/tmp/mig-flag",
					writeFn,
				}),
			);
		...
		it("does not call writeFn when markDone=false", async () => {
			...
			await migrateCredentialsToVault(
				makeOpts({ factsDb, credentialsDb, markDone: false, writeFn }),
			);
	...
	describe("partial failure — vault-store throws, migration continues, flag NOT written", () => {
		it("collects error and continues when credentialsDb.store throws on first item", async () => {
			...
			const factsDb = makeFactsDB({
				lookup: vi.fn().mockReturnValue([fact1, fact2]),
			});
			const credentialsDb = makeCredentialsDB({
				store: vi
					.fn()
			...
			const result = await migrateCredentialsToVault(
				makeOpts({ factsDb, credentialsDb, markDone: true, writeFn }),
			);
	...
	describe("vector delete failure — migration continues", () => {
		it("does not fail migration when vectorDb.delete throws a non-'not found' error", async () => {
			...
			const vectorDb = makeVectorDB({
				delete: vi
					.fn()
					.mockRejectedValue(new Error("lancedb connection timeout")),
			});
			const credentialsDb = makeCredentialsDB();

			const result = await migrateCredentialsToVault(
				makeOpts({ factsDb, vectorDb, credentialsDb }),
			);
	...
	describe("aliasDb — deleteByFactId called when provided", () => {
		it("calls aliasDb.deleteByFactId for migrated facts", async () => {
			...
			await migrateCredentialsToVault(
				makeOpts({
					factsDb,
					credentialsDb,
					aliasDb: aliasDb as unknown as MigrateCredentialsOptions["aliasDb"],
				}),
			);
		...
		it("works without aliasDb (aliasDb=null)", async () => {
			...
			const result = await migrateCredentialsToVault(
				makeOpts({ factsDb, credentialsDb, aliasDb: null }),
			);
	...
	describe("failure paths — factsDb.store, embeddings.embed, vectorDb.store", () => {
		...
		it("adds vector error and still counts migration when embeddings.embed rejects", async () => {
			...
			const result = await migrateCredentialsToVault(
				makeOpts({ factsDb, credentialsDb, embeddings }),
			);
		...
		it("suppresses AllEmbeddingProvidersFailed when the pointer embedding fails for expected reasons", async () => {
			const captureSpy = vi
				.spyOn(errorReporter, "capturePluginError")
			...
			const embeddings = makeEmbeddings({
				embed: vi
					.fn()
					.mockRejectedValue(new AllEmbeddingProvidersFailed([rateLimitErr])),
			});

			const result = await migrateCredentialsToVault(
				makeOpts({ factsDb, credentialsDb, vectorDb, embeddings }),
			);
		...
		it("adds vector error and still counts migration when vectorDb.store rejects", async () => {
			...
			const result = await migrateCredentialsToVault(
				makeOpts({ factsDb, credentialsDb, vectorDb }),
			);
	...
	describe("writeFn error path — error collected, result not re-thrown", () => {
		it("adds error to result when writeFn throws, does not re-throw", async () => {
			...
			const result = await migrateCredentialsToVault(
				makeOpts({ factsDb, markDone: true, writeFn }),
			);
