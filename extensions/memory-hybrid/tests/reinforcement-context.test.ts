...
describe("FIFO eviction at maxEventsPerFact (#259)", () => {
  it("evicts oldest events when limit is reached", () => {
    ...
    for (let i = 0; i < 5; i++) {
      db.reinforceFact(fact.id, `praise ${i}`, { querySnippet: `query ${i}` }, { maxEventsPerFact });
    }
...
describe("calculateDiversityScore (#259)", () => {
  ...
  it("returns 1.0 when all query snippets are unique", () => {
    const fact = storeFact();
    db.reinforceFact(fact.id, "praise", { querySnippet: "how to set up typescript" });
    db.reinforceFact(fact.id, "praise", { querySnippet: "how to configure eslint" });
  ...
  it("returns less than 1.0 when queries are repeated", () => {
    ...
    db.reinforceFact(fact.id, "praise", { querySnippet: "how to set up typescript" });
    db.reinforceFact(fact.id, "praise", { querySnippet: "how to set up typescript" });
    db.reinforceFact(fact.id, "praise", { querySnippet: "different query here" });
  ...
  it("returns low score when all queries are the same", () => {
    const fact = storeFact();
    for (let i = 0; i < 4; i++) {
      db.reinforceFact(fact.id, "praise", { querySnippet: "same query every time" });
    }
    const score = db.calculateDiversityScore(fact.id);
