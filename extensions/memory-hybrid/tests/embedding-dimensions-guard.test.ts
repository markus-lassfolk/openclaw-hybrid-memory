...
describe("assertEmbeddingDimensionsForVectorDb (#944)", () => {
	...
	it("rejects NaN, non-integer, and non-positive values", () => {
		expect(() => assertEmbeddingDimensionsForVectorDb(Number.NaN)).toThrow(
			/Invalid embedding dimensions/,
		);
		expect(() => assertEmbeddingDimensionsForVectorDb(1.5)).toThrow(
			/Invalid embedding dimensions/,
		);
		expect(() => assertEmbeddingDimensionsForVectorDb(0)).toThrow(
			/Invalid embedding dimensions/,
		);
		expect(() => assertEmbeddingDimensionsForVectorDb(-8)).toThrow(
			/Invalid embedding dimensions/,
		);
	});
});
