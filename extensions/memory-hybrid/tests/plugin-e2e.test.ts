...
const { FactsDB, VectorDB, findSimilarByEmbedding, VerificationStore } =
	_testing;
...
function buildE2EContext(opts: {
	...
	return {
		...
		timers: {
			proposalsPruneTimer: {
				value: null as ReturnType<typeof setInterval> | null,
			},
		},
	...
	it("register() does not throw and core memory tools are registered", () => {
		...
		const mockApi = {
			...
			resolvePath: (p: string) =>
				p.startsWith("/") || /^[A-Z]:/.test(p) ? p : join(tmpDir, p),
		};
...
describe("Store and recall e2e (real FactsDB + VectorDB, mock embeddings)", () => {
	...
	it("memory_store then memory_recall by id returns the stored fact", async () => {
		registerTools(
			buildE2EContext({ tmpDir, factsDb, vectorDb, cfg, api }) as never,
			api as never,
		);
		...
		const recallByIdResult = (await recallTool?.execute("call-2", {
			id: factId,
		})) as {
			content?: { type: string; text: string }[];
	...
	it("memory_store accepts why and memory_recall returns lineage context", async () => {
		...
		const why =
			"Previous in-place deploy caused a 12-minute outage during rollback";
		...
		const recallByIdResult = (await recallTool?.execute("call-why-2", {
			id: factId,
		})) as {
			content?: { type: string; text: string }[];
			details?: {
				count: number;
				memories?: { id: string; text: string; why?: string }[];
			};
		};
		expect(recallByIdResult.details?.count).toBe(1);
		...
		const matched = (recallByQueryResult.details?.memories ?? []).find((m) =>
	...
	it("memory_recall by query returns the stored fact (semantic path)", async () => {
		...
		const texts = (recallByQueryResult.details?.memories ?? []).map(
			(m) => m.text,
		);
	...
	it("memory_forget by memoryId removes the fact", async () => {
		...
		const beforeForget = (await recallTool?.execute("call-2", { id })) as {
			details?: { count: number };
		};
		expect(beforeForget.details?.count).toBe(1);

		await forgetTool?.execute("call-3", { memoryId: id });
		const afterForget = (await recallTool?.execute("call-4", { id })) as {
			details?: { count: number };
		};
...
describe("Core and common flows e2e", () => {
	...
	it("memory_store with entity/key/value then memory_recall by id returns structured fact", async () => {
		...
		const recallResult = (await recallTool?.execute("call-2", {
			...
			details?: {
				count: number;
				memories?: { entity?: string; key?: string; text: string }[];
			};
		};
	...
	it("memory_promote: store session-scoped then promote to global then recall", async () => {
		...
		const id = storeResult.details?.id;
		const promoteResult = (await promoteTool?.execute("call-2", {
			memoryId: id,
			scope: "global",
		})) as {
		...
		const recallResult = (await recallTool?.execute("call-3", { id })) as {
			details?: { count: number };
		};
		expect(recallResult.details?.count).toBe(1);
	});
	...
	it("memory_checkpoint save and restore returns saved intent and state", async () => {
		...
		const restoreResult = (await checkpointTool?.execute("call-2", {
			action: "restore",
		})) as {
			content?: { text: string }[];
			details?: {
				action: string;
				checkpoint?: { intent: string; state: string };
			};
		};
		expect(restoreResult.details?.action).toBe("restored");
		expect(restoreResult.details?.checkpoint?.intent).toBe(
			"Refactor auth module",
		);
		expect(restoreResult.details?.checkpoint?.state).toBe(
			"About to change login flow",
		);
	});
	...
	it("memory_prune hard mode removes expired fact", async () => {
		...
		const pruneResult = (await pruneTool?.execute("call-1", {
			mode: "hard",
		})) as {
	...
	it("memory_store duplicate text returns similar-already-exists when fuzzyDedupe on", async () => {
		...
		const secondResult = (await storeTool?.execute("call-2", {
			text,
			importance: 0.7,
			category: "fact",
		})) as {
		...
		expect(secondResult.content?.[0]?.text?.toLowerCase()).toMatch(
			/similar|already exists|duplicate/,
		);
	});
});
...
describe("Advanced features e2e", () => {
	...
	it("memory_link and memory_graph: link two facts then explore graph", async () => {
		...
		const graphResult = (await graphTool?.execute("call-4", {
			factId: a.details?.id,
			depth: 2,
		})) as {
			content?: { text: string }[];
	...
	it("memory_verify and memory_verified_list: verify fact then list verified", async () => {
		...
		const factId = storeResult.details?.id;
		const verifyResult = (await verifyTool?.execute("call-2", { factId })) as {
	...
	it("memory_issue_create and memory_issue_list: create issue then list", async () => {
		...
		expect(
			issues.some((i: { title: string }) => i.title === "E2E test issue"),
		).toBe(true);
		issueStore.close();
	});
	...
	it("memory_clusters: returns cluster count or no-clusters message", async () => {
		...
		const b = (await storeTool?.execute("call-2", {
			text: "Cluster fact two",
			importance: 0.7,
			category: "fact",
		})) as {
			details?: { id: string };
		};
		await linkTool?.execute("call-3", {
			sourceFact: a.details?.id,
			targetFact: b.details?.id,
			linkType: "RELATED_TO",
		});
		const result = (await clustersTool?.execute("call-4", {
			minClusterSize: 2,
			save: true,
		})) as {
	...
	it("memory_path: shortest path between two linked facts", async () => {
		...
		const pathResult = (await pathTool?.execute("call-4", {
			from: a.details?.id,
			to: b.details?.id,
			maxDepth: 5,
		})) as {
