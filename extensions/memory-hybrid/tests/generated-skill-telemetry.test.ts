...
describe("skills doctor — disk reconciler", () => {
  ...
  it("skills doctor CLI reports partial fix with exit code 2", async () => {
    const { skillName } = createGeneratedSkill("Doctor partial fix skill");
    db.getRawDb()
      .prepare(
        `CREATE TRIGGER block_doctor_uninstall
         BEFORE UPDATE OF skill_state ON procedures
         WHEN NEW.skill_state = 'uninstalled'
         BEGIN
           SELECT RAISE(IGNORE);
         END`,
      )
    ...
    const result = JSON.parse(String(logSpy.mock.calls[0]?.[0] ?? "{}")) as {
      ...
      failedCount?: number;
      exitCode?: number;
      issues?: Array<{ skillName: string; fixed: boolean }>;
    };

    expect(result.fixedCount).toBe(0);
    expect(result.failedCount).toBeGreaterThan(0);
    expect(result.exitCode).toBe(2);
    expect(result.issues?.find((i) => i.skillName === skillName)?.fixed).toBe(false);
