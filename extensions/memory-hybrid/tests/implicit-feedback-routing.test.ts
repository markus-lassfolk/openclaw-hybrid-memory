...
function makeCtx(
	...
	sessionsDir: string,
	implicitFeedbackOverrides?: Partial<
		NonNullable<HybridMemoryConfig["implicitFeedback"]>
	>,
): HandlerContext {
	const cfg = {
		enabled: true,
		...
		JSON.stringify({
			...
			message: {
				...
				content: [
					{
						type: "text",
						text: "Perfect, that is exactly what I needed. Thanks!",
					},
				],
			},
		}),
		...
		JSON.stringify({
			...
			message: {
				...
				content: [
					{
						type: "text",
						text: "How do I sort an array of objects by a property in TypeScript?",
					},
				],
			},
		}),
		JSON.stringify({
			...
			message: {
				...
				content: [
					{
						type: "text",
						text: "You can use Array.prototype.sort with a comparator.",
					},
				],
			},
		}),
		JSON.stringify({
			...
			message: {
				...
				content: [
					{
						type: "text",
						text: "No that is not what I meant, I need it sorted in descending order, please fix that.",
					},
				],
			},
		}),
		JSON.stringify({
			...
			message: {
				...
				content: [
					{
						type: "text",
						text: "My apologies, here is the corrected descending sort.",
					},
				],
			},
		}),
...
describe("implicit feedback routing — positive → reinforcement", () => {
	...
	it("stores implicit_signals but skips reinforcement_log when feedToReinforcement=false", async () => {
		...
		const logRows = rawDb(db)
			.prepare("SELECT COUNT(*) as cnt FROM reinforcement_log")
			.get() as { cnt: number };
...
describe("implicit feedback routing — negative → pattern facts", () => {
	...
	it("stores pattern facts tagged [implicit-feedback, negative] when feedToSelfCorrection=true", async () => {
		...
		const negFacts = rawDb(db)
			.prepare(`SELECT * FROM facts WHERE source = 'implicit-feedback'`)
			.all() as Array<{
			category: string;
			tags: string;
		}>;
...
describe("implicit feedback routing — dry-run mode", () => {
	...
	it("reports signals but does NOT persist anything when dryRun=true", async () => {
		...
		const sigRows = rawDb(db)
			.prepare("SELECT COUNT(*) as cnt FROM implicit_signals")
			.get() as { cnt: number };
...
describe("ImplicitFeedbackConfig — trajectoryLLMAnalysis", () => {
	it("parseImplicitFeedbackConfig defaults trajectoryLLMAnalysis to false", async () => {
		const { parseImplicitFeedbackConfig } = await import(
			"../config/parsers/features.js"
		);
	...
	it("parseImplicitFeedbackConfig respects trajectoryLLMAnalysis: true", async () => {
		...
		const result = parseImplicitFeedbackConfig({
			implicitFeedback: { trajectoryLLMAnalysis: true },
		});
