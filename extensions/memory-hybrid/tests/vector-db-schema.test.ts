...
// ---------------------------------------------------------------------------
// VectorDB init-timeout recovery — broad-swallowed-db-availability-flag fix
// Issue #1495: after a transient init timeout, lanceDbAvailable=false but
// lanceInitFailed=false. Vector ops must still call ensureInitialized() so
// the retry path can run on the next attempt.
// ---------------------------------------------------------------------------
...
describe("VectorDB init-timeout recovery (issue #1495)", () => {
  ...
  /**
   * Simulate the state left by an init timeout:
  ...
  it("permanent connect failure (lanceInitFailed=true) skips ensureInitialized()", async () => {
    ...
    // Permanent failure: ensureInitialized() should NOT be called (early-exit guard).
