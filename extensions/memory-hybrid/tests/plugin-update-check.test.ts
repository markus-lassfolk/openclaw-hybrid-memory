...
describe("plugin update check utilities", () => {
	...
	it("prefers the newest published version across npm and GitHub", async () => {
		const fetchMock = vi.fn(async (input: string | URL) => {
			...
			if (new URL(url).hostname === "registry.npmjs.org") {
				return new Response(JSON.stringify({ version: "2026.3.180" }), {
					status: 200,
				});
			}
			return new Response(JSON.stringify({ tag_name: "v2026.3.190" }), {
				status: 200,
			});
		});

		await expect(
			fetchLatestPublishedVersion(fetchMock as typeof fetch),
		).resolves.toEqual({
	...
	it("falls back cleanly when one upstream is unavailable", async () => {
		const fetchMock = vi.fn(async (input: string | URL) => {
			...
			return new Response(JSON.stringify({ tag_name: "v2026.3.188" }), {
				status: 200,
			});
		});
	...
	it("respects staleness and nudge intervals", () => {
		...
		expect(isVersionCheckCacheFresh(entry, nudge.cacheTtlHours, nowMs)).toBe(
			true,
		);
	...
	it("logs a warning and stamps lastNudgedAt when an outdated install is due for a nudge", () => {
		...
		expect(logger.warn).toHaveBeenCalledWith(
			expect.stringContaining("Telemetry is muted on outdated clients"),
		);
		expect(updated.lastNudgedAt).toBeDefined();
		expect(
			markUpdateNudged(entry, "2026-03-24T11:00:00.000Z").lastNudgedAt,
		).toBe("2026-03-24T11:00:00.000Z");
	});
});
