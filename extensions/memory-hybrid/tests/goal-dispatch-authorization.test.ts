...
describe("goal dispatch authorization", () => {
  it("supports custom mappings and multiple independently named classes", () => {
    ...
    expect(evaluateGoalDispatch(policy, write({ taskClass: "escalate", requestedAgent: "escalator-a", actualAgent: "escalator-a", writeScope: ["docs"] })).allowed).toBe(true);
  });
  ...
  it("rejects an agent outside the selected class", () =>
    expect(evaluateGoalDispatch(policy, write({ actualAgent: "reviewer-a", requestedAgent: "reviewer-a" })).reason).toContain("not allowed"));
  it("accepts only explicitly declared read-only work", () =>
    expect(evaluateGoalDispatch(policy, { taskClass: "review", requestedAgent: "reviewer-a", actualAgent: "reviewer-a", readOnly: true }).allowed).toBe(true));
  it("rejects read-only declaration mismatches and missing declarations", () => {
    ...
    expect(evaluateGoalDispatch(policy, { taskClass: "review", requestedAgent: "reviewer-a", actualAgent: "reviewer-a" }).reason).toContain("readOnly");
  });
  ...
  it("rejects sibling PR/branch and stale SHA", () => {
    expect(evaluateGoalDispatch(policy, write({ prNumber: 2233, branch: "fix/sibling" })).reason).toContain("non-canonical");
