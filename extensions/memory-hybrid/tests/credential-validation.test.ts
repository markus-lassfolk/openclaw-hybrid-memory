...
describe("validateCredentialValue", () => {
	...
	it("accepts sk- key when hasPatternMatch", () => {
		expect(
			validateCredentialValue("sk-proj-abc123def456ghi789", "api_key", true),
		).toEqual({ ok: true });
	});
	...
	it("rejects natural language when no pattern match", () => {
		expect(
			validateCredentialValue(
				"requires explicit login via the gateway",
				"other",
				false,
			),
		).toEqual({
	...
	it("accepts multi-word passphrase when hasPatternMatch is true (scanner extracted it directly)", () => {
		expect(
			validateCredentialValue(
				"my secret key with spaces 12345",
				"api_key",
				true,
			),
		).toEqual({ ok: true });
	...
	it("rejects path-like value when no pattern match", () => {
		expect(
			validateCredentialValue("/root/.config/systemd/user/ope", "other", false),
		).toEqual({
			ok: false,
			reason: "path",
		});
	...
	it("rejects empty value", () => {
		expect(validateCredentialValue("", "api_key", true)).toEqual({
			ok: false,
			reason: "empty",
		});
	});
	...
	it("rejects value shorter than 8 chars", () => {
		expect(validateCredentialValue("abc1234", "api_key", true)).toEqual({
			ok: false,
			reason: "value_too_short",
		});
	});
});
...
describe("validateAndNormalizeServiceName", () => {
	it("preserves hostname with dots", () => {
		expect(validateAndNormalizeServiceName("api.example.com")).toBe(
			"api.example.com",
		);
	});

	it("preserves URL-style service", () => {
		expect(validateAndNormalizeServiceName("postgres://host/db")).toBe(
			"postgres://host/db",
		);
	});

	it("normalizes and applies map", () => {
		expect(validateAndNormalizeServiceName("anthropic_api_key")).toBe(
			"anthropic",
		);
	});
	...
	it("rejects sentence-like slug", () => {
		expect(
			validateAndNormalizeServiceName(
				"be-root-or-plugin-is-blocked-5-kill-gateway",
			),
		).toBeNull();
	});
...
describe("auditCredentialValue", () => {
	it("flags natural language", () => {
		expect(
			auditCredentialValue("the token is required for login", "other"),
		).toContain("natural_language");
	});
...
describe("normalizeServiceForDedup", () => {
	...
	it("preserves :// for URL-style services", () => {
		expect(normalizeServiceForDedup("postgres://host/db")).toBe(
			"postgres://host/db",
		);
	});
...
describe("tryParseCredentialForVault — P2 hasPatternMatch bypass", () => {
	it("rejects narrative value even when pattern is present elsewhere in text", () => {
		...
		const text =
			"The login token is sk-abc123def456xyz789jkl012. It requires explicit login via the gateway";
