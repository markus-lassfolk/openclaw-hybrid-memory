...
describe("retrieval-mode-policy", () => {
	...
	it("clamps interactive orchestrator budget overrides to ambient budget", () => {
		...
		expect(
			resolveOrchestratorBudgetTokens(
				RETRIEVAL_MODE.INTERACTIVE_RECALL,
				retrievalCfg,
				2500,
			),
		).toBe(700);
	...
	it("skips HyDE only for interactive recall mode when skipForInteractiveTurns=true", () => {
		expect(shouldSkipHydeForMode(RETRIEVAL_MODE.INTERACTIVE_RECALL, true)).toBe(
			true,
		);
		expect(shouldSkipHydeForMode(RETRIEVAL_MODE.EXPLICIT_DEEP, true)).toBe(
			false,
		);
	});
});
