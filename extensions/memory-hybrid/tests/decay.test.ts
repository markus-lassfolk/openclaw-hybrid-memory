...
describe("classifyDecay — English keys (regression)", () => {
	...
	it('key "email" → permanent', () => {
		expect(classifyDecay(null, "email", null, "user@example.com")).toBe(
			"permanent",
		);
	});
	...
	it('key "architecture" → permanent', () => {
		expect(classifyDecay(null, "architecture", null, "monolith")).toBe(
			"permanent",
		);
	});

	it('key "birthday" → permanent', () => {
		expect(classifyDecay(null, "birthday", null, "1990-01-01")).toBe(
			"permanent",
		);
	});

	it('key "phone" → permanent', () => {
		expect(classifyDecay(null, "phone", null, "+46123456789")).toBe(
			"permanent",
		);
	});

	it('entity "decision" → permanent', () => {
		expect(classifyDecay("decision", null, null, "we chose React")).toBe(
			"permanent",
		);
	});

	it('entity "convention" → permanent', () => {
		expect(classifyDecay("convention", null, null, "use camelCase")).toBe(
			"permanent",
		);
	});

	it('key "current_file" → session', () => {
		expect(classifyDecay(null, "current_file", null, "index.ts")).toBe(
			"session",
		);
	});
	...
	it('key "checkpoint" → checkpoint', () => {
		expect(classifyDecay(null, "checkpoint", null, "build passed")).toBe(
			"checkpoint",
		);
	});

	it('key "preflight_check" → checkpoint', () => {
		expect(classifyDecay(null, "preflight_check", null, "ok")).toBe(
			"checkpoint",
		);
	});
...
describe("classifyDecay — text-based regex (regression)", () => {
	...
	it('"we decided to use X" → permanent via text regex', () => {
		expect(classifyDecay(null, null, null, "we decided to use X")).toBe(
			"permanent",
		);
	});
	...
	it('"currently debugging right now" → session via text regex', () => {
		expect(
			classifyDecay(null, null, null, "currently debugging right now"),
		).toBe("session");
	});

	it('"need to finish the blocker" → active via text regex', () => {
		expect(classifyDecay(null, null, null, "need to finish the blocker")).toBe(
			"active",
		);
	});
});
...
describe("classifyDecay — non-English keys (German translation in file)", () => {
	...
	it('key "aufgabe" (German: task) → active', () => {
		expect(classifyDecay(null, "aufgabe", null, "Aufgabe erledigen")).toBe(
			"active",
		);
	});
	...
	it('key "architektur" (German: architecture) → permanent', () => {
		expect(classifyDecay(null, "architektur", null, "Microservices")).toBe(
			"permanent",
		);
	});

	it('key "geburtsdatum" (German: birthday) → permanent', () => {
		expect(classifyDecay(null, "geburtsdatum", null, "1990-01-01")).toBe(
			"permanent",
		);
	});

	it('key "aktuelle_datei" (German: current_file) → session', () => {
		expect(classifyDecay(null, "aktuelle_datei", null, "index.ts")).toBe(
			"session",
		);
	});

	it('key "kontrollpunkt" (German: checkpoint) → checkpoint', () => {
		expect(classifyDecay(null, "kontrollpunkt", null, "passed")).toBe(
			"checkpoint",
		);
	});
	...
	it('entity "entscheidung" (German: decision) → permanent', () => {
		expect(
			classifyDecay("entscheidung", null, null, "wir haben React gewählt"),
		).toBe("permanent");
	});

	it('entity "konvention" (German: convention) → permanent', () => {
		expect(classifyDecay("konvention", null, null, "camelCase verwenden")).toBe(
			"permanent",
		);
	});
});
...
describe("classifyDecay — non-English keys (French translation in file)", () => {
	...
	it('key "anniversaire" (French: birthday) → permanent', () => {
		expect(classifyDecay(null, "anniversaire", null, "1990-01-01")).toBe(
			"permanent",
		);
	});
	...
	it('entity "décision" (French: decision) → permanent', () => {
		expect(
			classifyDecay("décision", null, null, "nous avons choisi React"),
		).toBe("permanent");
	});
});
