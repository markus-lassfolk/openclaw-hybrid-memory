...
`;
		...
		expect(parsed[0]).toBe(
			"User consistently favors functional composition over OOP",
		);
		expect(parsed[1]).toBe(
			"User prefers small, focused code units (functions <20 lines)",
		);
...
`;
	...
	it("returns empty for empty or non-matching input", () => {
		...
		expect(
			parsePatternsFromReflectionResponse("No PATTERN: here"),
		).toHaveLength(0);
		expect(
			parsePatternsFromReflectionResponse("RULE: This is a rule"),
		).toHaveLength(0);
	});
});
					...
					create: async () => {
						...
						return {
							choices: [
								{
									message: {
										content: "No patterns detected in the provided facts.",
									},
								},
							],
						};
	...
	it("LLM call is attributed to 'reflection-rules' feature", async () => {
		...
		const factsDb = {
			getByCategory: (cat: string) =>
				cat === "pattern" ? [pattern1, pattern2] : [],
			store: async () =>
				({
					id: "rule-1",
					text: "Always use functional patterns",
					category: "rule",
				}) as MemoryEntry,
			setEmbeddingModel: () => undefined,
		};
			...
			chat: {
				completions: {
					create: async () => {
						capturedFeature = getCurrentCostFeature();
						return {
							choices: [{ message: { content: "No rules detected." } }],
						};
					},
				},
			},
...
describe("runReflectionMeta cost attribution", () => {
	it("LLM call is attributed to 'reflection-meta' feature", async () => {
		...
		const factsDb = {
			getByCategory: (cat: string) =>
				cat === "pattern" ? [pattern1, pattern2, pattern3] : [],
			store: async () =>
				({
					id: "meta-1",
					text: "Core meta-pattern",
					category: "pattern",
				}) as MemoryEntry,
			setEmbeddingModel: () => undefined,
		};
			...
			chat: {
				completions: {
					create: async () => {
						capturedFeature = getCurrentCostFeature();
						return {
							choices: [{ message: { content: "No meta-patterns detected." } }],
						};
					},
				},
			},
...
describe("Reflection prompt template", () => {
	...
	it("fillPrompt substitutes window and facts", () => {
		const tmpl = loadPrompt("reflection");
		const filled = fillPrompt(tmpl, {
			window: "14",
			facts: "[preference] User likes dark mode",
		});
