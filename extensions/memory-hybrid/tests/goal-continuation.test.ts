...
const policy = {
  ...
  classes: {
    repair: {
      allowedAgents: ["forge"],
      readOnly: false,
      canonical: { repository: "org/plugin", prNumber: 963, branch: "fix/pr-963", remoteHead: "abc" },
      writeScope: ["services"],
      forbidNewPr: true,
...
describe("goal continuation controller", () => {
  it("turns the #963/#964-shaped worker/CI failure into a same-PR repair decision", async () => {
    ...
    const goal = await createGoal(
      ...
      {
        label: "pr963",
        description: "repair existing Armor PR",
    ...
    await updateGoalOnSubagentEnd(dir, {
      label: "armor-pr-963",
      sessionKey: "worker",
      success: false,
      outcome: "CI failed on existing PR #963",
    });
  ...
  it("is idempotent across duplicate heartbeat pulses", async () => {
    ...
    const goal = await createGoal(
      dir,
      { label: "pr964", description: "repair", acceptanceCriteria: ["green"], dispatchPolicy: policy },
      defaults,
    );
