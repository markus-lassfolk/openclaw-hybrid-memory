...
describe("FactsDB.store", () => {
	...
	it("stores and retrieves lineage why context", () => {
		...
		expect(entry.why).toBe(
			"Jest startup overhead was slowing local CI feedback loops",
		);
		const fetched = db.getById(entry.id);
		expect(fetched?.why).toBe(
			"Jest startup overhead was slowing local CI feedback loops",
		);
	});
});
...
describe("FactsDB.list / listForDashboard filter allowlist", () => {
	...
	it("listForDashboard returns empty facts and total 0 when decayClass is not allowlisted", () => {
		const r = db.listForDashboard({
			limit: 10,
			offset: 0,
			decayClass: "not-a-decay",
		});
...
describe("FactsDB.search", () => {
	...
	it("interactiveFtsFastPath returns FTS results (two-phase id fetch)", () => {
		...
		const fast = db.search("Unique marker", 5, {
...
describe("FactsDB bi-temporal", () => {
	...
	it("search with asOf returns only facts valid at that time", () => {
		...
		const afterSupersession = db.search("theme", 5, {
			asOf: supersessionTime + 1,
		});
	...
	it("search with includeSuperseded returns superseded facts", () => {
		...
		const withSuperseded = db.search("preference", 5, {
	...
	it("lookup with asOf returns only facts valid at that time", () => {
		...
		const afterSupersession = db.lookup("Entity", "key", undefined, {
			asOf: supersessionTime + 1,
		});
	...
	it("lookup with includeSuperseded returns superseded facts", () => {
		...
		const withSuperseded = db.lookup("X", "k", undefined, {
			includeSuperseded: true,
		});
...
describe("FactsDB.getRecentFacts", () => {
	it("returns facts from window and excludes pattern/rule by default", () => {
		...
		expect(recent.map((e) => e.category).sort()).toEqual([
			"decision",
			"preference",
		]);
	});
	...
	it("respects excludeCategories option", () => {
		...
		const withoutPref = db.getRecentFacts(14, {
...
describe("FactsDB.findSimilarForClassification", () => {
	it("finds facts by entity + key", () => {
		...
		const similar = db.findSimilarForClassification(
...
describe("FactsDB.createOrStrengthenRelatedLink", () => {
	...
	it("strengthens existing RELATED_TO link on repeated co-recall", () => {
		...
		db.createOrStrengthenRelatedLink(a.id, b.id);
		const afterFirst = db
			.getLinksFrom(a.id < b.id ? a.id : b.id)
			.find((l) => l.linkType === "RELATED_TO");
...
describe("FactsDB.strengthenRelatedLinksBatch", () => {
	...
	it("creates RELATED_TO links for all pairs in a single call", () => {
		...
		const allRelated = [...linksA, ...linksB, ...linksC].filter(
			(l) => l.linkType === "RELATED_TO",
		);
	...
	it("strengthens existing links on repeated calls", () => {
		...
		const strength1 =
			db.getLinksFrom(source1).find((l) => l.linkType === "RELATED_TO")
				?.strength ?? 0;
	...
	it("caps strength at 1.0 after many calls", () => {
		...
		const strength =
			db.getLinksFrom(source).find((l) => l.linkType === "RELATED_TO")
				?.strength ?? 0;
...
describe("FactsDB scoping", () => {
	...
	it("getById with scopeFilter returns null when not in scope", () => {
		...
		expect(
			db.getById(entry.id, { scopeFilter: { userId: "alice" } }),
		).not.toBeNull();
	...
	it("pruneSessionScope deletes session-scoped facts", () => {
		...
		const sessionNotes = remaining.filter((e) =>
			e.text.includes("Session note"),
		);
...
describe("FactsDB search reinforcement ranking", () => {
	it("reinforced fact ranks before non-reinforced when reinforcementBoost > 0", () => {
		...
		const results = db.search("auth API", 10, {
...
describe("FactsDB.findByIdPrefix", () => {
	...
	it("returns ambiguous when multiple IDs match prefix", () => {
		...
		const ambiguousPrefix = [...prefixMap.entries()].find(
			([, v]) => v.length >= 2,
		);
		if (ambiguousPrefix) {
...
describe("FactsDB migration #237: access_count and last_accessed_at", () => {
	...
	it("refreshAccessedFacts increments access_count and sets last_accessed_at", () => {
		...
		expect(updated?.lastAccessedAt!).toMatch(
...
describe("FactsDB new decay classes (#237)", () => {
	it.each([
		...
		"normal",
		"durable",
	] as const)("stores and retrieves decay_class=%s", (dc) => {
		const entry = db.store({
	...
	it("normal decay class has 2-week TTL", () => {
		...
		expect(Math.abs(entry.expiresAt! - (nowSec + twoWeeksSec))).toBeLessThan(
			60,
		);
	});
...
describe("FactsDB Episodes", () => {
	describe("recordEpisode", () => {
		...
		it("stores scope and related fact IDs", () => {
			...
			const relRows = (
				db as unknown as {
					liveDb: { prepare: (sql: string) => { all: () => unknown[] } };
				}
			).liveDb
			...
			expect(
				relRows.some(
					(r) => r.target_id === relatedId.id && r.relation_type === "PART_OF",
				),
			).toBe(true);
		});
	});
	...
	describe("searchEpisodes", () => {
		...
		it("filters by outcome", () => {
			...
			const successesAndFailures = db.searchEpisodes({
				outcome: ["success", "failure"],
			});
			expect(
				successesAndFailures.every(
		...
		it("filters by time range (since / until)", () => {
			...
			const window = db.searchEpisodes({
				since: now - 7200,
				until: now - 1800,
			});
			expect(
				window.every(
					(e) => e.timestamp >= now - 7200 && e.timestamp <= now - 1800,
				),
			).toBe(true);
		});
