...
function makeFactsDbStub(
	overrides?: Partial<
		Pick<FactsDB, "getCount" | "statsBreakdownByCategory" | "getBatch">
	>,
): FactsDB {
	return {
		getCount: overrides?.getCount ?? (() => 42),
		statsBreakdownByCategory:
			overrides?.statsBreakdownByCategory ??
			(() => ({ preference: 10, goal: 5, skill: 15, other: 12 })),
		getBatch:
			overrides?.getBatch ??
...
describe("sweepSessionHistory", () => {
	it("returns zero counts when session dir does not exist", async () => {
		...
		expect(result.eventsWritten + result.eventsSkipped).toBeGreaterThanOrEqual(
			0,
		);
	...
	it("writes one event when sessions exist", async () => {
		...
		const sessionData = [
			JSON.stringify({
				message: "user asked about TypeScript decorators",
				timestamp: new Date().toISOString(),
			}),
			JSON.stringify({
				message: "assistant explained the concept",
				timestamp: new Date().toISOString(),
			}),
		].join("\n");
...
describe("sweepSystemHealth", () => {
	it("writes a system-health event", async () => {
		...
		const result = await sweepSystemHealth(
			bus,
			cfg,
			"/nonexistent/facts.db",
			0,
		);
...
describe("sweepAll", () => {
	...
	it("returns error summary for failed sensors", async () => {
		...
		const memPatterns = result.sensors.find(
			(s) => s.sensor === "memory-patterns",
		);
