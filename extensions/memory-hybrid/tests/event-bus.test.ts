...
describe("EventBus.appendEvent", () => {
	it("inserts an event and returns a positive integer id", () => {
		const id = bus.appendEvent("sensor.garmin", "garmin-sensor", {
			steps: 8000,
		});
	...
	it("auto-increments ids for successive inserts", () => {
		...
		const id2 = bus.appendEvent("sensor.github", "github-sensor", {
			commits: 3,
		});
	...
	it("throws RangeError for importance below 0", () => {
		expect(() => bus.appendEvent("sensor.test", "test", {}, -0.1)).toThrow(
			RangeError,
		);
	});

	it("throws RangeError for importance above 1", () => {
		expect(() => bus.appendEvent("sensor.test", "test", {}, 1.1)).toThrow(
			RangeError,
		);
	});
	...
	it("throws RangeError for NaN importance", () => {
		expect(() =>
			bus.appendEvent("sensor.test", "test", {}, Number.NaN),
		).toThrow(RangeError);
	});
...
describe("EventBus.updateStatus", () => {
	...
	it("does not overwrite existing processed_at on subsequent transitions", () => {
		...
		bus.updateStatus(id, "surfaced");
		const afterSecond = bus
			.queryEvents()
			.find((e) => e.id === id)?.processed_at;
...
describe("EventBus.dedup", () => {
	...
	it("returns false when matching fingerprint is older than cooldown", () => {
		...
		db2
			.prepare("UPDATE memory_events SET created_at = ? WHERE fingerprint = ?")
			.run(twoHoursAgo, fp);
		db2.close();
...
describe("EventBus.pruneArchived", () => {
	it("deletes archived events older than N days and returns correct count", () => {
		...
		db2
			.prepare(
				"UPDATE memory_events SET created_at = '2000-01-01T00:00:00.000Z'",
			)
			.run();
		db2.close();
	...
	it("does not delete non-archived events", () => {
		...
		expect(count).toBe(0);
		expect(
			bus.queryEvents({ status: "processed" }).some((e) => e.id === id),
		).toBe(true);
	});
...
describe("EventBus integration", () => {
	it("full lifecycle: append → query → updateStatus → verify", () => {
		...
		const fp = computeFingerprint(
			"sensor.github:repo123:3 new commits:2026-03-11",
		);
		const id = bus.appendEvent(
			"sensor.github",
			"github-sensor",
			{ commits: 3, repo: "my-repo" },
			0.7,
			fp,
		);
		expect(id).toBeGreaterThan(0);
		...
		expect(processedEvents.some((e) => e.id === id)).toBe(true);
		expect(
			processedEvents.find((e) => e.id === id)?.processed_at,
		).not.toBeNull();
...
describe("EventBus lifecycle", () => {
	...
	it("throws when used after close", () => {
		bus.close();
		expect(() => bus.appendEvent("sensor.test", "test", {})).toThrow(
			"EventBus is closed",
		);
	});
});
