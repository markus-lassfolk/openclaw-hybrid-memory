  ...
  - alias: Test # automation comment
    trigger:
      - platform: state
        entity_id: sensor.test`;
			...
			expect(result.automation[0].trigger[0]).toHaveProperty(
				"platform",
				"state",
			);
		});
	...
	describe("Bug 2a: Flow sequence quoted strings preserve type (no coercion)", () => {
		it('keeps ["true"] as string, not boolean', () => {
			const result = parseYaml('flags: ["true", "false"]') as Record<
				string,
				any
			>;
		...
		it("parses unquoted booleans and numbers correctly in flow sequences", () => {
			const result = parseYaml("vals: [true, false, 42, 3.14]") as Record<
				string,
				any
			>;
