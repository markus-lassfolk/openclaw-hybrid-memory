...
describe("memory_provenance tool", () => {
	it("returns derivedFrom + consolidationChain for a fact", async () => {
		...
		const result = (await api.callTool("memory_provenance", {
			factId: consolidatedFact.id,
		})) as any;
...
describe("consolidation provenance chain", () => {
	it("records CONSOLIDATED_FROM edges for merged facts", async () => {
		...
		const openai = {
			chat: {
				completions: {
					create: vi.fn(async () => ({
						choices: [
							{ message: { content: "The sky is blue during daylight." } },
						],
					})),
				},
			},
		...
		const mergedFact = remainingFacts.find(
			(f) => f.id !== source1.id && f.id !== source2.id,
		);
		expect(mergedFact).toBeDefined();

		const edges = provenanceService.getEdges(mergedFact?.id);
		const consolidatedFrom = edges.filter(
			(e) => e.edgeType === "CONSOLIDATED_FROM",
		);
