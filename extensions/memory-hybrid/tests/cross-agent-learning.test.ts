...
describe("parseCrossAgentLearningConfig", () => {
	...
	it("clamps windowDays to max 90", () => {
		const cfg = parseCrossAgentLearningConfig({
			crossAgentLearning: { windowDays: 200 },
		});
	...
	it("respects custom model", () => {
		const cfg = parseCrossAgentLearningConfig({
			crossAgentLearning: { model: "gpt-4o" },
		});
	...
	it("ignores invalid batchSize (<5)", () => {
		const cfg = parseCrossAgentLearningConfig({
			crossAgentLearning: { batchSize: 2 },
		});
	...
	it("clamps batchSize to max 100", () => {
		const cfg = parseCrossAgentLearningConfig({
			crossAgentLearning: { batchSize: 200 },
		});
...
describe("runCrossAgentLearning — LLM mock", () => {
		...
		const mockOpenAI = {
			chat: {
				completions: {
					create: async () => ({
						choices: [
							{ message: { content: JSON.stringify([generalisedLesson]) } },
						],
					}),
				},
			},
	...
	it("handles LLM returning empty array", async () => {
       ...
       VALUES (?, ?, 'pattern', 'agent', 'hearth', 0.8, 0.7, 'test', ?, ?, 'stable')`,
			...
			.run(
				"agent-h-1",
				"Check HA entity state before toggling lights",
				now,
				now,
			);
		...
		const mockOpenAI = {
			chat: {
				completions: {
					create: async () => ({
						choices: [
							{ message: { content: "INVALID JSON RESPONSE FROM LLM" } },
						],
					}),
				},
			},
...
describe("parseFrustrationDetectionConfig", () => {
	...
	it("clamps windowSize to [2, 50]", () => {
		expect(
			parseFrustrationDetectionConfig({
				frustrationDetection: { windowSize: 1 },
			}).windowSize,
		).toBe(8);
		expect(
			parseFrustrationDetectionConfig({
				frustrationDetection: { windowSize: 100 },
			}).windowSize,
		).toBe(8);
		expect(
			parseFrustrationDetectionConfig({
				frustrationDetection: { windowSize: 10 },
			}).windowSize,
		).toBe(10);
	});
	...
	it("clamps decayRate to (0, 1]", () => {
		expect(
			parseFrustrationDetectionConfig({
				frustrationDetection: { decayRate: 1.5 },
			}).decayRate,
		).toBe(0.85);
		expect(
			parseFrustrationDetectionConfig({
				frustrationDetection: { decayRate: 0 },
			}).decayRate,
		).toBe(0.85);
		expect(
			parseFrustrationDetectionConfig({
				frustrationDetection: { decayRate: 0.85 },
			}).decayRate,
		).toBe(0.85);
	});
...
describe("parseToolEffectivenessConfig", () => {
	...
	it("clamps topN to max 50", () => {
		const cfg = parseToolEffectivenessConfig({
			toolEffectiveness: { topN: 100 },
		});
		expect(cfg.topN).toBe(50);
	});

	it("clamps decayFactor to (0, 1]", () => {
		const outOfRange = parseToolEffectivenessConfig({
			toolEffectiveness: { decayFactor: 1.5 },
		});
		expect(outOfRange.decayFactor).toBe(0.95);
		const valid = parseToolEffectivenessConfig({
			toolEffectiveness: { decayFactor: 0.8 },
		});
	...
	it("respects custom lowScoreThreshold", () => {
		const cfg = parseToolEffectivenessConfig({
			toolEffectiveness: { lowScoreThreshold: 0.45 },
		});
...
describe("getCrossAgentLessons", () => {
	...
	function insertCrossAgentFact(opts: {
		text: string;
		tags?: string[];
		confidence?: number;
	}): void {
	...
	it("returns facts tagged with cross-agent", async () => {
		insertCrossAgentFact({
			text: "Always validate before executing",
			tags: ["cross-agent", "forge"],
		});
		const lessons = await getCrossAgentLessons(
			db,
			"forge",
			"validate execution",
			10,
	...
	it("returns general cross-agent lessons when no agent-specific match", async () => {
		insertCrossAgentFact({
			text: "General lesson for all agents",
			tags: ["cross-agent"],
		});
		const lessons = await getCrossAgentLessons(
			db,
	...
	it("ranks lessons with higher relevance to context first", async () => {
		insertCrossAgentFact({
			text: "Always test your code before committing",
			tags: ["cross-agent", "forge"],
		});
		insertCrossAgentFact({
			text: "General data backup policy",
			tags: ["cross-agent", "forge"],
		});
		const lessons = await getCrossAgentLessons(
			db,
...
describe("formatBriefInjection", () => {
	...
	it("formats lessons with header and bullet points", () => {
		const lessons = [
			makeLesson("1", "Always verify before committing", {
				tags: ["cross-agent", "forge"],
				confidence: 0.85,
			}),
		];
...
describe("verifyLessonForAgent", () => {
	...
	it("does nothing for non-existent lesson ID", async () => {
		// Should not throw
		await expect(
			verifyLessonForAgent(db, "non-existent-id", "forge"),
