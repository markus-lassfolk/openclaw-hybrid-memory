...
describe("preFilterSessions", () => {
  ...
  it("returns all sessions when disabled", async () => {
    ...
    const result = await preFilterSessions([path], { ...defaultConfig, enabled: false });
  ...
  it("keeps session when local model returns YES", async () => {
    const path = join(tmpDir, "session.jsonl");
    writeFileSync(path, msg("user", "Remember that I prefer dark mode always"), "utf-8");
  ...
  it("skips session when local model returns NO", async () => {
    const path = join(tmpDir, "session.jsonl");
    writeFileSync(path, msg("user", "Can you help me fix this bug please?"), "utf-8");
  ...
  it("keeps session when model returns ambiguous response (conservative)", async () => {
    const path = join(tmpDir, "session.jsonl");
    writeFileSync(path, msg("user", "What is the capital of France please?"), "utf-8");
  ...
  it("skips sessions with no extractable user messages", async () => {
    ...
    expect((mockClient.chat.completions.create as ReturnType<typeof vi.fn>).mock.calls).toHaveLength(0);
  });
  ...
  it("handles Qwen3 thinking-model YES response embedded in think tags", async () => {
    const path = join(tmpDir, "session.jsonl");
    writeFileSync(path, msg("user", "From now on always use TypeScript"), "utf-8");

    const thinkingResponse = "<think>\nLet me analyze...\nThis seems like a preference.\n</think>\nYES";
    const mockClient = {
      chat: {
        completions: {
          create: vi.fn().mockResolvedValue({
  ...
  it("strips ollama/ prefix from model name when classifying", async () => {
    ...
    await preFilterSessions([path], { ...defaultConfig, model: "ollama/qwen3:8b" }, { ollamaClient: mockClient });
