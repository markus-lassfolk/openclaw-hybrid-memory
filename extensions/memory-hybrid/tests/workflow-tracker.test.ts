...
describe("WorkflowTracker.push", () => {
	...
	it("uses provided startTime for duration calculation", () => {
		...
		const call = (store.record as ReturnType<typeof vi.fn>).mock
			.calls[0][0] as CreateWorkflowTraceInput;
...
describe("WorkflowTracker.flush", () => {
	it("calls store.record with correct arguments and returns trace id", () => {
		const store = makeMockStore();
		(store.record as ReturnType<typeof vi.fn>).mockReturnValue({
			id: "trace-abc123",
		});
		const tracker = new WorkflowTracker(store, ENABLED_CFG);
...
describe("WorkflowTracker — rate limiting", () => {
	it("allows exactly maxTracesPerDay flushes", () => {
		const store = makeMockStore();
		const cfg: WorkflowTrackingConfig = {
			enabled: true,
			maxTracesPerDay: 2,
			retentionDays: 90,
		};
