...
describe("registerPublicApiRoutes", () => {
	...
	it("serves search, timeline, stats, export, and fact endpoints", async () => {
		...
		const search = routes.find(
			(r) => r.path === `${PUBLIC_API_PREFIX}${PUBLIC_API_PATHS.search}`,
		)!;
		...
		const timeline = routes.find(
			(r) => r.path === `${PUBLIC_API_PREFIX}${PUBLIC_API_PATHS.timeline}`,
		)!;
		...
		const stats = routes.find(
			(r) => r.path === `${PUBLIC_API_PREFIX}${PUBLIC_API_PATHS.stats}`,
		)!;
		const statsRes = await stats.handler(
			fakeReq(`${PUBLIC_API_PREFIX}${PUBLIC_API_PATHS.stats}`),
		);
		expect(statsRes.status).toBe(200);
		expect(JSON.parse(statsRes.body).facts.active).toBeGreaterThanOrEqual(1);

		const exported = routes.find(
			(r) => r.path === `${PUBLIC_API_PREFIX}${PUBLIC_API_PATHS.export}`,
		)!;
		...
		expect(
			JSON.parse(exportRes.body).manifest.counts.facts,
		).toBeGreaterThanOrEqual(1);

		const fact = routes.find(
			(r) => r.path === `${PUBLIC_API_PREFIX}${PUBLIC_API_PATHS.fact}`,
		)!;
		const factRes = await fact.handler(
