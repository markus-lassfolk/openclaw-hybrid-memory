...
describe("extractGoalKeywords", () => {
  ...
  it("strips punctuation", () => {
    ...
    for (const k of kw) expect(k).not.toMatch(/[^a-z0-9\-_]/);
  });
});
...
describe("WorkflowStore.getSuccessRate", () => {
  beforeEach(() => {
    ...
    store.record({ goal: "g1", toolSequence: ["exec", "read"], outcome: "success" });
    store.record({ goal: "g2", toolSequence: ["exec", "read"], outcome: "success" });
    store.record({ goal: "g3", toolSequence: ["exec", "read"], outcome: "failure" });
  });
...
describe("WorkflowStore.summarizeGoalKinds", () => {
  ...
  it("filters by sinceSec using ISO cutoff (lexicographic compare with created_at)", () => {
    ...
    db.prepare("UPDATE workflow_traces SET created_at = ? WHERE id = ?").run("2026-06-05T08:00:00.000Z", morning.id);
    db.prepare("UPDATE workflow_traces SET created_at = ? WHERE id = ?").run("2026-06-05T14:00:00.000Z", afternoon.id);
...
describe("WorkflowStore.getPatterns", () => {
  ...
  it("filters by minSuccessRate", () => {
    ...
    for (const p of patterns) expect(p.successRate).toBeGreaterThanOrEqual(0.9);
  });
...
describe("WorkflowTracker", () => {
  ...
  it("no-ops when disabled", () => {
    const disabledTracker = new WorkflowTracker(store, { ...cfg, enabled: false });
