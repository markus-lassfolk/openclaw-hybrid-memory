...
} from "../backends/facts-db/links.js";
import { appendReinforcementQuote } from "../backends/facts-db/reinforcement.js";
import {
	getScanCursor,
	migrateScanCursorsTable,
	updateScanCursor,
} from "../backends/facts-db/scan-cursors.js";
...
describe("facts-db links module", () => {
    ...
    `);
		...
		const rows = db
			.prepare(
				`SELECT id, strength FROM memory_links WHERE source_fact_id = 'a' AND target_fact_id = 'b'`,
			)
