...
describe("findShortestPath: 1-hop paths", () => {
	...
	it("finds direct path via incoming edge (B→A traversed as A←B)", () => {
		...
		const db = buildMockDb(
			...
			{
				a: [
					{ id: "l1", sourceFactId: "b", linkType: "CAUSED_BY", strength: 0.8 },
				],
			},
		);
	...
	it("returns correct chain for 1-hop path", () => {
		...
		const db = buildMockDb(
			...
			{
				a: [
					{ id: "l1", targetFactId: "b", linkType: "PART_OF", strength: 1.0 },
				],
			},
			{},
		);
	...
	it("finds 2-hop path A→B→C", () => {
		...
		const db = buildMockDb(
			[a, b, c],
			{
				...
				b: [
					{ id: "l2", targetFactId: "c", linkType: "PART_OF", strength: 0.7 },
				],
			},
			{},
		);
	...
	it("finds 3-hop path A→B→C→D", () => {
		...
		const db = buildMockDb(
			[a, b, c, d],
			{
				...
				b: [
					{ id: "l2", targetFactId: "c", linkType: "CAUSED_BY", strength: 1.0 },
				],
				c: [
					{ id: "l3", targetFactId: "d", linkType: "PART_OF", strength: 1.0 },
				],
			},
			{},
		);
	...
	it("returns 1-hop direct path even when longer alternative exists", () => {
		...
		const db = buildMockDb(
			[a, b, c],
			{
				...
				c: [
					{ id: "l3", targetFactId: "b", linkType: "CAUSED_BY", strength: 0.3 },
				],
			},
			{},
		);
	...
	it("finds 2-hop path when 1-hop is unavailable", () => {
		...
		const db = buildMockDb(
			[a, b, c],
			{
				...
				c: [
					{ id: "l2", targetFactId: "b", linkType: "PART_OF", strength: 1.0 },
				],
			},
			{},
		);
