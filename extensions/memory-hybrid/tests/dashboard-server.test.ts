...
const describeCreateDashboardServer = (await detectLoopbackBindSupport())
	? describe
	: describe.skip;
...
describe("Memory Viewer API (Issue #1023)", () => {
	...
	async function makeContextWithStores(tmpDir: string) {
		const { FactsDB, VectorDB } = await import("../index.js").then(
			(m) => m._testing,
		);
		const { EdictStore } = await import("../backends/edict-store.js");
		const { VerificationStore } = await import(
			"../services/verification-store.js"
		);
	...
	async function withServer(
		fn: (
			ctx: Awaited<ReturnType<typeof makeContextWithStores>>,
			port: number,
		) => Promise<void>,
	) {
	...
	it("GET /api/viewer/facts/:id returns 404 for unknown id", async () => {
		await withServer(async (_ctx, port) => {
			const { status } = await apiGet(
				port,
				"/api/viewer/facts/00000000-0000-0000-0000-000000000000",
			);
			expect(status).toBe(404);
		});
	});
	...
	it("POST /api/viewer/facts/:id/verify verifies a fact", async () => {
		await withServer(async (ctx, port) => {
			...
			const target = facts.find(
				(fact: { text: string; id: string }) => fact.text === "To verify",
			);
	...
	it("POST /api/viewer/facts/:id/forget forgets a fact", async () => {
		await withServer(async (ctx, port) => {
			...
			const { facts } = JSON.parse(lb);
			const target = facts.find(
				(fact: { text: string; id: string }) => fact.text === "To forget",
			);
			expect(target?.id).toBeTruthy();
			const { status, body } = await apiPost(
				port,
				`/api/viewer/facts/${target.id}/forget`,
				"{}",
			);
			expect(status).toBe(200);
			expect(JSON.parse(body).ok).toBe(true);
	...
	it("GET /api/viewer/timeline/sessions aggregates event log data", async () => {
		await withServer(async (ctx, port) => {
			...
			ctx.eventLog.append({
				...
				content: { text: "Correction: do Z instead" },
			});
	...
	it("GET /api/viewer/timeline/sessions/:id returns detail with events", async () => {
		await withServer(async (ctx, port) => {
			...
			const { status, body } = await apiGet(
				port,
				`/api/viewer/timeline/sessions/${sessionId}`,
			);
			expect(status).toBe(200);
			const data = JSON.parse(body);
	...
	it("GET /api/viewer/timeline/sessions/:id returns 404 for unknown session", async () => {
		await withServer(async (ctx, port) => {
			const { status } = await apiGet(
				port,
				"/api/viewer/timeline/sessions/no-such-session-id",
			);
			expect(status).toBe(404);
		});
	});
