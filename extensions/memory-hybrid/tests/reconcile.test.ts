...
describe("VectorDB.getAllIds()", () => {
  ...
  it("returns stored IDs", async () => {
    const id1 = await vectorDb.store({ text: "fact one", vector: makeVector(), importance: 0.8, category: "fact" });
    const id2 = await vectorDb.store({ text: "fact two", vector: makeVector(), importance: 0.6, category: "fact" });
    const ids = await vectorDb.getAllIds();
  ...
  it("IDs are lowercase UUIDs", async () => {
    const stored = await vectorDb.store({ text: "fact", vector: makeVector(), importance: 0.5, category: "fact" });
    const ids = await vectorDb.getAllIds();
    for (const id of ids) {
...
describe("Reconciliation fix — delete vector orphans", () => {
  it("removes orphan vectors from LanceDB", async () => {
    ...
    await vectorDb.store({ id: orphanId, text: "orphan", vector: makeVector(), importance: 0.4, category: "fact" });
