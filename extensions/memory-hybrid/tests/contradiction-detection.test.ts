...
describe("detectContradictions: same entity+key, different value", () => {
  it("detects contradiction and creates record", () => {
    ...
    const contradictions = db.detectContradictions(newFact.id, "user", "theme", "light");
  ...
  it("creates a record in the contradictions table", () => {
    ...
    const [{ contradictionId }] = db.detectContradictions(newFact.id, "user", "theme", "light");
...
describe("detectContradictions: confidence reduction", () => {
  ...
  it("does not reduce below the floor of 0.1", () => {
    const old = storeFact("user", "theme", "dark", "User prefers dark mode", 0.2);
    const new1 = storeFact("user", "theme", "light", "User switched to light mode");
...
describe("detectContradictions: same value is NOT a contradiction", () => {
  it("does not detect contradiction when values match (case-insensitive)", () => {
    ...
    const newFact = storeFact("user", "theme", "dark", "User still uses dark mode");

    const contradictions = db.detectContradictions(newFact.id, "user", "theme", "dark");
...
describe("detectContradictions: different entity", () => {
  it("does not flag contradiction when entities differ", () => {
    ...
    const newFact = storeFact("bob", "theme", "light", "Bob prefers light mode");

    const contradictions = db.detectContradictions(newFact.id, "bob", "theme", "light");
    expect(contradictions).toHaveLength(0);
  });
});
...
describe("detectContradictions: null fields skip detection", () => {
  it("skips detection when entity is null", () => {
    storeFact("user", "theme", "dark", "User prefers dark mode");
    const newFact = storeFact(null, "theme", "light", "Someone prefers light mode");

    const contradictions = db.detectContradictions(newFact.id, null, "theme", "light");
    expect(contradictions).toHaveLength(0);
  });
  ...
  it("skips detection when key is null", () => {
    ...
    const contradictions = db.detectContradictions(newFact.id, "user", null, "light");
    expect(contradictions).toHaveLength(0);
  });
  ...
  it("skips detection when value is null", () => {
    ...
    const contradictions = db.detectContradictions(newFact.id, "user", "theme", null);
    expect(contradictions).toHaveLength(0);
  });
});
...
describe("detectContradictions: multiple conflicts", () => {
  it("detects multiple conflicting facts for the same entity+key", () => {
    ...
    const newFact = storeFact("user", "theme", "light", "User now prefers light mode");
...
describe("updateConfidence", () => {
  it("reduces confidence by delta and floors at 0.1", () => {
    const fact = storeFact("user", "theme", "dark", "User prefers dark mode", 0.15);
