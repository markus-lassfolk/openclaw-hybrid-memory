...
describe("explicit_frustration signal", () => {
	it("detects 'frustrating' keyword", () => {
		...
		expect(result.triggers.some((t) => t.type === "explicit_frustration")).toBe(
			true,
		);
...
describe("imperative_tone signal", () => {
	it("detects short imperative", () => {
		...
		expect(result.triggers.some((t) => t.type === "imperative_tone")).toBe(
			true,
		);
	});
	...
	it("does NOT fire on long sentence", () => {
		const result = detectFrustration(
			turns([
				"Run the tests and then check if they all pass and report back to me with the results",
			]),
			defaultCfg,
		);
		expect(result.triggers.some((t) => t.type === "imperative_tone")).toBe(
...
describe("caps_or_emphasis signal", () => {
	it("detects ALL CAPS words", () => {
		...
		expect(result.triggers.some((t) => t.type === "caps_or_emphasis")).toBe(
			true,
		);
	});
...
describe("repeated_instruction signal", () => {
	it("fires when user repeats same instruction", () => {
		const conversationTurns: FrustrationConversationTurn[] = [
			{
				role: "user",
				content: "Use TypeScript strict mode for the config file",
			},
			{ role: "assistant", content: "I'll update the config." },
			{
				role: "user",
				content: "Make sure to use TypeScript strict mode in the config",
			},
			{ role: "assistant", content: "Updated." },
			{
				role: "user",
				content: "Please use TypeScript strict mode for the config file",
			},
		];
		const result = detectFrustration(conversationTurns, defaultCfg);
		expect(result.triggers.some((t) => t.type === "repeated_instruction")).toBe(
			true,
		);
	});
});
...
describe("question_to_command shift", () => {
	it("detects shift from questions to commands", () => {
		...
		const result = detectFrustration(conversationTurns, defaultCfg);
		expect(result.triggers.some((t) => t.type === "question_to_command")).toBe(
			true,
		);
	});
});
...
describe("scoring algorithm", () => {
	...
	it("respects custom windowSize", () => {
		...
		expect(
			result.triggers.filter((t) => t.type === "explicit_frustration"),
		).toHaveLength(0);
	});
	...
	it("respects custom signal weights", () => {
		...
		const normalResult = detectFrustration(
			turns(["This is frustrating!"]),
			defaultCfg,
		);
		const heavyResult = detectFrustration(
			turns(["This is frustrating!"]),
			heavyWeightCfg,
		);
...
describe("adaptation thresholds", () => {
	...
	it("returns 'acknowledge_struggle' when critical threshold exceeded", () => {
		const criticalCfg = {
			...defaultCfg,
			adaptationThresholds: { medium: 0.1, high: 0.2, critical: 0.3 },
		};
		const result = detectFrustration(
...
describe("edge cases", () => {
	it("handles unicode emoji in messages", () => {
		expect(() =>
			detectFrustration(turns(["Great work! 😊👍"]), defaultCfg),
		).not.toThrow();
	});
