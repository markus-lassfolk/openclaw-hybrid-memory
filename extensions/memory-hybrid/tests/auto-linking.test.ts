...
describe("FactsDB.extractEntitiesFromText", () => {
	...
	it("exact word-boundary match returns weight 1.0", () => {
		const result = db.extractEntitiesFromText("Alice is the project lead", [
			"Alice",
		]);
	...
	it("substring match (no word boundary) returns weight 0.7", () => {
		...
		const result = db.extractEntitiesFromText("Working on MyProjectFoo code", [
			"MyProject",
		]);
		expect(result).toHaveLength(1);
	...
	it("IPv4 address extracted with weight 0.5", () => {
		const result = db.extractEntitiesFromText(
			"Connect to server 192.168.1.100 now",
			[],
		);
		expect(result).toHaveLength(1);
	...
	it("multiple IPs in text each extracted separately", () => {
		const result = db.extractEntitiesFromText(
			"Route traffic from 10.0.0.1 to 10.0.0.2",
			[],
		);
	...
	it("known entity + IP both extracted", () => {
		const result = db.extractEntitiesFromText(
			"Server alice at 10.0.0.5 is offline",
			["alice"],
		);
	...
	it("no match returns empty array", () => {
		const result = db.extractEntitiesFromText("Nothing relevant here", [
			"alpha",
			"beta",
		]);
		expect(result).toEqual([]);
	...
	it("duplicate entity not returned twice", () => {
		...
		const result = db.extractEntitiesFromText("alice and alice again", [
			"alice",
		]);
...
describe("FactsDB.autoLinkEntities — entity-based RELATED_TO links", () => {
	...
	it("creates RELATED_TO link from new fact to entity anchor", () => {
		...
		const result = db.autoLinkEntities(
			newFact.id,
			newFact.text,
			null,
			null,
...
describe("FactsDB.autoLinkEntities — temporal co-occurrence", () => {
	...
	it("links to facts in the same session by sessionId match", () => {
		...
		const coLink = links.find(
			(l) => l.targetFactId === factA.id && l.linkType === "RELATED_TO",
		);
...
describe("FactsDB.autoLinkEntities — supersession detection", () => {
	...
	it("reduces confidence on old fact when autoSupersede is true", () => {
		...
		db.autoLinkEntities(newer.id, newer.text, "host", "name", null, {
			coOccurrenceWeight: 0.3,
			autoSupersede: true,
		});
	...
	it("does not create duplicate SUPERSEDES edges on repeated calls", () => {
		...
		db.autoLinkEntities(newer.id, newer.text, "svc", "port", null, {
			coOccurrenceWeight: 0.3,
			autoSupersede: false,
		});
		db.autoLinkEntities(newer.id, newer.text, "svc", "port", null, {
		...
		const links = db
			.getLinksFrom(newer.id)
			.filter((l) => l.targetFactId === old.id && l.linkType === "SUPERSEDES");
		expect(links).toHaveLength(1);
	});
	...
	it("no supersession when entity is null", () => {
		...
		db.autoLinkEntities(newer.id, newer.text, null, "k", null, {
			coOccurrenceWeight: 0.3,
			autoSupersede: true,
		});
		...
		const links = db
			.getLinksFrom(newer.id)
			.filter((l) => l.linkType === "SUPERSEDES");
	...
	it("no supersession when key is null", () => {
		...
		db.autoLinkEntities(newer.id, newer.text, "e", null, null, {
			coOccurrenceWeight: 0.3,
			autoSupersede: true,
		});
...
describe("autoLinkEntities — config variants", () => {
	it("coOccurrenceWeight=0 still creates links but with 0 strength", () => {
		...
		db.autoLinkEntities(factB.id, factB.text, null, null, sessionId, {
...
describe("FactsDB.autoLinkEntities — INSTANCE_OF detection", () => {
	...
	it("creates INSTANCE_OF link for 'is a' pattern", () => {
		...
		const links = db.getLinksFrom(newFact.id);
		const instanceLink = links.find(
			(l) => l.linkType === "INSTANCE_OF" && l.targetFactId === anchor.id,
		);
...
describe("autoLinkEntities — edge cases", () => {
	...
	it("handles empty text without throwing", () => {
		...
		expect(() =>
			db.autoLinkEntities(fact.id, "", null, null, null, cfg),
		).not.toThrow();
	});
	...
	it("handles entity with special regex characters safely", () => {
		...
		expect(() =>
			db.autoLinkEntities(fact.id, fact.text, null, null, null, cfg),
		).not.toThrow();
	});
