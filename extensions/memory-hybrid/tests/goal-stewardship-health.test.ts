...
function baseCfg(
	over: Partial<GoalStewardshipConfig> = {},
...
describe("runGoalHealthCheck", () => {
	...
	it("returns no updates when stewardship is disabled", async () => {
		...
		await createGoal(
			goalsDir,
			{ label: "x", description: "d", acceptanceCriteria: ["a"] },
			defaults,
		);
	...
	it("parseGithubPrTarget accepts owner/repo#N and github.com pull URLs", () => {
		expect(parseGithubPrTarget("foo/bar#42")).toEqual({
			owner: "foo",
			repo: "bar",
			number: 42,
		});
		expect(parseGithubPrTarget("https://github.com/a/b/pull/7")).toEqual({
			owner: "a",
			repo: "b",
			number: 7,
		});
	...
	it("pr_merged: transitions to verifying when GitHub API reports merged", async () => {
		...
		try {
			...
			vi.unstubAllGlobals();
			if (prev === undefined) process.env.GITHUB_TOKEN = undefined;
	...
	it("escalates goal after consecutive failures", async () => {
		...
		expect(
			r.actions.some(
				(a) => a.action === "escalated" && a.reason === "failures",
			),
		).toBe(true);
		const after = await readGoal(goalsDir, g.id);
		expect(after?.status).toBe("blocked");
		expect(
			after?.currentBlockers.some((b) => b.includes("Escalated after")),
		).toBe(true);
	});
	...
	it("goalsChecked and goalsUpdated counts are correct", async () => {
		...
		await createGoal(
			goalsDir,
			{ label: "count_a", description: "d", acceptanceCriteria: ["a"] },
			defaults,
		);
		await createGoal(
			goalsDir,
			{ label: "count_b", description: "d", acceptanceCriteria: ["b"] },
			defaults,
		);
		const r = await runGoalHealthCheck({
			goalsDir,
