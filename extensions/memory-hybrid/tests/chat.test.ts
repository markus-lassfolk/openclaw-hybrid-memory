...
describe("is403Like", () => {
	...
	it("returns true for Google '403 Country, region, or territory not supported'", () => {
		expect(
			is403Like(new Error("403 Country, region, or territory not supported")),
		).toBe(true);
	});
...
describe("is403QuotaOrRateLimitLike", () => {
	...
	it("unwraps LLMRetryError so wrapped quota 403 is not classified as geo 403", () => {
		...
		const wrapped = new LLMRetryError(
			`Failed after 4 attempts: ${cause.message}`,
			cause,
			4,
		);
...
describe("parseGoDurationToMs / parseRetryAfterMs (OpenAI x-ratelimit-reset-* #941)", () => {
	...
	it("parses plain retry-after seconds only when the value is all digits", () => {
		expect(parseRetryAfterMs({ headers: { "retry-after": "1282" } })).toBe(
			1_282_000,
		);
	});
});
			...
			chat: {
				completions: {
					create: vi
						...
						.mockResolvedValueOnce({
							choices: [
								{ message: { content: "Success with second fallback" } },
							],
						}),
				},
			},
			...
			chat: {
				completions: {
					create: vi
						...
						.mockRejectedValue(
							new Error(
								"429 429 Too Many Requests: you (clawout) have reached your weekly usage limit",
							),
						),
				},
			},
			...
			chat: {
				completions: {
					create: vi
						...
						.mockRejectedValue(
							new Error(
								"429 Too Many Requests: you have reached your weekly usage limit",
							),
						),
				},
			},
...
describe("chatCompleteWithRetry — UnconfiguredProviderError (#328)", () => {
	...
	it("#328: does not report to GlitchTip when all models fail with UnconfiguredProviderError", async () => {
		const err = new UnconfiguredProviderError(
			"anthropic",
	...
	it("#328: does not report UnconfiguredProviderError to GlitchTip when final model is unconfigured but earlier model had a different error", async () => {
		const unconfiguredErr = new UnconfiguredProviderError(
			"anthropic",
			"anthropic/claude-sonnet-4-6",
		);
		const mockOpenai = {
...
describe("isOllamaOOM (#387)", () => {
	...
	it("matches 'not enough memory to load' phrasing", () => {
		expect(
			isOllamaOOM(new Error("not enough memory to load model qwen3:8b")),
		).toBe(true);
	});

	it("matches 'requires X GiB' pattern", () => {
		expect(
			isOllamaOOM(new Error("model requires 18.2 GiB of system memory")),
		).toBe(true);
	});

	it("matches bare OOM: prefix from Ollama", () => {
		expect(
			isOllamaOOM(new Error("OOM: model 'qwen3:8b' cannot be loaded")),
		).toBe(true);
	});

	it("does not match generic 500 errors", () => {
		expect(isOllamaOOM(new Error("HTTP 500 Internal Server Error"))).toBe(
			false,
		);
	});

	it("does not match connection errors", () => {
		expect(isOllamaOOM(new Error("ECONNREFUSED http://localhost:11434"))).toBe(
			false,
		);
	});
...
describe("isConnectionErrorLike (#703)", () => {
	...
	it("does not match config or model errors", () => {
		...
		expect(
			isConnectionErrorLike(new Error("Provider 'openai' is not configured")),
		).toBe(false);
	});
});
...
describe("is500Like (#387)", () => {
	...
	it("matches OpenAI SDK InternalServerError with .status property", () => {
		const err = Object.assign(new Error("InternalServerError"), {
			status: 500,
		});
...
describe("chatCompleteWithRetry — Responses reasoning sequence fallback (#1034)", () => {
	...
	it("isResponsesReasoningSequenceError matches LLMRetryError wrapping the provider message", () => {
		...
		const wrapped = new LLMRetryError(
			`Failed after 2 attempts: ${inner.message}`,
			inner,
			2,
		);
	...
	it("#1034: matches variant phrasing without 'its' before required", () => {
		const msg =
			"Item 'rs_abc' of type 'reasoning' was provided without required following item.";
	...
	it("#1034: isResponsesReasoningSequenceError returns false for unrelated errors", () => {
		expect(
			isResponsesReasoningSequenceError(new Error("400 invalid_request_error")),
		).toBe(false);
		expect(
			isResponsesReasoningSequenceError(new Error("context length exceeded")),
		).toBe(false);
	});
...
describe("is403Like", () => {
	...
	it("#490: returns true for 'Country, region, or territory not supported' without numeric prefix", () => {
		expect(
			is403Like(new Error("Country, region, or territory not supported")),
		).toBe(true);
	});
	...
	it("#490: returns true for 'Country, region, or territory not supported' embedded in a longer message", () => {
		expect(
			is403Like(
				new Error(
					"upstream error: Country, region, or territory not supported",
				),
			),
		).toBe(true);
	});
	...
	it("#490: returns true for PERMISSION_DENIED gRPC status in message", () => {
		expect(
			is403Like(
				new Error(
					"PERMISSION_DENIED: Country, region, or territory not supported",
				),
			),
		).toBe(true);
	});
	...
	it("#490: returns false for filesystem 'access denied' (e.g. EACCES)", () => {
		...
		expect(is403Like(new Error("access denied to file /etc/hosts"))).toBe(
			false,
		);
	...
	it("#490: returns false for 'access forbidden' on a directory", () => {
		expect(is403Like(new Error("access forbidden: directory /var/run"))).toBe(
			false,
		);
	});
});
...
describe("isContextLengthError (#442)", () => {
	...
	it("does not match a 404 or 5xx error", () => {
		...
		expect(isContextLengthError(new Error("500 Internal Server Error"))).toBe(
			false,
		);
	});
	...
	it("#488: detects Ollama pattern with '400' prefix in message and status=400", () => {
		const err = Object.assign(
			new Error("400 Input length 768 exceeds maximum allowed token size 512"),
			{
				status: 400,
			},
		);
	...
	it("#488: detects Ollama pattern without numeric status property (message-only)", () => {
		...
		const err = new Error(
			"Input length 768 exceeds maximum allowed token size 512",
		);
		expect(isContextLengthError(err)).toBe(true);
	});
...
describe("isAbortOrTransientLlmError", () => {
	it("detects Request was aborted", () => {
		expect(isAbortOrTransientLlmError(new Error("Request was aborted."))).toBe(
			true,
		);
	});
	...
	it("detects gateway stopped / unreachable phrasing", () => {
		expect(
			isAbortOrTransientLlmError(new Error("gateway client stopped")),
		).toBe(true);
		expect(
			isAbortOrTransientLlmError(
				new Error("Gateway not reachable. Is it running?"),
			),
		).toBe(true);
	});
	...
	it("unwraps LLMRetryError cause", () => {
		...
		expect(
			isAbortOrTransientLlmError(new LLMRetryError("wrap", inner, 1)),
		).toBe(true);
	});

	it("returns false for arbitrary model failure", () => {
		expect(isAbortOrTransientLlmError(new Error("model not found"))).toBe(
			false,
		);
	});
});
...
describe("resolveWireApi", () => {
	it('returns "responses" for azure-foundry-responses prefix', () => {
		...
		expect(resolveWireApi("azure-foundry-responses/gpt-5.4-pro")).toBe(
			"responses",
		);
	});
	...
	it("respects explicit wireApi override", () => {
		...
		expect(resolveWireApi("azure-foundry-responses/o3-pro", "chat")).toBe(
			"chat",
		);
	});
...
describe("chatComplete with wireApi='responses'", () => {
	...
	it("uses responses.create when wireApi is 'responses'", async () => {
		...
		expect(
			mockOpenaiWithResponses.chat.completions.create,
