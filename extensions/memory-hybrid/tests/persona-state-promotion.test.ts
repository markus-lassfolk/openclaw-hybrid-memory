...
describe("persona-state promotion pipeline", () => {
	...
	beforeEach(() => {
		...
		reflectionStore = new IdentityReflectionStore(
			join(dir, "identity-reflections.db"),
		);
	...
	it("groups paraphrased durable reflections into one promotion candidate", () => {
		reflectionStore.create({
			...
			insight:
				"I work best when I explain tradeoffs clearly and keep the user looped into decisions.",
			durability: "durable",
			confidence: 0.82,
		...
		reflectionStore.create({
			...
			questionText: "What patterns define good partnership with the user?",
			insight:
				"Good partnership means I surface tradeoffs clearly and keep the user involved in decisions.",
			durability: "durable",
			confidence: 0.86,
			evidence: ["Surfaces tradeoffs", "Keeps user involved"],
		});

		const candidates = collectPersonaPromotionCandidates(
			reflectionStore.listRecent(20),
			PROMOTION_CFG,
		);
	...
	it("creates then updates durable persona state without duplicating state rows", () => {
		const first = reflectionStore.create({
			...
			insight:
				"I protect user context by preserving constraints before I optimize for speed.",
			durability: "durable",
			confidence: 0.81,
		...
		const second = reflectionStore.create({
			...
			questionText: "What do I reliably protect?",
			insight:
				"I reliably preserve user constraints before optimizing for speed or convenience.",
			durability: "durable",
			confidence: 0.88,
			evidence: [
				"Preserves user constraints",
				"Resists convenience-first shortcuts",
			],
		});

		const initial = promotePersonaStateFromReflections(
			reflectionStore,
			personaStateStore,
			PROMOTION_CFG,
		);
		...
		expect(initial.entries[0].sourceReflectionIds.sort()).toEqual(
		...
		reflectionStore.create({
			...
			questionText: "What do I reliably protect?",
			insight:
				"I protect the user's stated constraints before I optimize for speed.",
			durability: "durable",
			confidence: 0.9,
			evidence: ["Protects stated constraints"],
		});

		const updated = promotePersonaStateFromReflections(
			reflectionStore,
			personaStateStore,
			PROMOTION_CFG,
		);
	...
	it("formats durable persona state for proposal prompts", () => {
		reflectionStore.create({
			...
			insight:
				"My name and avatar should be treated as part of my stable identity presentation.",
			durability: "durable",
			confidence: 0.82,
