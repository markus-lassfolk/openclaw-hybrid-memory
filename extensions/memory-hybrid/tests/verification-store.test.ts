...
beforeEach(() => {
	...
	logger = {
		...
		error: vi.fn() as unknown as (msg: string) => void,
	};
...
describe("VerificationStore.verify", () => {
	...
	it("throws VerificationError when fact_id is already verified", async () => {
		...
		expect(() => store.verify("fact-dup", "Second", "agent")).toThrow(
			VerificationError,
		);
...
describe("VerificationStore.getVerified", () => {
	...
	it("throws VerificationError when text is corrupted after storing", async () => {
		...
		expect(() => store.getVerified("fact-corrupted")).toThrow(
			VerificationError,
		);
...
describe("VerificationStore.update", () => {
	...
	it("old version remains accessible via direct id query", async () => {
		...
		const oldRow = db
			.prepare("SELECT * FROM verified_facts WHERE id = ?")
			.get(id) as { canonical_text: string };
	...
	it("clears next_verification on superseded version so listDueForReverification does not return it", async () => {
		...
		const db = (store as unknown as { db: import("node:sqlite").DatabaseSync })
			.db;
		const oldRow = db
			.prepare("SELECT next_verification FROM verified_facts WHERE id = ?")
			.get(id) as {
	...
	it("throws VerificationError when updating a non-existent id", async () => {
		expect(() => store.update("no-such-id", "text", "agent")).toThrow(
			VerificationError,
		);
	});
	...
	it("throws VerificationError when updating from a non-latest version", async () => {
		...
		expect(() => store.update(id1, "V2-alt", "user")).toThrow(
			VerificationError,
		);
...
describe("shouldAutoVerify", () => {
	...
	it("returns true for various valid IPv4 addresses", () => {
		expect(
			shouldAutoVerify({ text: "Host: 10.0.0.1", category: "other", tags: [] }),
		).toBe(true);
		expect(
			shouldAutoVerify({
				text: "IP=172.16.254.1",
				category: "other",
				tags: [],
			}),
		).toBe(true);
...
describe("VerificationStore backup file", () => {
	...
	it("appends a JSON line on update", async () => {
		...
		const parsed = lines.map((l) => JSON.parse(l));
		expect(
			parsed.some((p) => p.fact_id === "fact-upd" && p.version === 2),
		).toBe(true);
	});
