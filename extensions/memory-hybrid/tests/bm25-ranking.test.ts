...
describe("BM25 score ordering", () => {
  ...
  it("results are sorted ascending by rank (closest to 0 first = most relevant)", () => {
    ...
    insertFact(db, "PostgreSQL has advanced features like JSONB, full-text search, and window functions");
...
describe("Exact keyword match outranks partial/weaker match", () => {
  it("exact single-word match ranks above document mentioning only a related term", () => {
    ...
    insertFact(db, "Redis is an in-memory database used for caching and pub/sub");
  ...
  it("document with the exact query phrase ranks first", () => {
    ...
    const kubernetesResults = results.filter((r) => r.text.toLowerCase().includes("kubernetes"));
    const dockerOnly = results.filter(
      (r) => r.text.toLowerCase().includes("docker") && !r.text.toLowerCase().includes("kubernetes"),
    );
    ...
    if (kubernetesResults.length > 0 && dockerOnly.length > 0) {
      const lastKubernetesIdx = results.indexOf(kubernetesResults[kubernetesResults.length - 1]);
...
describe("Term frequency effect", () => {
  it("document mentioning the query term more often tends to rank higher", () => {
    ...
    insertFact(db, "Python is popular for data science; JavaScript is used for frontend work.");
    ...
    const highTfIdx = results.findIndex((r) => (r.text.match(/JavaScript/g) ?? []).length >= 3);
    const lowTfIdx = results.findIndex((r) => (r.text.match(/JavaScript/g) ?? []).length < 3);
...
describe("Integration: searchFts result order", () => {
  it("searchFts returns results in relevance order for a simple query", () => {
    ...
    insertFact(db, "The server room temperature should be maintained below 20°C");
  ...
  it("snippet is present and non-empty when results are found", () => {
    insertFact(db, "Elasticsearch is a distributed search and analytics engine built on Lucene");
    insertFact(db, "Solr is another search platform based on Lucene from Apache");
...
describe("BM25 column weighting", () => {
  it("match in text column contributes to rank (negative rank for text match)", () => {
    ...
    insertFact(db, "Vim is a terminal-based text editor with modal editing", "vim-editor");
