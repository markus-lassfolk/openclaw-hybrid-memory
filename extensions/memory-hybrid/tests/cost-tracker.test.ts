...
import {
	getCurrentCostFeature,
	withCostFeature,
} from "../services/cost-context.js";
...
describe("CostTracker", () => {
	...
	describe("record()", () => {
		it("inserts a row into llm_cost_log", () => {
			...
			const row = db.prepare("SELECT * FROM llm_cost_log").get() as Record<
				string,
				unknown
			>;
		...
		it("records estimated_cost_usd for known model", () => {
			...
			const row = db
				.prepare("SELECT estimated_cost_usd FROM llm_cost_log")
	...
	describe("getReport()", () => {
		it("returns features aggregated by feature name", () => {
			...
			const classifyFeature = report.features.find(
				(f) => f.feature === "auto-classify",
			);
	...
	describe("pruneOldEntries()", () => {
		it("deletes entries older than retainDays", () => {
         ...
         VALUES (1000, 'auto-classify', 'openai/gpt-4.1-nano', 100, 20, 1)`,
			...
			const remaining = db
				.prepare("SELECT COUNT(*) AS n FROM llm_cost_log")
				.get() as { n: number };
	...
	describe("record() error isolation", () => {
		it("does not throw when the DB is closed", () => {
			...
			let storeOpen = true;
			const tmpTracker = new CostTracker(
				factsDbStubForTest(tmpDb, () => storeOpen),
			);
	...
	describe("schema initialization", () => {
		...
		it("creates the llm_savings_log table on construction", () => {
			const tableInfo = db
				.prepare(
					"SELECT name FROM sqlite_master WHERE type='table' AND name='llm_savings_log'",
				)
				.get() as { name: string } | undefined;
			expect(tableInfo).toBeDefined();
		...
		it("creates the expected indexes", () => {
			const indexes = db
				.prepare(
					"SELECT name FROM sqlite_master WHERE type='index' AND tbl_name='llm_cost_log'",
				)
		...
		it("creates the expected savings indexes", () => {
			const indexes = db
				.prepare(
					"SELECT name FROM sqlite_master WHERE type='index' AND tbl_name='llm_savings_log'",
				)
				.all() as Array<{ name: string }>;
			const indexNames = indexes.map((i) => i.name);
		...
		it("is idempotent — constructing twice does not throw", () => {
			expect(
				() => new CostTracker(factsDbStubForTest(db, () => true)),
			).not.toThrow();
		});
	});
	...
	describe("recordSavings()", () => {
		it("inserts a row into llm_savings_log", () => {
			...
			const row = db.prepare("SELECT * FROM llm_savings_log").get() as Record<
				string,
				unknown
			>;
		...
		it("stores zero estimatedSavingUsd without throwing", () => {
			...
			const row = db
				.prepare("SELECT estimated_saving_usd FROM llm_savings_log")
				.get() as {
	...
	describe("getSavingsReport()", () => {
		...
		it("respects the days window", () => {
			...
			expect(report7.features.map((f) => f.feature)).not.toContain(
				"old-feature",
			);
	...
	describe("pruneOldEntries() — extended", () => {
		it("also prunes llm_savings_log entries older than retainDays", () => {
			...
			const remaining = db
				.prepare("SELECT feature FROM llm_savings_log")
				.all() as Array<{ feature: string }>;
