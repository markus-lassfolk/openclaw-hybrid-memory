...
describe("memory_issue_list", () => {
	...
	it("filters by status", async () => {
		...
		await api.callTool("memory_issue_update", {
			id: c1.details.id,
			status: "diagnosed",
		});

		const result = (await api.callTool("memory_issue_list", {
			status: ["diagnosed"],
		})) as any;
...
describe("memory_issue_search", () => {
	it("finds issues by title keyword", async () => {
		...
		const result = (await api.callTool("memory_issue_search", {
			query: "database",
		})) as any;
		expect(result.details).toHaveLength(1);
...
describe("memory_issue_link_fact", () => {
	...
	it("throws on nonexistent issue id", async () => {
		await expect(
			api.callTool("memory_issue_link_fact", {
				issueId: "nonexistent",
				factId: "fact-001",
			}),
		).rejects.toThrow();
...
describe("End-to-end issue lifecycle via tools", () => {
	it("full lifecycle: create → diagnose → fix-attempt → resolve → verify", async () => {
		...
		const created = (await api.callTool("memory_issue_create", {
			...
			symptoms: [
				"All login attempts return 500",
				"Error logs show NullPointerException",
			],
			severity: "critical",
		...
		// 4. Resolve
		const resolved = (await api.callTool("memory_issue_update", {
			id,
			status: "resolved",
		})) as any;
		...
		// 5. Verify
		const verified = (await api.callTool("memory_issue_update", {
			id,
			status: "verified",
		})) as any;
		...
		const listResult = (await api.callTool("memory_issue_list", {
			status: ["verified"],
		})) as any;
