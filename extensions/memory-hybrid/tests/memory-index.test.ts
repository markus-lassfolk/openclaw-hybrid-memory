...
describe("memory index", () => {
	...
	it("builds a compact awareness snapshot from clusters, decisions, entities, and patterns", () => {
		...
		const snapshot = buildMemoryIndexSnapshot(factsDb, {
			recentWindowDays: 30,
		});
		const markdown = renderMemoryIndexMarkdown(snapshot);

		expect(snapshot.clusters).toHaveLength(1);
		expect(snapshot.recentDecisions[0]?.ref).toBe(
			`decision:${decision.id.slice(0, 8)}`,
		);
		expect(snapshot.recentPatterns[0]?.ref).toBe(
			`pattern:${pattern.id.slice(0, 8)}`,
		);
		expect(snapshot.keyEntities.map((entity) => entity.entity)).toContain(
			"TypeScript",
		);
