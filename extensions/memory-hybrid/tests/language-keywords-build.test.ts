...
describe("language-keywords-build", () => {
  ...
  describe("runBuildLanguageKeywords", () => {
    it("writes .language-keywords.json when LLM returns languages and translations", async () => {
            ...
            create: async (opts: { model: string; messages: unknown[] }) => {
              ...
              const msg =
                typeof content === "object" && content && "content" in content
                  ? String((content as { content: string }).content)
                  : "";
      ...
      const result = await runBuildLanguageKeywords(facts, mockOpenai, tmpDir, { model: "gpt-4o-mini", dryRun: false });
    ...
    it("dryRun does not write file", async () => {
      ...
      const result = await runBuildLanguageKeywords(facts, mockOpenai, tmpDir, { model: "gpt-4o-mini", dryRun: true });
      expect(result.ok).toBe(true);
      if (!result.ok) return;
    ...
    it("returns ok with path and zero languages when no samples", async () => {
      ...
      const result = await runBuildLanguageKeywords([], mockOpenai, tmpDir, { model: "gpt-4o-mini", dryRun: false });
      expect(result.ok).toBe(true);
      if (!result.ok) return;
      expect(result.path).toBe(join(tmpDir, ".language-keywords.json"));
    });
