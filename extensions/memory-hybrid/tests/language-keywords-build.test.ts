...
describe("language-keywords-build", () => {
	...
	describe("collectSamplesFromFacts", () => {
		...
		it("returns samples when facts have enough text", () => {
			const facts = [
				{
					text: "This is a long enough fact to be sampled for language detection.",
				},
			];
	...
	describe("runBuildLanguageKeywords", () => {
		it("writes .language-keywords.json when LLM returns languages and translations", async () => {
			...
			const result = await runBuildLanguageKeywords(facts, mockOpenai, tmpDir, {
				model: "gpt-4o-mini",
				dryRun: false,
			});
		...
		it("dryRun does not write file", async () => {
			...
			const facts = [
				{
					text: "Ett tillräckligt långt faktum på svenska för att detektera språk.",
				},
			];
			const result = await runBuildLanguageKeywords(facts, mockOpenai, tmpDir, {
				model: "gpt-4o-mini",
		...
		it("returns ok with path and zero languages when no samples", async () => {
			...
			const result = await runBuildLanguageKeywords([], mockOpenai, tmpDir, {
				model: "gpt-4o-mini",
				dryRun: false,
			});
			expect(result.ok).toBe(true);
