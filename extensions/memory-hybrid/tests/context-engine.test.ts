...
function makeEngine(
	overrides?: Partial<ContextEngineOptions>,
): HybridMemoryContextEngine {
...
describe("HybridMemoryContextEngine.compact()", () => {
	...
	it("skips delete WAL entries without attempting deletion (issue #334)", async () => {
		...
		await wal.write({
			...
			operation: "delete",
			data: {
				text: "MiniMax M2.5 limitations for council reviews (2026-02-22)",
				source: "test",
			},
		});
	...
	it("includes top-fact summary in result when facts are present", async () => {
		...
		const summary = result.result as
			| { topFacts?: unknown[]; factCount?: number }
			| undefined;
		if (summary) {
...
describe("HybridMemoryContextEngine.onSubagentEnded()", () => {
	it("logs fact count when child session has captured facts", async () => {
		...
		const infoCalls = logger.info.mock.calls.map((c: unknown[]) =>
			String(c[0]),
		);
		const relevant = infoCalls.find((msg: string) =>
			msg.includes(childSessionKey),
		);
	...
	it("logs debug (not info) when child session has no captured facts", async () => {
		...
		const debugCalls = logger.debug.mock.calls.map((c: unknown[]) =>
			String(c[0]),
		);
		const relevant = debugCalls.find((msg: string) =>
			msg.includes(childSessionKey),
		);
		expect(relevant).toBeDefined();
		...
		expect(infoCalls.some((msg: string) => msg.includes(childSessionKey))).toBe(
			false,
		);
	});
	...
	it("handles different completion reasons without error", async () => {
		...
		for (const reason of reasons) {
			await expect(
				engine.onSubagentEnded?.({
					childSessionKey: `child-${randomUUID()}`,
					reason,
				}),
			).resolves.not.toThrow();
		}
...
describe("HybridMemoryContextEngine.assemble()", () => {
	...
	it("respects tokenBudget — truncates facts when budget is very small", async () => {
		...
		const resultFull = await engineFull.assemble({
			sessionId: "s1",
			messages: [],
			tokenBudget: 10000,
		});
		...
		const resultTight = await engineTight.assemble({
			sessionId: "s1",
			messages: [],
			tokenBudget: 85,
		});
...
describe("buildContextBlock()", () => {
	...
	it("respects tokenBudget by omitting entries that would exceed it", () => {
		...
		const facts = Array.from({ length: 20 }, (_, i) =>
			makeEntry(
				`Fact ${i}: This is a longer fact to consume more tokens in the budget.`,
			),
		);
