...
describe("computeSimpleSimilarity", () => {
	it("returns 1.0 for identical strings", () => {
		expect(
			computeSimpleSimilarity("hello world foo bar", "hello world foo bar"),
		).toBeCloseTo(1.0, 2);
	});

	it("returns 0 for completely different strings", () => {
		const sim = computeSimpleSimilarity(
			"apple banana orange",
	...
	it("is symmetric", () => {
		...
		expect(computeSimpleSimilarity(a, b)).toBeCloseTo(
			computeSimpleSimilarity(b, a),
			5,
		);
	});
});
...
describe("detectRephrase", () => {
	it("detects rephrase when similar user messages appear in sequence", () => {
		...
		const turns: ConversationTurn[] = [
			{
				role: "user",
				content: "How do I deploy my application to AWS production environment",
			},
			{ role: "assistant", content: "You can use the AWS CLI or console." },
			{
				role: "user",
				content: "How can I deploy my app to AWS production environment",
			}, // similar topic, slightly different wording
		];
...
describe("detectImmediateAction", () => {
	it("detects positive action words after agent message", () => {
		const turns: ConversationTurn[] = [
			{
				role: "assistant",
				content: "Try running npm install to fix the issue.",
			},
...
describe("detectTopicChange", () => {
		...
		const turns: ConversationTurn[] = [
			{
				role: "assistant",
				content:
					"webpack optimization splitChunks treeshaking production configuration settings.",
			},
			{
				role: "user",
				content:
					"What webpack optimization splitChunks treeshaking production settings should I adjust?",
			},
		];
		const signal = detectTopicChange(turns, { topicChangeThreshold: 0.3 }, 1);
		expect(signal).toBeNull();
	...
	it("skips grateful close messages", () => {
		const turns: ConversationTurn[] = [
			{
				role: "assistant",
				content: "Your deployment is complete and the app is running.",
			},
...
describe("detectGratefulClose", () => {
	it("detects thanks keyword", () => {
		const turns: ConversationTurn[] = [
			{
				role: "assistant",
				content: "Your application is now deployed successfully.",
			},
...
describe("detectTerseResponse", () => {
	it("detects terse response when message is much shorter than average", () => {
		const turns: ConversationTurn[] = [
			...
			{
				role: "assistant",
				content: "The entry config specifies starting points.",
			},
			{
				role: "user",
				content:
	...
	it("returns null when message length is normal", () => {
		const turns: ConversationTurn[] = [
			...
			{
				role: "user",
				content: "I need help with the next configuration setting",
			},
		...
		const turns: ConversationTurn[] = [
			{
				role: "user",
				content:
					"webpack splitChunks optimization configuration production settings",
			},
			{ role: "assistant", content: "Use mode: production." },
			{
				role: "user",
				content:
					"webpack splitChunks optimization configuration production bundles",
			},
			{ role: "assistant", content: "Enable tree shaking." },
			{
				role: "user",
				content:
					"webpack splitChunks optimization configuration production chunks",
			},
			{ role: "assistant", content: "Use optimization config." },
			{
				role: "user",
				content:
					"webpack splitChunks optimization configuration production splitting",
			},
		];
...
describe("detectCorrectionCascade", () => {
	it("detects cascade when 2+ corrections in 5 turns", () => {
		const turns: ConversationTurn[] = [
			...
			{
				role: "user",
				content: "No that's wrong, I said I need the TypeScript version.",
			},
	...
	it("returns null for single correction", () => {
		const turns: ConversationTurn[] = [
			...
			{
				role: "user",
				content: "No, that is wrong, I need something different.",
			},
			{ role: "assistant", content: "Let me try again." },
...
describe("detectCopyPaste", () => {
	it("detects high overlap between agent output and user message", () => {
		...
		const shared =
			"deploy application production kubernetes cluster configuration settings optimization scaling";
		const turns: ConversationTurn[] = [
			{
				role: "assistant",
				content: `${shared} ingress replicas namespace service`,
			},
			{
				role: "user",
				content: `${shared} ingress replicas namespace service verified`,
			},
		];
...
describe("detectSelfService", () => {
	it("detects 'I'll do it myself'", () => {
		const turns: ConversationTurn[] = [
			{
				role: "assistant",
				content: "I can help you configure the deployment pipeline.",
			},
...
describe("detectEscalation", () => {
	it("detects 'I'll ask someone'", () => {
		const turns: ConversationTurn[] = [
			{
				role: "assistant",
				content: "I'm not sure about the exact configuration for your setup.",
			},
...
describe("detectSilenceAfterAction", () => {
	it("detects silence after tool call without user acknowledgment", () => {
		const turns: ConversationTurn[] = [
			...
			{
				role: "assistant",
				content: "Deployment complete. The app is now running.",
			},
		];
...
describe("extractImplicitSignals", () => {
	...
	it("filters signals below minConfidence", () => {
		...
		const silenceSignals = signals.filter(
			(s) => s.type === "silence_after_action",
		);
	...
	it("attaches sessionFile to all signals", () => {
		...
		expect(
			signals.every((s) => s.context.sessionFile === "my-session.jsonl"),
		).toBe(true);
	});
	...
	it("detects correction_cascade in a mixed conversation", () => {
		const turns: ConversationTurn[] = [
			{
				role: "user",
				content: "Please write a TypeScript function to sort an array.",
			},
			{ role: "assistant", content: "Here is a JavaScript function..." },
			{
				role: "user",
				content: "No that's wrong, I said TypeScript not JavaScript.",
			},
	...
	it("detects grateful_close as positive signal", () => {
		const turns: ConversationTurn[] = [
			{
				role: "assistant",
				content: "Here is the complete implementation you requested.",
			},
