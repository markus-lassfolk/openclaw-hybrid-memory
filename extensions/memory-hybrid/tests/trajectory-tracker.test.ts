...
describe("detectTrajectoryBoundaries", () => {
	...
	it("detects single trajectory in a simple conversation", () => {
		const turns: ConversationTurn[] = [
			...
			{
				role: "user",
				content: "How do I set up the webpack TypeScript compiler options?",
			},
			{ role: "assistant", content: "Use the compilerOptions in tsconfig." },
			{
				role: "user",
				content: "Perfect, the webpack TypeScript setup is working now.",
			},
		];
		const boundaries = detectTrajectoryBoundaries(turns);
		...
		const turns: ConversationTurn[] = [
			{
				role: "user",
				content:
					"How do I configure webpack for production builds with optimization?",
			},
			{
				role: "assistant",
				content: "Use mode: production and optimization settings.",
			},
			{
				role: "user",
				content:
					"What webpack configuration do I need for production deployment?",
			},
			...
			{
				role: "user",
				content: "Can you recommend a good recipe for pasta carbonara?",
			},
			{
				role: "assistant",
				content: "Classic carbonara uses eggs, pecorino, guanciale.",
			},
			{
				role: "user",
				content: "What ingredients do I need for pasta carbonara sauce?",
			},
		];
		const boundaries = detectTrajectoryBoundaries(turns);
	...
	it("closes trajectory at grateful message", () => {
		const turns: ConversationTurn[] = [
			{
				role: "user",
				content: "How do I deploy my application to kubernetes cluster?",
			},
			...
			{ role: "user", content: "Thanks, that worked perfectly!" }, // grateful close
			{
				role: "user",
				content:
					"Now I need to set up monitoring for my kubernetes pods and services.",
			},
			{ role: "assistant", content: "Use Prometheus and Grafana." },
			{
				role: "user",
				content:
					"How do I configure Prometheus monitoring for kubernetes services?",
			},
		];
		const boundaries = detectTrajectoryBoundaries(turns);
		expect(boundaries.length).toBeGreaterThanOrEqual(1);
...
describe("extractTrajectoryLessons", () => {
	const base = (
		overrides: Partial<FeedbackTrajectory>,
	): FeedbackTrajectory => ({
		id: "test-id",
...
describe("buildTrajectories", () => {
		...
		const turns: ConversationTurn[] = [
			{
				role: "user",
				content:
					"How do I configure webpack for TypeScript with decorators support?",
			},
			{
				role: "assistant",
				content: "You need ts-loader and tsconfig with experimentalDecorators.",
			},
			{
				role: "user",
				content:
					"What typescript webpack configuration options do I need for decorators?",
			},
			{
				role: "assistant",
				content: "Enable experimentalDecorators in tsconfig.json.",
			},
			{
				role: "user",
				content:
					"That worked, the webpack typescript config is set up now. Thanks!",
			},
		];
	...
	it("correctly identifies success outcome", () => {
		...
		const turns: ConversationTurn[] = [
			{
				role: "user",
				content: "How do I set up webpack configuration for my project?",
			},
			{
				role: "assistant",
				content: "Create a webpack.config.js with entry and output.",
			},
			{
				role: "user",
				content:
					"How do I set up the webpack configuration entry point and output?",
			},
			{
				role: "assistant",
				content: "Set entry to src/index.js and output to dist.",
			},
	...
	it("assigns tools used from tool calls", () => {
		...
		expect(trajectories.length).toBeGreaterThanOrEqual(1);
		const hasTool = trajectories.some(
			(t) => t.toolsUsed.includes("exec") || t.toolsUsed.includes("write"),
		);
