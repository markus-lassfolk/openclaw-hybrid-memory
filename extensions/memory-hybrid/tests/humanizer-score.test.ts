...
describe("parseHumanizerOutput", () => {
	...
	it("returns null when score field is missing", () => {
		expect(
			parseHumanizerOutput(JSON.stringify({ patterns_triggered: [] })),
		).toBeNull();
	});
	...
	it("clamps score to [0, 1]", () => {
		...
		expect(parseHumanizerOutput(JSON.stringify({ score: -0.3 }))?.score).toBe(
			0,
		);
	});
	...
	it("extracts patterns_triggered as strings", () => {
		const result = parseHumanizerOutput(
			JSON.stringify({
				score: 0.7,
				patterns_triggered: ["great_question", "happy_to_help", 42, null],
			}),
		);
		expect(result?.patternsTriggered).toEqual([
			"great_question",
	...
	it("extracts category_breakdown as number map", () => {
		const result = parseHumanizerOutput(
			JSON.stringify({
				score: 0.3,
				category_breakdown: { gush: 0.2, filler: 0.1, invalid: "x" },
			}),
		);
...
describe("formatQualityLoopEntry", () => {
	...
	it("includes patterns when present", () => {
		const entry = formatQualityLoopEntry(
			makeResult(0.5, ["great_question", "happy_to_help"]),
		);
	...
	it("produces compact single-line output", () => {
		const entry = formatQualityLoopEntry(makeResult(0.73, ["gush"]), {
			modelTag: "sonnet",
			skillTag: "weather",
		});
		expect(entry).toBe(
			"humanizer_score: 0.73, patterns: ['gush'], model: sonnet, skill: weather",
		);
	});
...
describe("runHumanizerScore", () => {
	...
	it("returns null for text shorter than minTextLength", async () => {
		const result = await runHumanizerScore("short", {
			...defaultCfg,
			minTextLength: 100,
		});
...
describe("parseHumanizerConfig", () => {
	...
	it("uses custom bin path", () => {
		const cfg = parseHumanizerConfig({
			humanizer: { bin: "/usr/local/bin/humanizer" },
		});
	...
	it("parses modelTag and skillTag", () => {
		const cfg = parseHumanizerConfig({
			humanizer: { modelTag: "sonnet", skillTag: "weather" },
		});
