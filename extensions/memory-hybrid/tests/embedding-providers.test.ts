...
function makeMockOpenAI(vector: number[]): import("openai").default {
	const mockCreate = vi
		.fn()
		.mockImplementation((params: { input: string | string[] }) => {
...
describe("Embeddings (OpenAI) implements EmbeddingProvider interface", () => {
			...
			.mockImplementation((params: { model: string }) => {
				if (params.model === "text-embedding-3-small") {
					return Promise.reject(
						Object.assign(new Error("upstream 404 from primary"), {
							status: 404,
						}),
					);
				}
	...
	it("#969: embedBatch() throws when data[] length mismatches inputs", async () => {
		const mockCreate = vi
			.fn()
			.mockResolvedValue({ data: [{ index: 0, embedding: [0.1, 0.2] }] });
		const client = {
			embeddings: { create: mockCreate },
		} as unknown as import("openai").default;
		const provider = new Embeddings(client, "text-embedding-3-small", 2);
	...
	it("#589: embedBatch() handles mixed cached/uncached inputs, only calling API for uncached", async () => {
		...
		const seedCreate = vi
			.fn()
			.mockResolvedValue({ data: [{ index: 0, embedding: cachedVec }] });
		...
		const mockCreate = vi
			...
			.mockImplementation((params: { input: string | string[] }) => {
				const inputs = Array.isArray(params.input)
					...
					: [params.input];
				return Promise.resolve({
					data: inputs.map((_, i) => ({ index: i, embedding: vec })),
				});
			});
		const client = {
			embeddings: { create: mockCreate },
	...
	it("throws when dimensions exceed model max", () => {
		...
		expect(
			() => new Embeddings(client, "text-embedding-3-small", 2000),
		).toThrow(/exceed/i);
		expect(
			() => new Embeddings(client, "text-embedding-3-large", 4000),
		).toThrow(/exceed/i);
	});
	...
	it("does not throw when dimensions are within model max", () => {
		const client = makeMockOpenAI([]);
		expect(
			() => new Embeddings(client, "text-embedding-3-small", 1536),
		).not.toThrow();
		expect(
			() => new Embeddings(client, "text-embedding-3-large", 3072),
		).not.toThrow();
		expect(
			() => new Embeddings(client, "text-embedding-ada-002", 1536),
		).not.toThrow();
	});

	it("throws when non-text-embedding-3 model is used with custom dimensions", () => {
		const client = makeMockOpenAI([]);
		expect(() => new Embeddings(client, "text-embedding-ada-002", 768)).toThrow(
		...
		).toThrow(/does not support custom dimensions/);
	});
			...
			const mockCreate = vi
				...
				.mockImplementation(
					(params: { model: string; input: string | string[] }) => {
						...
						const count = Array.isArray(params.input) ? params.input.length : 1;
						return Promise.resolve({
							data: Array.from({ length: count }, () => ({ embedding: [0.1] })),
						});
					},
				);
			const client = {
...
describe("OllamaEmbeddingProvider", () => {
	...
	it("embedBatch() throws on empty embeddings array in response", async () => {
		...
		await expect(p.embedBatch(["a"])).rejects.toThrow(
...
describe("FallbackEmbeddingProvider", () => {
	...
	it("uses primary provider when it succeeds", async () => {
		...
		const primary = new OllamaEmbeddingProvider({
			model: "nomic-embed-text",
			dimensions: 2,
		});
...
describe("createEmbeddingProvider factory", () => {
	...
	it("falls back to OpenAI when ONNX runtime is missing and apiKey is set", async () => {
		...
		const embedSpy = vi
			.spyOn(Embeddings.prototype, "embed")
			.mockResolvedValue(openaiVec);
		const provider = createEmbeddingProvider(cfg);
	...
	it("throws for unknown provider", () => {
		...
		expect(() => createEmbeddingProvider(cfg)).toThrow(
			...
			InferenceSession: {
				create: async () => ({
					...
					run: async (feeds: Record<string, { dims: number[] }>) => {
						...
						return {
							sentence_embedding: new FakeTensor("float32", data, [batch, dim]),
						};
					},
				}),
			},
...
describe("Embeddings (OpenAI) — context-length truncation (#442)", () => {
	...
	it("#442: truncates text > 32768 chars before calling the API", async () => {
		...
		const mockCreate = vi
			.fn()
			.mockImplementation((_params: { input: string }) => {
...
describe("#385: safeEmbed suppresses capturePluginError for config-error-only chain failures", () => {
	it("returns null and does NOT report when all causes are config errors (404)", async () => {
		...
		const p1 = {
			embed: vi.fn().mockRejectedValue(configErr),
			embedBatch: vi.fn(),
			dimensions: 768,
			modelName: "p1",
		};
	...
	it("returns null and does NOT report when all causes are config errors (401)", async () => {
		const authErr = Object.assign(new Error("401 Incorrect API key"), {
			status: 401,
		});
		const p1 = {
			embed: vi.fn().mockRejectedValue(authErr),
			embedBatch: vi.fn(),
			dimensions: 768,
			modelName: "p1",
		};
...
describe("#385: ChainEmbeddingProvider does not report 404/401 config errors", () => {
	...
	it("does not capturePluginError for 401 from non-last provider", async () => {
		const authErr = Object.assign(new Error("401 Incorrect API key provided"), {
			status: 401,
		});
		const vec = [0.5, 0.6];
		const p1 = {
	...
	it("does not capturePluginError for Ollama connection failure from non-last provider", async () => {
		const ollamaConnErr = new Error(
			"Ollama connection failed (http://localhost:11434): TypeError: fetch failed",
		);
		const vec = [0.5, 0.6];
		const p1 = {
	...
	it("does not capturePluginError for 401 message-based (Ollama HTTP 401 Unauthorized)", async () => {
		const ollamaAuthErr = new Error(
			"Ollama embed failed: HTTP 401 Unauthorized",
		);
		const vec = [0.7, 0.8];
		const p1 = {
	...
	it("throws AllEmbeddingProvidersFailed when all providers exhaust", async () => {
		...
		await expect(chain.embed("test")).rejects.toBeInstanceOf(
			AllEmbeddingProvidersFailed,
		);
	});
});
...
describe("#486: shouldSuppressEmbeddingError — suppression helper", () => {
	...
	it("suppresses Ollama circuit breaker open errors", () => {
		expect(
			shouldSuppressEmbeddingError(
				new Error("Ollama circuit breaker open — skipping embed"),
			),
		).toBe(true);
	});
	...
	it("does NOT suppress generic network/connection transient errors", () => {
		...
		expect(shouldSuppressEmbeddingError(new Error("network timeout"))).toBe(
			false,
		);
	});
	...
	it("suppresses 500 server errors", () => {
		expect(
			shouldSuppressEmbeddingError(new Error("500 Internal Server Error")),
		).toBe(true);
		expect(shouldSuppressEmbeddingError(new Error("500 internal error"))).toBe(
			true,
		);
	});
	...
	it("suppresses AllEmbeddingProvidersFailed when all causes are config errors", () => {
		const configErr = Object.assign(new Error("404 Not Found"), {
			status: 404,
		});
	...
	it("suppresses AllEmbeddingProvidersFailed when causes are a mix of suppressible errors", () => {
		const configErr = Object.assign(new Error("401 Unauthorized"), {
			status: 401,
		});
		const rateLimitErr = Object.assign(new Error("429 Too Many Requests"), {
			status: 429,
