...
import { listDumpTypeAliases, resolveDumpTableType, runSqliteTableDump } from "../services/cli-sql-dump.js";
...
describe("cli-sql-dump", () => {
  ...
  it("runSqliteTableDump reads rows with order last", () => {
    ...
    const last = runSqliteTableDump(db, { type: "fact_entity", limit: 10, order: "last", json: true });
    expect(last.ok).toBe(true);
    if (last.ok) {
      expect(last.rows).toHaveLength(2);
      expect((last.rows[0] as { surface_text: string }).surface_text).toBe("ACME");
    }

    const first = runSqliteTableDump(db, { type: "fact_entity", limit: 1, order: "first", json: true });
    expect(first.ok).toBe(true);
    if (first.ok) {
      expect(first.rows).toHaveLength(1);
      expect((first.rows[0] as { surface_text: string }).surface_text).toBe("Ada");
    }
  });

  it("runSqliteTableDump errors on unknown type", () => {
    const db = new DatabaseSync(":memory:");
    const r = runSqliteTableDump(db, { type: "nope", limit: 5, order: "last", json: true });
    expect(r.ok).toBe(false);
  });

  it("runSqliteTableDump errors when table missing", () => {
    const db = new DatabaseSync(":memory:");
    const r = runSqliteTableDump(db, { type: "organizations", limit: 5, order: "last", json: true });
    expect(r.ok).toBe(false);
