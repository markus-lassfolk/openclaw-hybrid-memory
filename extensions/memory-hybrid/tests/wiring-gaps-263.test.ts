...
function makeMetrics(
	overrides: Partial<ToolMetrics> & { tool: string },
): ToolMetrics {
	return {
		tool: overrides.tool,
...
function frustrationTurns(
	userMessages: string[],
): FrustrationConversationTurn[] {
...
describe("Gap 1 — exportAsImplicitSignals wiring into implicit_signals table", () => {
    ...
    `);
		...
		const rows = rawDb
			.prepare(
				`SELECT * FROM implicit_signals WHERE session_file = ? AND source = 'frustration'`,
			)
...
describe("Gap 3 — generateMonthlyReport monthly gating in nightly cycle", () => {
	...
	it("monthly report fact has correct key format YYYY-MM", async () => {
		...
		const row = rawDb
			.prepare(
				`SELECT key FROM facts WHERE key LIKE 'tool-effectiveness-monthly-%' AND superseded_at IS NULL LIMIT 1`,
			)
