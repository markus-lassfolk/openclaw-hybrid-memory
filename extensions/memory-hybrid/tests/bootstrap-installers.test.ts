...
describe("bootstrap installers", () => {
	it("orders core storage before optional adjacent services", () => {
		expect(
			bootstrapInstallers.map(
				(installer) => `${installer.bootstrapPhase}:${installer.id}`,
			),
		).toEqual(["core:memoryCore", "optional:adjacentFeatures"]);
	});
	...
	it("wraps failed FTS5 probes with an actionable startup error", () => {
		...
		expect(() => FactsDB.verifyFts5Support({ exec } as never)).toThrow(
			/FTS5|vector-only|startup/i,
		);
	...
	it("fails core bootstrap loudly when SQLite FTS5 support is unavailable", () => {
		...
		const verifySpy = vi
			.spyOn(factsDbConnection, "verifyFts5Support")
			.mockImplementation(() => {
				throw new Error(
		...
		try {
			...
			expect(api.logger.error).toHaveBeenCalledWith(
				expect.stringMatching(
					/SQLite FTS5 capability check failed during startup/,
				),
			);
		} finally {
