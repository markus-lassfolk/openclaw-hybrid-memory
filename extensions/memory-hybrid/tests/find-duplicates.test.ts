...
describe("runFindDuplicates", () => {
  it("uses embedBatch instead of per-item embed calls", async () => {
    ...
    expect(embedBatch).toHaveBeenCalledWith(["User prefers TypeScript", "User likes TypeScript"]);
  });
  ...
  it("reports pairs found by vector search", async () => {
    ...
    expect(result.pairs[0]).toMatchObject({ idA: "a", idB: "b", score: 0.95, textA: "Fact A", textB: "Fact B" });
  ...
  it("handles partial embedding failure — skips facts where embedBatch returns null for their slot", async () => {
    ...
    expect(warnLogger.warn).toHaveBeenCalledWith(expect.stringContaining("skipping fact b"));
  ...
  it("all batches fail — graceful degradation with zero pairs", async () => {
    const facts = Array.from({ length: 25 }, (_, i) => makeEntry({ id: `f${i}`, text: `Fact ${i}` }));
    const factsDb = { getFactsForConsolidation: vi.fn().mockReturnValue(facts) };
    const embedBatch = vi.fn().mockRejectedValue(new Error("all providers failed"));
    const embeddings = { embedBatch, embed: vi.fn(), dimensions: 2, modelName: "test" };
    const vectorDb = { search: vi.fn() };
  ...
  it("batches 20 items at a time — does not call embedBatch per-item", async () => {
    const facts = Array.from({ length: 35 }, (_, i) => makeEntry({ id: `f${i}`, text: `Fact ${i}` }));
    const factsDb = { getFactsForConsolidation: vi.fn().mockReturnValue(facts) };
    const embedBatch = vi
      .fn()
  ...
  it("skips structured facts when includeStructured is false", async () => {
    const facts = [
      ...
      makeEntry({ id: "a", text: "user@example.com", entity: "user", key: "email", value: "user@example.com" }),
    ...
    expect(embedBatch).toHaveBeenCalledWith(["Prefers dark mode", "Uses VSCode"]);
