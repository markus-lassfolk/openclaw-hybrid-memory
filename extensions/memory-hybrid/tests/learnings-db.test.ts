...
describe("LearningsDB — create", () => {
	it("stores an error entry with correct defaults", () => {
		const entry = db.create({
			type: "error",
			area: "forge-dispatch",
			content: "ctx.logger instead of api.logger",
		});
...
describe("LearningsDB — transition", () => {
	it("transitions open → promoted with promotedTo", () => {
		...
		const promoted = db.transition(
			entry.id,
			"promoted",
			"memory_store(category=technical)",
		);
	...
	it("throws when transitioning to promoted without promotedTo", () => {
		...
		expect(() => db.transition(entry.id, "promoted")).toThrow(
			"promotedTo is required",
		);
	});
});
