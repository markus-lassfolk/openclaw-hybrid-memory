...
describe("VectorDB semantic query cache", () => {
	...
	it("caps semantic cache growth per filter key", async () => {
		...
		const rows = await cacheTable
			.query()
			.where("filterKey = 'bounded'")
			.select(["cachedAt"])
			.toArray();
		const cachedAtValues = rows
			.map((row: { cachedAt: number }) => Number(row.cachedAt))
			.sort((a, b) => a - b);
...
describe("runRetrievalPipeline production RAG additions", () => {
	...
	it("returns cached semantic results before retrieval strategies run", async () => {
		...
		expect(
			vectorDb.storeSemanticQueryCache as ReturnType<typeof vi.fn>,
	...
	it("rewrites the query when all graded documents are irrelevant", async () => {
		...
		const grader = {
			gradeDocuments: vi
				...
				.mockResolvedValueOnce([
					{ factId: banana.id, answer: "yes", relevant: true },
				]),
	...
	it("uses distinct semantic cache keys when grading-related behavior changes", async () => {
		...
		const lookupCalls = (
			vectorDb.getSemanticQueryCacheMatch as ReturnType<typeof vi.fn>
		).mock.calls;
		expect(lookupCalls[0]?.[1]?.filterKey).not.toBe(
			lookupCalls[1]?.[1]?.filterKey,
		);
	});
});
