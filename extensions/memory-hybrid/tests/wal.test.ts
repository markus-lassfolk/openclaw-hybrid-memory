...
describe("WriteAheadLog", () => {
	...
	describe("write and read operations", () => {
		...
		it("returns empty array for non-existent WAL file", async () => {
			const emptyWal = new WriteAheadLog(
				join(testDir, "nonexistent.wal"),
				DEFAULT_MAX_AGE_MS,
			);
			await emptyWal.init();
	...
	describe("error handling", () => {
		...
		it("throws error when remove fails", async () => {
			...
			await expect(badWal.remove("some-id")).rejects.toThrow(
				/WAL remove failed/,
			);
		});
