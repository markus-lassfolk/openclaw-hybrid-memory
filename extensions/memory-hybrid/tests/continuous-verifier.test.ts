...
describe("buildVerificationPrompt", () => {
	it("includes the fact text in the prompt", () => {
		const prompt = buildVerificationPrompt("Server IP is 10.0.0.1", "server", [
			"fact A",
		]);
...
describe("parseVerificationOutcome", () => {
	it("returns CONFIRMED when response contains CONFIRMED", () => {
		expect(parseVerificationOutcome("CONFIRMED – still accurate")).toBe(
			"CONFIRMED",
		);
	});
	...
	it("returns UNCERTAIN when response contains UNCERTAIN", () => {
		expect(parseVerificationOutcome("UNCERTAIN – can't tell")).toBe(
			"UNCERTAIN",
		);
	});
	...
	it("prefers CONFIRMED over STALE when both appear (CONFIRMED checked first)", () => {
		expect(parseVerificationOutcome("CONFIRMED but maybe STALE")).toBe(
			"CONFIRMED",
		);
	});

	it("NOT CONFIRMED and UNCONFIRMED do not return CONFIRMED", () => {
		expect(parseVerificationOutcome("NOT CONFIRMED – no recent data")).toBe(
			"UNCERTAIN",
		);
		expect(parseVerificationOutcome("UNCONFIRMED")).toBe("UNCERTAIN");
		expect(parseVerificationOutcome("The fact is unconfirmed")).toBe(
			"UNCERTAIN",
		);
	});
});
...
describe("ContinuousVerifier.verifyFact", () => {
	it("returns CONFIRMED when LLM responds with CONFIRMED", async () => {
		...
		const outcome = await verifier.verifyFact(vf!, [
			"The server is still running at 10.0.0.1",
		]);
...
describe("ContinuousVerifier.runCycle — CONFIRMED", () => {
	it("creates a new version in the store and increments confirmed counter", async () => {
		...
		const id = await store.verify(
			"fact-c",
			"Critical infrastructure IP",
			"agent",
		);
...
describe("ContinuousVerifier.runCycle — STALE", () => {
	it("reduces fact confidence to 0.5 and adds 'needs-verification' tag", async () => {
		...
		const storeId = await store.verify(
			factId,
			"Server IP is 10.0.0.1",
			"agent",
		);
...
describe("ContinuousVerifier.runCycle — error handling", () => {
	it("counts errors but continues processing remaining facts", async () => {
		...
		const db = (store as unknown as { db: import("node:sqlite").DatabaseSync })
			.db;
		db.prepare(
			`UPDATE verified_facts SET next_verification = '2020-01-01T00:00:00.000Z' WHERE id IN (?, ?)`,
		).run(id1, id2);
...
describe("VerificationConfig — new fields", () => {
	...
	it("parses continuousVerification: true correctly", () => {
		const cfg = hybridConfigSchema.parse({
			...BASE_CONFIG,
			verification: { continuousVerification: true },
		});
	...
	it("parses verificationModel correctly", () => {
		const cfg = hybridConfigSchema.parse({
			...BASE_CONFIG,
			verification: { verificationModel: "openai/gpt-4o-mini" },
		});
