...
import {
	type FutureDateProtectionConfig,
	detectFutureDate,
} from "../utils/date-detector.js";
...
const ENABLED_CFG: FutureDateProtectionConfig = {
	enabled: true,
	maxFreezeDays: 365,
};
const DISABLED_CFG: FutureDateProtectionConfig = {
	enabled: false,
	maxFreezeDays: 365,
};
...
describe("detectFutureDate — ISO dates", () => {
	...
	it("detects ISO date with time component", () => {
		const result = detectFutureDate(
			"2026-03-20T14:00:00 appointment",
			ENABLED_CFG,
			NOW_MS,
		);
		expect(result).not.toBeNull();
...
describe("detectFutureDate — vague dates: no freeze", () => {
	...
	it("returns null for 'eventually'", () => {
		expect(
			detectFutureDate("Will handle eventually", ENABLED_CFG, NOW_MS),
		).toBeNull();
	});
	...
	it("returns null for text with no date at all", () => {
		expect(
			detectFutureDate("User prefers dark mode", ENABLED_CFG, NOW_MS),
		).toBeNull();
	});
});
...
describe("detectFutureDate — maxFreezeDays cap", () => {
	it("caps at maxFreezeDays when date is beyond the limit", () => {
		const cfgWith30Days: FutureDateProtectionConfig = {
			enabled: true,
			maxFreezeDays: 30,
		};
		// 2027-01-01 is ~300 days away
		const result = detectFutureDate(
			"Contract ends 2027-01-01",
			cfgWith30Days,
			NOW_MS,
		);
		expect(result).not.toBeNull();
	...
	it("does not cap when date is within maxFreezeDays", () => {
		const cfgWith365Days: FutureDateProtectionConfig = {
			enabled: true,
			maxFreezeDays: 365,
		};
		const result = detectFutureDate(
...
describe("FactsDB — decayConfidence skips frozen facts", () => {
	it("does NOT decay a fact that is frozen (decay_freeze_until > now)", () => {
		...
		rawDb
			.prepare(
				"UPDATE facts SET last_confirmed_at = ?, expires_at = ?, confidence = 0.9 WHERE id = ?",
			)
...
describe("FactsDB — decayConfidence deletes low-confidence frozen facts that expired", () => {
	it("does not delete a frozen fact even if confidence would drop below 0.1", () => {
		...
		rawDb
			.prepare(
				"UPDATE facts SET last_confirmed_at = ?, expires_at = ?, confidence = 0.15 WHERE id = ?",
			)
			.run(nowSec - 20 * 86400, expiresAt, entry.id);
...
describe("detectFutureDate — recurring events return null (no freeze)", () => {
	it("returns null for 'every Monday'", () => {
		expect(
			detectFutureDate("Stand-up every Monday at 9am", ENABLED_CFG, NOW_MS),
		).toBeNull();
	});

	it("returns null for 'every week'", () => {
		expect(
			detectFutureDate("Team sync every week", ENABLED_CFG, NOW_MS),
		).toBeNull();
	});
	...
	it("returns null for 'daily' recurring events", () => {
		expect(
			detectFutureDate("Daily standup at 9am", ENABLED_CFG, NOW_MS),
		).toBeNull();
	});

	it("returns null for 'weekly' recurring events", () => {
		expect(
			detectFutureDate("Weekly review on Fridays", ENABLED_CFG, NOW_MS),
		).toBeNull();
	});

	it("returns null for 'monthly' recurring events", () => {
		expect(
			detectFutureDate("Monthly all-hands meeting", ENABLED_CFG, NOW_MS),
		).toBeNull();
	});
	...
	it("returns null for 'yearly' recurring events", () => {
		expect(
			detectFutureDate(
				"Yearly performance review in March",
				ENABLED_CFG,
				NOW_MS,
			),
		).toBeNull();
	...
	it("returns null for 'each' recurring events", () => {
		expect(
			detectFutureDate("Send report each Friday", ENABLED_CFG, NOW_MS),
		).toBeNull();
	});
	...
	it("returns null for 'recurring' keyword", () => {
		expect(
			detectFutureDate(
				"This is a recurring event on 2026-04-01",
				ENABLED_CFG,
				NOW_MS,
			),
		).toBeNull();
	...
	it("returns null for 'biweekly' recurring events", () => {
		expect(
			detectFutureDate("Biweekly sync with the team", ENABLED_CFG, NOW_MS),
		).toBeNull();
	});
	...
	it("returns null for recurring even when a specific future date is also present", () => {
		...
		expect(
			detectFutureDate(
				"Team sync every Monday starting 2026-03-20",
				ENABLED_CFG,
				NOW_MS,
			),
		).toBeNull();
	...
	it("is case-insensitive for recurring keywords", () => {
		expect(
			detectFutureDate("DAILY backup at midnight", ENABLED_CFG, NOW_MS),
		).toBeNull();
		expect(
			detectFutureDate("Every FRIDAY at noon", ENABLED_CFG, NOW_MS),
		).toBeNull();
	...
	it("does NOT suppress a non-recurring date with similar substring ('everyday' is not 'every')", () => {
		...
		const result = detectFutureDate(
			"Everyday reminder for 2026-03-20",
			ENABLED_CFG,
			NOW_MS,
		);
...
describe("detectFutureDate — explicit paramDecayFreezeUntil override", () => {
	it("explicit finite override takes precedence over auto-detected date", () => {
		...
		const autoDetected = detectFutureDate(
			"Meeting in 30 days",
			ENABLED_CFG,
			NOW_MS,
		);
