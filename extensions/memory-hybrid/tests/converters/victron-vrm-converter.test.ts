...
const SAMPLE_CSV_HEADERS =
	"Timestamp,Battery Voltage (V),SOC (%),PV Power (W),AC Output (W)";
const makeCsvRow = (
	ts: string,
	bv: number,
	soc: number,
...
function makeCsv(rowCount: number): string {
	...
	for (let i = 0; i < rowCount; i++) {
		...
		rows.push(
			makeCsvRow(date, 48 + Math.random(), 80 + i, 1000 + i * 10, 500 + i * 5),
		);
	}
	return rows.join("\n");
}
...
describe("victronVrmConverter (CSV)", () => {
	it("parses a small CSV with Victron headers", () => {
		const csv = makeCsv(5);
		const result = victronVrmConverter.convert(
			csv,
			"/exports/victron_data.csv",
		);
