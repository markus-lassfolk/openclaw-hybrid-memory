...
describe("architecture center contract", () => {
  ...
  it("keeps core runtime and adjacent ownership disjoint", () => {
    ...
    expect(corePaths.size).toBe(ARCHITECTURE_CENTER.coreRuntime.flatMap((area) => area.ownership).length);
    expect(adjacentPaths.size).toBe(ARCHITECTURE_CENTER.adjacentSubsystems.flatMap((area) => area.ownership).length);
  ...
  it("documents the canonical decision, boundaries, heuristics, and guardrails", () => {
    ...
    expect(normalizedDoc).toContain("Architecture Center: Core Runtime vs Adjacent Subsystems");
    ...
    for (const area of [...ARCHITECTURE_CENTER.coreRuntime, ...ARCHITECTURE_CENTER.adjacentSubsystems]) {
