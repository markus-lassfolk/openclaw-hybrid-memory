...
describe("Error Reporter", () => {
	describe("Module Loading", () => {
		...
		it("should export DEFAULT_GLITCHTIP_DSN pointing to the community GlitchTip instance", async () => {
			const { DEFAULT_GLITCHTIP_DSN } = await import(
				"../services/error-reporter.js"
			);
			expect(DEFAULT_GLITCHTIP_DSN).toBe(
	...
	describe("Configuration Validation", () => {
		it("should not initialize without consent", async () => {
			const { initErrorReporter, isErrorReporterActive } = await import(
				"../services/error-reporter.js"
			);
		...
		it("should allow DSN override in community mode", async () => {
			const { initErrorReporter } = await import(
				"../services/error-reporter.js"
			);
		...
		it("should initialize by default with no config (opt-out: enabled+consent default to true)", async () => {
			...
			const { initErrorReporter, DEFAULT_GLITCHTIP_DSN } = await import(
				"../services/error-reporter.js"
			);
	...
	describe("Privacy Requirements (via internal functions)", () => {
		it("should scrub old and modern OpenAI API keys", async () => {
			...
			expect(scrubString("Error with key sk-1234567890abcdefghij")).toBe(
				"Error with key [REDACTED]",
			);
			expect(
				scrubString(
					"Error with key sk-proj-abcdefghijklmnopqrstuvwxyz1234567890",
				),
			).toBe("Error with key [REDACTED]");
			expect(
				scrubString(
					"Error with key sk-ant-api03-1234567890abcdefghijklmnopqrstuvwxyz",
				),
			).toBe("Error with key [REDACTED]");
		});
		...
		it("should scrub database connection strings", async () => {
			...
			expect(scrubString("Error: postgres://user:pass@localhost/db")).toBe(
				"Error: postgres://[REDACTED]",
			);
			expect(
				scrubString("Error: mysql://admin:secret@db.example.com/prod"),
			).toBe("Error: mysql://[REDACTED]");
			expect(
				scrubString("Error: redis://default:password@redis.local:6379"),
			).toBe("Error: redis://[REDACTED]");
			expect(
				scrubString(
					"Error: mongodb://root:pass123@mongo.example.com:27017/mydb",
				),
			).toBe("Error: mongodb://[REDACTED]");
		});

		it("should scrub GitHub tokens and Bearer tokens", async () => {
			const { scrubString } = await import("../services/error-reporter.js");

			expect(scrubString(`Token: ghp_${"a".repeat(36)}`)).toBe(
				"Token: [REDACTED]",
			);
		...
		it("should scrub home paths and PII", async () => {
			...
			expect(scrubString("File at /home/alice/file.txt")).toBe(
				"File at $HOME/file.txt",
			);
			expect(scrubString("File at /Users/bob/file.txt")).toBe(
				"File at $HOME/file.txt",
			);
		...
		it("should sanitize file paths with multiple directory markers", async () => {
			...
			expect(
				sanitizePath("/home/alice/project/extensions/memory-hybrid/index.ts"),
			).toBe("extensions/memory-hybrid/index.ts");
			expect(
				sanitizePath(
					"/home/alice/project/extensions/openclaw-hybrid-memory/index.ts",
				),
			).toBe("extensions/openclaw-hybrid-memory/index.ts");
			expect(
				sanitizePath(
					"/some/path/openclaw-hybrid-memory/services/error-reporter.ts",
				),
			).toBe("openclaw-hybrid-memory/services/error-reporter.ts");
					...
					values: [
						{
							type: "Error",
							value:
								"Failed with key sk-proj-abc123def456ghi789jkl012mno345pqr678",
							stacktrace: {
								frames: [
									{
										filename:
											"/home/user/project/extensions/memory-hybrid/index.ts",
										function: "test",
										lineno: 10,
									},
								],
		...
		it("should verify error reporter enforces security config", async () => {
			const serviceCode = await import("node:fs").then((fs) =>
				fs.promises.readFile(
					new URL("../services/error-reporter.ts", import.meta.url),
					"utf-8",
				),
			);
	...
	describe("Integration Tests", () => {
		it("should verify capturePluginError accepts extended context and returns event ID", async () => {
			const { capturePluginError, isErrorReporterActive } = await import(
				"../services/error-reporter.js"
			);

			const testError = new Error(
				"Test error with sk-proj-sensitive123456789012345678901234567890",
			);
	...
	describe("Version Helpers", () => {
		...
		it("extractVersion: parses valid release string", async () => {
			...
			expect(extractVersion("openclaw-hybrid-memory@2026.3.110")).toBe(
				"2026.3.110",
			);
		});
	...
	describe("resolvedIssues version-aware filtering", () => {
		it("drops event when fingerprint matches and version is older than fix", async () => {
			const { shouldDropForResolvedIssue } = await import(
				"../services/error-reporter.js"
			);
			const event: any = {
		...
		it("lets event through when release is missing but fallback is also unparseable", async () => {
			...
			const resolvedIssues = { "Error:Connection refused": "2026.3.110" };
			expect(
				shouldDropForResolvedIssue(event, resolvedIssues, "bad-release"),
			).toBe(false);
		});
		...
		it("uses fallbackRelease when event.release is absent", async () => {
			...
			expect(
				shouldDropForResolvedIssue(
					event,
					resolvedIssues,
					"openclaw-hybrid-memory@2026.3.100",
				),
			).toBe(true);
			...
			expect(
				shouldDropForResolvedIssue(
					event,
					resolvedIssues,
					"openclaw-hybrid-memory@2026.3.120",
				),
			).toBe(false);
		});
	...
	describe("beforeSend integration: version-aware filtering pipeline", () => {
		it("drops event matching a resolved issue on an older release (beforeSend pipeline)", async () => {
			const { sanitizeEvent, shouldDropForResolvedIssue } = await import(
				"../services/error-reporter.js"
			);
			...
			const shouldDrop = shouldDropForResolvedIssue(
				sanitized!,
				resolvedIssues,
				rawEvent.release,
			);
	...
	describe("New Exports", () => {
		it("should export flushErrorReporter", async () => {
			const { flushErrorReporter, isErrorReporterActive } = await import(
				"../services/error-reporter.js"
			);
		...
		it("should export addOperationBreadcrumb", async () => {
			const { addOperationBreadcrumb } = await import(
				"../services/error-reporter.js"
			);
	...
	describe("Noisy error filtering", () => {
		it("drops transient network transport errors", async () => {
			const { shouldDropNoisyError } = await import(
				"../services/error-reporter.js"
			);

			expect(
				shouldDropNoisyError(new Error("ECONNREFUSED http://localhost:11434")),
			).toBe(true);
			expect(shouldDropNoisyError(new Error("TypeError: fetch failed"))).toBe(
				true,
			);
		});
		...
		it("drops external-provider auth errors", async () => {
			...
			expect(shouldDropNoisyError(new Error("invalid api key provided"))).toBe(
				true,
			);
			expect(
				shouldDropNoisyError(
					new Error("Country, region, or territory not supported"),
				),
			).toBe(true);
		});
		...
		it("drops Ollama circuit-breaker-open errors", async () => {
			...
			expect(
				shouldDropNoisyError(
					new Error("Ollama circuit breaker open — retrying in 30s"),
				),
			).toBe(true);
		});
		...
		it("drops wrapped or aggregate errors only when every cause is noisy", async () => {
			...
			const aggregateAllNoisy = Object.assign(
				new Error("all providers failed"),
				{
					causes: [
						new Error("ECONNREFUSED"),
						new Error("Ollama circuit breaker open"),
					],
				},
			);
		...
		it("does not drop unrelated errors or file-permission failures", async () => {
			...
			expect(
				shouldDropNoisyError(new Error("Access denied to file /tmp/test.txt")),
			).toBe(false);
		});
	});
});
...
describe("UnconfiguredProviderError suppression", () => {
	it("capturePluginError returns undefined immediately and never reaches GlitchTip for UnconfiguredProviderError", async () => {
		...
		const { capturePluginError } = await import(
			"../services/error-reporter.js"
		);
		...
		const err = Object.assign(
			new Error(
				"Provider 'openrouter' is not configured for model openrouter/qwen/qwen3-14b",
			),
			{
