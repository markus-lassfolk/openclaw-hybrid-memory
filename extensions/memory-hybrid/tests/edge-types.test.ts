...
const { FactsDB, packIntoBudget, serializeFactForContext, runConsolidate } =
	_testing;
...
describe("CONTRADICTS unidirectional", () => {
	it("recordContradiction creates forward link (new → old)", () => {
		...
		const contradicts = outboundFromB.filter(
			(l) => l.linkType === "CONTRADICTS",
		);
	...
	it("recordContradiction does NOT create reverse link (old → new) — single directed link only", () => {
		...
		const contradicts = outboundFromA.filter(
			(l) => l.linkType === "CONTRADICTS",
		);
	...
	it("detectContradictions creates a directed CONTRADICTS link (new → old only)", () => {
		...
		const fromB = db
			.getLinksFrom(factB.id)
			.filter((l) => l.linkType === "CONTRADICTS");
		const fromA = db
			.getLinksFrom(factA.id)
			.filter((l) => l.linkType === "CONTRADICTS");
...
describe("autoDetectInstanceOf", () => {
	it("detects 'is a' pattern and creates INSTANCE_OF link", () => {
		...
		const outbound = db
			.getLinksFrom(pollyFact.id)
			.filter((l) => l.linkType === "INSTANCE_OF");
	...
	it("detects 'type of' pattern and creates INSTANCE_OF link", () => {
		...
		const fact = storeFact(
			"Labrador is a type of retriever",
			"labrador",
			null,
			null,
		);
		const linked = db.autoDetectInstanceOf(
			fact.id,
			"Labrador is a type of retriever",
		);
...
describe("DERIVED_FROM", () => {
	...
	it("DERIVED_FROM links visible in getLinksFrom", () => {
		...
		const links = db
			.getLinksFrom(derived.id)
			.filter((l) => l.linkType === "DERIVED_FROM");
	...
	it("DERIVED_FROM provenance chain — multiple levels", () => {
		...
		const level1 = db
			.getLinksFrom(insight.id)
			.filter((l) => l.linkType === "DERIVED_FROM");
		expect(level1[0].targetFactId).toBe(processed.id);

		const level2 = db
			.getLinksFrom(processed.id)
			.filter((l) => l.linkType === "DERIVED_FROM");
	...
	it("DERIVED_FROM links survive when target fact is deleted (provenance preservation)", () => {
		...
		const links = db
			.getLinksFrom(merged.id)
			.filter((l) => l.linkType === "DERIVED_FROM");
		expect(links).toHaveLength(2);
		expect(links.map((l) => l.targetFactId).sort()).toEqual(
			[source1.id, source2.id].sort(),
		);
	});
});
...
describe("packIntoBudget with contradictedIds", () => {
	it("marks contradicted facts with WARNING in packed output", () => {
		...
		const { packed } = packIntoBudget(entries, 10_000, {
			contradictedIds: new Set(["a"]),
		});
	...
	it("is backward-compatible when options are omitted", () => {
		...
		const { packed } = packIntoBudget(
			[{ factId: "d", entry: entry as MemoryEntry }],
			10_000,
		);
		expect(packed[0]).not.toContain("WARNING");
	});
...
describe("runConsolidate DERIVED_FROM regression", () => {
	it("DERIVED_FROM links survive after source facts are hard-deleted during consolidation", async () => {
		...
		const source1 = storeFact(
			"The sky appears blue during daylight hours",
			"sky",
			null,
			null,
		);
		const source2 = storeFact(
			"Sky looks blue when sun is up",
			"sky",
			null,
			null,
		);
		...
		const linksToSource1 = db
			.getLinksTo(source1.id)
			.filter((l) => l.linkType === "DERIVED_FROM");
		const linksToSource2 = db
			.getLinksTo(source2.id)
			.filter((l) => l.linkType === "DERIVED_FROM");
		...
		const mergedFactId =
			linksToSource1[0]?.sourceFactId ?? linksToSource2[0]?.sourceFactId;
		expect(mergedFactId).toBeDefined();
		const outbound = db
			.getLinksFrom(mergedFactId!)
			.filter((l) => l.linkType === "DERIVED_FROM");
