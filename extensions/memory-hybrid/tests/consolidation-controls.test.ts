...
describe("consolidation controls", () => {
  it("recognizes derived consolidation facts by source, key, or tag", () => {
    ...
    expect(isConsolidatedDerivedFact({ tags: ["topic", "consolidated"] })).toBe(true);
    expect(isConsolidatedDerivedFact({ source: "conversation", tags: ["topic"] })).toBe(false);
  });

  it("applies a retrieval penalty to derived consolidation facts", () => {
    expect(applyConsolidationRetrievalControls(1, { source: "consolidation" })).toBeCloseTo(0.85, 5);
    expect(applyConsolidationRetrievalControls(1, { source: "conversation" })).toBe(1);
  });
});
