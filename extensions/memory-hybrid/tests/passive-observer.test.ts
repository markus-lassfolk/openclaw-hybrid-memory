...
describe("extractTextFromJsonlChunk", () => {
	...
	it("extracts user text blocks", () => {
		...
		expect(extractTextFromJsonlChunk(line)).toContain(
	...
	it("extracts assistant text blocks", () => {
		...
		expect(extractTextFromJsonlChunk(line)).toContain(
			"assistant: The project uses Node.js 20 LTS",
		);
	});
...
describe("parseObserverResponse", () => {
	...
	it("parses a valid JSON array response", () => {
		const raw = JSON.stringify([
			...
			{
				text: "Project uses PostgreSQL 15",
				category: "fact",
				importance: 0.75,
			},
		]);
	...
	it("clamps importance to 0–1 range", () => {
		const raw = JSON.stringify([
			{
				text: "Some fact about the system configuration",
				category: "fact",
				importance: 1.5,
			},
			{
				text: "Another fact about user workflow habits",
				category: "fact",
				importance: -0.2,
			},
		]);
	...
	it("defaults to 'fact' category for unknown categories", () => {
		const raw = JSON.stringify([
			{
				text: "Some observation about team dynamics",
	...
	it("skips items with text shorter than 10 chars", () => {
		const raw = JSON.stringify([
			...
			{
				text: "This is a long enough fact to be included",
				category: "fact",
				importance: 0.8,
			},
		]);
	...
	it("returns empty array for invalid JSON", () => {
		expect(parseObserverResponse("not json at all", categories)).toHaveLength(
			0,
		);
	...
	it("handles string importance (e.g., '0.75')", () => {
		const raw = JSON.stringify([
			{
				text: "System runs on Ubuntu 22.04 servers in production",
				category: "fact",
				importance: "0.75",
			},
		]);
...
describe("cursor management", () => {
	...
	it("loadCursors ignores non-numeric cursor values", async () => {
		...
		writeFileSync(
			path,
			JSON.stringify({ good: 100, bad: "string", negative: -1 }),
		);
...
describe("runPassiveObserver", () => {
	...
	it("skips sessions where cursor equals file size (nothing new)", async () => {
		...
		vi.doMock("../services/chat.js", () => ({
			chatCompleteWithRetry: chatMock,
		}));
	...
	it("respects minImportance threshold — skips low-importance facts", async () => {
		...
		const llmResponse = JSON.stringify([
			{
				text: "The user mentioned having lunch today",
				category: "fact",
				importance: 0.2,
			}, // below threshold
			{
				text: "The team uses React for all frontend development",
				category: "fact",
				importance: 0.8,
			}, // above threshold
		]);
		...
		expect(filtered[0].text).toBe(
			"The team uses React for all frontend development",
		);
	});
	...
	it("deduplication: skips facts when vectorDb.search returns a match", async () => {
		...
		const chatSpy = vi
			...
			.mockResolvedValue(
				JSON.stringify([
					{
						text: "The team uses React for frontend",
						category: "fact",
						importance: 0.8,
					},
				]),
	...
	it("updates cursor to file size after processing", async () => {
		const sessionContent = [
			JSON.stringify({
				message: { role: "user", content: "I always use eslint for linting" },
			}),
			"",
		].join("\n");
	...
	it("dryRun mode does not call factsDb.store", async () => {
		const sessionContent = `${JSON.stringify({
			message: {
				role: "user",
				content: "The system is built with TypeScript and Node.js",
			},
		})}\n`;
	...
	it("stores extracted facts end-to-end with mocked LLM + DBs", async () => {
		const sessionContent = `${JSON.stringify({
			message: {
				role: "user",
				content: "We decided to use Rust for the CLI tool.",
			},
		})}\n`;
		...
		const chatSpy = vi
			...
			.mockResolvedValue(
				JSON.stringify([
					{
						text: "The team decided to use Rust for the CLI tool",
						category: "decision",
						importance: 0.8,
					},
				]),
...
describe("runPassiveObserver — LanceDB dedup (Issue #499)", () => {
	...
	it("calls vectorDb.search once per extracted fact", async () => {
		...
		const chatSpy = vi.spyOn(chat, "chatCompleteWithRetry").mockResolvedValue(
			JSON.stringify([
				{
					text: "The team uses TypeScript everywhere",
					category: "fact",
					importance: 0.8,
				},
				{
					text: "The project targets Node.js 22 LTS",
					category: "fact",
					importance: 0.75,
				},
			]),
	...
	it("uses deduplicationThreshold as minScore for vectorDb.search", async () => {
		...
		const chatSpy = vi
			...
			.mockResolvedValue(
				JSON.stringify([
					{
						text: "The project uses PostgreSQL as the main database",
						category: "fact",
						importance: 0.8,
					},
				]),
	...
	it("reinforcement: boosts matched fact when vectorDb.search finds a near-duplicate", async () => {
		...
		const matchResult = [
			{ entry: { id: matchedFactId }, score: 0.95, backend: "lancedb" },
		];
		const vectorDb = makeVectorDb(matchResult);
		const factsDb = makeFactsDb({
		...
		expect(result.factsStored).toBe(0);
		expect(factsDb.boostConfidence).toHaveBeenCalledWith(
			matchedFactId,
			0.1,
			1.0,
		);
	...
	it("search failure does not crash observer — proceeds without dedup", async () => {
		...
		const chatSpy = vi
			...
			.mockResolvedValue(
				JSON.stringify([
					{
						text: "Team adopted Kubernetes for container orchestration",
						category: "decision",
						importance: 0.8,
					},
				]),
	...
	it("does not call getRecentFacts — LanceDB is the single source of truth", async () => {
		...
		const chatSpy = vi
			...
			.mockResolvedValue(
				JSON.stringify([
					{
						text: "The service runs on AWS Lambda functions",
						category: "fact",
						importance: 0.8,
					},
				]),
...
describe("runPassiveObserver event_log integration", () => {
	...
	it("does not write to event_log when eventLog is null", async () => {
		...
		const chatSpy = vi
			...
			.mockResolvedValue(
				JSON.stringify([
					{ text: "The team uses Rust", category: "fact", importance: 0.8 },
				]),
			);
	...
	it("writes to event_log before factsDb.store (Layer 1 write order)", async () => {
		...
		const chatSpy = vi
			...
			.mockResolvedValue(
				JSON.stringify([
					{
						text: "We use Postgres as primary database",
						category: "fact",
						importance: 0.8,
					},
				]),
...
describe("PassiveObserverConfig defaults via hybridConfigSchema", () => {
	...
	it("expands $HOME in passiveObserver.sessionsDir", async () => {
		...
		expect(cfg.passiveObserver.sessionsDir).toBe(
	...
	it("expands $HOME in procedures.sessionsDir", async () => {
		...
		expect(cfg.procedures.sessionsDir).toBe(
			`${home}/.openclaw/agents/main/sessions`,
		);
	});
...
describe("isIdentityFact", () => {
	...
	it("detects 'the bot's role'", () => {
		expect(isIdentityFact("The bot's role is infrastructure assistant")).toBe(
			true,
		);
	});
	...
	it("detects 'role is ...'", () => {
		expect(isIdentityFact("Role is senior infrastructure assistant")).toBe(
			true,
		);
	});
	...
	it("does NOT flag generic facts about the user", () => {
		expect(isIdentityFact("The user prefers TypeScript over JavaScript")).toBe(
			false,
		);
	});
	...
	it("detects agent name-specific pattern when agentName is provided", () => {
		expect(
			isIdentityFact("TestBot's email is agent@example.com", "TestBot"),
		).toBe(true);
	});

	it("detects agent name without apostrophe when agentName is provided", () => {
		expect(
			isIdentityFact("TestBot email is agent@example.com", "TestBot"),
		).toBe(true);
	});

	it("is case-insensitive", () => {
		expect(isIdentityFact("YOUR EMAIL IS TESTBOT@EXAMPLE.COM")).toBe(true);
		expect(isIdentityFact("the ASSISTANT's name is TESTBOT", "testbot")).toBe(
			true,
		);
	});
	...
	it("stores identity fact with scope=global, decayClass=permanent", async () => {
		...
		const chatSpy = vi
			...
			.mockResolvedValue(
				JSON.stringify([
					{
						text: "The agent's email is agent@example.com",
						category: "fact",
						importance: 0.8,
					},
				]),
		...
		const infoMessages = logger.info.mock.calls.map((c: unknown[]) =>
			String(c[0]),
		);
		expect(
			infoMessages.some((m) => m.includes("promoting identity fact")),
		).toBe(true);
	...
	it("stores non-identity fact with scope=session (default unchanged)", async () => {
		...
		const chatSpy = vi
			...
			.mockResolvedValue(
				JSON.stringify([
					{
						text: "User mentioned they like coffee a lot",
						category: "fact",
						importance: 0.7,
					},
				]),
	...
	it("promotes fact with explicit agentName='TestBot'", async () => {
		...
		const chatSpy = vi
			...
			.mockResolvedValue(
				JSON.stringify([
					{
						text: "TestBot email is agent@example.com",
						category: "fact",
						importance: 0.75,
					},
				]),
