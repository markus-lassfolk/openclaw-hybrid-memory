...
import {
	RRF_K_DEFAULT,
	mergeResults as mergeResultsModule,
...
describe("mergeResults", () => {
	it("RRF: prioritizes sqlite over lancedb when RRF ties (each appears in one list only)", () => {
		const sqlite = [
			makeResult({ text: "sqlite fact", score: 0.9, backend: "sqlite" }),
		];
		const lance = [
			makeResult({ text: "lance fact", score: 0.8, backend: "lancedb" }),
		];
	...
	it("RRF: boosts items appearing in both keyword and semantic results", () => {
		...
		const sqlite = [
			inBoth,
			makeResult({ text: "sqlite only", score: 0.9, backend: "sqlite" }),
		];
		const lance = [
			makeResult({
				id: sharedId,
	...
	it("excludes Lance results whose text is superseded", () => {
		...
		const sqlite = [
			makeResult({ text: "Current fact", score: 0.8, backend: "sqlite" }),
		];
		const lance = [
			makeResult({
	...
	it("RRF: returned score is RRF score (not raw)", () => {
		...
		const sqlite = [
			inBoth,
			makeResult({ text: "sqlite only", score: 0.99, backend: "sqlite" }),
		];
		const lance = [
			makeResult({
				id: sharedId,
	...
	it("RRF: optional k parameter changes scores", () => {
		...
		const mergedK60 = mergeResultsModule(sqlite, lance, 10, undefined, {
			k: 60,
		});
		const mergedK10 = mergeResultsModule(sqlite, lance, 10, undefined, {
			k: 10,
		});
...
describe("filterByScope", () => {
	...
	it("filters out results not in scope", () => {
		...
		const getById = (
			id: string,
			opts?: { scopeFilter?: { userId?: string | null } | null },
		) => {
