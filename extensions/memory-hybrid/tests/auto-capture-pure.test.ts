...
describe("SENSITIVE_PATTERNS", () => {
	it("matches 'password' keyword", () => {
		expect(
			SENSITIVE_PATTERNS.some((r) => r.test("the password is secret")),
		).toBe(true);
	});

	it("matches 'api_key' variant", () => {
		expect(
			SENSITIVE_PATTERNS.some((r) => r.test("set the api_key value")),
		).toBe(true);
	});

	it("matches 'secret' keyword", () => {
		expect(
			SENSITIVE_PATTERNS.some((r) => r.test("the client secret here")),
		).toBe(true);
	});

	it("matches 'token is' phrase", () => {
		expect(
			SENSITIVE_PATTERNS.some((r) => r.test("the token is abc123def456")),
		).toBe(true);
	});

	it("matches AWS access key format AKIA...", () => {
		expect(
			SENSITIVE_PATTERNS.some((r) => r.test("AKIAIOSFODNN7EXAMPLE is the key")),
		).toBe(true);
	});

	it("matches private key header", () => {
		expect(
			SENSITIVE_PATTERNS.some((r) => r.test("-----BEGIN RSA PRIVATE KEY-----")),
		).toBe(true);
	});

	it("matches connection strings with embedded passwords", () => {
		expect(
			SENSITIVE_PATTERNS.some((r) => r.test("mongodb://user:pass@host/db")),
		).toBe(true);
	});

	it("does not match plain safe text", () => {
		expect(
			SENSITIVE_PATTERNS.some((r) =>
				r.test("user prefers dark mode in the editor"),
			),
		).toBe(false);
	});
});
...
describe("detectCredentialPatterns", () => {
	...
	it("returns empty array for plain text with no credentials", () => {
		const results = detectCredentialPatterns(
...
describe("extractCredentialMatch", () => {
	...
	it("returns null when no credential pattern found", () => {
		const result = extractCredentialMatch(
			"this is just plain text with no secrets",
		);
...
describe("inferServiceFromText", () => {
	it("returns 'home-assistant' for Home Assistant text", () => {
		expect(inferServiceFromText("my home assistant token for local ha")).toBe(
			"home-assistant",
		);
	});

	it("returns 'home-assistant' for hass keyword", () => {
		expect(inferServiceFromText("hass long-lived token here")).toBe(
			"home-assistant",
		);
	});
	...
	it("returns 'github' for GitHub token text", () => {
		expect(
			inferServiceFromText("my github personal access token ghp_abc"),
		).toBe("github");
	});
	...
	it("returns 'slack' for xoxb- token", () => {
		...
		expect(inferServiceFromText(`${slackToken} is the slack bot token`)).toBe(
			"slack",
		);
	});
	...
	it("returns 'twilio' for twilio text", () => {
		expect(inferServiceFromText("twilio auth token for SMS service")).toBe(
			"twilio",
		);
	});

	it("returns 'duckdns' for duckdns text", () => {
		expect(inferServiceFromText("duckdns token for dynamic DNS")).toBe(
			"duckdns",
		);
	});

	it("returns 'imported' for unrecognised service text", () => {
		expect(inferServiceFromText("some random service token value")).toBe(
			"imported",
		);
	});
});
...
describe("isCredentialLike", () => {
	...
	it("returns true when entity contains 'secret'", () => {
		expect(isCredentialLike("some text", "client_secret", null, null)).toBe(
			true,
		);
	});
	...
	it("returns true when text matches a credential pattern", () => {
		expect(
			isCredentialLike(
				"use Bearer eyJhbGciOiJIUzI1NiJ9.eyJzdWIiOiIxIn0.abc123 for auth",
				null,
				null,
				null,
			),
	...
	it("returns true when text matches a sensitive pattern (password)", () => {
		expect(isCredentialLike("the password is hunter2", null, null, null)).toBe(
			true,
		);
	});
	...
	it("returns false for plain non-credential text", () => {
		expect(
			isCredentialLike(
				"the user prefers dark mode in VS Code",
				null,
				null,
				null,
			),
...
describe("tryParseCredentialForVault", () => {
	...
	it("returns null for non-credential text", () => {
		expect(
			tryParseCredentialForVault("user prefers dark mode", null, null, null),
		).toBeNull();
	});
