...
describe("deepMerge - Prototype Pollution Prevention", () => {
	...
	it("blocks constructor key from being merged", () => {
		...
		expect(
			((target.constructor as any).prototype as any)?.isAdmin,
		).toBeUndefined();
	});
	...
	it("blocks prototype key from being merged", () => {
		...
		expect(
			(Object.prototype as Record<string, unknown>).evilProperty,
		).toBeUndefined();
	});
	...
	it("handles multiple dangerous keys in same object", () => {
      ...
      "safeKey": "safeValue"
		...
		// Verify Object.prototype was not polluted
		expect(
			(Object.prototype as Record<string, unknown>).polluted1,
		).toBeUndefined();
		expect(
			(Object.prototype as Record<string, unknown>).polluted2,
		).toBeUndefined();
		expect(
			(Object.prototype as Record<string, unknown>).polluted3,
		).toBeUndefined();
	...
	it("prevents pollution through deeply nested __proto__", () => {
		...
		expect(
			(Object.prototype as Record<string, unknown>).deepPollution,
		).toBeUndefined();
