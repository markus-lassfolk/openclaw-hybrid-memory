...
describe("self-correction-extract", () => {
	...
	it("extracts incident when user message matches correction phrase", () => {
		...
		expect(result.incidents[0].precedingAssistant).toContain(
			"I ran the command",
		);
	...
	it("truncates long user and assistant messages", () => {
		...
		expect(result.incidents[0].precedingAssistant.length).toBeLessThanOrEqual(
			503,
		);
		expect(result.incidents[0].followingAssistant.length).toBeLessThanOrEqual(
			503,
		);
	});
});
