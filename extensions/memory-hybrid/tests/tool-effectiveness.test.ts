...
describe("aggregateTraceRows", () => {
	...
	it("computes redundancy score between 0 and 1", () => {
		const rows = Array.from({ length: 5 }, (_, i) =>
			makeTrace(
				["repeat_tool", "repeat_tool", "repeat_tool"],
				"success",
				`s${i}`,
			),
		);
	...
	it("returns results sorted by compositeScore DESC", () => {
		const rows = [
			...
			...Array.from({ length: 5 }, () =>
				makeTrace(["always_succeed"], "success"),
			),
		];
		const result = aggregateTraceRows(rows, 3);
		if (result.length >= 2) {
			for (let i = 0; i < result.length - 1; i++) {
				expect(result[i]?.compositeScore).toBeGreaterThanOrEqual(
					result[i + 1]?.compositeScore,
				);
			}
		}
	});
...
describe("generateRecommendations", () => {
	...
	it("returns recommendation for low-success tools", () => {
		...
		const recs = generateRecommendations(metrics, 0.3);
		expect(recs.some((r) => r.toLowerCase().includes("failing_tool"))).toBe(
			true,
		);
	});
...
describe("ToolEffectivenessStore — context-aware", () => {
	...
	it("same tool can have different scores per context", () => {
		store.upsert(
			makeMetrics({ tool: "exec", context: "general", compositeScore: 0.6 }),
		);
		store.upsert(
			makeMetrics({
				tool: "exec",
	...
	it("getToolEffectiveness without context returns all rows for tool", () => {
		store.upsert(
			makeMetrics({ tool: "browser", context: "general", compositeScore: 0.5 }),
		);
		store.upsert(
			makeMetrics({
				tool: "browser",
