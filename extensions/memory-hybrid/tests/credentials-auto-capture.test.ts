...
describe("extractCredentialsFromToolCalls", () => {
	describe("sshpass pattern", () => {
		it("extracts password from sshpass -p <pass> ssh user@host", () => {
			const input = JSON.stringify({
				command: "sshpass -p s3cr3tPass ssh root@192.168.1.19",
			});
		...
		it("extracts password from sshpass with extra ssh options", () => {
			const input =
				"sshpass -p myP@ssw0rd ssh -o StrictHostKeyChecking=no admin@10.0.0.1";
			const results = extractCredentialsFromToolCalls(input);
			const cred = results.find((r) => r.type === "password");
			expect(cred).toBeDefined();
	...
	describe("curl -u user:pass pattern", () => {
		it("extracts password from curl -u user:pass URL", () => {
			const input =
				"curl -u admin:superSecret123 https://jenkins.example.com/api";
			const results = extractCredentialsFromToolCalls(input);
			const cred = results.find((r) => r.type === "password");
			expect(cred).toBeDefined();
		...
		it("ignores curl -u with short password", () => {
			...
			const results = extractCredentialsFromToolCalls(input);
			const cred = results.find(
				(r) => r.type === "password" && r.service === "example.com",
			);
			expect(cred).toBeUndefined();
		});
	});
	...
	describe("connection string patterns", () => {
		it("extracts postgres connection string credentials", () => {
			const input =
				"psql postgres://dbuser:dbpassword123@db.example.com:5432/mydb";
			const results = extractCredentialsFromToolCalls(input);
			const cred = results.find(
				(r) => r.type === "password" && r.service.startsWith("postgres://"),
			);
			expect(cred).toBeDefined();
	...
	describe("export VAR=value pattern", () => {
		...
		it("extracts token from export *_TOKEN=value", () => {
			const input =
				"export GITHUB_TOKEN=test_token_abcdefghijklmnopqrstuvwxyz123456";
			const results = extractCredentialsFromToolCalls(input);
	...
	describe("deduplication", () => {
		it("returns only one entry per service+type combination", () => {
			...
			const results = extractCredentialsFromToolCalls(input);
			const apiKeyCreds = results.filter(
				(r) => r.service === "api" && r.type === "api_key",
			);
		...
		it("captures multiple distinct credentials from the same input", () => {
			...
			const results = extractCredentialsFromToolCalls(input);
			const sshCreds = results.filter(
				(r) => r.type === "password" && r.service.startsWith("ssh://"),
			);
			expect(sshCreds.length).toBe(2);
			expect(sshCreds.map((c) => c.service)).toContain(
...
describe("tool call credential auto-capture integration", () => {
	...
	it("stores SSH credential extracted from tool call into vault", () => {
		const toolCallArgs = JSON.stringify({
			command: "sshpass -p secret1234 ssh root@192.168.1.19",
		});
	...
	it("does not store credential values in facts DB (vault only)", () => {
		...
		const args =
			"export PAYMENT_SECRET_KEY=test_key_abcdefghijklmnopqrst123456";
		const creds = extractCredentialsFromToolCalls(args);
		for (const cred of creds) {
...
describe("council review fixes", () => {
	...
	describe("Fix #19: connection string extraction", () => {
		it("extracts credentials from connection strings in .env format", () => {
			const input =
				"DATABASE_URL=postgres://user:pass12345@db.example.com:5432/mydb";
			const results = extractCredentialsFromToolCalls(input);
			const cred = results.find((r) => r.service.startsWith("postgres://"));
			expect(cred).toBeDefined();
		...
		it("handles mongodb+srv connection strings", () => {
			const input =
				"MONGO_URI=mongodb+srv://admin:secret789@cluster.mongodb.net/dbname";
			const results = extractCredentialsFromToolCalls(input);
...
describe("shouldCapture — unified SENSITIVE_PATTERNS (issue #555)", () => {
	...
	it("blocks private key header text that also contains a memory trigger", () => {
		const text =
			"remember that -----BEGIN RSA PRIVATE KEY----- is the cert header";
		expect(shouldCapture(text, maxChars, alwaysMatch)).toBe(false);
	});

	it("blocks connection string with embedded password that also contains a memory trigger", () => {
		const text =
			"remember mongodb://admin:Pa$$w0rd@localhost/mydb for the service";
		expect(shouldCapture(text, maxChars, alwaysMatch)).toBe(false);
	});
	...
	it("blocks text with token=value format that also contains a memory trigger", () => {
		const text =
			"remember my API token=ghp_abc123def456 for the staging environment";
		expect(shouldCapture(text, maxChars, alwaysMatch)).toBe(false);
	});

	it("blocks text with token: value format that also contains a memory trigger", () => {
		const text =
			"remember to use token: sk-abcdefghijklmnopqrstuvwxyz for the service";
		expect(shouldCapture(text, maxChars, alwaysMatch)).toBe(false);
	});
});
