...
function buildMinimalCtx(
	...
	const cfg = hybridConfigSchema.parse({
		embedding: {
			apiKey: "sk-test-key-long-enough-to-pass",
...
describe("createPluginService startup — version check wiring", () => {
	...
	it("emits a version warning when api.version is undefined (old gateway)", async () => {
		...
		const warnCalls = api.logger.warn.mock.calls.map(
			(c: unknown[]) => c[0] as string,
		);
		const versionWarn = warnCalls.find((msg) =>
			msg.includes(MIN_OPENCLAW_VERSION),
		);
	...
	it("mutes telemetry and warns when the published plugin version is newer", async () => {
		...
		const fetchMock = vi.fn(async (input: string | URL) => {
			...
			if (new URL(url).hostname === "registry.npmjs.org") {
				return new Response(JSON.stringify({ version: publishedNewer }), {
					status: 200,
				});
			}
			if (new URL(url).hostname === "api.github.com") {
				return new Response(
					JSON.stringify({ tag_name: `v${publishedNewer}` }),
					{ status: 200 },
				);
			}
		...
		expect(getErrorReporterMuteReason()).toContain(
			`outdated-plugin:${publishedNewer}`,
		);
		expect(
			capturePluginError(new Error("should stay local"), {
		...
		expect(
			warnCalls.some(
				(msg) =>
					msg.includes("update available") && msg.includes(publishedNewer),
			),
		).toBe(true);
