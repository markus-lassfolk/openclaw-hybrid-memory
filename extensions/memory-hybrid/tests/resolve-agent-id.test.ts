...
import {
	sliceHookAgentContext,
	withHookResolutionApi,
} from "../lifecycle/hook-resolution-api.js";
import {
	resolveAgentIdFromHookEvent,
	tryParseAgentIdFromOpenClawSessionKey,
} from "../lifecycle/resolve-agent-id.js";
...
describe("tryParseAgentIdFromOpenClawSessionKey", () => {
	it("parses agent:main:main", () => {
		expect(tryParseAgentIdFromOpenClawSessionKey("agent:main:main")).toBe(
			"main",
		);
	});
	...
	it("parses multi-segment cron-style keys", () => {
		expect(
			tryParseAgentIdFromOpenClawSessionKey("agent:ralph:cron:job-abc"),
		).toBe("ralph");
	});
...
describe("resolveSessionKeyFromHookEvent", () => {
	...
	it("falls back to api.context.sessionKey when session id fields are absent", () => {
		expect(
			resolveSessionKeyFromHookEvent(
				{},
				{ context: { sessionKey: "agent:ralph:cron:j1" } },
			),
		).toBe("agent:ralph:cron:j1");
	});
	...
	it("prefers api.context.sessionId over api.context.sessionKey", () => {
		expect(
			resolveSessionKeyFromHookEvent(
				{},
				{ context: { sessionId: "sid", sessionKey: "sk" } },
			),
		).toBe("sid");
	});
	...
	it("reads event.context.sessionId when session and api.context are absent (newer OpenClaw payloads)", () => {
		expect(
			resolveSessionKeyFromHookEvent(
				{ context: { sessionId: "agent:nova:chan:1" } },
				{},
			),
		).toBe("agent:nova:chan:1");
	});
			...
			resolveSessionKeyFromHookEvent(
				{
					...
					context: { sessionId: "from-context" },
				},
				{},
			),
		).toBe("from-session");
	...
	it("uses typed-hook context merged into api when event is minimal (#1005)", () => {
		const api = withHookResolutionApi(mockApi(), {
			sessionKey: "agent:ralph:cron:job-99",
		});
		expect(resolveSessionKeyFromHookEvent({ prompt: "hi" }, api)).toBe(
			"agent:ralph:cron:job-99",
		);
	});

	it("prefers event session id over hook-merged api.context.sessionKey", () => {
		const api = withHookResolutionApi(mockApi(), {
			sessionKey: "agent:hook:only",
		});
		expect(
			resolveSessionKeyFromHookEvent({ session: { id: "from-event" } }, api),
		).toBe("from-event");
	});
});
...
describe("withHookResolutionApi / sliceHookAgentContext", () => {
	...
	it("hook sessionKey overrides api.context.sessionKey for resolution merge", () => {
		...
		const merged = withHookResolutionApi(base, {
			sessionKey: "agent:hook:wins",
		});
		expect(merged.context?.sessionKey).toBe("agent:hook:wins");
	});

	it("hook agentId overrides api.context.agentId on merged api", () => {
		const merged = withHookResolutionApi(mockApi("from-api"), {
			agentId: "from-hook",
		});
...
describe("resolveAgentIdFromHookEvent", () => {
	...
	it("falls back to api.context.agentId", () => {
		expect(resolveAgentIdFromHookEvent({}, mockApi("ctx-agent"))).toBe(
			"ctx-agent",
		);
	});
	...
	it("derives agent id from event.context.sessionId when only payload context carries the key", () => {
		expect(
			resolveAgentIdFromHookEvent(
				{ context: { sessionId: "agent:ralph:cron:task-1" } },
				mockApi(),
			),
		).toBe("ralph");
	});
	...
	it("derives agent id from api.context.sessionKey when that is all we have", () => {
		expect(
			resolveAgentIdFromHookEvent(
				{},
				mockApi(undefined, { sessionKey: "agent:ralph:cron:task-1" }),
			),
		).toBe("ralph");
	});
	...
	it("prefers explicit api.context.agentId over session key parsing", () => {
		expect(
			resolveAgentIdFromHookEvent(
				{},
				mockApi("explicit", { sessionKey: "agent:other:cron:x" }),
			),
		).toBe("explicit");
	});
	...
	it("returns null when absent and session key is not agent:-prefixed", () => {
		expect(
			resolveAgentIdFromHookEvent(
				{},
				mockApi(undefined, { sessionKey: "cron:only" }),
			),
		).toBeNull();
	});
	...
	it("derives agent id from hook-only sessionKey via merged api (#1005)", () => {
		const api = withHookResolutionApi(mockApi(), {
			sessionKey: "agent:ralph:cron:task-1",
		});
	...
	it("uses hook agentId on merged api when event has no structured id", () => {
		...
		expect(resolveAgentIdFromHookEvent({ prompt: "x" }, api)).toBe(
			"hook-agent",
		);
	});

	it("prefers event.agentId over hook merged context", () => {
		const api = withHookResolutionApi(mockApi("ctx"), {
			agentId: "hook-agent",
		});
		expect(resolveAgentIdFromHookEvent({ agentId: "event-agent" }, api)).toBe(
			"event-agent",
		);
	});
	...
	it("logs at debug when resolving from session key", () => {
		...
		expect(debug).toHaveBeenCalledWith(
			expect.stringContaining(
				'memory-hybrid: Resolved agentId "ralph" from session key pattern',
			),
		);
	});
});
...
describe("hook identity parity (#1005)", () => {
	it("prefers sessionId over sessionKey on merged api.context when both are set", () => {
		...
		expect(resolveSessionKeyFromHookEvent({ prompt: "x" }, api)).toBe(
	...
	it("resolves the same hook sessionKey to a session string and a matching agent id", () => {
		...
		const ev = { prompt: "hi" };
		expect(resolveSessionKeyFromHookEvent(ev, api)).toBe(
			"agent:forge:cron:task-9",
		);
