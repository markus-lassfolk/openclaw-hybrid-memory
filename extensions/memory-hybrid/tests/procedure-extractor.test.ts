...
describe("procedure-extractor", () => {
			...
			const lines = [
				JSON.stringify({
					...
					message: {
						role: "user",
						content: [
							{ type: "text", text: "  Check Moltbook notifications  " },
						],
					},
				}),
				JSON.stringify({
						...
						content: [
							{
								...
								id: "t1",
								name: "web_fetch",
								input: { url: "https://api.example.com/agents" },
							},
							{
								type: "tool_use",
								id: "t2",
	...
	describe("minimalRecipe", () => {
		...
		it("truncates long string args to 200 chars", () => {
			...
			expect((out[0].args as Record<string, string>)?.url?.endsWith("…")).toBe(
				true,
			);
		});
