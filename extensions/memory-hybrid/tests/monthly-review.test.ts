...
import {
	...
	runMonthlyReview,
} from "../services/monthly-review.js";
...
describe("MonthlyReviewService coverage analysis", () => {
	it("counts facts by category and entity", async () => {
		...
		expect(report.coverageAnalysis.topEntities[0]).toEqual({
			entity: "Alpha",
			count: 2,
		});
	});
	...
	it("ranks top and sparse entities correctly", async () => {
		...
		const openai = makeOpenAIMock(["", ""]);
		const report = await new MonthlyReviewService(
			db,
			openai as never,
			"openai/gpt-test",
		).runReview();
		...
		expect(report.coverageAnalysis.sparseEntities[0]).toEqual({
			entity: "B",
			count: 1,
		});
	});
...
describe("MonthlyReviewService quality trends", () => {
	it("computes average confidence and previous month average", async () => {
		...
		expect(report.qualityTrends.averageConfidence).toBeCloseTo(
			(0.2 + 0.8 + 0.6) / 3,
			5,
		);
		expect(report.qualityTrends.averageConfidencePreviousMonth).toBeCloseTo(
			(0.2 + 0.8) / 2,
			5,
		);
	});
