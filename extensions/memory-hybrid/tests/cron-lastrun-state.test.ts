...
describe("syncCronLastRunFromGuards", () => {
		...
		writeCronStore(openclawDir, [
			{
				...
				enabled: true,
				state: {
					nextRunAtMs: Date.now() + 1_000_000,
					lastStatus: "ok",
					consecutiveErrors: 0,
				},
			},
	...
	it("converts epoch-seconds guard file to epoch-ms when syncing", () => {
		...
		writeCronStore(openclawDir, [
			{ name: "nightly-dream-cycle", enabled: true },
		]);
		// Write seconds (legacy format)
		const guardDir = join(openclawDir, GUARD_SUBDIR);
		mkdirSync(guardDir, { recursive: true });
		writeFileSync(
			join(guardDir, "nightly-dream-cycle.ms"),
			String(secs),
			"utf-8",
		);
	...
	it("logs info when jobs are updated", () => {
		...
		writeCronStore(openclawDir, [
			{ name: "weekly-deep-maintenance", enabled: true },
		]);
		writeGuardFile(openclawDir, "weekly-deep-maintenance", ts);

		const messages: string[] = [];
		syncCronLastRunFromGuards(
			{ info: (m) => messages.push(m), warn: noop.warn },
			openclawDir,
		);

		expect(messages.some((m) => m.includes("synced") && m.includes("1"))).toBe(
			true,
		);
	});
...
describe("guard prefix format — persistent path", () => {
	it("buildGuardPrefix output does not mention /tmp/", () => {
		...
		for (const name of jobs) {
			...
			expect(prefix, `${name} guard prefix should not use /tmp/`).not.toContain(
				"/tmp/",
			);
			expect(prefix, `${name} guard prefix should use cron/guard/`).toContain(
				"cron/guard",
			);
		}
	});
