...
describe("extractTags", () => {
	...
	it("detects home assistant", () => {
		expect(extractTags("Home Assistant integration")).toContain(
			"homeassistant",
		);
	});
	...
	it("detects multiple tags from one text", () => {
		const tags = extractTags(
			"Nibe heat pump with Zigbee sensors via Home Assistant",
		);
...
describe("formatProgressiveIndexLine", () => {
	it("formats line as position, category, title, and token cost", () => {
		const line = formatProgressiveIndexLine(
			"preference",
			"IDE settings",
			48,
			1,
	...
	it("uses correct position and category", () => {
		const line = formatProgressiveIndexLine(
			"technical",
			"PostgreSQL connection config for prod",
			185,
			2,
		);
		expect(line).toBe(
			"  2. [technical] PostgreSQL connection config for prod (185 tok)",
		);
	});
...
describe("classifyDecay", () => {
	...
	it("returns permanent for decision-like text", () => {
		expect(classifyDecay(null, null, null, "decided to use Vitest")).toBe(
			"permanent",
		);
		expect(classifyDecay(null, null, null, "architecture choice")).toBe(
			"permanent",
		);
		expect(classifyDecay(null, null, null, "always use TypeScript")).toBe(
			"permanent",
		);
	});
	...
	it("returns session for session-like text", () => {
		expect(
			classifyDecay(null, null, null, "currently debugging the auth module"),
		).toBe("session");
		expect(classifyDecay(null, null, null, "right now working on tests")).toBe(
			"session",
		);
	});
	...
	it("returns active for task-like text", () => {
		expect(classifyDecay(null, null, null, "working on memory tests")).toBe(
			"active",
		);
		expect(classifyDecay(null, null, null, "todo: fix the prune logic")).toBe(
			"active",
		);
	});
	...
	it("defaults to stable when no patterns match", () => {
		expect(classifyDecay(null, null, null, "general information")).toBe(
			"stable",
		);
		expect(classifyDecay("user", "color", "blue", "user likes blue")).toBe(
			"stable",
		);
	});
});
...
describe("detectCredentialPatterns", () => {
	it("detects OpenAI-style API keys", () => {
		const results = detectCredentialPatterns(
			"My key is sk-1234567890abcdefghijklmn",
		);
...
describe("inferServiceFromText", () => {
	...
	it("infers home-assistant", () => {
		expect(inferServiceFromText("HA token for Home Assistant")).toBe(
			"home-assistant",
		);
	});
...
describe("isStructuredForConsolidation", () => {
	it("returns true for IP address", () => {
		expect(
			isStructuredForConsolidation("Server at 192.168.1.1", null, null),
		).toBe(true);
	});

	it("returns true for email", () => {
		expect(isStructuredForConsolidation("john@example.com", null, null)).toBe(
			true,
		);
	});

	it("returns true for phone-like number", () => {
		expect(isStructuredForConsolidation("+1234567890123", null, null)).toBe(
			true,
		);
	});
