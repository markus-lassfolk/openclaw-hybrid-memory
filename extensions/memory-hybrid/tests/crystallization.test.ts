...
beforeEach(() => {
	...
	cStore = new CrystallizationStore(
		join(tmpDir, "crystallization-proposals.db"),
	);
});
...
describe("PatternDetector.detect", () => {
	...
	it("detects candidates meeting thresholds", () => {
		...
		expect(candidates[0].pattern.toolSequence).toEqual([
			"exec",
			"exec",
			"read",
		]);
...
describe("SkillValidator", () => {
	...
	it("denies eval() in code block", () => {
		...
		expect(result.violations.some((v: string) => v.includes("eval"))).toBe(
			true,
		);
	});
	...
	it("denies curl in code block", () => {
		...
		const result = validator.validate(content);
		expect(result.valid).toBe(false);
		expect(result.violations.some((v: string) => v.includes("curl"))).toBe(
			true,
		);
	});
	...
	it("denies credential env var in code block", () => {
		...
		expect(result.valid).toBe(false);
		expect(
			result.violations.some((v: string) => v.includes("credential")),
		).toBe(true);
	});
	...
	it("denies command substitution in code block", () => {
		...
		const result = validator.validate(content);
		expect(result.valid).toBe(false);
		expect(result.violations.some((v: string) => v.includes("subst"))).toBe(
			true,
		);
	});
...
describe("CrystallizationProposer.runCycle", () => {
	...
	it("stops at maxCrystallized approved limit", () => {
		...
		expect(result.proposed).toBe(0);
		expect(
			result.reasons.some((r: string) => r.includes("maxCrystallized")),
		).toBe(true);
	});
...
describe("parseCrystallizationConfig", () => {
	...
	it("defaults to disabled with sensible values when omitted", async () => {
		...
		expect(cfg.crystallization.autoApprove).toBe(false);
		expect(cfg.crystallization.outputDir).toBe(
