...
describe("extractEntitiesFromMessage", () => {
	...
	it("extracts PascalCase word", () => {
		const result = extractEntitiesFromMessage(
			"Deploy MyService to production",
			[],
		);
	...
	it("extracts known entity from knownEntities list", () => {
		const result = extractEntitiesFromMessage("Deploy to alice's server", [
			"alice",
		]);
		expect(result).toContain("alice");
	});
	...
	it("deduplicate: same entity from multiple sources appears once", () => {
		...
		const result = extractEntitiesFromMessage("@alice is doing well", [
			"alice",
		]);
	...
	it("known entity match is case-insensitive", () => {
		const result = extractEntitiesFromMessage("Talk about Alice today", [
			"alice",
		]);
		expect(result).toContain("alice");
	});
...
describe("generateAmbientQueries — multiQuery enabled", () => {
	...
	it("caps at maxQueriesPerTrigger = 2", () => {
		...
		const result = generateAmbientQueries(
			"Big message with @alice and #tag and 1.2.3.4",
			cfg,
		);
	...
	it("generates entity queries when entities detected", () => {
		const result = generateAmbientQueries(
			"Update the MyService deployment",
			DEFAULT_CFG,
		);
	...
	it("generates temporal queries when no entities fill the slots", () => {
		...
		const result = generateAmbientQueries("hello world", DEFAULT_CFG, {
			nowMs: Date.now(),
		});
	...
	it("generates context query when userId provided", () => {
		const result = generateAmbientQueries("hello", DEFAULT_CFG, {
			userId: "alice",
		});
	...
	it("uses knownEntities to produce entity queries from plain words", () => {
		...
		const entityQ = result.find(
			(q) => q.type === "entity" && q.entity === "production",
		);
