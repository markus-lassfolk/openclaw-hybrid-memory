...
if grep -rn "console\.log\|console\.warn\|console\.error" backends/ cli/ config/ lifecycle/ prompts/ routes/ services/ setup/ tools/ types/ utils/ --include="*.ts" 2>/dev/null | grep -v ".test.ts" | grep -v "logger" | head -5 | grep -q .; then
  grep -rn "console\.log\|console\.warn\|console\.error" backends/ cli/ config/ lifecycle/ prompts/ routes/ services/ setup/ tools/ types/ utils/ --include="*.ts" 2>/dev/null | grep -v ".test.ts" | grep -v "logger" | head -5
...
found=$(find backends cli config lifecycle prompts routes services setup tests tools types utils -name "*.spec.ts" -type f 2>/dev/null | head -1)
if [ -n "$found" ]; then
