...
if grep -rn "from ['\"]fs['\"]\\|require(['\"]fs['\"])" src/services/ --include="*.ts" 2>/dev/null | grep -v ".test.ts" | head -5 | grep -q .; then
  grep -rn "from ['\"]fs['\"]\\|require(['\"]fs['\"])" src/services/ --include="*.ts" 2>/dev/null | grep -v ".test.ts" | head -5
...
if grep -rn "console\.log\|console\.warn\|console\.error" src/ --include="*.ts" 2>/dev/null | grep -v ".test.ts" | grep -v "logger" | head -5 | grep -q .; then
  grep -rn "console\.log\|console\.warn\|console\.error" src/ --include="*.ts" 2>/dev/null | grep -v ".test.ts" | grep -v "logger" | head -5
...
found=$(find src -name "*.spec.ts" -type f 2>/dev/null | head -1)
if [ -n "$found" ]; then
