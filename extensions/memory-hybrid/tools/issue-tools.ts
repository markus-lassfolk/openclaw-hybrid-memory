	...
	api.registerTool(
		{
			...
			async execute(_toolCallId: string, params: Record<string, unknown>) {
				...
				const issue = withErrorTracking(
					() => issueStore.create({ title, symptoms, severity, tags }),
					{
						subsystem: "issues",
	...
	api.registerTool(
		{
			...
			parameters: Type.Object({
				...
				symptoms: Type.Optional(
					Type.Array(Type.String(), {
						description: "Updated list of symptoms",
					}),
				),
			}),
			async execute(_toolCallId: string, params: Record<string, unknown>) {
				...
				const issue = withErrorTracking(
					() => {
						...
						if (status) {
							return issueStore.transition(id, status, {
								rootCause,
								fix,
								rollback,
								symptoms,
	...
	api.registerTool(
		{
			...
			label: "List Issues",
			description:
				"List tracked issues with optional filters by status, severity, and tags.",
			parameters: Type.Object({
				status: Type.Optional(
					Type.Array(
						stringEnum(ISSUE_STATUSES as unknown as readonly string[]),
			...
			async execute(_toolCallId: string, params: Record<string, unknown>) {
				...
				const issues = withErrorTracking(
					() => issueStore.list({ status, severity, tags, limit }),
					{
						subsystem: "issues",
	...
	api.registerTool(
		{
			...
			label: "Search Issues",
			description:
				"Search issues by title and symptoms using LIKE-based text matching.",
			parameters: Type.Object({
				query: Type.String({
					description: "Search query to match against issue title and symptoms",
				}),
			}),
			async execute(_toolCallId: string, params: Record<string, unknown>) {
	...
	api.registerTool(
		{
			...
			async execute(_toolCallId: string, params: Record<string, unknown>) {
				const { issueId, factId } = params as {
					issueId: string;
					factId: string;
				};
