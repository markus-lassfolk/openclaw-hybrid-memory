      ...
      async execute(_toolCallId: string, params: Record<string, unknown>) {
        ...
        try {
          const result = await runReflection(
            ...
            { window, dryRun: false, model: reflectionCfg.model, fallbackModels: cfg.distill?.fallbackModels },
            api.logger,
          );
          return {
            content: [
      ...
      async execute() {
        ...
        try {
          const result = await runReflectionRules(
            ...
            embeddings,
            openai,
            { dryRun: false, model: reflectionCfg.model, fallbackModels: cfg.distill?.fallbackModels },
            api.logger,
          );
          return {
            content: [
