	...
	api.registerTool(
		{
			...
			async execute(_toolCallId: string, params: Record<string, unknown>) {
				...
				if (isCompactVerbosity(verbosity)) {
					...
					const linksNote =
						linksPruned > 0 ? ` ${linksPruned} orphaned links.` : "";
			...
			async execute(_toolCallId: string, params: Record<string, unknown>) {
				...
				try {
					const result = await runReflection(
						...
						openai,
						{
							defaultWindow: reflectionCfg.defaultWindow,
							minObservations: reflectionCfg.minObservations,
						},
				...
				if (!reflectionCfg.enabled) {
					return {
						content: [
							{
								type: "text",
								text: "Reflection is disabled. Enable reflection.enabled in plugin config.",
							},
						],
						details: { error: "reflection_disabled" },
					};
				...
				try {
					...
					return {
						...
						details: {
							...
							rulesStored: result.rulesStored,
						},
					};
				} catch (err) {
					capturePluginError(
			...
			async execute() {
				...
				try {
					...
					return {
						...
						details: {
							metaExtracted: result.metaExtracted,
							metaStored: result.metaStored,
						},
					};
				} catch (err) {
					capturePluginError(
	...
	api.registerTool(
		{
			...
			parameters: Type.Object({
				minClusterSize: Type.Optional(
					Type.Number({
						description:
							"Minimum facts to form a cluster (default from config, typically 3)",
						minimum: 2,
					}),
				),
				save: Type.Optional(
					Type.Boolean({
						description:
							"Persist detected clusters to the database (default: true)",
					}),
				),
			}),
			async execute(_toolCallId: string, params: Record<string, unknown>) {
				...
				const minClusterSize =
					typeof params.minClusterSize === "number" &&
				...
				try {
					...
					const existingClusterIds = new Map<
						string,
					...
					for (const cluster of existingClusters) {
						...
						existingClusterIds.set(componentKey, {
							id: cluster.id,
							createdAt: cluster.createdAt,
						});
					}

					const result = detectClusters(factsDb, {
						minClusterSize,
						existingClusterIds,
					});
