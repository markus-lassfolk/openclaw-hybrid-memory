		...
		api.registerTool(
			{
				...
				parameters: Type.Object({
					...
					linkType: stringEnum(
						MEMORY_LINK_TYPES as unknown as readonly string[],
					),
				...
				async execute(_toolCallId: string, params: Record<string, unknown>) {
					...
					if (!src) {
						return {
							content: [
								{ type: "text", text: `Source fact not found: ${sourceFact}` },
							],
					...
					if (!tgt) {
						return {
							content: [
								{ type: "text", text: `Target fact not found: ${targetFact}` },
							],
							details: { error: "target_not_found", id: targetFact },
						};
					}
					if (linkType === "CONTRADICTS") {
						const contradictionId = factsDb.recordContradiction(
							sourceFact,
							targetFact,
						);
					...
					const linkId = factsDb.createLink(
						sourceFact,
						targetFact,
						linkType,
						strength,
		...
		api.registerTool(
			{
				...
				label: "Memory Graph",
				description:
					"Explore connections from a memory: show direct links and optionally traverse up to depth 3.",
				parameters: Type.Object({
					factId: Type.String({ description: "ID of the fact to explore" }),
					depth: Type.Optional(
						Type.Number({
							description: "Max hops to traverse (default 2, max 3)",
						}),
					),
				}),
				async execute(_toolCallId: string, params: Record<string, unknown>) {
					const { factId, depth = 2 } = params as {
						factId: string;
						depth?: number;
					};
					...
					lines.push("");
					lines.push(
						`Total connected facts (depth ${maxD}): ${connectedIds.length}`,
					);
					return {
		...
		api.registerTool(
			{
				...
				parameters: Type.Object({
					...
					maxDepth: Type.Optional(
						Type.Number({
							description: `Max hops to traverse (default 5, max ${cfg.path.maxPathDepth})`,
						}),
					),
				}),
				async execute(_toolCallId: string, params: Record<string, unknown>) {
					...
					if (!fromId) {
						return {
							content: [
								{
									type: "text",
									text: `Could not resolve start: "${from}" (not a known fact ID or entity name)`,
								},
							],
					...
					if (!toId) {
						return {
							content: [
								{
									type: "text",
									text: `Could not resolve end: "${to}" (not a known fact ID or entity name)`,
								},
							],
					...
					const result = findShortestPath(factsDb, fromId, toId, {
						maxDepth: depthCap,
					});
					...
					if (!result) {
						return {
							content: [
								{
									type: "text",
									text: `No path found between "${from}" and "${to}" within ${depthCap} hops.`,
								},
							],
					...
					for (let i = 0; i < result.chain.length; i++) {
						...
						lines.push(
							`  [${entry.id.slice(0, 8)}…] ${entry.text.slice(0, 80)}${entry.text.length > 80 ? "…" : ""}`,
						);
					}
