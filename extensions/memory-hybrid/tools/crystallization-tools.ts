...
export function registerCrystallizationTools(
	ctx: CrystallizationToolsContext,
		...
		async execute(_toolCallId: string, _params: Record<string, unknown>) {
			try {
				...
				if (result.proposed > 0) {
					...
					lines.push(
						"Use memory_crystallize_approve <id> to approve and write a skill.",
					);
				}
		...
		parameters: Type.Object({
			status: Type.Optional(
				Type.Union(
					...
					{
						description:
							"Filter by proposal status. Omit to list all proposals.",
					},
				),
			),
			limit: Type.Optional(
		...
		async execute(_toolCallId: string, params: Record<string, unknown>) {
			...
			try {
						...
						content: [
							{
								type: "text",
								text: status
									? `No ${status} crystallization proposals found.`
									: "No crystallization proposals found.",
							},
						],
						details: [],
					};
				...
				const lines = proposals.map((p, i) => {
					...
					const rejectInfo = p.rejectionReason
	...
	api.registerTool({
		...
		parameters: Type.Object({
			id: Type.String({
				description:
					"The proposal ID to approve (from memory_crystallize_list).",
			}),
		}),
		async execute(_toolCallId: string, params: Record<string, unknown>) {
	...
	api.registerTool({
		...
		parameters: Type.Object({
			id: Type.String({
				description:
					"The proposal ID to reject (from memory_crystallize_list).",
			}),
			reason: Type.Optional(
				Type.String({
					description:
						"Optional reason for rejection (stored for audit trail).",
				}),
			),
		}),
		async execute(_toolCallId: string, params: Record<string, unknown>) {
