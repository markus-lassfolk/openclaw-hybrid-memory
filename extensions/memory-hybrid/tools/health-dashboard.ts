...
export async function buildHealthReport(
	...
	const totalRow = db.prepare("SELECT COUNT(*) AS cnt FROM facts").get() as {
		cnt: number;
	};
	...
	const decayRows = db
		.prepare(
			"SELECT decay_class, COUNT(*) AS cnt FROM facts GROUP BY decay_class ORDER BY cnt DESC",
		)
	...
	const avgConfidence =
		confRow.avg_conf != null ? Math.round(confRow.avg_conf * 1000) / 1000 : 0;
	...
	const avgLinksPerFact =
		activeFacts > 0
			? Math.round((totalLinks * 2 * 1000) / activeFacts) / 1000
			: 0;
	...
	const [sqliteSize, sqliteWalSize, sqliteShmSize, lanceSize] =
		await Promise.all([
