...
function isSupportedExtension(
	...
	const custom =
		customExts && customExts.length > 0 ? new Set(customExts) : null;
	const extOk = custom
		? custom.has(lowerExt)
		: SUPPORTED_EXTENSIONS.has(lowerExt);
...
async function describeImageWithVision(opts: {
	...
	if (modelsToTry.length === 0) {
		throw new Error(
			"No vision model configured (documents.visionModel or llm.default must be set).",
		);
	}
...
export function registerDocumentTools(
	...
	async function ingestSingleDocument(opts: {
		...
		opts.onProgress?.({
			stage: "start",
			pct: 0,
			message: `Starting ingestion of ${basename(filePath)}`,
		});
			...
			return {
				...
				response: makeErrorResponse(
					`Error: Path is not under any allowed directory. Allowed: ${docCfg.allowedPaths?.join(", ") ?? ""}`,
					{
						error: "path_not_allowed",
						path: realPath,
						allowedPaths: docCfg.allowedPaths,
					},
				),
			};
		}
		...
		try {
			...
			return {
				...
				progress: progress.steps,
				response: makeErrorResponse(
					`Error: File not found or inaccessible: ${realPath}`,
					{
						error: "file_not_found",
						path: realPath,
					},
		...
		if (chunks.length === 0) {
			const response = makeErrorResponse(
				"Document produced no storable chunks after chunking.",
				{
					error: "no_chunks",
					path: realPath,
				},
		...
		opts.onProgress?.({
			stage: "chunking",
			pct: 40,
			message: `Chunking (${chunks.length} chunks)...`,
		});
		...
		for (let i = 0; i < chunks.length; i++) {
			...
			const headingTag = chunk.sectionHeading
				? headingTagSafe(chunk.sectionHeading)
				: null;
			const chunkTags = [
				...baseTags,
			...
			if ((i + 1) % 25 === 0 || i === chunks.length - 1) {
				api.logger.info(
					`memory-hybrid: ${fileName} storing chunk ${i + 1}/${chunks.length}`,
				);
			}
		}
	...
	api.registerTool(
		{
			...
			parameters: Type.Object({
				...
				dryRun: Type.Optional(
					Type.Boolean({
						description:
							"When true, convert and chunk but do NOT store — returns preview only",
					}),
				),
			}),
			async execute(_toolCallId: string, params: Record<string, unknown>) {
				...
				const categoryParam =
					typeof params.category === "string" ? params.category : "fact";
	...
	api.registerTool(
		{
			...
			parameters: Type.Object({
				path: Type.String({
					description:
						"Absolute path to the folder containing documents to ingest",
				}),
				...
				dryRun: Type.Optional(
					Type.Boolean({
						description:
							"When true, only list matching files without ingesting",
					}),
				),
			}),
			async execute(_toolCallId: string, params: Record<string, unknown>) {
				...
				if (!isAbsolute(folderPath)) {
					return makeErrorResponse(
						`Error: Path must be absolute. Got: ${folderPath}`,
						{
							error: "path_not_absolute",
							path: folderPath,
						},
				...
				try {
					...
					return makeErrorResponse(
						...
						{
							error: isNotFound ? "folder_not_found" : "path_inaccessible",
							path: folderPath,
						},
					);
				}
				...
				try {
					...
					return makeErrorResponse(
						`Error: Folder not found or inaccessible: ${realFolder}`,
						{
				...
				if (!folderStat.isDirectory()) {
					return makeErrorResponse(
						`Error: Path is not a directory: ${realFolder}`,
						{
				...
				const customExts = Array.isArray(filter.extensions)
					? normalizeExtensions(filter.extensions)
					: undefined;
				const glob =
					typeof filter.glob === "string" ? filter.glob.trim() : undefined;
				const files = collectFilesRecursive(realFolder, (fp) =>
					isSupportedExtension(fp, customExts, glob),
				);
				...
				for (let i = 0; i < files.length; i++) {
					...
					totalErrors +=
						summary.errorCount ?? (summary.status === "error" ? 1 : 0);
				}

				const progressLines = summaries.map(
					(s) => `${s.fileName}: ${s.progress.join(" ")}`,
				);
