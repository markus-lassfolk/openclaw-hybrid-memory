...
function preprocessHAYaml(content: string): string {
	...
	return content
		.replace(
			/(^|\n)(\s*)([\w.-]+):[ \t]+!include_dir_merge_named\s+(\S+)/g,
			`$1$2$3: "${HA_INCLUDE_DIR_PREFIX}$4"`,
		)
		.replace(
			/(^|\n)(\s*)-\s+!include_dir_merge_named\s+(\S+)/g,
			`$1$2- "${HA_INCLUDE_DIR_PREFIX}$3"`,
		)
		.replace(
			/(^|\n)(\s*)([\w.-]+):[ \t]+!include_dir_list\s+(\S+)/g,
			`$1$2$3: "${HA_INCLUDE_DIR_LIST_PREFIX}$4"`,
		)
		.replace(
			/(^|\n)(\s*)-\s+!include_dir_list\s+(\S+)/g,
			`$1$2- "${HA_INCLUDE_DIR_LIST_PREFIX}$3"`,
		)
		.replace(
			/(^|\n)(\s*)([\w.-]+):[ \t]+!include_dir_named\s+(\S+)/g,
			`$1$2$3: "${HA_INCLUDE_DIR_NAMED_PREFIX}$4"`,
		)
		.replace(
			/(^|\n)(\s*)-\s+!include_dir_named\s+(\S+)/g,
			`$1$2- "${HA_INCLUDE_DIR_NAMED_PREFIX}$3"`,
		)
		.replace(
			/(^|\n)(\s*)([\w.-]+):[ \t]+!include\s+(\S+)/g,
			`$1$2$3: "${HA_INCLUDE_PREFIX}$4"`,
		)
		.replace(
			/(^|\n)(\s*)-\s+!include\s+(\S+)/g,
			`$1$2- "${HA_INCLUDE_PREFIX}$3"`,
		)
		.replace(
			/(^|\n)(\s*)([\w.-]+):[ \t]+!secret\s+(\S+)/g,
			`$1$2$3: "${HA_SECRET_PREFIX}$4"`,
		)
		.replace(
			/(^|\n)(\s*)-\s+!secret\s+(\S+)/g,
			`$1$2- "${HA_SECRET_PREFIX}$3"`,
		);
}
...
function renderRef(val: unknown): string {
	if (isIncludeRef(val))
		return `*(includes: ${val.substring(HA_INCLUDE_PREFIX.length)})*`;
	if (isSecretRef(val))
		return `*(secret: ${val.substring(HA_SECRET_PREFIX.length)})*`;
...
function getStr(
	...
	if (isIncludeRef(v) || isSecretRef(v) || isIncludeDirRef(v))
		return renderRef(v);
	return String(v);
}
...
function renderAutomation(auto: unknown, idx: number): string {
	if (
		typeof auto === "string" &&
		(isIncludeRef(auto) || isSecretRef(auto) || isIncludeDirRef(auto))
	) {
		return `- ${renderRef(auto)}`;
	}
	if (typeof auto !== "object" || auto === null)
		return `- Automation ${idx + 1}: *(invalid)*`;
...
function summariseTrigger(t: unknown): string {
	...
	if (platform === "numeric_state")
		return `numeric_state(${getStr(obj, "entity_id")})`;
	return platform;
}
...
function renderScene(scene: unknown, idx: number): string {
	if (
		typeof scene === "string" &&
		(isIncludeRef(scene) || isSecretRef(scene) || isIncludeDirRef(scene))
	) {
		return `- ${renderRef(scene)}`;
	}
	if (typeof scene !== "object" || scene === null)
		return `- Scene ${idx + 1}: *(invalid)*`;
	const s = scene as Record<string, unknown>;
	const name = getStr(s, "name", `scene_${idx + 1}`);
	const entities = s.entities;
	const count =
		typeof entities === "object" && entities !== null
...
function renderEntity(entity: unknown, idx: number): string {
	if (
		...
		(isIncludeRef(entity) || isSecretRef(entity) || isIncludeDirRef(entity))
	) {
		return `- ${renderRef(entity)}`;
	}
	if (typeof entity !== "object" || entity === null)
		return `- Entity ${idx + 1}: *(invalid)*`;
	...
	convert(content: string, filePath: string): ConversionResult {
		...
		if (scripts && typeof scripts === "object" && !Array.isArray(scripts)) {
			...
			if (scriptEntries.length > 0) {
				const lines = scriptEntries.map(([name, script]) =>
		...
		return {
			...
			metadata: {
				...
				scriptCount:
					typeof doc.script === "object" &&
					doc.script !== null &&
					!Array.isArray(doc.script)
