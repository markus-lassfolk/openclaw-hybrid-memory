...
function summariseCSV(headers: string[], rows: string[][]): string {
	...
	const firstDate =
		dateIdx !== -1 ? (rows[0]?.[dateIdx] ?? "unknown") : "unknown";
	const lastDate =
		dateIdx !== -1
			? (rows[rows.length - 1]?.[dateIdx] ?? "unknown")
			: "unknown";
	...
	if (pvIdx !== -1) {
		const vals = rows
			.map((r) => parseNum(r[pvIdx]))
			.filter((v): v is number => v !== null);
		if (vals.length > 0) {
			const peak = Math.max(...vals);
			const total = vals.reduce((a, b) => a + b, 0);
			lines.push(
				`- PV Power: peak ${peak.toFixed(0)} W, sum ${total.toFixed(0)} W·intervals`,
			);
		}
	}
	...
	if (voltIdx !== -1) {
		...
		if (vals.length > 0) {
			...
			lines.push(
				`- Battery Voltage: avg ${avg.toFixed(2)} V, min ${min.toFixed(2)} V, max ${max.toFixed(2)} V`,
			);
		}
	}
...
function summariseJSONRecords(records: VictronRecord[]): string {
	...
	for (const [field, vals] of Object.entries(numericFields)) {
		...
		const max = Math.max(...vals);
		lines.push(
			`- ${field}: avg ${avg.toFixed(2)}, min ${min.toFixed(2)}, max ${max.toFixed(2)}`,
		);
	}
	...
	convert(content: string, filePath: string): ConversionResult {
		...
		if (ext === ".csv") {
			...
			if (!isVictronCSV(headers)) {
				sections.push(
					"*Note: CSV may not be a Victron VRM export (unrecognised headers)*\n",
				);
			}
