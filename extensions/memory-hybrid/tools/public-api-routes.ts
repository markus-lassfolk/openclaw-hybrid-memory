...
function extractFactId(url: URL): string | null {
	...
	if (url.pathname.startsWith(routePrefix)) {
		try {
			const maybe = decodeURIComponent(
...
export function registerPublicApiRoutes(
	...
	makeRoute(`${PUBLIC_API_PREFIX}${PUBLIC_API_PATHS.health}`, async () =>
		toJson(200, {
			...
			endpoints: Object.values(PUBLIC_API_PATHS).map(
				(path) => `${PUBLIC_API_PREFIX}${path}`,
			),
		}),
	);
	...
	makeRoute(`${PUBLIC_API_PREFIX}${PUBLIC_API_PATHS.export}`, async (req) => {
		...
		const narrativeLimit = parseLimitParam(
			url.searchParams.get("narrativeLimit"),
			20,
			500,
		);
	...
	makeRoute(`${PUBLIC_API_PREFIX}${PUBLIC_API_PATHS.fact}`, async (req) => {
		...
		if (!factId) {
			return toJson(400, {
				error:
					"Missing fact id. Use /fact?id=<uuid> (or /fact/<uuid> when supported by your gateway).",
			});
		}
