	...
	api.registerTool(
		{
			...
			parameters: Type.Object({
				...
				title: Type.String({
					description:
						"Short title for the proposal (e.g., 'Add tone-matching guidance')",
				}),
				...
				suggestedChange: Type.String({
					description:
						"The specific change to make to the file (be precise about location and wording)",
				}),
				confidence: Type.Number({
					description:
						"Confidence score 0-1 (must be >= minConfidence from config)",
					minimum: 0,
					maximum: 1,
				}),
				evidenceSessions: Type.Array(Type.String(), {
					description:
						"List of session IDs or references that support this proposal",
				}),
			}),
			async execute(_toolCallId: string, params: Record<string, unknown>) {
				...
				if (observation.length > MAX_OBSERVATION_LENGTH) {
					return {
						...
						details: {
							error: "observation_too_long",
							length: observation.length,
							max: MAX_OBSERVATION_LENGTH,
						},
					};
				}
				...
				const invalidSessions = evidenceSessions.filter(
				...
				if (invalidSessions.length > 0) {
					return {
						...
						details: {
							error: "invalid_evidence_sessions",
				...
				const expiresAt =
					cfg.personaProposals.proposalTTLDays > 0
						? Math.floor(Date.now() / 1000) +
							cfg.personaProposals.proposalTTLDays * 24 * 3600
						: null;
				...
				api.logger.info(
					`memory-hybrid: persona proposal created — ${proposal.id} (${title})`,
				);
	...
	api.registerTool(
		{
			...
			async execute(_toolCallId: string, params: Record<string, unknown>) {
				const { status, targetFile } = params as {
					status?: string;
					targetFile?: string;
				};
				const verbosity = cfg.verbosity ?? "normal";
				...
				const lines = proposals.map((p) => {
					const age = Math.floor(
						(Date.now() / 1000 - p.createdAt) / SECONDS_PER_DAY,
					);
					const expires = p.expiresAt
						? Math.floor((p.expiresAt - Date.now() / 1000) / SECONDS_PER_DAY)
						: null;
					if (isCompactVerbosity(verbosity)) {
