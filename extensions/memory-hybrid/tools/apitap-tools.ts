...
import {
	...
	isEndpointBlocked,
	validateUrl,
} from "../services/apitap-service.js";
...
function persistAndFormatEndpoints(
	...
	const expiresAt =
		cfg.apiTap.endpointTtlDays > 0
			? new Date(
					Date.now() + cfg.apiTap.endpointTtlDays * 24 * 60 * 60_000,
				).toISOString()
			: null;
	...
	const allowedEndpoints = result.endpoints.filter((ep) => {
		let fullUrl: string;
		if (
			ep.endpoint.startsWith("http://") ||
			ep.endpoint.startsWith("https://")
		) {
			fullUrl = ep.endpoint;
		} else {
			try {
	...
	if (blockedCount > 0) {
		lines.push(
			`  Blocked (filtered): ${blockedCount} endpoint(s) matched blocked patterns and were not stored.`,
		);
	}
...
export function registerApitapTools(
	ctx: ApitapToolsContext,
	...
	api.registerTool({
		...
		parameters: Type.Object({
			url: Type.String({
				description:
					"Target site URL to capture (e.g. https://api.example.com).",
			}),
			timeoutSeconds: Type.Optional(
				Type.Integer({
					...
					maximum: 300,
					description:
						"Capture duration in seconds (default: from config, typically 60).",
				}),
			),
		}),
		async execute(_toolCallId: string, params: Record<string, unknown>) {
			const { url, timeoutSeconds } = params as {
				url: string;
			...
			try {
				...
				if (result.error) {
					return {
						content: [
							{ type: "text", text: `Capture failed: ${result.error}` },
						],
					};
				}
		...
		parameters: Type.Object({
			...
			status: Type.Optional(
				Type.Union(
					...
					{
						description:
							"Filter by review status. Omit to list all non-expired endpoints.",
					},
				),
			),
			limit: Type.Optional(
		...
		async execute(_toolCallId: string, params: Record<string, unknown>) {
			...
			try {
				...
				const lines = endpoints.map((ep, i) => {
					const expiry = ep.expiresAt
						? ` | expires ${ep.expiresAt.slice(0, 10)}`
						: "";
					return (
	...
	api.registerTool({
		...
		async execute(_toolCallId: string, params: Record<string, unknown>) {
			...
			try {
				...
				if (!endpoint) {
					return {
						content: [
							{
								type: "text",
								text: `Endpoint ${id} not found. Use apitap_list to see available endpoints.`,
							},
						],
					};
				}
				...
				const lines: string[] = [];
				lines.push(
					`Skill scaffold for [${scaffold.method}] ${scaffold.endpoint}:`,
				);
				lines.push("");
				...
				lines.push("");
				lines.push(
					"Review this spec before enabling. Use apitap_list to see all discovered endpoints.",
				);
