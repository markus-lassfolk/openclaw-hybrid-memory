...
import type {
	BuildToolScopeFilterFn,
	FindSimilarByEmbeddingFn,
} from "../api/memory-plugin-api.js";
...
import {
	formatNarrativeRange,
	recallNarrativeSummaries,
} from "../services/narrative-recall.js";
...
function hasBoundMemoryToolHelpers(
...
async function storeRegistryEmbeddings({
	...
	if (embeddingRegistry.isMultiModel()) {
		...
		if (!vector) {
			try {
				const vec = await embedCallWithTimeoutAndRetry(
					() => embeddingRegistry.embed(text),
					`${operation}:primary`,
				);
				const modelName =
					embeddings.modelName || embeddingRegistry.getPrimaryModel().name;
	...
	for (const [model, vec] of vectors) {
		try {
			...
			logger.warn(
				`memory-hybrid: fact_embeddings store failed (${model}): ${err}`,
			);
			capturePluginError(err instanceof Error ? err : new Error(String(err)), {
...
export function registerMemoryTools(
	...
	const agentIdForAudit = () =>
		currentAgentIdRef.value || cfg.multiAgent.orchestratorId || "unknown";

	function auditAppend(
		input: import("../backends/audit-store.js").AuditEventInput,
	): void {
	...
	api.registerTool(
		{
			...
			parameters: Type.Object({
				query: Type.Optional(
					Type.String({
						description:
							"Search query (omit when using id to fetch a specific memory)",
					}),
				),
				id: Type.Optional(
				...
				includeSuperseded: Type.Optional(
					Type.Boolean({
						description:
							"Include superseded (historical) facts in results. Default: only current facts.",
					}),
				),
				asOf: Type.Optional(
					Type.String({
				...
				includeCold: Type.Optional(
					Type.Boolean({
						description:
							"Set true to include COLD tier (slower / deeper retrieval). Default: false (HOT + WARM only).",
					}),
				),
	...
	api.registerTool(
		{
			...
			description:
				"Recall chronological summaries of recent sessions, decisions, and attempts.",
			parameters: Type.Object({
				query: Type.Optional(
					Type.String({
						description:
							"Optional topic or project query used to rank narrative summaries.",
					}),
				),
				sessionId: Type.Optional(
					Type.String({
				...
				days: Type.Optional(
					Type.Number({
						description:
							"Look back window in days when sessionId is omitted (default: 7).",
						minimum: 1,
						maximum: 365,
					}),
				),
	...
	async function memoryRecallImpl(params: Record<string, unknown>) {
		...
		const asOfSec =
			asOfParam != null && asOfParam !== ""
				? parseSourceDate(asOfParam)
				: undefined;
		...
		if (idParam !== undefined && idParam !== null && idParam !== "") {
			...
			if (factId) {
				...
				const entry = factsDb.getById(
					factId,
					asOfSec != null || scopeFilter
						? (getByIdOpts as { asOf?: number; scopeFilter?: ScopeFilter })
						: undefined,
				);
				if (entry) {
							...
							memories: [
								{
									...
									sourceDate: entry.sourceDate
										? new Date(entry.sourceDate * 1000)
												.toISOString()
												.slice(0, 10)
										: undefined,
								},
			...
			auditAppend({
				...
				action: "memory_recall",
				target:
					typeof idParam === "number" ? `index ${idParam}` : `id ${idParam}`,
				outcome: "partial",
				durationMs: Date.now() - recallStartedAt,
				sessionId: api.context?.sessionId ?? undefined,
			});
		...
		if (entity) {
			entityResults = factsDb.lookup(entity, undefined, tag, {
				...recallOpts,
				limit: 100,
			});
		}
		...
		try {
			...
			const queryExpander =
				...
				cfg.retrieval.strategies.includes("semantic")
				...
				queryVector != null
					? (text: string) =>
							embedCallWithTimeoutAndRetry(
								...
								"memory-tools:rrf-deep-retrieval",
							)
					: null;
			const rrfOutput = await runExplicitDeepRetrieval(
				query,
				queryVector,
				factsDb.getRawDb(),
				vectorDb,
			...
			const packedFactIdSet =
				rrfOutput.packed.length > 0 ? new Set(rrfOutput.packedFactIds) : null;
			for (const fusedResult of rrfOutput.fused) {
				if (packedFactIdSet && !packedFactIdSet.has(fusedResult.factId))
					continue;
			...
			api.logger.warn(
				`memory-hybrid: RRF pipeline failed, falling back to legacy merge: ${err}`,
			);
		...
		if (useExpandGraph) {
			const rawDepth =
				typeof expandDepthParam === "number"
					? expandDepthParam
					: cfg.retrieval.graphWalkDepth;
			const depth = Math.min(
				Math.max(0, rawDepth),
				cfg.graphRetrieval.maxExpandDepth,
			);
			const seedInputs = results.map((r) => ({
				factId: r.entry.id,
				score: r.score,
				entry: r.entry,
			}));
			...
			for (const e of expanded) {
				const backend =
					e.expansionSource === "direct"
						? (originalBackendMap.get(e.factId) ?? "sqlite")
						: "sqlite";
			...
			const connectedIds = factsDb.getConnectedFactIds(
				[...initialIds],
		...
		api.registerTool(
			{
				...
				parameters: Type.Object({
					taskDescription: Type.String({
						description:
							"What you are trying to do (e.g. 'check Moltbook', 'HA health checks')",
					}),
					limit: Type.Optional(
						Type.Number({
							description: "Max procedures to return (default: 5)",
						}),
					),
					agentId: Type.Optional(
						Type.String({
							description:
								"⚠️ SECURITY: Caller-controlled parameter. Filter procedures for specific agent.",
						}),
					),
					userId: Type.Optional(
						Type.String({
							description:
								"⚠️ SECURITY: Caller-controlled parameter. Filter procedures for specific user.",
						}),
					),
					sessionId: Type.Optional(
						Type.String({
							description:
								"⚠️ SECURITY: Caller-controlled parameter. Filter procedures for specific session.",
						}),
					),
					confirmCrossTenantScope: Type.Optional(
						Type.Boolean({
				...
				async execute(_toolCallId: string, params: Record<string, unknown>) {
					try {
						...
						if (!q) {
							return {
								content: [
									{
										type: "text" as const,
										text: "Provide a task description to recall procedures.",
									},
								],
								details: { count: 0 },
							};
						...
						const negatives = factsDb.getNegativeProceduresMatching(
							q,
							3,
							scopeFilter,
						);
						const lines: string[] = [];
						const positiveList = procedures.filter(
							(p) => p.procedureType === "positive",
						);
							...
							for (const p of positiveList) {
								...
								const rate =
									p.successRate !== undefined
										? `, rate ${(p.successRate * 100).toFixed(0)}%`
										: "";
								const ver = p.version !== undefined ? `, v${p.version}` : "";
								const outcome =
									p.lastOutcome === "failure"
										? " ⚠️"
										: p.lastOutcome === "success"
											? " ✅"
											: "";
								lines.push(
						...
						if (negatives.length > 0) {
							...
							for (const p of negatives) {
								...
								const ver = p.version !== undefined ? ` (v${p.version})` : "";
								lines.push(
									`- ${p.taskPattern.slice(0, 80)}… ${steps ? `(${steps})` : ""}${ver}`,
								);
								if (p.avoidanceNotes && p.avoidanceNotes.length > 0) {
									for (const note of p.avoidanceNotes.slice(0, 2)) {
										lines.push(`  ⚠ ${note}`);
						...
						if (lines.length === 0) {
							return {
								content: [
									{
										type: "text" as const,
										text: "No procedures found for this task.",
									},
								],
								details: { count: 0 },
							};
		...
		api.registerTool(
			{
				...
				parameters: Type.Object({
					procedureId: Type.String({
						description: "ID of the procedure that was used",
					}),
					success: Type.Boolean({
						description: "true if the procedure succeeded, false if it failed",
					}),
					context: Type.Optional(
						Type.String({
							description:
								"What happened — error message, environment notes, or a summary of the outcome.",
						}),
					),
					...
					duration: Type.Optional(
						Type.Number({
							description:
								"Optional: how long the procedure took in milliseconds.",
						}),
					),
					tags: Type.Optional(
						Type.Array(Type.String(), {
							description:
								"Optional tags to associate with the episode (e.g. 'production', 'doris').",
						}),
					),
				}),
				async execute(_toolCallId: string, params: Record<string, unknown>) {
					try {
						...
						if (!result) {
							return {
								content: [
									{
										type: "text" as const,
										text: `Procedure not found: ${procedureId}`,
									},
								],
						...
						if (success) {
							...
							if (result.avoidanceNotes && result.avoidanceNotes.length > 0) {
								lines.push(
									`  Avoidance notes: ${result.avoidanceNotes.join("; ")}`,
								);
							}
						}
	...
	api.registerTool(
		{
			...
			parameters: Type.Object({
				...
				tags: Type.Optional(
					Type.Array(Type.String(), {
						description:
							"Topic tags for sharper retrieval (e.g. nibe, zigbee). Auto-inferred if omitted.",
					}),
				),
			...
			async execute(_toolCallId: string, params: Record<string, unknown>) {
				try {
					...
					const recordActiveStoreProvenance = (
						factId: string,
						sourceText?: string,
					) => {
					...
					let scopeTarget: string | null =
						scope === "global" ? null : (paramScopeTarget?.trim() ?? null);
					...
					const explicitVerificationTier = (verificationTier ?? "")
						.trim()
						.toLowerCase();
					...
					const maybeAutoVerify = (
						...
						try {
							const verifiedBy =
								explicitVerificationTier === "critical" ? "agent" : "system";
							verificationStore.verify(factId, factText, verifiedBy);
						} catch (err) {
							api.logger.warn?.(
								`memory-hybrid: auto-verify failed for ${factId}: ${err}`,
							);
						}
					};
					...
					if (
						cfg.credentials.enabled &&
						credentialsDb &&
						isCredentialLike(textToStore, entity, key, value)
					) {
					...
					try {
						...
						if (err instanceof AllEmbeddingProvidersFailed) {
							...
							api.logger.warn(
								"memory-hybrid: Stored fact without embeddings — all providers unavailable",
							);
							...
							api.logger.warn(
								`memory-hybrid: embedding skipped (expected): ${err}`,
							);
						} else {
							capturePluginError(
								err instanceof Error ? err : new Error(String(err)),
							...
							api.logger.warn(
								`memory-hybrid: embedding generation failed: ${err}`,
							);
						}
					}
					...
					if (cfg.store.classifyBeforeWrite) {
						...
						if (vector) {
							similarFacts = await findSimilarByEmbedding(
								vectorDb,
								factsDb,
								vector,
								5,
							...
							if (classification.action === "NOOP") {
								return {
									content: [
										{
											type: "text",
											text: `Already known: ${classification.reason}`,
										},
									],
							...
							if (
								...
								return {
									content: [
										{
											type: "text",
											text: `Retracted fact ${classification.targetId}: ${classification.reason}`,
										},
									],
									details: {
										action: "delete",
					...
					if (paramScope) {
						...
						const agentId =
							currentAgentIdRef.value || cfg.multiAgent.orchestratorId;
						...
						if (
							...
							!currentAgentIdRef.value &&
							(cfg.multiAgent.defaultStoreScope === "agent" ||
								cfg.multiAgent.defaultStoreScope === "auto")
						) {
							throw new Error(
					...
					const decayFreezeUntil =
						...
						Number.isFinite(paramDecayFreezeUntil)
							? paramDecayFreezeUntil
							: detectFutureDate(
									textToStore,
									cfg.futureDateProtection ?? { enabled: false },
								);
					...
					if (cfg.graph.enabled) {
						const enrichModel = getDefaultCronModel(
							...
							.catch((err) => {
								const msg =
									err instanceof Error
										...
										: String(err);
								api.logger.warn?.(
									`memory-hybrid: entity enrichment failed: ${msg}`,
								);
							});
					}
					...
					return {
						...
						details: {
							...
							...(autoSupersededIds.length > 0
								? { autoSuperseded: autoSupersededIds }
								: {}),
							...(contradictions.length > 0
								? {
	...
	api.registerTool(
		{
			...
			parameters: Type.Object({
				...
				name_prefix: Type.Optional(
					Type.String({
						description:
							"For list_contacts: filter by display name prefix (optional).",
					}),
				),
				org_name: Type.Optional(
					Type.String({
						description:
							"For org_view: organization name or key (resolves canonical org).",
					}),
				),
				limit: Type.Optional(
					Type.Number({
			...
			async execute(_toolCallId: string, params: Record<string, unknown>) {
				try {
					...
					const cap = Math.min(
						...
						Math.max(
							1,
							typeof limitRaw === "number" ? Math.floor(limitRaw) : 40,
						),
					);
					if (operation === "list_contacts") {
						const prefix =
							typeof params.name_prefix === "string" ? params.name_prefix : "";
					...
					if (operation === "org_view") {
						const orgName =
							typeof params.org_name === "string" ? params.org_name.trim() : "";
						if (!orgName) {
							return {
								content: [
									{ type: "text", text: "org_view requires org_name." },
								],
						...
						const peopleLines = people.map(
							(p) => `- ${p.displayName} (contact id=${p.id})`,
						);
						const factLines = factSummaries.map(
							(f) => `- [${f.id}] ${f.text}${f.text.length >= 240 ? "…" : ""}`,
						);
						return {
							content: [
								{
	...
	api.registerTool(
		{
			...
			parameters: Type.Object({
				...
				scope: Type.Union([Type.Literal("global"), Type.Literal("agent")], {
					description:
						"New scope: global (available to all) or agent (this agent only).",
				}),
				scopeTarget: Type.Optional(
					Type.String({
					...
					if (scope === "agent" && !scopeTarget?.trim()) {
						return {
							content: [
								{
									type: "text",
									text: "Scope 'agent' requires scopeTarget (agent identifier).",
								},
							],
							details: { error: "scope_target_required" },
						};
					}
					const scopeTargetValue =
						scope === "agent" ? (scopeTarget?.trim() ?? null) : null;
					const ok = factsDb.promoteScope(memoryId, scope, scopeTargetValue);
					if (!ok) {
						return {
							content: [
								{ type: "text", text: `Could not promote memory ${memoryId}.` },
							],
				...
				try {
					...
					if (memoryId) {
						...
						try {
							...
							api.logger.warn(
								`memory-hybrid: LanceDB delete during tool failed: ${err}`,
							);
						}
					...
					if (query) {
						...
						if (results.length === 1 && results[0].score > 0.9) {
							...
							try {
								...
								api.logger.warn(
									`memory-hybrid: LanceDB delete during supersede failed: ${err}`,
								);
							}
	...
	/** memory_record_episode — store a structured event with explicit outcome. */
	{
		const _recordEpisodeParams = Type.Object({
			event: Type.String({
				description: "What happened (e.g. 'deployed openclaw to production').",
			}),
			outcome: stringEnum(
				["success", "failure", "partial", "unknown"] as const,
				{
			...
			duration: Type.Optional(
				Type.Number({
					description:
						"Duration in milliseconds (e.g. how long a deployment took).",
				}),
			),
			context: Type.Optional(
				Type.String({
					description: "Context: environment state, what led up to it, etc.",
				}),
			),
			relatedFactIds: Type.Optional(
				Type.Array(Type.String(), {
					description: "IDs of related memory facts to link to this episode.",
				}),
			),
			...
			importance: Type.Optional(
				Type.Number({
					description:
						"Importance 0–1 (default 0.5). Failures are auto-boosted to ≥0.8.",
				}),
			),
			tags: Type.Optional(
				Type.Array(Type.String(), { description: "Topic tags for filtering." }),
			),
			scope: Type.Optional(
		...
		const _execRecordEpisode = async (
			_toolCallId: string,
	...
	/** memory_search_episodes — search structured episodic memories with filters. */
	{
		const _searchEpisodesParams = Type.Object({
			query: Type.Optional(
				Type.String({
					description: "Full-text search over event and context fields.",
				}),
			),
			outcome: Type.Optional(
				Type.Array(
					stringEnum(["success", "failure", "partial", "unknown"] as const),
				),
			),
			since: Type.Optional(
				Type.Number({
					description: "Unix epoch seconds — only events after this time.",
				}),
			),
			until: Type.Optional(
				Type.Number({
					description: "Unix epoch seconds — only events before this time.",
				}),
			),
			procedureId: Type.Optional(
				Type.String({
					description: "Filter to episodes linked to a specific procedure.",
				}),
			),
			limit: Type.Optional(
				Type.Number({
		...
		const _execSearchEpisodes = async (
			_toolCallId: string,
			params: Record<string, unknown>,
		) => {
			try {
				...
				if (episodes.length === 0) {
					return {
						content: [
							{
								type: "text",
								text: "No episodes found matching the criteria.",
							},
						],
	...
	/** memory_add_edict — create a verified ground-truth fact (human-only; agents propose via GitHub). */
	{
		...
		const _addEdictParams = Type.Object({
			text: Type.String({
				description: "The verified statement of fact to store",
			}),
			source: Type.Optional(
				Type.String({
					description:
						"Who or what verified this edict (e.g. 'human:markus', 'ops:runbook')",
				}),
			),
			tags: Type.Optional(
				Type.Array(Type.String(), {
					description: "Labels for filtering (e.g. ['operations', 'ssh'])",
				}),
			),
			ttl: Type.Optional(
				Type.String({
					description:
						"TTL mode: 'never' (permanent), 'event' (use expiresAt date), or seconds as number",
				}),
			),
			expiresAt: Type.Optional(
				Type.String({
					description:
						"ISO 8601 date when this edict expires (for ttl='event')",
				}),
			),
		});
		const _addEdictDesc =
				...
				if (!text || text.trim().length === 0) {
					return {
						content: [
							{
								type: "text",
								text: "Provide the 'text' parameter: the verified statement to store.",
							},
						],
	...
	/** memory_list_edicts — list edicts, optionally filtered by tags. */
	{
		...
		const _listEdictsParams = Type.Object({
			tags: Type.Optional(
				Type.Array(Type.String(), {
					description: "Filter to edicts with any of these tags",
				}),
			),
			limit: Type.Optional(
		...
		const _listEdictsDesc =
			"List all non-expired edicts, optionally filtered by tags.";
		const _execListEdicts = async (
			_toolCallId: string,
			params: Record<string, unknown>,
		) => {
			try {
	...
	/** memory_get_edicts — get edicts formatted for system prompt injection. */
	{
		...
		const _getEdictsParams = Type.Object({
			...
			format: Type.Optional(
				Type.String({
					description:
						"Output format: 'prompt' (Markdown block, default) or 'full' (structured)",
				}),
			),
			limit: Type.Optional(
				Type.Number({ description: "Max results (default 100)" }),
		...
		const _execGetEdicts = async (
			_toolCallId: string,
			params: Record<string, unknown>,
		) => {
			try {
				const { tags, format, limit } = params as {
					tags?: string[];
					format?: string;
					limit?: number;
				};
				const result = edictStore.getEdicts({
					tags,
					format: format as "prompt" | "full",
					limit,
				});
					...
					content: [
						{
							type: "text",
							text:
								result.renderForPrompt ||
								JSON.stringify(result.edicts, null, 2),
						},
					],
					details: {
						count: result.edicts.length,
						edicts: result.edicts,
						renderForPrompt: result.renderForPrompt,
					},
				};
			} catch (err) {
				capturePluginError(
	...
	/** memory_update_edict — update an existing edict's text, tags, source, or expiry. */
	{
		...
		const _updateEdictParams = Type.Object({
			...
			text: Type.Optional(
				Type.String({ description: "New verified statement text" }),
			),
			source: Type.Optional(
				Type.String({ description: "New or updated source" }),
			),
			tags: Type.Optional(
				Type.Array(Type.String(), { description: "Replacement tags array" }),
			),
			ttl: Type.Optional(
				Type.String({
					description: "New TTL mode: 'never', 'event', or seconds as number",
				}),
			),
			expiresAt: Type.Optional(
				Type.String({ description: "ISO 8601 expiry date for ttl='event'" }),
			),
		});
		const _updateEdictDesc =
			"Update the text, tags, source, or expiry of an existing edict.";
		const _execUpdateEdict = async (
			_toolCallId: string,
			params: Record<string, unknown>,
		) => {
			try {
	...
	/** memory_edict_stats — get statistics about the edict store. */
	{
		...
		const _edictStatsDesc =
			"Get statistics about the edict store: total, by-tag counts, expired, and expiring soon.";
