		...
		api.registerTool(
			{
				...
				parameters: Type.Object({
					service: Type.String({
						description:
							"Service name (e.g. 'home-assistant', 'github', 'openai')",
					}),
					type: stringEnum(CREDENTIAL_TYPES as unknown as readonly string[]),
					value: Type.String({
						description: "The secret value (token, password, API key)",
					}),
					url: Type.Optional(
						Type.String({ description: "Optional URL or endpoint" }),
					),
					notes: Type.Optional(Type.String({ description: "Optional notes" })),
					expires: Type.Optional(
						Type.Number({
							description: "Optional Unix timestamp when credential expires",
						}),
					),
				}),
				async execute(_toolCallId: string, params: Record<string, unknown>) {
					...
					if (!credentialsDb)
						throw new Error("Credentials store not available");
					const urlTrim =
					...
					const notesTrim =
						typeof notes === "string" &&
						notes.length > CREDENTIAL_NOTES_MAX_CHARS
					...
					return {
						content: [
							{
								type: "text",
								text: `Stored credential for ${service} (${type}).`,
							},
						],
		...
		api.registerTool(
			{
				...
				parameters: Type.Object({
					service: Type.String({
						description: "Service name (e.g. 'home-assistant', 'github')",
					}),
					type: Type.Optional(
						stringEnum(CREDENTIAL_TYPES as unknown as readonly string[]),
					),
				}),
				async execute(_toolCallId: string, params: Record<string, unknown>) {
					const { service, type } = params as {
						service: string;
						type?: CredentialType;
					};
					if (!credentialsDb)
					...
					const expiresSoon =
						entry.expires != null &&
						entry.expires - nowSec < warnDays * 24 * 3600;
					...
					if (expiresSoon) {
						if (secLeft <= 0) {
							expiryWarning =
								" [WARNING: Credential has expired — rotate immediately]";
						} else if (daysLeft < 1) {
				...
				async execute(_toolCallId: string, params: Record<string, unknown>) {
					...
					if (!deleted) {
						return {
							content: [
								{
									type: "text",
									text: `No credential found for "${service}"${type ? ` (type: ${type})` : ""}.`,
								},
							],
					...
					return {
						content: [
							{
								type: "text",
								text: `Deleted credential for ${service}${type ? ` (${type})` : ""}.`,
							},
						],
