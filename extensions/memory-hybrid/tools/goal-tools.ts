...
async function flushGoalOutcomeToMemory(
	memoryDir: string,
	title: string,
	lines: string[],
): Promise<void> {
	...
	api.registerTool(
		{
			...
			parameters: Type.Object({
				...
				priority: Type.Optional(
					stringEnum(PRIORITIES as unknown as readonly string[]),
				),
			...
			async execute(_id: string, params: Record<string, unknown>) {
				...
				try {
					...
					if (active.length >= gs.globalLimits.maxActiveGoals) {
						return {
							content: [
								{
									type: "text",
									text: `Max active goals (${gs.globalLimits.maxActiveGoals}) reached.`,
								},
							],
					...
					if (
						gs.confirmationPolicy.requireRegisterAckForPriorities.includes(
							effectivePriority,
						) &&
						p.confirmed !== true
					) {
					...
					return {
						content: [
							{
								type: "text",
								text: `Goal registered: ${goal.label} (${goal.id})`,
							},
						],
						details: { goal },
					};
			...
			async execute(_id: string, params: Record<string, unknown>) {
				...
				try {
					...
					if (isTerminalStatus(goal.status)) {
						return {
							content: [{ type: "text", text: `Goal already ${goal.status}` }],
					...
					if (goal.assessmentCount >= goal.maxAssessments) {
						await updateGoal(
							...
							{
								status: "blocked",
								currentBlockers: ["Assessment budget exhausted"],
							},
							{
						...
						return {
							content: [{ type: "text", text: "Assessment budget exhausted." }],
							details: { error: "budget" },
						};
					}
					...
					if (p.dispatched) {
						if (isGlobalRateLimited(gs.globalLimits.maxDispatchesPerHour)) {
							return {
								content: [
									{ type: "text", text: "Global dispatch rate limit reached." },
								],
					...
					const cbState = blockersExplicitlyProvided
						...
						: {
								...
								circuitBreakerLastProgressAssessmentCount:
									goal.circuitBreakerLastProgressAssessmentCount,
							};
					const tripEval = evaluateCircuitBreakerTrip(
						gs.circuitBreaker,
						cbState,
						newAssessmentCount,
					);
					...
					if (tripEval.trip) {
						...
						const summary = composeCircuitBreakerHumanSummary(
							goalPreview,
							tripEval.reason,
							gs.circuitBreaker,
						);
						...
						const updated = await updateGoal(goalsDir, goal.id, blockedPatch, [
							assessEntry,
							cbEntry,
						]);
						try {
						...
						if (gs.circuitBreaker.appendMemoryEscalation) {
							await flushGoalOutcomeToMemory(
								memoryDir,
								`Circuit breaker: ${updated.label}`,
					...
					return {
						content: [
							{
								type: "text",
								text: `Assessed ${updated.label}. Next: ${p.next_action}`,
							},
						],
	...
	api.registerTool(
		{
			...
			async execute(_id: string, params: Record<string, unknown>) {
				...
				if (p.acceptance_criteria !== undefined)
					patch.acceptanceCriteria = p.acceptance_criteria;
				...
				return {
					content: [{ type: "text", text: `Updated ${updated.label}` }],
					details: { goal: updated },
				};
			},
		},
	...
	api.registerTool(
		{
			...
			async execute(_id: string, params: Record<string, unknown>) {
				...
				if (isTerminalStatus(goal.status)) {
					return {
						content: [
							{
								type: "text",
								text: `Goal ${goal.label} is already ${goal.status}.`,
							},
						],
						details: { error: "terminal" },
					};
				}
				const completed = await terminateGoal(
					goalsDir,
					goal.id,
					"completed",
					p.reason,
				...
				if (cfg.activeTask.flushOnComplete !== false) {
					await flushGoalOutcomeToMemory(
						memoryDir,
						`Goal completed: ${completed.label}`,
						[`**Outcome:** ${p.reason}`],
					);
				}
				try {
				...
				return {
					content: [{ type: "text", text: `Completed ${completed.label}` }],
	...
	api.registerTool(
		{
			...
			async execute(_id: string, params: Record<string, unknown>) {
				...
				const abandoned = await terminateGoal(
					goalsDir,
					goal.id,
					"abandoned",
					p.reason,
				...
				if (cfg.activeTask.flushOnComplete !== false) {
					await flushGoalOutcomeToMemory(
						memoryDir,
						`Goal abandoned: ${abandoned.label}`,
						[`**Reason:** ${p.reason}`],
					);
				}
				try {
				...
				return {
					content: [{ type: "text", text: `Abandoned ${abandoned.label}` }],
					details: { goal: abandoned },
				};
			},
		},
		{ name: "goal_abandon" },
