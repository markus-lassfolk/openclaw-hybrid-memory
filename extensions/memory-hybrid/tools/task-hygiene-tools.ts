...
export function resolveActiveTaskPathForTools(
	cfg: HybridMemoryConfig,
	workspaceRoot: string,
): string {
...
export function registerTaskHygieneTools(
	ctx: TaskHygieneToolsContext,
	...
	api.registerTool(
		{
			...
			async execute(_id: string, params: Record<string, unknown>) {
				...
				try {
					...
					if (!file || file.active.length === 0) {
						return {
							content: [
								{
									type: "text",
									text: "No active tasks in ACTIVE-TASKS.md (or file missing).",
								},
							],
					...
					const want = String(
						(params as { task_label: string }).task_label,
					).trim();
					const task =
