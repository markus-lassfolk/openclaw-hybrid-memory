	...
	api.registerTool(
		{
			...
			label: "Memory Verify",
			description:
				"Mark a fact as verified/critical and store it in the verification store.",
			...
			async execute(_toolCallId: string, params: Record<string, unknown>) {
				...
				try {
					...
					if (existing) {
						return {
							content: [
								{
									type: "text",
									text: `Fact ${factId} is already verified (version ${existing.version}).`,
								},
							],
							details: {
					...
					return {
						content: [
							{
								type: "text",
								text: `Verification status check failed for ${factId}: ${err}`,
							},
						],
				...
				try {
					const verifiedId = verificationStore.verify(
						factId,
						fact.text,
						"agent",
					);
					return {
						content: [
							{
								type: "text",
								text: `Verified fact ${factId} (verification id: ${verifiedId}).`,
							},
						],
						details: {
							status: "verified",
					...
					if (err instanceof VerificationError) {
						return {
							content: [
								{
									type: "text",
									text: `Verification failed for ${factId}: ${err.message}`,
								},
							],
					...
					return {
						content: [
							{
								type: "text",
								text: `Verification failed for ${factId}: ${err}`,
							},
						],
						details: { error: "verification_failed", id: factId },
					};
	...
	api.registerTool(
		{
			...
			description:
				"List all verified facts with their latest verification metadata.",
			parameters: Type.Object({}),
			async execute() {
				...
				const lines = verified.map((vf) => {
					const next = vf.nextVerification
						? ` next=${vf.nextVerification}`
						: "";
	...
	api.registerTool(
		{
			...
			description:
				"Check whether a specific fact is verified and return its status.",
			parameters: Type.Object({
			...
			async execute(_toolCallId: string, params: Record<string, unknown>) {
				...
				try {
					...
					if (!verified) {
						return {
							content: [
								{ type: "text", text: `Fact ${factId} is not verified.` },
							],
