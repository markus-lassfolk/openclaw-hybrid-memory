...
export function pruneLogTables(
	db: DatabaseSync,
	...
	const recall = db
		.prepare("DELETE FROM recall_log WHERE occurred_at < ?")
		.run(cutoff);
	const reinforcement = db
		.prepare("DELETE FROM reinforcement_log WHERE occurred_at < ?")
		.run(cutoff);
	const feedback = db
		.prepare("DELETE FROM feedback_trajectories WHERE created_at < ?")
		.run(cutoff);
	return (
		Number(recall.changes ?? 0) +
		Number(reinforcement.changes ?? 0) +
		Number(feedback.changes ?? 0)
	);
}
...
export function selfCorrectionIncidentsCount(db: DatabaseSync): number {
	const row = db
		.prepare(
			`SELECT COUNT(*) as count FROM facts WHERE superseded_at IS NULL AND source = 'self-correction'`,
		)
		.get() as { count: number } | undefined;
	return row?.count ?? 0;
}
...
export function countBySource(db: DatabaseSync, source: string): number {
	const row = db
		.prepare(
			"SELECT COUNT(*) as count FROM facts WHERE superseded_at IS NULL AND source = ?",
		)
...
export function statsBySource(db: DatabaseSync): Record<string, number> {
	const rows = db
		.prepare(
			"SELECT source, COUNT(*) as count FROM facts WHERE superseded_at IS NULL GROUP BY source",
		)
		.all() as Array<{ source: string; count: number }>;
	return Object.fromEntries(rows.map((r) => [r.source, r.count]));
}

export function uniqueScopes(
	db: DatabaseSync,
): Array<{ scope: string; scopeTarget: string | null }> {
	const rows = db
		.prepare(
			"SELECT DISTINCT scope, scope_target as scopeTarget FROM facts WHERE scope IS NOT NULL",
		)
...
export function pruneScopedFacts(
	db: DatabaseSync,
