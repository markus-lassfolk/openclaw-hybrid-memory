 ...
 */
export function enrichProcedureWithFeedback(
	db: DatabaseSync,
	base: ProcedureEntry,
): ProcedureEntry {
	try {
		...
		if (base.lastFailed !== null && base.lastValidated !== null) {
			lastOutcome =
				base.lastFailed > base.lastValidated ? "failure" : "success";
...
export function procedureRowToEntry(
	db: DatabaseSync,
	...
	const base: ProcedureEntry = {
		...
		procedureType:
			(row.procedure_type as "positive" | "negative") || "positive",
...
export function procedureFeedback(
	...
	if (input.success) {
		...
		const existingVer = db
			...
			.get(input.procedureId) as
			| { id: string; success_count: number }
			| undefined;

		if (existingVer) {
			db.prepare(
				"UPDATE procedure_versions SET success_count = success_count + 1 WHERE id = ?",
			).run(existingVer.id);
		} else {
		...
		db.prepare(
			...
			Math.max(
				...
				Math.min(
					0.95,
					0.5 + 0.1 * (versionCounts.total_succ - versionCounts.total_fail),
				),
			),
			nowSec,
			input.procedureId,
		...
		const prevNotes = db
			.prepare(
				"SELECT avoidance_notes FROM procedure_versions WHERE procedure_id = ?",
			)
		...
		const notesJson =
			avoidanceNotes.length > 0 ? JSON.stringify(avoidanceNotes) : null;
	...
	return rows.map((r) => ({
		...
		avoidanceNotes: (() => {
			...
			try {
				const parsed = JSON.parse(raw);
				return Array.isArray(parsed)
					? parsed.filter((n: unknown) => typeof n === "string")
					: null;
			} catch {
				return null;
			}
...
export function upsertProcedure(
	...
	if (existing) {
		...
		const confidence =
			proc.confidence ??
			Math.max(0.1, Math.min(0.95, 0.5 + 0.1 * (successCount - failureCount)));
...
export function listProcedures(
	...
	try {
		const rows = db
			.prepare(
				"SELECT * FROM procedures ORDER BY updated_at DESC, created_at DESC LIMIT ?",
			)
...
export function listProceduresUpdatedInLastNDays(
	db: DatabaseSync,
	days: number,
	limit = 500,
): ProcedureEntry[] {
...
export function getProcedureById(
	...
	const row = db.prepare("SELECT * FROM procedures WHERE id = ?").get(id) as
...
export function searchProcedures(
	...
	try {
		...
		const rows = db
			.prepare(baseSql)
			.all(safeQuery, ...scopeParams, limit * 2) as Array<
			Record<string, unknown>
		>;
		...
		const scored: ScoredRow[] = rows.map((r) => {
			...
			const rawFtsScore =
				1 - ((r.fts_score as number) - minFtsScore) / ftsRange;
			...
			const boostedScore = Math.min(
				1.0,
				ftsScore * 0.6 + confidence * 0.4 + reinforcement,
			);
	...
	try {
		...
		const rows = db
         ...
         LIMIT ?`,
			...
			.all(safeQuery, ...scopeParams, limit * 3) as Array<
			Record<string, unknown>
		>;
...
export function recordProcedureSuccess(
	...
	if (sessionId) {
		const sourceSessions = proc.sourceSessions
			? proc.sourceSessions.split(",")
			: [];
		...
		const confidence = Math.max(
			0.1,
			Math.min(0.95, 0.5 + 0.1 * (successCount - proc.failureCount)),
		);
...
export function recordProcedureFailure(
	...
	if (sessionId) {
		...
		const confidence = Math.max(
			0.1,
			Math.min(0.95, 0.5 + 0.1 * (proc.successCount - failureCount)),
		);
		if (recipeJson !== undefined) {
			db.prepare(
...
export function markProcedurePromoted(
	db: DatabaseSync,
	id: string,
	skillPath: string,
): boolean {
	const result = db
		.prepare(
			"UPDATE procedures SET promoted_to_skill = 1, skill_path = ?, updated_at = ? WHERE id = ?",
		)
...
export function proceduresCount(db: DatabaseSync): number {
	try {
		const row = db.prepare("SELECT COUNT(*) as cnt FROM procedures").get() as {
			cnt: number;
		};
		return row?.cnt ?? 0;
	} catch (err) {
...
export function proceduresValidatedCount(db: DatabaseSync): number {
	try {
		const row = db
			.prepare(
				"SELECT COUNT(*) as cnt FROM procedures WHERE last_validated IS NOT NULL",
			)
			.get() as {
			cnt: number;
		};
...
export function proceduresPromotedCount(db: DatabaseSync): number {
	try {
		const row = db
			.prepare(
				"SELECT COUNT(*) as cnt FROM procedures WHERE promoted_to_skill = 1",
			)
			.get() as {
			cnt: number;
		};
