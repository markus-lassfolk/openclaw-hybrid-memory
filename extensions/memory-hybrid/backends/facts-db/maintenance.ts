...
export function logRecall(
	...
	db.prepare(
		"INSERT INTO recall_log (id, occurred_at, hit) VALUES (?, ?, ?)",
...
export function pruneRecallLog(db: DatabaseSync, olderThanDays = 30): number {
	...
	return Number(
		db.prepare("DELETE FROM recall_log WHERE occurred_at < ?").run(cutoff)
			.changes ?? 0,
	);
}

export function setFactTier(
	db: DatabaseSync,
	id: string,
	tier: MemoryTier,
): boolean {
...
export function trimToBudget(
	...
	for (const row of rows) {
		...
		if (isEdict || isVerified || hasPreserveUntil || hasPreserveTags) {
			...
			if (hasPreserveTags)
				reasons.push(`preserveTags=${preserveTags.join(",")}`);
	...
	const trimPoolTokens = trimRows.reduce(
		(sum, r) => sum + tokenEstimate(r.text),
		0,
	);
	...
	for (const fact of toTrim) {
		...
		if (!simulate) {
			db.prepare("UPDATE facts SET superseded_at = ? WHERE id = ?").run(
				nowSec,
				fact.id,
			);
			db.prepare(
...
export function setPreserveUntil(
	...
	if (untilSec !== null && untilSec <= nowSec) {
		throw new Error(
			`preserve_until must be in the future or null. Got: ${untilSec}`,
		);
	}
	db.prepare("UPDATE facts SET preserve_until = ? WHERE id = ?").run(
		untilSec,
		id,
	);
	return getById(id);
...
export function setPreserveTags(
	...
	db.prepare("UPDATE facts SET preserve_tags = ? WHERE id = ?").run(
		preserveTagsStr,
		id,
	);
	return getById(id);
