...
export function getDuplicateIdByNormalizedHash(
	db: DatabaseSync,
	text: string,
): string | null {
...
export function storeFact(
	...
	const rawFreeze =
		(entry as { decayFreezeUntil?: number | null }).decayFreezeUntil ?? null;
	const decayFreezeUntil =
		rawFreeze !== null && Number.isFinite(rawFreeze) ? rawFreeze : null;
	const adjustedExpiresAt =
		decayFreezeUntil !== null &&
		expiresAt !== null &&
		expiresAt < decayFreezeUntil
			? decayFreezeUntil
			: expiresAt;
	...
	if (!loaded) {
		throw new Error(
			`memory-hybrid: store() failed to read back inserted fact ${id}`,
		);
	}
	return loaded;
}
...
export function deleteFact(db: DatabaseSync, id: string): boolean {
	db.prepare(
		"DELETE FROM contradictions WHERE fact_id_new = ? OR fact_id_old = ?",
	).run(id, id);
	db.prepare(
		`DELETE FROM memory_links WHERE target_fact_id = ? AND link_type != 'DERIVED_FROM'`,
	).run(id);
...
export function hasDuplicateText(
	...
	if (fuzzyDedupe && getDuplicateIdByNormalizedHash(db, text) !== null)
		return true;
	return false;
}
