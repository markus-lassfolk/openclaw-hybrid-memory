...
export function migrateEntityLayerTables(db: DatabaseSync): void {
  ...
  `);
	...
	db.exec(
		"CREATE UNIQUE INDEX IF NOT EXISTS idx_contacts_normalized_key_unique ON contacts(normalized_key)",
	);
...
function upsertContact(
	...
	const existing = db
		.prepare(
			"SELECT id, primary_org_id FROM contacts WHERE normalized_key = ? LIMIT 1",
		)
		.get(nk) as { id: string; primary_org_id: string | null } | undefined;
	const now = Math.floor(Date.now() / 1000);
	if (existing) {
		if (primaryOrgId && !existing.primary_org_id) {
			db.prepare(
				"UPDATE contacts SET primary_org_id = ?, updated_at = ?, display_name = ? WHERE id = ?",
			).run(primaryOrgId, now, displayName.trim(), existing.id);
		} else {
			db.prepare(
				"UPDATE contacts SET updated_at = ?, display_name = ? WHERE id = ?",
			).run(now, displayName.trim(), existing.id);
		}
		return { id: existing.id, created: false };
	}
	const id = randomUUID();
...
export function replaceFactEntityMentions(
	...
	const tx = createTransaction(db, () => {
		db.prepare("DELETE FROM fact_entity_mentions WHERE fact_id = ?").run(
			factId,
		);
		db.prepare(
			"DELETE FROM org_fact_links WHERE fact_id = ? AND reason = 'ner_mention'",
		).run(factId);
		...
		if (orgIds.length > 0 && personRows.length > 0) {
			...
			for (const p of personRows) {
				db.prepare(
					"UPDATE contacts SET primary_org_id = COALESCE(primary_org_id, ?), updated_at = ? WHERE id = ?",
				).run(primaryOrg, now, p.contactId);
			}
		}

		db.prepare("UPDATE facts SET entity_enrichment_at = ? WHERE id = ?").run(
			now,
			factId,
		);
	});
...
export function getOrganizationByKeyOrName(
	db: DatabaseSync,
	query: string,
	...
	if (!nk) return null;
	const byKey = db
		.prepare("SELECT * FROM organizations WHERE canonical_key = ?")
		.get(nk) as Record<string, unknown> | undefined;
...
export function listContactsForOrg(
	db: DatabaseSync,
	orgId: string,
	limit: number,
): ContactRow[] {
...
export function listContactsByNamePrefix(
	db: DatabaseSync,
	prefix: string,
	limit: number,
): ContactRow[] {
	...
	if (!p) {
		const rows = db
			.prepare(
				"SELECT * FROM contacts ORDER BY display_name COLLATE NOCASE LIMIT ?",
			)
...
export function listFactIdsForOrg(
	db: DatabaseSync,
	orgId: string,
	limit: number,
): string[] {
	const rows = db
		.prepare(
			"SELECT DISTINCT fact_id FROM org_fact_links WHERE org_id = ? ORDER BY created_at DESC LIMIT ?",
		)
