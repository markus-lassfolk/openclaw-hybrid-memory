...
export function statsBreakdownBySource(
	db: DatabaseSync,
...
export function statsBreakdownByCategory(
	db: DatabaseSync,
): Record<string, number> {
	const rows = db
		.prepare(
...
export function statsBreakdownByDecayClass(
	db: DatabaseSync,
): Record<string, number> {
	const rows = db
		.prepare(
...
export function listForDashboard(
	...
	if (
		(opts.category != null &&
			opts.category !== "" &&
			!isValidCategory(opts.category)) ||
		(opts.tier != null &&
			opts.tier !== "" &&
			!DASHBOARD_TIER_FILTER.has(opts.tier)) ||
		(opts.decayClass != null &&
			opts.decayClass !== "" &&
			!DECAY_CLASS_FILTER.has(opts.decayClass))
	) {
		return { facts: [], total: 0 };
	}

	let where =
		"superseded_at IS NULL AND (expires_at IS NULL OR expires_at > ?)";
	...
	if (opts.search?.trim()) {
		...
		for (let i = 0; i < allFtsIds.length; i += CHUNK_SIZE) {
			...
			const chunkRows = db
				.prepare(
					`SELECT id FROM facts WHERE id IN (${idPlaceholders}) AND ${where}`,
				)
...
export function countFacts(db: DatabaseSync): number {
	const row = db.prepare("SELECT COUNT(*) as cnt FROM facts").get() as Record<
		string,
		number
	>;
	return row.cnt;
...
export function estimateStoredTokens(db: DatabaseSync): number {
	const rows = db
		.prepare("SELECT summary, text FROM facts WHERE superseded_at IS NULL")
		.all() as Array<{
		summary: string | null;
		text: string;
	}>;
	return rows.reduce(
		(sum, r) => sum + estimateTokensForDisplay(r.summary || r.text),
		0,
	);
}
