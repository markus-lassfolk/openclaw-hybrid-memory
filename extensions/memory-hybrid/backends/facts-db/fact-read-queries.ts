...
export function findSimilarForClassification(
	...
	if (entity && results.length < limit) {
		...
		const rows = db
			...
			.all(entity, nowSec, remaining + results.length) as Array<
	...
	if (results.length < limit) {
		...
		if (words) {
			try {
				...
				for (;;) {
					const ftsRows = ftsStmt.all(words, fetchLimit) as Array<{
						rowid: number;
					}>;
					...
					for (let i = 0; i < ftsRows.length; i += CHUNK_SIZE) {
						...
						allRows.push(
							...(db
								...
								.all(...chunk.map((r) => r.rowid), nowSec) as Array<
								Record<string, unknown>
							>),
						);
					}
...
export function listFacts(
	...
	if (
		(filters?.category != null &&
			filters.category !== "" &&
			!isValidCategory(filters.category)) ||
		(filters?.tier != null &&
			filters.tier !== "" &&
			!DASHBOARD_TIER_FILTER.has(filters.tier))
	) {
		return [];
	}
	const parts: string[] = [
...
export function listFactsByCategory(
	db: DatabaseSync,
	category: string,
	limit = 100,
): MemoryEntry[] {
