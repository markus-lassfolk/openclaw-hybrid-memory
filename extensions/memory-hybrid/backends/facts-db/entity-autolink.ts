...
const knownEntitiesCacheByDb = new WeakMap<
	DatabaseSync,
	{ list: string[]; time: number }
>();
...
export function autoLinkEntities(
	...
	if (entity?.trim() && key?.trim()) {
		...
		const newVal =
			((
				db.prepare("SELECT value FROM facts WHERE id = ?").get(newFactId) as
					| { value: string | null }
					| undefined
	...
	linkedCount += autoDetectInstanceOf(
		db,
		newFactId,
		text,
		knownEntities,
