...
export class FactsDBLayer1 extends BaseSqliteStore {
	...
	protected readonly supersededTextsCacheMgr = new SupersededTextsCache(
		5 * 60_000,
	);
	...
	findSimilarForClassification(
		...
		return findSimilarForClassificationImpl(
			this.liveDb,
			text,
			entity,
			key,
	...
	createOrStrengthenRelatedLink(
		...
		createOrStrengthenRelatedLinkHelper(
			this.liveDb,
			factIdA,
			factIdB,
			deltaStrength,
	...
	expandGraphWithCTE(
		...
		return expandGraphWithCTEHelper(
			this.liveDb,
			seedFactIds,
			maxDepth,
			options,
