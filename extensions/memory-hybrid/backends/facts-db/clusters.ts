...
export function getAllLinks(
	db: DatabaseSync,
): Array<{ sourceFactId: string; targetFactId: string }> {
	const rows = db
...
export function getAllEdges(
	...
	const rows = db
		.prepare(
			"SELECT source_fact_id, target_fact_id, link_type, strength FROM memory_links LIMIT ?",
		)
		.all(limit) as Array<{
		source_fact_id: string;
		target_fact_id: string;
...
export function getClusters(db: DatabaseSync): Array<{
	...
	const rows = db
		.prepare(
			"SELECT id, label, fact_count, created_at, updated_at FROM clusters ORDER BY fact_count DESC",
		)
		.all() as Array<{
		id: string;
		label: string;
...
export function getFactClusterId(
	db: DatabaseSync,
	factId: string,
): string | null {
	const row = db
