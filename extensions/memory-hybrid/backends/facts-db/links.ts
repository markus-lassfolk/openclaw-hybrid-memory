...
export function createOrStrengthenRelatedLink(
	...
	const [source, target] =
		factIdA < factIdB ? [factIdA, factIdB] : [factIdB, factIdA];
	...
	if (existing) {
		db.prepare("UPDATE memory_links SET strength = ? WHERE id = ?").run(
			newStrength,
			existing.id,
		);
	} else {
...
export function strengthenRelatedLinksBatch(
	...
	const updateStmt = db.prepare(
		"UPDATE memory_links SET strength = ? WHERE id = ?",
	);
	...
	const tx = createTransaction(db, () => {
		for (const [factIdA, factIdB] of pairs) {
			...
			const [source, target] =
				factIdA < factIdB ? [factIdA, factIdB] : [factIdB, factIdA];
			const existing = selectStmt.get(source, target) as
				| { id: string; strength: number }
				| undefined;
			const newStrength = Math.max(
				0,
				Math.min(1, (existing?.strength ?? 0) + deltaStrength),
			);
			if (existing) {
...
export function getConnectedFactIds(
  ...
  `;
	...
	const rows = db
		.prepare(query)
		.all(JSON.stringify(factIds), maxDepth, maxDepth) as Array<{
		fact_id: string;
	}>;
...
export function expandGraphWithCTE(
	...
	if (asOf != null || scopeFilter) {
		...
		if (asOf != null) {
			baseWhere +=
				" AND COALESCE(f.valid_from, f.created_at) <= ? AND (f.valid_until IS NULL OR f.valid_until > ?)";
			filterParamsOut.push(asOf, asOf);
			filterParamsIn.push(asOf, asOf);
		}
		if (
			scopeFilter &&
			(scopeFilter.userId || scopeFilter.agentId || scopeFilter.sessionId)
		) {
			...
			filterParamsOut.push(
				scopeFilter.userId ?? null,
				scopeFilter.agentId ?? null,
				scopeFilter.sessionId ?? null,
			);
			filterParamsIn.push(
				scopeFilter.userId ?? null,
				scopeFilter.agentId ?? null,
