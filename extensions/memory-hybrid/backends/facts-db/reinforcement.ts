...
export function appendReinforcementQuote(
	...
	if (existingJson) {
		try {
			...
			if (Array.isArray(parsed))
				quotes = parsed.filter((q): q is string => typeof q === "string");
		} catch (err) {
...
export function reinforceFact(
	...
	const tx = createTransaction(db, () => {
		...
		const quotesJson = appendReinforcementQuote(
			row.reinforced_quotes,
			quoteSnippet,
		);
		...
		if (trackContext) {
         ...
         VALUES (?, ?, 'positive', ?, ?, ?, ?, ?)`,
			...
			const countRow = db
				.prepare(
					"SELECT COUNT(*) as cnt FROM reinforcement_log WHERE fact_id = ?",
				)
				.get(id) as {
				cnt: number;
			};
...
export function getReinforcementEvents(
	db: DatabaseSync,
	factId: string,
): ReinforcementEvent[] {
	const rows = db
		.prepare(
			"SELECT * FROM reinforcement_log WHERE fact_id = ? ORDER BY occurred_at DESC",
		)
		.all(factId) as Array<{
		id: string;
		fact_id: string;
	...
	return rows.map((r) => ({
		id: r.id,
		factId: r.fact_id,
		signal: (r.signal === "negative" ? "negative" : "positive") as
			| "positive"
			| "negative",
		querySnippet: r.query_snippet,
		topic: r.topic,
		toolSequence: r.tool_sequence
			? (JSON.parse(r.tool_sequence) as string[])
			: null,
...
export function computeDiversityFromEvents(
...
export function batchGetReinforcementEvents(
	...
	const rows = db
		.prepare(
			`SELECT * FROM reinforcement_log WHERE fact_id IN (${placeholders}) ORDER BY fact_id, occurred_at DESC`,
		)
...
export function reinforceProcedure(
	...
	const tx = createTransaction(db, () => {
		const row = db
			.prepare(
				"SELECT reinforced_quotes, reinforced_count, confidence FROM procedures WHERE id = ?",
			)
			.get(id) as
			| {
