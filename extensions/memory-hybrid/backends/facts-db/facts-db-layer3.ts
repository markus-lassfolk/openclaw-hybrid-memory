...
export class FactsDB extends FactsDBLayer2 {
	...
	findSessionFactsForPromotion(
		...
		return findSessionFactsForPromotionImpl(
			this.liveDb,
			thresholdDays,
			minImportance,
		);
	...
	findConflictingFacts(
		...
		return findConflictingFactsImpl(
			this.liveDb,
			entity,
			key,
			value,
	...
	detectContradictions(
		...
		return detectContradictionsImpl(
			this.liveDb,
			newFactId,
			entity,
			key,
	...
	listFactIdsNeedingEntityEnrichment(limit: number, minTextLen = 24): string[] {
		return entityLayerListFactsNeedingEnrichment(
			this.liveDb,
			limit,
			minTextLen,
		);
