...
function migrateProcedureVersionsTable(db: DatabaseSync): void {
  ...
  `);
	db.exec(
		"CREATE INDEX IF NOT EXISTS idx_proc_ver_procedure ON procedure_versions(procedure_id)",
	);
	db.exec(
...
function migrateProcedureFailuresTable(db: DatabaseSync): void {
  ...
  `);
	db.exec(
		"CREATE INDEX IF NOT EXISTS idx_proc_fail_procedure ON procedure_failures(procedure_id)",
	);
	db.exec(
		"CREATE INDEX IF NOT EXISTS idx_proc_fail_version ON procedure_failures(procedure_id, version_number)",
	);
	db.exec(
		"CREATE INDEX IF NOT EXISTS idx_proc_fail_ts ON procedure_failures(timestamp DESC)",
	);
}
...
function migrateSourceDateColumn(db: DatabaseSync): void {
	...
	db.exec(
		"UPDATE facts SET source_date = created_at WHERE source_date IS NULL",
	);
	db.exec(
...
function migrateAccessTracking(db: DatabaseSync): void {
	...
	if (!colNames.has("last_accessed")) {
		...
		db.exec(
			"UPDATE facts SET last_accessed = last_confirmed_at WHERE last_accessed IS NULL",
		);
	}
	db.exec(
...
function migrateReinforcementColumns(db: DatabaseSync): void {
	...
	db.exec(
		"ALTER TABLE facts ADD COLUMN reinforced_count INTEGER NOT NULL DEFAULT 0",
	);
...
function migrateReinforcementColumnsProcedures(db: DatabaseSync): void {
	...
	if (colNames.has("reinforced_count")) return;
	db.exec(
		"ALTER TABLE procedures ADD COLUMN reinforced_count INTEGER NOT NULL DEFAULT 0",
	);
...
function migrateProcedureScopeColumns(db: DatabaseSync): void {
	...
	if (!colNames.has("scope")) {
		db.exec(
			`ALTER TABLE procedures ADD COLUMN scope TEXT NOT NULL DEFAULT 'global'`,
		);
	}
	if (!colNames.has("scope_target")) {
...
function migrateContradictionsTable(db: DatabaseSync): void {
  ...
  `);
	db.exec(
		"CREATE INDEX IF NOT EXISTS idx_contradictions_new ON contradictions(fact_id_new)",
	);
	db.exec(
		"CREATE INDEX IF NOT EXISTS idx_contradictions_old ON contradictions(fact_id_old)",
	);
	db.exec(
	...
	const cols = db.prepare("PRAGMA table_info(contradictions)").all() as Array<{
		name: string;
	}>;
	if (!cols.some((c) => c.name === "old_fact_original_confidence")) {
		db.exec(
			"ALTER TABLE contradictions ADD COLUMN old_fact_original_confidence REAL",
		);
	}
}
...
function migrateRecallLog(db: DatabaseSync): void {
  ...
  `);
	db.exec(
		"CREATE INDEX IF NOT EXISTS idx_recall_log_time ON recall_log(occurred_at)",
	);
...
function migrateDecayFreezeColumn(db: DatabaseSync): void {
	...
	db.exec(
		"ALTER TABLE facts ADD COLUMN decay_freeze_until INTEGER DEFAULT NULL",
	);
	db.exec(
...
function migrateProvenanceColumns(db: DatabaseSync): void {
	...
	if (!colNames.has("provenance_session")) {
		...
		if (colNames.has("source_session")) {
			db.exec(
				"UPDATE facts SET provenance_session = source_session WHERE source_session IS NOT NULL",
			);
		}
	}
...
function migrateVerifiedFactsAddFk(db: DatabaseSync): void {
	const tableInfo = db
		.prepare(
			"SELECT name FROM sqlite_master WHERE type='table' AND name='verified_facts'",
		)
		.get();
	if (!tableInfo) return;
	const fkCheck = db.prepare("PRAGMA foreign_key_list(verified_facts)").all() as
		| Array<{ table: string }>
		| undefined;
...
function migrateReinforcementLogTable(db: DatabaseSync): void {
  ...
  `);
	db.exec(
		"CREATE INDEX IF NOT EXISTS idx_rl_fact_id ON reinforcement_log(fact_id)",
	);
	db.exec(
		"CREATE INDEX IF NOT EXISTS idx_rl_occurred ON reinforcement_log(occurred_at)",
	);
	...
	const hasFk = fkList.some(
		(fk) => fk.from === "fact_id" && fk.table === "facts",
	);
	if (!hasFk) {
		createTransaction(db, () => {
...
function migrateReinforcedCountToReal(db: DatabaseSync): void {
	const factsCols = db.prepare("PRAGMA table_info(facts)").all() as Array<{
		name: string;
		type: string;
	}>;
	const factsReinforcedCol = factsCols.find(
		(c) => c.name === "reinforced_count",
	);
	if (factsReinforcedCol && factsReinforcedCol.type !== "REAL") {
		createTransaction(db, () => {
			db.exec("DROP INDEX IF EXISTS idx_facts_reinforced");
			db.exec(
				"ALTER TABLE facts ADD COLUMN reinforced_count_real REAL NOT NULL DEFAULT 0",
			);
			db.exec(
				"UPDATE facts SET reinforced_count_real = CAST(reinforced_count AS REAL)",
			);
			db.exec("ALTER TABLE facts DROP COLUMN reinforced_count");
			db.exec(
				"ALTER TABLE facts RENAME COLUMN reinforced_count_real TO reinforced_count",
			);
			db.exec(
				"CREATE INDEX IF NOT EXISTS idx_facts_reinforced ON facts(reinforced_count) WHERE reinforced_count > 0",
			);
	...
	const proceduresCols = db
		.prepare("PRAGMA table_info(procedures)")
		.all() as Array<{
		name: string;
		type: string;
	}>;
	const proceduresReinforcedCol = proceduresCols.find(
		(c) => c.name === "reinforced_count",
	);
	if (proceduresReinforcedCol && proceduresReinforcedCol.type !== "REAL") {
		createTransaction(db, () => {
			...
			db.exec(
				"ALTER TABLE procedures ADD COLUMN reinforced_count_real REAL NOT NULL DEFAULT 0",
			);
			db.exec(
				"UPDATE procedures SET reinforced_count_real = CAST(reinforced_count AS REAL)",
			);
			db.exec("ALTER TABLE procedures DROP COLUMN reinforced_count");
			db.exec(
				"ALTER TABLE procedures RENAME COLUMN reinforced_count_real TO reinforced_count",
			);
			db.exec(
				"CREATE INDEX IF NOT EXISTS idx_procedures_reinforced ON procedures(reinforced_count) WHERE reinforced_count > 0",
			);
...
function migrateImplicitSignalsTable(db: DatabaseSync): void {
  ...
  `);
	db.exec(
		"CREATE INDEX IF NOT EXISTS idx_is_created ON implicit_signals(created_at)",
	);
	db.exec(
		"CREATE INDEX IF NOT EXISTS idx_is_polarity ON implicit_signals(polarity)",
	);
	db.exec(
...
function migrateFeedbackTrajectoriesTable(db: DatabaseSync): void {
  ...
  `);
	db.exec(
		"CREATE INDEX IF NOT EXISTS idx_ft_session ON feedback_trajectories(session_file)",
	);
	db.exec(
		"CREATE INDEX IF NOT EXISTS idx_ft_outcome ON feedback_trajectories(outcome)",
	);
	db.exec(
		"CREATE INDEX IF NOT EXISTS idx_ft_created_at ON feedback_trajectories(created_at)",
	);
}
...
function migrateFeedbackEffectivenessTable(db: DatabaseSync): void {
  ...
  `);
	db.exec(
		"CREATE INDEX IF NOT EXISTS idx_fe_measured ON feedback_effectiveness(measured_at)",
	);
	...
	const hasFk = fkList.some(
		(fk) => fk.from === "rule_id" && fk.table === "facts",
	);
	if (!hasFk) {
		createTransaction(db, () => {
			db.exec(
				"ALTER TABLE feedback_effectiveness RENAME TO feedback_effectiveness_v1",
			);
			db.exec(`
...
function migrateTrimMetricsTable(db: DatabaseSync): void {
  ...
  `);
	db.exec(
		"CREATE INDEX IF NOT EXISTS idx_trim_metrics_trimmed_at ON trim_metrics(trimmed_at)",
	);
	db.exec(
		"CREATE INDEX IF NOT EXISTS idx_trim_metrics_fact_id ON trim_metrics(fact_id)",
	);
}
...
function migrateEpisodesTable(db: DatabaseSync): void {
  ...
  `);
	...
	db.exec(
		"CREATE INDEX IF NOT EXISTS idx_episodes_procedure ON episodes(procedure_id)",
	);
	db.exec(
	...
	const hasOldFtsSchema =
		ftsInfo?.sql?.includes("content='episodes'") ||
		ftsInfo?.sql?.includes('content="episodes"');
	...
	if (!ftsExists || hasOldFtsSchema) {
      ...
      `);
			...
			db.exec(
				"INSERT INTO episodes_fts(rowid, event, context) SELECT rowid, event, context FROM episodes",
			);
		});
		migrate();
	}
  ...
  `);
	db.exec(
		"CREATE INDEX IF NOT EXISTS idx_episode_relations_episode ON episode_relations(episode_id)",
	);
	db.exec(
		"CREATE INDEX IF NOT EXISTS idx_episode_relations_target ON episode_relations(target_id)",
	);
}
