...
export class VectorDB {
	 ...
	 */
	private async ensureMemoriesWhyColumnIfMissing(
		table: lancedb.Table,
	): Promise<void> {
		try {
			const schema = await table.schema();
			...
			this.logWarn(
				`memory-hybrid: could not add 'why' column to LanceDB: ${err}`,
			);
			this.schemaValid = false;
		}
	}
	...
	async getSemanticQueryCacheMatch(
		...
		try {
			...
			await this.ensureInitialized();
			if (
				!this.lanceDbAvailable ||
				this.lanceInitFailed ||
				!this.semanticQueryCacheTable
			)
				return null;
	 ...
	 */
	async optimize(olderThanMs: number = 7 * 24 * 60 * 60 * 1000): Promise<{
		compacted: number;
		removedFragments: number;
		freedBytes: number;
	}> {
