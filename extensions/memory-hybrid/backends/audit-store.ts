...
const SENSITIVE_KEYS =
	/(api[_-]?key|password|secret|authorization|bearer|cookie|\btoken\b)/i;
...
export class AuditStore extends BaseSqliteStore {
	...
	query(opts: {
		...
		if (opts.targetContains) {
			const escapedTarget = opts.targetContains
				.replace(/\\/g, "\\\\")
				.replace(/[%_]/g, "\\$&");
		...
		return rows.map((r) => ({
			...
			error: r.error,
			context: r.context
				? (JSON.parse(r.context) as Record<string, unknown>)
				: null,
	...
	summary24h(): {
		...
		const rows = this.liveDb
			.prepare(
				"SELECT outcome, agent_id, COUNT(*) as c FROM audit_log WHERE timestamp >= ? GROUP BY outcome, agent_id",
			)
...
export function auditDbPathForMemorySqlite(
