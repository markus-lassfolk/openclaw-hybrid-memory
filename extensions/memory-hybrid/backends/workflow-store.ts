...
export class WorkflowStore extends BaseSqliteStore {
	...
	getByGoal(keywords: string[], limit = 20): WorkflowTrace[] {
		...
		return this.runSqliteOp("getByGoal", () => {
			...
			const candidates = this.liveDb
				.prepare(
					"SELECT * FROM workflow_traces ORDER BY created_at DESC LIMIT 500",
				)
	...
	getPatterns(options?: {
		...
		return this.runSqliteOp("getPatterns", () => {
			...
			const allRows = this.liveDb
				.prepare(
					"SELECT goal, tool_sequence, outcome, duration_ms FROM workflow_traces ORDER BY created_at DESC",
				)
				.all() as {
				goal: string;
				tool_sequence: string;
