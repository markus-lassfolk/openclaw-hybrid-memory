...
export class VectorDB {
  ...
  async runWithAutoOptimizePaused<T>(fn: () => Promise<T>): Promise<T> {
    const current = autoOptimizePauseByPath.get(this.dbPath) ?? 0;
    autoOptimizePauseByPath.set(this.dbPath, current + 1);
    try {
      return await fn();
    } finally {
      const next = (autoOptimizePauseByPath.get(this.dbPath) ?? 1) - 1;
      if (next <= 0) autoOptimizePauseByPath.delete(this.dbPath);
      else autoOptimizePauseByPath.set(this.dbPath, next);
    }
  }
  ...
  async store(entry: {
    ...
    try {
      ...
      this.storeCount++;
      if (
        !optimizingByPath.get(this.dbPath) &&
        (autoOptimizePauseByPath.get(this.dbPath) ?? 0) === 0 &&
        ...
        this.optimize(24 * 60 * 60 * 1000)
          .then(() => {
            optimizeFailuresByPath.delete(this.dbPath);
          })
          .catch((err) => {
            const failures = (optimizeFailuresByPath.get(this.dbPath) ?? 0) + 1;
            optimizeFailuresByPath.set(this.dbPath, failures);
