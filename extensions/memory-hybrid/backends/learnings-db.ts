...
export class LearningsDB extends BaseSqliteStore {
	...
	incrementRecurrence(id: string): LearningEntry {
		...
		this.liveDb
			.prepare(
				"UPDATE learnings SET recurrence = recurrence + 1, updated_at = ? WHERE id = ?",
			)
			.run(now, id);
	...
	transition(
		...
		if (newStatus === "promoted" && !promotedTo) {
			throw new Error(
				`promotedTo is required when transitioning to "promoted" status`,
			);
		}
		...
		params.push(id);
		this.liveDb
			.prepare(`UPDATE learnings SET ${sets.join(", ")} WHERE id = ?`)
			.run(...params);
	...
	get(id: string): LearningEntry | null {
		const row = this.liveDb
			...
			.get(id) as unknown as LearningRow | undefined;
		if (!row) return null;
	...
	getBySlug(slug: string): LearningEntry | null {
		const row = this.liveDb
			.prepare("SELECT * FROM learnings WHERE slug = ?")
			.get(slug) as unknown as LearningRow | undefined;
		if (!row) return null;
		return this.rowToEntry(row);
	}
	...
	count(filter?: {
		...
		const row = this.liveDb.prepare(query).get(...params) as unknown as {
			n: number;
		};
		return row.n;
	}
	...
	findByAreaContent(
		...
		const row = this.liveDb
			.prepare(
				"SELECT * FROM learnings WHERE type = ? AND area = ? AND content = ? AND status = ?",
			)
	...
	prune(olderThanDays: number): number {
		...
		const result = this.liveDb
			.prepare(
				`DELETE FROM learnings WHERE status IN ('promoted', 'wont_promote') AND updated_at <= ?`,
			)
			.run(cutoff);
	...
	private nextSeq(type: LearningEntryType): number {
		...
		const rows = this.liveDb
			.prepare("SELECT slug FROM learnings WHERE slug LIKE ?")
