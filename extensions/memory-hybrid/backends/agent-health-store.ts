...
export class AgentHealthStore extends BaseSqliteStore {
	...
	listAll(): AgentHealthRecord[] {
		...
		return rows.map((r) => ({
			...
			nextAgent: r.next_agent_json
				? (JSON.parse(r.next_agent_json) as string[])
				: [],
		}));
	}
	...
	get(agentId: string): AgentHealthRecord | null {
		...
		return {
			...
			nextAgent: row.next_agent_json
				? (JSON.parse(row.next_agent_json) as string[])
				: [],
		};
	}
	...
	prune(retentionDays = 30): number {
		...
		const r = this.liveDb
			.prepare("DELETE FROM agent_health WHERE updated_at < ?")
			.run(cutoff);
		return Number(r.changes ?? 0);
	}
}

export function agentHealthDbPathForMemorySqlite(
 ...
 */
export function mergeAgentHealthDashboard(
	forge: ForgeTaskItem[],
	dbRows: AgentHealthRecord[],
): AgentHealthView[] {
	const byId = new Map<string, AgentHealthRecord>();
	for (const r of dbRows) {
		byId.set(r.agentId.toLowerCase(), {
			...r,
			agentId: r.agentId.toLowerCase(),
		});
	}
	...
	const ids = new Set<string>([
		...DEFAULT_AGENT_IDS.map((a) => a.toLowerCase()),
		...byId.keys(),
	]);
...
function computeHealthView(
	...
	const wasDegradedFromStale =
		status === "stale" && (r.errorCount >= 2 || r.anomalyScore >= 0.6);
	...
	let score = 75;
	if (status === "healthy")
		score = 88 + Math.min(12, Math.floor((1 - r.anomalyScore) * 12));
