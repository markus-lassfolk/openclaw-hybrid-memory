...
export class NarrativesDB extends BaseSqliteStore {
	...
	store(input: StoreNarrativeInput): NarrativeEntry {
		...
		const row = this.liveDb
			.prepare(
				"SELECT * FROM narratives WHERE session_id = ? AND tag = ? LIMIT 1",
			)
	...
	listRecent(
		limit = 3,
		tag: "session" | "weekly-rollup" | "all" = "session",
	): NarrativeEntry[] {
		const sql =
			tag === "all"
		...
		const rows =
			tag === "all"
				...
				: (this.liveDb
						.prepare(sql)
						.all(tag, limit) as unknown as NarrativeRow[]);
	...
	listBySession(
		sessionId: string,
		limit = 5,
		tag: "session" | "weekly-rollup" | "all" = "all",
	): NarrativeEntry[] {
		const sql =
			tag === "all"
		...
		const rows =
			tag === "all"
				? (this.liveDb
						.prepare(sql)
						.all(sessionId, limit) as unknown as NarrativeRow[])
				: (this.liveDb
						.prepare(sql)
						.all(sessionId, tag, limit) as unknown as NarrativeRow[]);
		return rows.map((r) => this.rowToEntry(r));
	}
	...
	pruneOlderThan(days: number): number {
		...
		const result = this.liveDb
			.prepare("DELETE FROM narratives WHERE created_at < ?")
			.run(cutoff);
