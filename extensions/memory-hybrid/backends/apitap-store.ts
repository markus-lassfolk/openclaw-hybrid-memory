...
export class ApitapStore extends BaseSqliteStore {
	...
	create(input: CreateApitapEndpointInput): ApitapEndpoint {
		...
		const sampleResponseStr =
			typeof input.sampleResponse === "string"
				? input.sampleResponse
				: JSON.stringify(input.sampleResponse ?? null);

		// Compute expiresAt: explicit value wins; endpointTtlDays computes if no explicit expiresAt
		let expiresAt: string | null =
			input.expiresAt !== undefined ? (input.expiresAt ?? null) : null;
		if (
			expiresAt === null &&
			!("expiresAt" in input) &&
			input.endpointTtlDays &&
			input.endpointTtlDays > 0
		) {
			expiresAt = new Date(
				Date.now() + input.endpointTtlDays * 24 * 60 * 60_000,
			).toISOString();
		}
	...
	getById(id: string): ApitapEndpoint | null {
		const row = this.liveDb
			.prepare("SELECT * FROM apitap_endpoints WHERE id = ?")
	...
	list(filter?: ApitapEndpointFilter): ApitapEndpoint[] {
		...
		if (filter?.siteUrl) {
			const escapedSiteUrl = filter.siteUrl
				.replace(/\\/g, "\\\\")
				.replace(/[%_]/g, "\\$&");
	...
	updateStatus(
		...
		const result = this.liveDb
			.prepare(
				"UPDATE apitap_endpoints SET status = ?, updated_at = ? WHERE id = ?",
			)
			.run(status, now, id);
	...
	count(status?: ApitapEndpointStatus): number {
		const row = status
			? (this.liveDb
					.prepare(
						"SELECT COUNT(*) as n FROM apitap_endpoints WHERE status = ?",
					)
					.get(status) as {
					n: number;
				})
			: (this.liveDb
					.prepare("SELECT COUNT(*) as n FROM apitap_endpoints")
					.get() as { n: number });
		return row.n;
	}
	...
	countForSession(sessionId: string): number {
		const row = this.liveDb
			.prepare(
				"SELECT COUNT(*) as n FROM apitap_endpoints WHERE session_id = ?",
			)
			.get(sessionId) as {
			n: number;
		};
