...
export class EventLog extends BaseSqliteStore {
	...
	appendBatch(entries: Omit<EventLogEntry, "id" | "createdAt">[]): string[] {
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
		...
		const insertAll = createTransaction(
			this.liveDb,
	...
	getByTimeRange(
		...
		let query =
			"SELECT * FROM event_log WHERE timestamp >= ? AND timestamp <= ?";
		if (eventType) {
			...
			try {
				const lineStream = Readable.from(
					(async function* () {
						...
						for (const r of stmt.iterate(cutoff, month) as Iterable<
				...
				const deleteBatch = createTransaction(
					this.liveDb,
					(batch: string[]) => {
