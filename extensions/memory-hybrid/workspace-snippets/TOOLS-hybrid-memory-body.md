...
- **Credentials vault (when enabled):** Secrets live in the vault only; memory holds pointers. Use **`credential_get`** / **`credential_store`** — not `memory_store` for secrets. CLI: `credentials list`, `credentials get --service <name>`, `credentials vault-status`.
