...
- **More detail:** Workspace skill `skills/hybrid-memory/` (`SKILL.md` + `references/memory-optimization.md` — copied on first gateway start if missing, or refreshed via `hybrid-mem install`) and repo docs: [Memory Protocol](https://github.com/markus-lassfolk/openclaw-hybrid-memory/blob/main/docs/MEMORY-PROTOCOL.md).
