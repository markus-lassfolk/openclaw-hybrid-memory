...
import {
	...
	runAutoClassify,
	runClassifyForCli,
} from "./services/auto-classifier.js";
import {
	detectCategory as detectCategoryUtil,
	shouldCapture as shouldCaptureUtil,
...
import {
	AuditStore,
	auditDbPathForMemorySqlite,
} from "./backends/audit-store.js";
import {
...
function shouldCapture(text: string): boolean {
	return shouldCaptureUtil(
		text,
		runtimeRef.value?.cfg.captureMaxChars ?? 5000,
		getMemoryTriggers(),
	);
}
...
function runMemoryHybridRegister(api: ClawdbotPluginApi): void {
	...
	try {
		...
		const toParse =
			rawPc && typeof rawPc === "object" && !Array.isArray(rawPc)
				? (() => {
						const clone = shallowClonePluginConfigForGatewayMerge(
	...
	if (old) {
		...
		if (old.timers.classifyTimer.value)
			clearInterval(old.timers.classifyTimer.value);
		if (old.timers.classifyStartupTimeout.value)
			clearTimeout(old.timers.classifyStartupTimeout.value);
		if (old.timers.proposalsPruneTimer.value)
			clearInterval(old.timers.proposalsPruneTimer.value);
		if (old.timers.languageKeywordsTimer.value)
			clearInterval(old.timers.languageKeywordsTimer.value);
		if (old.timers.languageKeywordsStartupTimeout.value)
			clearTimeout(old.timers.languageKeywordsStartupTimeout.value);
		if (old.timers.postUpgradeTimeout.value)
			clearTimeout(old.timers.postUpgradeTimeout.value);
		if (old.timers.passiveObserverTimer.value)
			clearInterval(old.timers.passiveObserverTimer.value);
		if (old.timers.watchdogTimer.value)
			clearInterval(old.timers.watchdogTimer.value);
	...
	if (cfg.sensorSweep.enabled) {
		try {
			...
			api.logger.info(
				`memory-hybrid: event bus initialized at ${eventBusPath}`,
			);
		} catch (err) {
			capturePluginError(err instanceof Error ? err : new Error(String(err)), {
				subsystem: "registration",
	...
	const pythonBridge = cfg.documents.enabled
		? new PythonBridge(cfg.documents.pythonPath)
		: null;
	...
	if (cfg.contextualVariants.enabled) {
		const variantGenerator = new ContextualVariantGenerator(
			cfg.contextualVariants,
			dbContext.openai,
		);
		variantQueue = new VariantGenerationQueue(
			variantGenerator,
			async (factId, variantType, variants) => {
	...
	try {
		...
		api.logger.info(
			`memory-hybrid: learnings DB initialized at ${learningsDbPath}`,
		);
	} catch (err) {
		capturePluginError(err instanceof Error ? err : new Error(String(err)), {
			subsystem: "registration",
	...
	try {
		...
		if (ahPath) {
			...
			api.logger.info(
				`memory-hybrid: agent health store initialized at ${ahPath}`,
			);
		}
	} catch (err) {
		capturePluginError(err instanceof Error ? err : new Error(String(err)), {
	...
	import("./services/context-engine.js")
		...
		.catch((err: unknown) => {
			api.logger.warn?.(
				`memory-hybrid: ContextEngine registration skipped: ${err}`,
			);
		});
			...
			void (async () => {
				try {
					...
					if (currentCrontab.includes("hybrid-mem backup")) {
						...
						api.logger.debug?.(
							"memory-hybrid: boot-check -- weekly backup cron already present",
						);
						return;
					}
					...
					api.logger.debug?.(
						`memory-hybrid: boot-check -- could not verify backup cron (non-fatal): ${err}`,
					);
				}
			})();
		});
