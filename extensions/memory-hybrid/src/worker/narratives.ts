...
import {
	NARRATIVE_CHAT_TIMEOUT_MS,
	getSessionLogFileSuffix,
...
export async function buildDailyNarrative(
	params: BuildDailyNarrativeParams,
): Promise<boolean> {
	const {
		sessionId,
		eventLog,
	...
	try {
		...
		const workflows = workflowStore
			? workflowStore.list({ sessionId, limit: MAX_WORKFLOWS_FOR_PROMPT })
			: [];
		...
		const eventsBlock = clip(
			events
				.map((e) => {
					const content = (() => {
						if (
							e.eventType === "action_taken" &&
							typeof e.content.action === "string"
						) {
							return `action=${e.content.action}`;
						}
						if (
							e.eventType === "decision_made" &&
							typeof e.content.decision === "string"
						) {
							return `decision=${e.content.decision}`;
						}
						if (typeof e.content.text === "string")
		...
		const transient =
			isAbortOrTransientLlmError(err) || is500OrWrapped(asError);
		if (!transient) {
		...
		if (transient) {
			logger.info?.(
				`memory-hybrid: narrative skipped (LLM unavailable or aborted) for ${sessionId}: ${detail}`,
			);
		} else {
			logger.warn(
				`memory-hybrid: narrative build failed for ${sessionId}: ${err}`,
			);
		}
		return false;
	}
