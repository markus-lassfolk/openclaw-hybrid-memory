...
export const ARCHITECTURE_CENTER = {
  ...
  classificationHeuristics: [
    {
      prompt: "If removed, would live turn-time capture, persistence, retrieval, injection, or provenance break?",
    ...
    {
      prompt: "Can it be disabled or replaced without breaking the baseline memory runtime contract?",
...
export function allArchitectureOwnershipPaths(): string[] {
  return [...ARCHITECTURE_CENTER.coreRuntime, ...ARCHITECTURE_CENTER.adjacentSubsystems].flatMap(
    (area) => area.ownership,
  );
}
