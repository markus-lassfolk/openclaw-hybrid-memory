...
function parseHaConfig(
	raw: Record<string, unknown> | undefined,
): HomeAssistantSensorConfig | undefined {
...
export function parseSensorSweepConfig(
	...
	const garmin: GarminSensorConfig = {
		...
		entityPrefix:
			typeof garminRaw?.entityPrefix === "string" &&
			garminRaw.entityPrefix.trim().length > 0
				? garminRaw.entityPrefix.trim()
				: "sensor.garmin",
	};

	const sessionHistoryRaw = raw?.sessionHistory as
	...
	const sessionHistory: SessionHistorySensorConfig = {
		...
		recentSessions:
			typeof sessionHistoryRaw?.recentSessions === "number" &&
			sessionHistoryRaw.recentSessions >= 1
				? Math.floor(sessionHistoryRaw.recentSessions)
				: 10,
	};

	const memoryPatternsRaw = raw?.memoryPatterns as
		| Record<string, unknown>
		| undefined;
	const memoryPatterns: MemoryPatternsSensorConfig = {
		...
		hotAccessThreshold:
			typeof memoryPatternsRaw?.hotAccessThreshold === "number" &&
			memoryPatternsRaw.hotAccessThreshold >= 1
				? Math.floor(memoryPatternsRaw.hotAccessThreshold)
				: 3,
		staleAfterDays:
			typeof memoryPatternsRaw?.staleAfterDays === "number" &&
			memoryPatternsRaw.staleAfterDays >= 1
	...
	const github: GitHubSensorConfig = {
		...
		repo:
			typeof githubRaw?.repo === "string" &&
			/^[A-Za-z0-9._-]+\/[A-Za-z0-9._-]+$/.test(githubRaw.repo.trim())
				? githubRaw.repo.trim()
				: undefined,
		includeReviewRequests: githubRaw?.includeReviewRequests !== false,
		staleIssueDays:
			typeof githubRaw?.staleIssueDays === "number" &&
			githubRaw.staleIssueDays >= 1
				? Math.floor(githubRaw.staleIssueDays)
				: 7,
	};

	const haAnomalyRaw = raw?.homeAssistantAnomaly as
		| Record<string, unknown>
		| undefined;
	const homeAssistantAnomaly: HomeAssistantAnomalySensorConfig = {
		...
		watchEntities: Array.isArray(haAnomalyRaw?.watchEntities)
			? (haAnomalyRaw.watchEntities as unknown[]).filter(
					(e): e is string => typeof e === "string",
				)
			: [],
	};

	const systemHealthRaw = raw?.systemHealth as
		| Record<string, unknown>
		| undefined;
	...
	const weather: WeatherSensorConfig = {
		...
		location:
			typeof weatherRaw?.location === "string" &&
			weatherRaw.location.trim().length > 0
	...
	const yarbo: YarboSensorConfig = {
		...
		entityPrefix:
			typeof yarboRaw?.entityPrefix === "string" &&
			yarboRaw.entityPrefix.trim().length > 0
