...
export function parsePassiveObserverConfig(cfg: Record<string, unknown>): PassiveObserverConfig {
  ...
  return {
    ...
    intervalMinutes: typeof observerRaw?.intervalMinutes === "number" && observerRaw.intervalMinutes >= 1
      ? Math.floor(observerRaw.intervalMinutes)
      : 15,
    model: typeof observerRaw?.model === "string" && observerRaw.model.trim().length > 0
      ? observerRaw.model.trim()
      : undefined,
    maxCharsPerChunk: typeof observerRaw?.maxCharsPerChunk === "number" && observerRaw.maxCharsPerChunk >= 100
    ...
    minImportance:
      typeof observerRaw?.minImportance === "number" && observerRaw.minImportance >= 0 && observerRaw.minImportance <= 1
        ...
        : 0.5,
    deduplicationThreshold: typeof observerRaw?.deduplicationThreshold === "number" &&
        observerRaw.deduplicationThreshold >= 0 &&
        observerRaw.deduplicationThreshold <= 1
      ? observerRaw.deduplicationThreshold
      : 0.92,
    sessionsDir: typeof observerRaw?.sessionsDir === "string" && observerRaw.sessionsDir.trim().length > 0
...
export function parseIdentityReflectionConfig(cfg: Record<string, unknown>): IdentityReflectionConfig {
  ...
  return {
    ...
    defaultWindow: typeof raw?.defaultWindow === "number" && raw.defaultWindow > 0
    ...
    maxInsightsPerRun: typeof raw?.maxInsightsPerRun === "number" && raw.maxInsightsPerRun >= 1
      ? Math.min(20, Math.floor(raw.maxInsightsPerRun))
      : 8,
    questions: parsedQuestions.length > 0
      ? parsedQuestions
...
export function parseProceduresConfig(cfg: Record<string, unknown>): ProceduresConfig {
  ...
  return {
    ...
    maxInjectionTokens: typeof proceduresRaw?.maxInjectionTokens === "number" &&
...
export function parseExtractionConfig(cfg: Record<string, unknown>): ExtractionConfig {
  ...
  return {
    ...
    extractionModelTier: (() => {
      const v = typeof extractionRaw?.extractionModelTier === "string"
