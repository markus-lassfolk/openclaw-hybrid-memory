...
export function parsePassiveObserverConfig(
	cfg: Record<string, unknown>,
): PassiveObserverConfig {
	const observerRaw = cfg.passiveObserver as
		| Record<string, unknown>
		| undefined;
	return {
		enabled: observerRaw?.enabled === true,
		intervalMinutes:
			typeof observerRaw?.intervalMinutes === "number" &&
			observerRaw.intervalMinutes >= 1
				? Math.floor(observerRaw.intervalMinutes)
				: 15,
		model:
			typeof observerRaw?.model === "string" &&
			observerRaw.model.trim().length > 0
				? observerRaw.model.trim()
				: undefined,
		maxCharsPerChunk:
			typeof observerRaw?.maxCharsPerChunk === "number" &&
			observerRaw.maxCharsPerChunk >= 100
				? Math.floor(observerRaw.maxCharsPerChunk)
				: 8000,
		minImportance:
			typeof observerRaw?.minImportance === "number" &&
			observerRaw.minImportance >= 0 &&
			observerRaw.minImportance <= 1
		...
		deduplicationThreshold:
			...
			observerRaw.deduplicationThreshold <= 1
				...
				: 0.92,
		sessionsDir:
			typeof observerRaw?.sessionsDir === "string" &&
			observerRaw.sessionsDir.trim().length > 0
...
export function parseReflectionConfig(
	...
	return {
		...
		model:
			typeof reflectionRaw?.model === "string"
				? reflectionRaw.model
				: undefined,
		defaultWindow:
			typeof reflectionRaw?.defaultWindow === "number" &&
			reflectionRaw.defaultWindow > 0
				? Math.min(90, Math.floor(reflectionRaw.defaultWindow))
				: 14,
		minObservations:
			typeof reflectionRaw?.minObservations === "number" &&
			reflectionRaw.minObservations >= 1
				? Math.floor(reflectionRaw.minObservations)
				: 2,
	};
}

export function parseIdentityReflectionConfig(
	cfg: Record<string, unknown>,
): IdentityReflectionConfig {
	...
	return {
		...
		defaultWindow:
			typeof raw?.defaultWindow === "number" && raw.defaultWindow > 0
				...
				: 30,
		minInsights:
			typeof raw?.minInsights === "number" && raw.minInsights >= 1
				? Math.floor(raw.minInsights)
				: 3,
		maxInsightsPerRun:
		...
		questions:
			parsedQuestions.length > 0
				? parsedQuestions
				: DEFAULT_IDENTITY_REFLECTION_QUESTIONS,
	};
}

export function parseIdentityPromotionConfig(
	cfg: Record<string, unknown>,
): IdentityPromotionConfig {
	...
	return {
		...
		lookbackDays:
			typeof raw?.lookbackDays === "number" && raw.lookbackDays >= 1
				? Math.min(365, Math.floor(raw.lookbackDays))
				: 90,
		minDurableReflections:
			typeof raw?.minDurableReflections === "number" &&
			raw.minDurableReflections >= 1
		...
		similarityThreshold:
			...
			raw.similarityThreshold >= 0 &&
			raw.similarityThreshold <= 1
				? raw.similarityThreshold
				: 0.72,
		maxPromotionsPerRun:
			typeof raw?.maxPromotionsPerRun === "number" &&
			raw.maxPromotionsPerRun >= 1
				? Math.min(50, Math.floor(raw.maxPromotionsPerRun))
				: 8,
	};
}

export function parseProceduresConfig(
	cfg: Record<string, unknown>,
): ProceduresConfig {
	...
	return {
		...
		sessionsDir:
			typeof proceduresRaw?.sessionsDir === "string" &&
			proceduresRaw.sessionsDir.length > 0
		...
		minSteps:
			typeof proceduresRaw?.minSteps === "number" && proceduresRaw.minSteps >= 1
				...
				: 2,
		validationThreshold:
			typeof proceduresRaw?.validationThreshold === "number" &&
			proceduresRaw.validationThreshold >= 1
				? Math.floor(proceduresRaw.validationThreshold)
				: 3,
		skillTTLDays:
			typeof proceduresRaw?.skillTTLDays === "number" &&
			proceduresRaw.skillTTLDays >= 1
				? Math.floor(proceduresRaw.skillTTLDays)
				: 30,
		skillsAutoPath:
			typeof proceduresRaw?.skillsAutoPath === "string" &&
			proceduresRaw.skillsAutoPath.length > 0
				? proceduresRaw.skillsAutoPath
				: "skills/auto",
		requireApprovalForPromote:
			proceduresRaw?.requireApprovalForPromote !== false,
		maxInjectionTokens:
...
function parsePreFilterConfig(
	raw: Record<string, unknown> | undefined,
): ExtractionPreFilterConfig | undefined {
	...
	return {
		...
		endpoint:
			typeof raw.endpoint === "string" && raw.endpoint.trim().length > 0
				...
				: undefined,
		maxCharsPerSession:
			typeof raw.maxCharsPerSession === "number" &&
			raw.maxCharsPerSession >= 100
...
export function parseExtractionConfig(
	...
	return {
		...
		extractionModel:
			typeof extractionRaw?.extractionModel === "string" &&
			extractionRaw.extractionModel.trim().length > 0
				? extractionRaw.extractionModel.trim()
				: undefined,
		implicitModel:
			typeof extractionRaw?.implicitModel === "string" &&
			extractionRaw.implicitModel.trim().length > 0
				? extractionRaw.implicitModel.trim()
				: undefined,
		verificationModel:
			typeof extractionRaw?.verificationModel === "string" &&
			extractionRaw.verificationModel.trim().length > 0
				? extractionRaw.verificationModel.trim()
				: undefined,
		preFilter: parsePreFilterConfig(
			extractionRaw?.preFilter as Record<string, unknown> | undefined,
		),
		extractionModelTier: (() => {
			const v =
