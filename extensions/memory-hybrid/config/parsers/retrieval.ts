...
export function parseAutoClassifyConfig(
	...
	return {
		...
		minFactsForNewCategory:
			typeof acCfg?.minFactsForNewCategory === "number"
				? acCfg.minFactsForNewCategory
				: 10,
		discoveryIntervalHours:
			typeof acCfg?.discoveryIntervalHours === "number" &&
			acCfg.discoveryIntervalHours >= 0
				? acCfg.discoveryIntervalHours
				: 72,
	};
}

export function parseAutoRecallConfig(
	cfg: Record<string, unknown>,
): AutoRecallConfig {
	...
	if (typeof arRaw === "object" && arRaw !== null && !Array.isArray(arRaw)) {
		...
		const format =
			...
			VALID_FORMATS.includes(
				ar.injectionFormat as (typeof VALID_FORMATS)[number],
			)
				? (ar.injectionFormat as AutoRecallInjectionFormat)
				: "full";
		const limit =
			typeof ar.limit === "number" && ar.limit > 0 ? Math.floor(ar.limit) : 10;
		const minScore =
			typeof ar.minScore === "number" && ar.minScore >= 0 && ar.minScore <= 1
				? ar.minScore
				: 0.3;
		const preferLongTerm = ar.preferLongTerm === true;
		const useImportanceRecency = ar.useImportanceRecency === true;
		const entityLookupRaw = ar.entityLookup as
			| Record<string, unknown>
			| undefined;
		const maxAutoRaw =
			typeof entityLookupRaw?.maxAutoEntities === "number" &&
			entityLookupRaw.maxAutoEntities > 0
		...
		const entityLookup: EntityLookupConfig = {
			...
			entities: Array.isArray(entityLookupRaw?.entities)
				? (entityLookupRaw.entities as string[]).filter(
						(e) => typeof e === "string" && e.length > 0,
					)
				: [],
			maxFactsPerEntity:
				typeof entityLookupRaw?.maxFactsPerEntity === "number" &&
				entityLookupRaw.maxFactsPerEntity > 0
		...
		const summaryThreshold =
			typeof ar.summaryThreshold === "number" && ar.summaryThreshold >= 0
				? ar.summaryThreshold
				: 300;
		const summaryMaxChars =
			typeof ar.summaryMaxChars === "number" && ar.summaryMaxChars > 0
				? Math.min(ar.summaryMaxChars, 500)
				: 80;
		...
		const summarizeModel =
			typeof ar.summarizeModel === "string" ? ar.summarizeModel : undefined;
		const directivesRaw = ar.retrievalDirectives as
			| Record<string, unknown>
			| undefined;
		...
		const retrievalDirectives: RetrievalDirectivesConfig = {
			...
			limit:
				typeof directivesRaw?.limit === "number" && directivesRaw.limit > 0
					...
					: 3,
			maxPerPrompt:
				typeof directivesRaw?.maxPerPrompt === "number" &&
				directivesRaw.maxPerPrompt > 0
					? Math.floor(directivesRaw.maxPerPrompt)
					: 4,
		};
		const progressiveMaxCandidates =
			typeof ar.progressiveMaxCandidates === "number" &&
			ar.progressiveMaxCandidates > 0
				? Math.floor(ar.progressiveMaxCandidates)
				: 15;
		let progressiveIndexMaxTokens: number | undefined =
			typeof ar.progressiveIndexMaxTokens === "number" &&
			ar.progressiveIndexMaxTokens > 0
		...
		if (
			(format === "progressive" || format === "progressive_hybrid") &&
		...
		const progressivePinnedRecallCount =
			typeof ar.progressivePinnedRecallCount === "number" &&
			ar.progressivePinnedRecallCount >= 0
			...
			recallTimingRaw === true
				...
				: recallTimingRaw === false
					...
					: typeof recallTimingRaw === "string" &&
							(VALID_RECALL_TIMING as readonly string[]).includes(
								recallTimingRaw,
							)
						? (recallTimingRaw as (typeof VALID_RECALL_TIMING)[number])
						: "off";
		const scopeFilterRaw = ar.scopeFilter as
			| Record<string, unknown>
			| undefined;
		const scopeFilter =
			scopeFilterRaw &&
			typeof scopeFilterRaw === "object" &&
			!Array.isArray(scopeFilterRaw)
				? {
						userId:
							typeof scopeFilterRaw.userId === "string" &&
							scopeFilterRaw.userId.trim().length > 0
								? scopeFilterRaw.userId.trim()
								: undefined,
						agentId:
							typeof scopeFilterRaw.agentId === "string" &&
							scopeFilterRaw.agentId.trim().length > 0
								? scopeFilterRaw.agentId.trim()
								: undefined,
						sessionId:
							typeof scopeFilterRaw.sessionId === "string" &&
							scopeFilterRaw.sessionId.trim().length > 0
								? scopeFilterRaw.sessionId.trim()
								: undefined,
					}
				: undefined;
		// Auth failure recall config
		const authFailureRaw = ar.authFailure as
			| Record<string, unknown>
			| undefined;
		const authFailure: AuthFailureRecallConfig = {
			...
			patterns: Array.isArray(authFailureRaw?.patterns)
				? (authFailureRaw.patterns as string[]).filter(
						(p) => typeof p === "string" && p.length > 0,
					)
				: [],
			maxRecallsPerTarget:
				typeof authFailureRaw?.maxRecallsPerTarget === "number" &&
				authFailureRaw.maxRecallsPerTarget >= 0
		...
		return {
			...
			recallTiming,
			maxTokens:
				typeof ar.maxTokens === "number" && ar.maxTokens > 0
					? ar.maxTokens
					: 800,
			maxPerMemoryChars:
				typeof ar.maxPerMemoryChars === "number" && ar.maxPerMemoryChars >= 0
					? ar.maxPerMemoryChars
					: 0,
			...
			degradationQueueDepth:
				typeof ar.degradationQueueDepth === "number" &&
				ar.degradationQueueDepth >= 0
					? Math.floor(ar.degradationQueueDepth)
					: 10,
			degradationMaxLatencyMs:
				typeof ar.degradationMaxLatencyMs === "number" &&
				ar.degradationMaxLatencyMs >= 0
	...
	const parsedStrategies = Array.isArray(retrievalRaw?.strategies)
		? (retrievalRaw.strategies as string[]).filter(
				(s): s is "semantic" | "fts5" | "graph" =>
					typeof s === "string" &&
					VALID_STRATEGIES.includes(s as (typeof VALID_STRATEGIES)[number]),
			)
		: (["semantic", "fts5"] as Array<"semantic" | "fts5" | "graph">);
	return {
		strategies:
			parsedStrategies.length > 0 ? parsedStrategies : ["semantic", "fts5"],
		rrf_k:
			typeof retrievalRaw?.rrf_k === "number" && retrievalRaw.rrf_k > 0
				? Math.floor(retrievalRaw.rrf_k)
				: 60,
		ambientBudgetTokens:
			typeof retrievalRaw?.ambientBudgetTokens === "number" &&
			retrievalRaw.ambientBudgetTokens > 0
				? Math.floor(retrievalRaw.ambientBudgetTokens)
				: 2000,
		explicitBudgetTokens:
			typeof retrievalRaw?.explicitBudgetTokens === "number" &&
			retrievalRaw.explicitBudgetTokens > 0
				? Math.floor(retrievalRaw.explicitBudgetTokens)
				: 4000,
		graphWalkDepth:
			typeof retrievalRaw?.graphWalkDepth === "number" &&
			retrievalRaw.graphWalkDepth > 0
				? Math.floor(retrievalRaw.graphWalkDepth)
				: 2,
		semanticTopK:
			typeof retrievalRaw?.semanticTopK === "number" &&
			retrievalRaw.semanticTopK > 0
...
export function parseSearchConfig(
	...
	return {
		...
		hydeModel:
			typeof searchRaw.hydeModel === "string" &&
			searchRaw.hydeModel.trim().length > 0
				? searchRaw.hydeModel.trim()
				: undefined,
	};
}

export function parseQueryExpansionConfig(
	cfg: Record<string, unknown>,
): QueryExpansionConfig {
	...
	const mode: "always" | "conditional" | "off" = !enabled
		? "off"
		: (parsedMode ?? "always");

	const threshold =
		typeof qeRaw?.threshold === "number" &&
		qeRaw.threshold >= 0 &&
		qeRaw.threshold <= 1
			? qeRaw.threshold
			: 0.03;
	...
	const rawQeTimeout =
		typeof rawQeTimeoutRaw === "number" &&
		Number.isFinite(rawQeTimeoutRaw) &&
		rawQeTimeoutRaw > 0
	...
	return {
		...
		timeoutMs:
			rawQeTimeout !== null
				? Math.max(MIN_QE_TIMEOUT_MS, rawQeTimeout)
				: defaultTimeout,
		skipForInteractiveTurns,
	};
}

export function parseRerankingConfig(
	cfg: Record<string, unknown>,
): RerankingConfig {
	...
	const model =
		typeof rrRaw?.model === "string" && rrRaw.model.trim().length > 0
			? rrRaw.model.trim()
			: undefined;
	...
	const rawRerankTimeout =
		typeof rawRerankTimeoutRaw === "number" &&
		Number.isFinite(rawRerankTimeoutRaw) &&
		rawRerankTimeoutRaw > 0
	...
	return {
		...
		outputCount,
		timeoutMs:
			rawRerankTimeout !== null
				? Math.max(MIN_RERANK_TIMEOUT_MS, rawRerankTimeout)
				: 10000,
	};
}

export function parseContextualVariantsConfig(
	cfg: Record<string, unknown>,
): ContextualVariantsConfig {
	...
	return {
		...
		model:
			typeof cvRaw?.model === "string" && cvRaw.model.trim().length > 0
				? cvRaw.model.trim()
				: undefined,
		maxVariantsPerFact:
			typeof cvRaw?.maxVariantsPerFact === "number" &&
			cvRaw.maxVariantsPerFact > 0
		...
		maxPerMinute:
			typeof cvRaw?.maxPerMinute === "number" && cvRaw.maxPerMinute > 0
				...
				: 30,
		categories:
			Array.isArray(cvRaw?.categories) &&
			(cvRaw.categories as unknown[]).length > 0
				? (cvRaw.categories as unknown[]).filter(
...
export function parseDocumentGradingConfig(
	cfg: Record<string, unknown>,
): DocumentGradingConfig {
	...
	const model =
		typeof dgRaw?.model === "string" && dgRaw.model.trim().length > 0
			? dgRaw.model.trim()
			: undefined;
	...
	const rawTimeout =
		typeof rawTimeoutRaw === "number" &&
		Number.isFinite(rawTimeoutRaw) &&
		rawTimeoutRaw > 0
