...
export const DEFAULT_LANCE_PATH = join(
	homedir(),
	".openclaw",
	"memory",
	"lancedb",
...
export function normalizeResolvedSecretValue(
...
export function resolveEnvVars(value: string): string {
	if (value.length > MAX_ENV_RESOLVE_LENGTH) {
		throw new Error(
			`Config value too long for environment variable resolution (max ${MAX_ENV_RESOLVE_LENGTH} chars).`,
		);
	}
...
export function parseStoreConfig(cfg: Record<string, unknown>): StoreConfig {
	...
	return {
		...
		classifyModel:
			typeof storeRaw?.classifyModel === "string"
				? storeRaw.classifyModel
				: undefined,
	};
}
...
export function parseWALConfig(cfg: Record<string, unknown>): WALConfig {
	...
	return {
		...
		maxAge:
			typeof walRaw?.maxAge === "number" && walRaw.maxAge > 0
				? walRaw.maxAge
				: 5 * 60 * 1000,
	};
}

export function parseEventLogConfig(
	cfg: Record<string, unknown>,
): EventLogConfig {
	const eventLogRaw = cfg.eventLog as Record<string, unknown> | undefined;
	return {
		archivalDays:
			typeof eventLogRaw?.archivalDays === "number" &&
			eventLogRaw.archivalDays >= 1
				? Math.min(3650, Math.floor(eventLogRaw.archivalDays))
				: 90,
		archivePath:
			typeof eventLogRaw?.archivePath === "string" &&
			eventLogRaw.archivePath.trim().length > 0
...
function parseCredentialOptions(credRaw: Record<string, unknown> | undefined): {
	...
	const autoCaptureRaw = credRaw?.autoCapture as
	...
	return {
		...
		expiryWarningDays:
			typeof credRaw?.expiryWarningDays === "number" &&
			credRaw.expiryWarningDays >= 0
				? Math.floor(credRaw.expiryWarningDays)
				: 7,
	};
}

export function parseCredentialsConfig(
	cfg: Record<string, unknown>,
): CredentialsConfig {
	const credRaw = cfg.credentials as Record<string, unknown> | undefined;
	const explicitlyDisabled = credRaw?.enabled === false;
	const encKeyRaw =
		typeof credRaw?.encryptionKey === "string" ? credRaw.encryptionKey : "";
	let encryptionKey = "";
	...
	const shouldEnable =
		!explicitlyDisabled && (credRaw?.enabled === true || hasValidKey);
...
export function parseActiveTaskConfig(
	cfg: Record<string, unknown>,
): ActiveTaskConfig {
	...
	const staleWarningRaw = activeTaskRaw?.staleWarning as
		| Record<string, unknown>
		| undefined;
	const thRaw = activeTaskRaw?.taskHygiene as
		| Record<string, unknown>
		| undefined;
	const suggestDays =
		typeof thRaw?.suggestGoalAfterTaskAgeDays === "number" &&
		thRaw.suggestGoalAfterTaskAgeDays >= 0
			? Math.floor(thRaw.suggestGoalAfterTaskAgeDays)
			: 0;
	const heartbeatNudgeMaxChars =
		typeof thRaw?.heartbeatNudgeMaxChars === "number" &&
		thRaw.heartbeatNudgeMaxChars >= 200
			? Math.floor(thRaw.heartbeatNudgeMaxChars)
			: 2500;
	const ledgerRaw = activeTaskRaw?.ledger;
	const ledger = ledgerRaw === "facts" ? "facts" : "markdown";
	const projRaw = activeTaskRaw?.projection as
		| Record<string, unknown>
		| undefined;
	const projection: ActiveTaskProjectionConfig = {
		...
		titleMinChars:
			typeof projRaw?.titleMinChars === "number" && projRaw.titleMinChars > 0
				? Math.floor(projRaw.titleMinChars)
				: 0,
		dedupeBy:
			projRaw?.dedupeBy === "label" || projRaw?.dedupeBy === "normalizedTitle"
				? projRaw.dedupeBy
				: "none",
		maxRowsPerSection:
			typeof projRaw?.maxRowsPerSection === "number" &&
			projRaw.maxRowsPerSection > 0
	...
	return {
		...
		ledger,
		filePath:
			typeof activeTaskRaw?.filePath === "string" &&
			activeTaskRaw.filePath.trim().length > 0
		...
		injectionBudget:
			typeof activeTaskRaw?.injectionBudget === "number" &&
			activeTaskRaw.injectionBudget > 0
...
export function parseGoalStewardshipConfig(
	cfg: Record<string, unknown>,
): GoalStewardshipConfig {
	...
	const goalsDir =
		typeof raw?.goalsDir === "string" && raw.goalsDir.trim().length > 0
			? raw.goalsDir.trim()
			: "state/goals";
	...
	const attentionWeights = {
		critical:
			typeof aw?.critical === "number" && aw.critical > 0 ? aw.critical : 4,
	...
	const multiGoalMaxGoals =
		typeof raw?.multiGoalMaxGoals === "number" && raw.multiGoalMaxGoals >= 1
			? Math.floor(raw.multiGoalMaxGoals)
			: 8;

	const confRaw = raw?.confirmationPolicy as
		| Record<string, unknown>
		| undefined;
	const reqAck = confRaw?.requireRegisterAckForPriorities;
	const requireRegisterAckForPriorities: Array<
	...
	const escPolRaw = raw?.escalationPolicy as
		| Record<string, unknown>
		| undefined;
	const cbRaw = raw?.circuitBreaker as Record<string, unknown> | undefined;
	const sameBlockerRepeatLimit =
		typeof cbRaw?.sameBlockerRepeatLimit === "number" &&
		cbRaw.sameBlockerRepeatLimit >= 1
			? Math.floor(cbRaw.sameBlockerRepeatLimit)
			: 0;
	const maxAssessmentsWithoutProgress =
		typeof cbRaw?.maxAssessmentsWithoutProgress === "number" &&
		cbRaw.maxAssessmentsWithoutProgress >= 1
	...
	return {
		...
		globalLimits: {
			maxDispatchesPerHour:
				typeof limits?.maxDispatchesPerHour === "number" &&
				limits.maxDispatchesPerHour >= 1
...
export function parseSelfCorrectionConfig(
	cfg: Record<string, unknown>,
): SelfCorrectionConfig | undefined {
	...
	return {
		...
		semanticDedupThreshold:
			...
			scRaw.semanticDedupThreshold <= 1
				...
				: 0.92,
		toolsSection:
			typeof scRaw.toolsSection === "string" &&
			scRaw.toolsSection.trim().length > 0
		...
		positiveRulesSection:
			typeof scRaw.positiveRulesSection === "string" &&
			scRaw.positiveRulesSection.trim().length > 0
...
function isPlaceholderApiKey(key: string): boolean {
	...
	return (
		/YOUR_.*_HERE|REPLACE_ME|INSERT_.*_HERE|<.*API.*KEY.*>|^x+$|^sk-x+$|^placeholder$/i.test(
			key,
		) ||
...
export function parseAuthConfig(
	...
	for (const [provider, profiles] of Object.entries(
	...
	const backoffScheduleMinutes =
		Array.isArray(backoffScheduleRaw) && backoffScheduleRaw.length > 0
			? (backoffScheduleRaw as number[]).filter(
					(n) => typeof n === "number" && n > 0,
				)
			: undefined;
	const resetHoursRaw = authRaw.resetBackoffAfterHours;
	const resetBackoffAfterHours =
		typeof resetHoursRaw === "number" && resetHoursRaw > 0
			? Math.min(168, resetHoursRaw)
			: undefined;
	return {
		order,
		preferOAuthWhenBoth,
		...(backoffScheduleMinutes && backoffScheduleMinutes.length > 0
...
export function parseLLMConfig(
	...
	const defaultList =
		llmRaw && Array.isArray(llmRaw.default)
			? (llmRaw.default as string[]).filter(
					(m) => typeof m === "string" && m.trim().length > 0,
				)
			: [];
	const heavyList =
		llmRaw && Array.isArray(llmRaw.heavy)
			? (llmRaw.heavy as string[]).filter(
					(m) => typeof m === "string" && m.trim().length > 0,
				)
			: [];
	...
	const llmProviders:
		...
		| undefined =
		llmProvidersRaw &&
		typeof llmProvidersRaw === "object" &&
		!Array.isArray(llmProvidersRaw)
			? Object.fromEntries(
					Object.entries(llmProvidersRaw as Record<string, unknown>).map(
						([k, v]) => {
							if (!v || typeof v !== "object" || Array.isArray(v))
								return [k, undefined];
							const pv = v as Record<string, unknown>;
							const rawKey =
								typeof pv.apiKey === "string" && pv.apiKey.trim().length > 0
									? pv.apiKey.trim()
									: undefined;
							const validKey =
								rawKey && !isPlaceholderApiKey(rawKey) ? rawKey : undefined;
							if (rawKey && !validKey) {
								pluginLogger.warn(
							...
							const baseURLRaw =
								(typeof pv.baseURL === "string" && pv.baseURL.trim().length > 0
									? pv.baseURL.trim()
									: undefined) ??
								(typeof pv.baseUrl === "string" && pv.baseUrl.trim().length > 0
									? pv.baseUrl.trim()
	...
	const llm: LLMConfig | undefined =
		...
		disabledProviders.length > 0
			? {
					...
					fallbackModel:
						typeof llmRaw?.fallbackModel === "string" &&
						(llmRaw.fallbackModel as string).trim().length > 0
...
export function parseGatewayConfig(
	...
	const tokenRaw =
		typeof authRaw.token === "string" ? authRaw.token.trim() : undefined;
