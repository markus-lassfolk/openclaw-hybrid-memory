...
export function parseGraphConfig(cfg: Record<string, unknown>): GraphConfig {
	...
	return {
		...
		autoLinkMinScore:
			typeof graphRaw?.autoLinkMinScore === "number" &&
			graphRaw.autoLinkMinScore >= 0 &&
			graphRaw.autoLinkMinScore <= 1
		...
		autoLinkLimit:
			typeof graphRaw?.autoLinkLimit === "number" && graphRaw.autoLinkLimit > 0
				...
				: 3,
		maxTraversalDepth:
			typeof graphRaw?.maxTraversalDepth === "number" &&
			graphRaw.maxTraversalDepth > 0
...
export function parseGraphRetrievalConfig(
	cfg: Record<string, unknown>,
): GraphRetrievalConfig {
	const graphRetrievalRaw = cfg.graphRetrieval as
	...
	return {
		...
		maxExpandDepth:
			typeof graphRetrievalRaw?.maxExpandDepth === "number" &&
			graphRetrievalRaw.maxExpandDepth >= 0
				? Math.min(5, Math.floor(graphRetrievalRaw.maxExpandDepth))
				: 3,
		maxExpandedResults:
			typeof graphRetrievalRaw?.maxExpandedResults === "number" &&
			graphRetrievalRaw.maxExpandedResults >= 0
				? Math.min(50, Math.floor(graphRetrievalRaw.maxExpandedResults))
				: 20,
	};
}

export function parseClustersConfig(
	cfg: Record<string, unknown>,
): ClustersConfig {
	...
	return {
		...
		minClusterSize:
			typeof clustersRaw?.minClusterSize === "number" &&
			clustersRaw.minClusterSize >= 1
				? Math.floor(clustersRaw.minClusterSize)
				: 3,
		refreshIntervalDays:
			typeof clustersRaw?.refreshIntervalDays === "number" &&
			clustersRaw.refreshIntervalDays >= 0
				? Math.floor(clustersRaw.refreshIntervalDays)
				: 7,
		labelModel:
			typeof clustersRaw?.labelModel === "string" &&
			clustersRaw.labelModel.trim().length > 0
...
export function parseAliasesConfig(
	cfg: Record<string, unknown>,
): AliasesConfig {
...
export function parseIngestConfig(
	...
	if (
		!ingestRaw ||
		!Array.isArray(ingestRaw.paths) ||
		ingestRaw.paths.length === 0
	)
		return undefined;
	return {
		paths: (ingestRaw.paths as string[]).filter(
			(p) => typeof p === "string" && p.length > 0,
		),
		chunkSize:
			typeof ingestRaw.chunkSize === "number" && ingestRaw.chunkSize > 0
				? Math.floor(ingestRaw.chunkSize)
				: 800,
		overlap:
			typeof ingestRaw.overlap === "number" && ingestRaw.overlap >= 0
				? Math.floor(ingestRaw.overlap)
				: 100,
	};
}

export function parseMemoryTieringConfig(
	cfg: Record<string, unknown>,
): MemoryTieringConfig {
	...
	return {
		...
		hotMaxTokens:
			typeof tierRaw?.hotMaxTokens === "number" && tierRaw.hotMaxTokens > 0
				? Math.floor(tierRaw.hotMaxTokens)
				: 2000,
		compactionOnSessionEnd: tierRaw?.compactionOnSessionEnd !== false,
		inactivePreferenceDays:
			typeof tierRaw?.inactivePreferenceDays === "number" &&
			tierRaw.inactivePreferenceDays >= 0
				? Math.floor(tierRaw.inactivePreferenceDays)
				: 7,
		hotMaxFacts:
			typeof tierRaw?.hotMaxFacts === "number" && tierRaw.hotMaxFacts > 0
				? Math.floor(tierRaw.hotMaxFacts)
				: 50,
	};
}

export function parseAmbientConfig(
	cfg: Record<string, unknown>,
): AmbientConfig {
	...
	return {
		...
		maxQueriesPerTrigger:
			...
			ambientRaw.maxQueriesPerTrigger >= 1
				...
				: 4,
		budgetTokens:
			typeof ambientRaw?.budgetTokens === "number" &&
			ambientRaw.budgetTokens > 0
				? Math.floor(ambientRaw.budgetTokens)
				: 2000,
	};
}

export function parseReinforcementConfig(
	cfg: Record<string, unknown>,
): ReinforcementConfig {
	const reinforcementRaw = cfg.reinforcement as
		| Record<string, unknown>
		| undefined;
	return {
		...
		similarityThreshold:
			...
			reinforcementRaw.similarityThreshold <= 1
				...
				: 0.85,
		maxEventsPerFact:
			typeof reinforcementRaw?.maxEventsPerFact === "number" &&
			reinforcementRaw.maxEventsPerFact > 0
				? Math.floor(reinforcementRaw.maxEventsPerFact)
				: 50,
		diversityWeight:
			typeof reinforcementRaw?.diversityWeight === "number" &&
			reinforcementRaw.diversityWeight >= 0
				? Math.min(1.0, reinforcementRaw.diversityWeight)
				: 1.0,
		trackContext: reinforcementRaw?.trackContext !== false,
		boostAmount:
			typeof reinforcementRaw?.boostAmount === "number" &&
			reinforcementRaw.boostAmount > 0
				? reinforcementRaw.boostAmount
				: 1.0,
	};
}

export function parseFutureDateProtectionConfig(
	cfg: Record<string, unknown>,
): FutureDateProtectionConfig {
	const fdpRaw = cfg.futureDateProtection as
		| Record<string, unknown>
		| undefined;
	return {
...
export function parseDocumentsConfig(
	cfg: Record<string, unknown>,
): DocumentsConfig {
	...
	const chunkSize =
		typeof documentsRaw?.chunkSize === "number" && documentsRaw.chunkSize >= 100
			...
			: 2000;
	const chunkOverlap =
		typeof documentsRaw?.chunkOverlap === "number" &&
		documentsRaw.chunkOverlap >= 0
			? Math.floor(documentsRaw.chunkOverlap)
			: 200;
	return {
		enabled: documentsRaw?.enabled === true,
		pythonPath:
			typeof documentsRaw?.pythonPath === "string" &&
			documentsRaw.pythonPath.trim().length > 0
				? documentsRaw.pythonPath.trim()
				: "python3",
		chunkSize,
		chunkOverlap: Math.min(chunkOverlap, Math.max(0, chunkSize - 100)),
		maxDocumentSize:
			typeof documentsRaw?.maxDocumentSize === "number" &&
			documentsRaw.maxDocumentSize > 0
				? Math.floor(documentsRaw.maxDocumentSize)
				: 50 * 1024 * 1024,
		autoTag: documentsRaw?.autoTag !== false,
		visionEnabled: documentsRaw?.visionEnabled === true,
		visionModel:
			typeof documentsRaw?.visionModel === "string" &&
			documentsRaw.visionModel.trim().length > 0
...
export function parsePersonaProposalsConfig(
	cfg: Record<string, unknown>,
): PersonaProposalsConfig {
	const proposalsRaw = cfg.personaProposals as
		| Record<string, unknown>
		| undefined;
	return {
		...
		maxProposalsPerWeek:
			typeof proposalsRaw?.maxProposalsPerWeek === "number" &&
			proposalsRaw.maxProposalsPerWeek > 0
		...
		minConfidence:
			...
			proposalsRaw.minConfidence <= 1
				...
				: 0.7,
		proposalTTLDays:
			typeof proposalsRaw?.proposalTTLDays === "number" &&
			proposalsRaw.proposalTTLDays >= 0
				? Math.floor(proposalsRaw.proposalTTLDays)
				: 30,
		minSessionEvidence:
			typeof proposalsRaw?.minSessionEvidence === "number" &&
			proposalsRaw.minSessionEvidence > 0
				? Math.floor(proposalsRaw.minSessionEvidence)
				: 10,
	};
}

export function parseMultiAgentConfig(
	cfg: Record<string, unknown>,
): MultiAgentConfig {
	const multiAgentRaw = cfg.multiAgent as Record<string, unknown> | undefined;
	return {
		orchestratorId:
			typeof multiAgentRaw?.orchestratorId === "string" &&
			multiAgentRaw.orchestratorId.trim().length > 0
 ...
 */
export function parseErrorReportingConfig(
	cfg: Record<string, unknown>,
): ErrorReportingConfig {
	const errorReportingRaw = cfg.errorReporting as
		| Record<string, unknown>
		| undefined;
	...
	const dsnRaw =
		typeof errorReportingRaw.dsn === "string"
			? errorReportingRaw.dsn.trim()
			: "";
	const modeRaw =
		typeof errorReportingRaw.mode === "string"
			? errorReportingRaw.mode
			: "community";
	const mode: "community" | "self-hosted" =
		modeRaw === "self-hosted" ? "self-hosted" : "community";

	if (enabled && !consent) {
		pluginLogger.warn(
			"memory-hybrid: errorReporting.enabled=true but consent is false; disabling error reporting.",
		);
		enabled = false;
	}
	...
	if (enabled && mode === "self-hosted") {
		...
		const placeholderPatterns =
			/<key>|<host>|<project-id>|YOUR_DSN|PLACEHOLDER/i;
	...
	const botIdRaw =
		typeof errorReportingRaw.botId === "string"
			? errorReportingRaw.botId.trim()
			: "";
	const uuidLike =
		/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;
	const botId =
		botIdRaw.length > 0 && uuidLike.test(botIdRaw) ? botIdRaw : undefined;

	// Optional friendly name for readable GlitchTip reports
	const botNameRaw =
		typeof errorReportingRaw.botName === "string"
			? errorReportingRaw.botName.trim()
			: "";
	const botName = botNameRaw.length > 0 ? botNameRaw.slice(0, 64) : undefined;

	const updateNudgeRaw =
		errorReportingRaw.updateNudge &&
		typeof errorReportingRaw.updateNudge === "object"
	...
	const updateNudge = {
		...
		intervalHours:
			updateNudgeRaw?.intervalHours === 0
				? 0
				: typeof updateNudgeRaw?.intervalHours === "number" &&
						updateNudgeRaw.intervalHours > 0
		...
		cacheTtlHours:
			updateNudgeRaw?.cacheTtlHours === 0
				? 0
				: typeof updateNudgeRaw?.cacheTtlHours === "number" &&
						updateNudgeRaw.cacheTtlHours > 0
	...
	const resolvedIssues =
		...
		!Array.isArray(errorReportingRaw.resolvedIssues)
			? Object.fromEntries(
					Object.entries(errorReportingRaw.resolvedIssues).filter(
	...
	return {
		...
		dsn:
			mode === "community"
				? dsnRaw || DEFAULT_GLITCHTIP_DSN
				: dsnRaw || undefined,
		environment:
			typeof errorReportingRaw.environment === "string"
				? errorReportingRaw.environment
				: undefined,
		sampleRate:
...
export function parseWorkflowTrackingConfig(
	cfg: Record<string, unknown>,
): WorkflowTrackingConfig {
	...
	return {
		...
		retentionDays:
			typeof raw?.retentionDays === "number" && raw.retentionDays > 0
				...
				: 90,
		goalExtractionModel:
			typeof raw?.goalExtractionModel === "string" &&
			raw.goalExtractionModel.trim().length > 0
				? raw.goalExtractionModel.trim()
				: undefined,
	};
}

export function parseCrystallizationConfig(
	cfg: Record<string, unknown>,
): CrystallizationConfig {
	...
	return {
		...
		minUsageCount:
			typeof raw?.minUsageCount === "number" && raw.minUsageCount > 0
				...
				: 5,
		minSuccessRate:
			typeof raw?.minSuccessRate === "number" &&
			raw.minSuccessRate >= 0 &&
			raw.minSuccessRate <= 1
				? raw.minSuccessRate
				: 0.7,
...
export function parseSelfExtensionConfig(
	cfg: Record<string, unknown>,
): SelfExtensionConfig {
...
export function parseImplicitFeedbackConfig(
	cfg: Record<string, unknown>,
): ImplicitFeedbackConfig {
	...
	const signalTypes: ImplicitSignalType[] = Array.isArray(raw?.signalTypes)
		? (raw.signalTypes as unknown[]).filter(
				(t): t is ImplicitSignalType =>
	...
	const topLevelTrajectoryLLMAnalysis =
		typeof cfg.trajectoryLLMAnalysis === "boolean"
			? cfg.trajectoryLLMAnalysis
			: undefined;
	const topLevelFeedToSelfCorrection =
		typeof cfg.feedToSelfCorrection === "boolean"
			? cfg.feedToSelfCorrection
			: undefined;

	// Deprecation warnings for old nested keys when top-level is also set
	if (
		topLevelTrajectoryLLMAnalysis !== undefined &&
		raw?.trajectoryLLMAnalysis !== undefined
	) {
		pluginLogger.warn(
	...
	if (
		...
		raw?.feedToSelfCorrection !== undefined
	) {
		pluginLogger.warn(
	...
	return {
		...
		signalTypes,
		rephraseThreshold:
			typeof raw?.rephraseThreshold === "number" &&
			raw.rephraseThreshold > 0 &&
			raw.rephraseThreshold <= 1
				? raw.rephraseThreshold
				: 0.8,
		topicChangeThreshold:
			typeof raw?.topicChangeThreshold === "number" &&
			raw.topicChangeThreshold >= 0 &&
			raw.topicChangeThreshold <= 1
				? raw.topicChangeThreshold
				: 0.3,
		terseResponseRatio:
			typeof raw?.terseResponseRatio === "number" &&
			raw.terseResponseRatio > 0 &&
			raw.terseResponseRatio <= 1
		...
		feedToSelfCorrection:
			topLevelFeedToSelfCorrection !== undefined
				...
				: raw?.feedToSelfCorrection !== false,
		trajectoryLLMAnalysis:
			topLevelTrajectoryLLMAnalysis !== undefined
				? topLevelTrajectoryLLMAnalysis
				: raw?.trajectoryLLMAnalysis === true,
	};
}

export function parseClosedLoopConfig(
	cfg: Record<string, unknown>,
): ClosedLoopConfig {
	...
	return {
		...
		measurementWindowDays:
			typeof raw?.measurementWindowDays === "number" &&
			raw.measurementWindowDays > 0
...
export function parseFrustrationDetectionConfig(
	cfg: Record<string, unknown>,
): FrustrationDetectionConfig {
	...
	if (weightsRaw && typeof weightsRaw === "object") {
		...
		for (const sig of validSignals) {
			if (typeof weightsRaw[sig] === "number") {
				(parsed as Record<string, number>)[sig] = Math.max(
					0,
					Math.min(1, weightsRaw[sig] as number),
				);
			}
		}
	...
	const thresholdsRaw = raw?.adaptationThresholds as
		| Record<string, unknown>
		| undefined;
	...
	return {
		...
		signalWeights,
		injectionThreshold:
			typeof raw?.injectionThreshold === "number" &&
			raw.injectionThreshold >= 0 &&
			raw.injectionThreshold <= 1
				? raw.injectionThreshold
				: 0.3,
		adaptationThresholds: {
			medium:
				typeof thresholdsRaw?.medium === "number" &&
				thresholdsRaw.medium >= 0 &&
				thresholdsRaw.medium <= 1
					? thresholdsRaw.medium
					: 0.3,
			high:
				typeof thresholdsRaw?.high === "number" &&
				thresholdsRaw.high >= 0 &&
				thresholdsRaw.high <= 1
					? thresholdsRaw.high
					: 0.5,
			critical:
				typeof thresholdsRaw?.critical === "number" &&
				thresholdsRaw.critical >= 0 &&
				thresholdsRaw.critical <= 1
...
export function parseCrossAgentLearningConfig(
	cfg: Record<string, unknown>,
): CrossAgentLearningConfig {
	...
	return {
		...
		fallbackModels: Array.isArray(raw?.fallbackModels)
			? (raw.fallbackModels as string[]).filter(
		...
		batchSize:
			typeof raw?.batchSize === "number" && raw.batchSize >= 5
				...
				: 20,
		minSourceConfidence:
			typeof raw?.minSourceConfidence === "number" &&
			raw.minSourceConfidence >= 0 &&
			raw.minSourceConfidence <= 1
...
export function parseToolEffectivenessConfig(
	cfg: Record<string, unknown>,
): ToolEffectivenessConfig {
	...
	return {
		...
		topN:
			typeof raw?.topN === "number" && raw.topN >= 1
				...
				: 10,
		lowScoreThreshold:
			typeof raw?.lowScoreThreshold === "number" &&
			raw.lowScoreThreshold >= 0 &&
			raw.lowScoreThreshold <= 1
				? raw.lowScoreThreshold
				: 0.3,
		decayFactor:
			typeof raw?.decayFactor === "number" &&
			raw.decayFactor > 0 &&
			raw.decayFactor <= 1
				? raw.decayFactor
				: 0.95,
...
export function parseCostTrackingConfig(
	cfg: Record<string, unknown>,
): CostTrackingConfig {
...
export function parseApiTapConfig(cfg: Record<string, unknown>): ApiTapConfig {
	...
	return {
		...
		captureTimeoutSeconds:
			typeof raw?.captureTimeoutSeconds === "number" &&
			raw.captureTimeoutSeconds >= 5
...
export function parseHumanizerConfig(
	cfg: Record<string, unknown>,
): HumanizerConfig {
	...
	return {
		...
		maxTextLength,
		modelTag:
			typeof raw?.modelTag === "string" && raw.modelTag.trim().length > 0
				? raw.modelTag.trim()
				: undefined,
		skillTag:
			typeof raw?.skillTag === "string" && raw.skillTag.trim().length > 0
				? raw.skillTag.trim()
				: undefined,
	};
}

export function parseFrequencyCaptureConfig(
	cfg: Record<string, unknown>,
): FrequencyCaptureConfig {
	...
	return {
		enabled: raw?.enabled === true,
		mentionThreshold:
			typeof raw?.mentionThreshold === "number" && raw.mentionThreshold >= 1
				? Math.floor(raw.mentionThreshold)
				: 3,
		lookbackSessions:
			typeof raw?.lookbackSessions === "number" && raw.lookbackSessions >= 1
				? Math.floor(raw.lookbackSessions)
				: 5,
		defaultImportance:
			typeof raw?.defaultImportance === "number" &&
			raw.defaultImportance >= 0 &&
			raw.defaultImportance <= 1
				? raw.defaultImportance
				: 0.6,
		captureCredentials: raw?.captureCredentials !== false,
		ttlDays:
			typeof raw?.ttlDays === "number" && raw.ttlDays >= 1
				? Math.floor(raw.ttlDays)
				: 30,
	};
}
