...
export const PRESET_OVERRIDES: Record<ConfigMode, Record<string, unknown>> = {
  ...
  minimal: {
    ...
    graph: { enabled: true, autoLink: false, useInRecall: false, strengthenOnRecall: false, hubDegreeCap: 500 },
  ...
  enhanced: {
    ...
    autoRecall: {
      enabled: true,
      interactiveEnrichment: "fast",
      entityLookup: { enabled: true, autoFromFacts: true },
      authFailure: { enabled: true },
    },
    autoClassify: { enabled: true, suggestCategories: true },
    ...
    graphRetrieval: { enabled: true, defaultExpand: true, maxExpandDepth: 2, maxExpandedResults: 12 },
    procedures: { enabled: true, requireApprovalForPromote: true },
    ...
    retrieval: { strategies: ["semantic", "fts5", "graph"], graphWalkDepth: 2 },
    verbosity: "normal",
  },
  complete: {
    autoCapture: true,
