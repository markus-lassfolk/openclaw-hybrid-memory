...
function hasEffectiveKey(apiKey: string | undefined): boolean {
	...
	const k = apiKey.trim();
	if (k.startsWith("env:") || k.startsWith("file:"))
		return resolveSecretRef(k) !== undefined;
	return true;
}
...
function getDefaultPreferredModelList(
	...
	if (hasKey(pluginConfig.embedding?.apiKey)) {
		if (tier === "nano") list.push(OPENAI_NANO_CRON_MODEL);
			...
			list.push(
				tier === "heavy" ? OPENAI_HEAVY_CRON_MODEL : OPENAI_DEFAULT_CRON_MODEL,
			);
	}
	if (hasKey(pluginConfig.claude?.apiKey)) {
 ...
 */
function getDisabledProviderSet(
	pluginConfig: CronModelConfig | undefined,
): Set<string> {
...
function filterModelsByDisabled(
	models: string[],
...
export function getLLMModelPreferenceUnfiltered(
	...
	const list =
		tier === "heavy" ? pluginConfig?.llm?.heavy : pluginConfig?.llm?.default;
...
export function getCronModelConfig(cfg: HybridMemoryConfig): CronModelConfig {
	return {
		...
		claude: (cfg as Record<string, unknown>)
			.claude as CronModelConfig["claude"],
		llm: cfg.llm,
	};
}
...
function appendUniqueFallbackList(
	chain: string[],
