...
export function isConsolidatedDerivedFact(
	entry: ConsolidatedFactLike,
): boolean {
	...
	return (entry.tags ?? []).some(
		(tag) => tag.trim().toLowerCase() === "consolidated",
	);
}

export function applyConsolidationRetrievalControls(
	score: number,
	entry: ConsolidatedFactLike,
): number {
	return isConsolidatedDerivedFact(entry)
		? score * CONSOLIDATED_RETRIEVAL_SCORE_MULTIPLIER
		: score;
}
