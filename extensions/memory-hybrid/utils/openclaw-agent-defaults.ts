...
export function readAgentsPrimaryModelFromOpenclawJsonRoot(root: Record<string, unknown>): string | undefined {
  const modelCfg = (root.agents as Record<string, unknown> | undefined)?.defaults as
...
export function extractCronStoreJobModel(job: Record<string, unknown>): string | undefined {
  if (typeof job.model === "string" && job.model.trim()) return job.model.trim();
...
export function setCronStoreJobModelFields(job: Record<string, unknown>, model: string): void {
  job.model = model;
  const p = job.payload;
