...
function extractCandidates(text: string, nowMs: number): number[] {
	...
	while ((m = mdRe.exec(text)) !== null) {
		...
		for (const yearOffset of [0, 1]) {
			...
			if (!Number.isNaN(ts)) {
				...
				const check = new Date(ts);
				if (check.getUTCMonth() !== moIdx || check.getUTCDate() !== dayNum)
					continue;
	...
	while ((m = inOffsetRe.exec(text)) !== null) {
		...
		if (daysAhead > 0) {
			results.push(
				Math.floor((todayMidnightMs + daysAhead * 86400 * 1000) / 1000),
			);
		}
	}
...
export function detectFutureDate(
	...
	if (
		/\b(every|daily|weekly|monthly|yearly|recurring|biweekly|each)\b/i.test(
			text,
		)
	)
		return null;
