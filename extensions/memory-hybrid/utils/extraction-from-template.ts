 ...
 */
function buildExtractionRunners(
	...
	if (
		template.choiceOver &&
		template.choiceOver.verbs.length > 0 &&
		template.choiceOver.rejectors.length > 0
	) {
	...
	if (
		template.convention &&
		(template.convention.always.length > 0 ||
			template.convention.never.length > 0)
	) {
	...
	if (
		template.possessive &&
		template.possessive.possessiveWords.length > 0 &&
		template.possessive.isWords.length > 0
	) {
	...
	if (
		template.preference &&
		template.preference.subject.length > 0 &&
		template.preference.verbs.length > 0
	) {
		...
		runners.push((text) => {
			...
			const verbMatch = text.match(
				new RegExp(
					`(?:${subject})\\s+(${template.preference?.verbs.map(escapeRegex).join("|")})\\s+(.+?)\\.?$`,
					"i",
				),
			);
