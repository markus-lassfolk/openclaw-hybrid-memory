...
export function isNanoModel(m: string): boolean {
  return m.split("/")[0]?.toLowerCase() === "ollama" || /nano|\bmini\b|haiku|\blite\b|\bturbo-mini\b/.test((m.split("/").pop() ?? m).toLowerCase());
}
...
export function isHeavyModel(m: string): boolean {
  return m.split("/")[0]?.toLowerCase() !== "ollama" && /\bpro\b|opus|\bo3\b|\bo1\b|\blarge\b|ultra|heavy|gpt-5/.test((m.split("/").pop() ?? m).toLowerCase());
}
