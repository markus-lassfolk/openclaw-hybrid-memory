...
export async function fetchLatestPublishedVersion(
	...
	const newest = candidates.reduce<{
		...
		return compareVersions(candidate.version, latest.version) > 0
			? candidate
			: latest;
	}, null);
...
): VersionCheckCacheEntry | null {
	try {
		const raw = JSON.parse(
			readFileSync(cacheFilePath, "utf-8"),
		) as Partial<VersionCheckCacheEntry>;
		if (
		...
		return {
			...
			lastNudgedAt:
				typeof raw.lastNudgedAt === "string" ? raw.lastNudgedAt : undefined,
		};
	} catch {
		return null;
	}
