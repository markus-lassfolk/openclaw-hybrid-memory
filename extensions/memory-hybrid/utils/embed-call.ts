...
export function embedCallWithTimeout<T>(
	promise: Promise<T>,
	label: string,
): Promise<T> {
	...
	const timeoutPromise = new Promise<T>((_, reject) => {
		timeoutId = setTimeout(
			() =>
				reject(
					new Error(
						`embedding timed out after ${EMBED_CALL_TIMEOUT_MS}ms (${label})`,
					),
				),
		...
		const maybeTimer = timeoutId as unknown as
			| { unref?: () => void }
			| undefined;
 ...
 */
export async function embedCallWithTimeoutAndRetry<T>(
	producer: () => Promise<T>,
	label: string,
): Promise<T> {
	let last: unknown;
	for (let attempt = 0; attempt < EMBED_CALL_MAX_ATTEMPTS; attempt++) {
		try {
			return await embedCallWithTimeout(
				producer(),
			...
			if (attempt < EMBED_CALL_MAX_ATTEMPTS - 1) {
				await new Promise((r) =>
					setTimeout(r, EMBED_CALL_BASE_DELAY_MS * 2 ** attempt),
				);
			}
		}
	}
