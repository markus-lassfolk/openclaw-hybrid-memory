...
export async function runVerifyForCli(
			...
			try {
				...
				const minimalEmbCfg: EmbeddingConfig = {
					...
					...(p === "openai" && {
						...
						...(typeof (cfg.embedding as Record<string, unknown>).endpoint ===
							"string" && (cfg.embedding as Record<string, unknown>).endpoint
							? {
									endpoint: (cfg.embedding as Record<string, unknown>)
										.endpoint as string,
								}
							: {}),
					}),
	...
	if (llmRows.length === 0) {
		...
		for (const row of llmRows) {
			...
			const inConfigStr = row.inConfig
				? noEmoji
					? "Yes"
					: "✅ Yes"
				: noEmoji
	...
	if (opts.reconcile) {
		...
		if (!sqliteOk || !lanceOk || !vectorDb.isLanceAvailable()) {
			log(
