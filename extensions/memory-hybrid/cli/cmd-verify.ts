...
function readApproxFactsRowCount(db: DatabaseSync): number | null {
	try {
		const row = db
			.prepare(`SELECT stat FROM sqlite_stat1 WHERE tbl = 'facts' LIMIT 1`)
			.get() as { stat: string | number } | undefined;
...
export async function runVerifyForCli(
	...
	const log: typeof rawLog = isCompactVerbosity(verbosity)
		? (msg: string) => {
				...
				const isIndentedStatus =
					/^\s{2,}/.test(msg) && !/❌|\[FAIL\]|FAIL —|Error|error/.test(msg);
	...
	if (!cfg.embedding.model) {
		...
		fixes.push(
			'Set "embedding.model" to "text-embedding-3-small" or "text-embedding-3-large" in plugin config',
		);
		configOk = false;
	}
	...
	try {
		if (existsSync(defaultConfigPath)) {
			...
			const agentsDefaults = (rawConfig.agents as Record<string, unknown>)
				?.defaults as Record<string, unknown> | undefined;
			if (agentsDefaults != null && "pruning" in agentsDefaults) {
				const WARN = noEmoji ? "[WARN]" : "⚠️";
				log(
					`${WARN} Config: agents.defaults.pruning is set but not supported by OpenClaw core — it has no effect`,
				);
				log(
				...
				issues.push(
					"agents.defaults.pruning is set but unsupported (has no effect)",
				);
				fixes.push(
	...
	try {
		...
		if (isBindingsError(msg)) {
			...
			fixes.push(
				"node:sqlite is not available. Upgrade Node.js to >=22.12.0 or use a compatible version.",
			);
		} else {
			fixes.push(
		...
		capturePluginError(e as Error, {
			subsystem: "cli",
			operation: "runVerifyForCli:sqlite-check",
		});
	}
	...
	try {
		...
		capturePluginError(e as Error, {
			subsystem: "cli",
			operation: "runVerifyForCli:lancedb-check",
		});
	}
	...
	const rawPluginConfig =
		"error" in rawPluginConfigResult ? undefined : rawPluginConfigResult.config;
	...
	function rawEmbeddingApiKey(): unknown {
		const emb = rawPluginConfig?.embedding as
	...
	function rawLlmProviderApiKey(provider: string): unknown {
		const prov = (rawPluginConfig?.llm as Record<string, unknown>)?.providers as
			| Record<string, unknown>
			| undefined;
	...
	const cfgGoogleKey = (cfg.embedding as Record<string, unknown>)
		.googleApiKey as string | undefined;
	const llmProviders = (
		rawPluginConfig?.llm as Record<string, unknown> | undefined
	...
	const resolvedGoogleKeyForHasKey =
		typeof cfgGoogleKey === "string" && cfgGoogleKey.length >= 10
			? cfgGoogleKey
			: typeof rawGoogleKeyForHasKey === "string" &&
					rawGoogleKeyForHasKey.trim()
				? resolveSecretRef(rawGoogleKeyForHasKey.trim())
				: undefined;
	const hasGoogleKey = Boolean(
		resolvedGoogleKeyForHasKey && resolvedGoogleKeyForHasKey.length >= 10,
	);
	const embProvidersToShow: ("openai" | "ollama" | "onnx" | "google")[] =
		cfg.embedding.preferredProviders &&
		cfg.embedding.preferredProviders.length > 0
		...
		if (opts.testLlm) {
			try {
				...
				const minimalEmbCfg: EmbeddingConfig = {
					...
					...(typeof cfg.embedding.deployment === "string" &&
					cfg.embedding.deployment.trim()
						? { deployment: cfg.embedding.deployment.trim() }
						: {}),
					...(cfg.embedding.models?.length
						? { models: cfg.embedding.models }
						: {}),
					...(p === "openai" && {
						apiKey: cfg.embedding.apiKey,
						...(typeof cfg.embedding.endpoint === "string" &&
						cfg.embedding.endpoint.trim()
							? { endpoint: cfg.embedding.endpoint.trim() }
							: {}),
					}),
					...(p === "google" && {
						googleApiKey:
							resolvedGoogleKeyForHasKey ??
				...
				capturePluginError(e as Error, {
					subsystem: "cli",
					operation: "runVerifyForCli:embedding-test",
					phase: p,
				});
				success = false;
	...
	tableLog(
		`  ${"-".repeat(embW1 + embW2 + embW3 + 4 + (opts.testLlm ? embW4 + 2 : 0))}`,
	);
	...
	const failedEmbRows = opts.testLlm
		? embTableRows.filter((r) => r.success === false && r.error)
		: [];
	...
	if (!anyEmbOk && opts.testLlm) {
		...
		loadBlocking.push(
			"No supported providers with Embedding support available",
		);
	...
	if (!sqliteOk || !lanceOk || !vectorDb.isLanceDbAvailable()) {
		...
		try {
			...
			tableLog(
				`  Active embedding provider: ${providerDims} dimensions (model: ${embeddings.modelName})`,
			);
			tableLog(
				`  Config embedding.dimensions: ${configDims ?? "(default from model/catalog)"}`,
			);
			...
			if (!schemaOk) {
				...
				if (probeLen === lanceDims) {
					log(
						`${OK} Embedding ↔ Lance: OK (${probeLen} dimensions; index matches API output)`,
					);
				} else {
					embeddingAlignmentOk = false;
					log(
	...
	const authOrder = (cfg as Record<string, unknown>).auth as
	...
	const nanoExplicitConfigured =
		Array.isArray(cronCfg.llm?.nano) &&
		cronCfg.llm.nano.some((m) => typeof m === "string" && m.trim().length > 0);
	...
	const fmtTierList = (arr: string[]) =>
		arr.length > 0 ? arr.join(", ") : "(none)";
	tableLog(
		"  Effective tier lists (first model in each tier wins when that tier is selected):",
	);
	tableLog(
	...
	const dreamOverride =
		typeof cfg.nightlyCycle?.model === "string" &&
		cfg.nightlyCycle.model.trim().length > 0
	...
	const disabledSet = new Set(
		(cfg.llm?.disabledProviders ?? []).map((p) =>
	...
	function llmCredentialSource(provider: string): string {
		if (
			gatewayAvailable &&
			hasOAuthProfiles(authOrder?.order?.[provider], provider)
		)
			return "gateway";
		if (provider === "openai")
			return (
				credentialSource(rawEmbeddingApiKey()) ||
				credentialSource(rawLlmProviderApiKey("openai"))
			);
		if (provider === "google")
			return (
				credentialSource(rawDistillApiKey()) ||
				credentialSource(rawLlmProviderApiKey("google"))
			);
		if (provider === "anthropic")
			return (
				credentialSource(rawClaudeApiKey()) ||
				credentialSource(rawLlmProviderApiKey("anthropic"))
			);
	...
	function resolveKey(raw: unknown): string | undefined {
		...
		const resolved =
			trimmed.startsWith("env:") || trimmed.startsWith("file:")
				? resolveSecretRef(trimmed)
				: trimmed;
		return typeof resolved === "string" && resolved.length >= 10
			? resolved
			: undefined;
	}
	function getDirectApiKey(provider: string): string | undefined {
		const prov = cronCfg.llm?.providers as
		...
		if (provider === "google") {
			const fromProv = resolveKey(
				prov?.google?.apiKey ?? cronCfg.distill?.apiKey,
			);
			if (fromProv) return fromProv;
		...
		if (provider === "anthropic") {
			const fromProv = resolveKey(
				prov?.anthropic?.apiKey ??
					(cronCfg.claude as { apiKey?: string } | undefined)?.apiKey,
			);
			if (fromProv) return fromProv;
	...
	function buildDirectClient(provider: string): OpenAI | undefined {
		...
		const provEntry = (
			cronCfg.llm?.providers as
				| Record<string, { baseURL?: string; baseUrl?: string }>
				| undefined
		)?.[provider];
		let baseURL =
			(typeof provEntry?.baseURL === "string" && provEntry.baseURL.trim()
				? provEntry.baseURL.trim()
				: undefined) ??
			(typeof provEntry?.baseUrl === "string" && provEntry.baseUrl.trim()
				? provEntry.baseUrl.trim()
				: undefined) ??
	...
	const uniqueModels = [
		...new Set([...allModelsUnfiltered, ...VERIFY_REFERENCE_MODELS]),
	];
	uniqueModels.sort(
		(a, b) =>
			providerFromModel(a).localeCompare(providerFromModel(b)) ||
			a.localeCompare(b),
	);
	const llmRows: {
		model: string;
	...
	for (const model of uniqueModels) {
		...
		const hasOAuth =
			gatewayAvailable &&
			Boolean(hasOAuthProfiles(authOrder?.order?.[provider], provider));
		...
		if (opts.testLlm && enabled && (hasOAuth || hasApi)) {
			...
			if (hasApi) {
				...
				const isResponsesOnlyModel =
					...
					(provider === "openai" &&
						(bareModel === "gpt-5-codex" || bareModel === "codex"));
				...
				if (isResponsesOnlyModel) {
					...
					if (!directClient) {
						...
						try {
							...
							if (
								errMsg.includes("does not expose responses.create") ||
								/\b(404|405)\b/.test(errMsg)
							) {
								apiResult = undefined;
								apiError = undefined;
								apiSkippedReason =
									"N/A (Responses API not available on endpoint)";
							} else {
								apiError = errMsg;
								if (
	...
	if (llmRows.length === 0) {
		tableLog(
			"  No LLM models configured (add llm.nano / llm.default / llm.heavy or API keys / OAuth).",
		);
		tableLog(
			"  LLMs: add model tiers or API keys in config. See docs/LLM-AND-PROVIDERS.md.",
		);
		tableLog("");
		tableLog(
			"  Summary: Configure LLM tiers or API keys to use memory and cron jobs.",
		);
	} else {
		const llmCols = [
			"Model",
		...
		const failedRows = opts.testLlm
			? llmRows.filter((r) => r.oauthError || r.apiError)
			: [];
		if (failedRows.length > 0) {
			...
			for (const row of failedRows) {
				if (row.oauthError)
					tableLog(`    ${row.model} (OAuth): ${row.oauthError}`);
				if (row.apiError) {
					tableLog(`    ${row.model} (API): ${row.apiError}`);
					if (
						row.provider === "openai" &&
						/401|incorrect api key/i.test(row.apiError)
					)
						has401Openai = true;
				}
			}
		...
		const llmProvidersWithCreds = new Set(
			llmRows.filter((r) => r.hasApi || r.hasOAuth).map((r) => r.provider),
		).size;
		...
		if (anyEmbOk && llmOk) {
			...
			tableLog(
				"  Summary: Fix the issue(s) above (or in --- Fixes --- below) before using memory features.",
			);
		}
	}
	...
	log("\n───── Config ─────");
	log(
		`  Config source: ${defaultConfigPath} (plugins.entries["${PLUGIN_ID}"].config)`,
	);
	...
	if (cfg.credentials.enabled) {
		if (credentialsDb) {
			try {
				...
				capturePluginError(e as Error, {
					subsystem: "cli",
					operation: "runVerifyForCli:credentials-check",
				});
			}
		} else {
	...
	if (existsSync(distillLastRunPath)) {
		try {
			const line =
				readFileSync(distillLastRunPath, "utf-8").split("\n")[0]?.trim() || "";
			log(
			...
			capturePluginError(e as Error, {
				subsystem: "cli",
				operation: "runVerifyForCli:read-distill-marker",
			});
		}
	} else {
	...
	const nightlyMemorySweepRe =
		/nightly[- ]?memory[- ]?sweep|memory distillation.*nightly|nightly.*memory.*distill/i;
	const weeklyReflectionRe =
		/weekly[- ]?reflection|memory reflection|pattern synthesis/i;
	const extractProceduresRe =
		/extract[- ]?procedures|weekly[- ]?extract[- ]?procedures|procedural memory/i;
	const selfCorrectionRe =
		/self[- ]?correction[- ]?analysis|self[- ]?correction\b/i;
	const weeklyDeepMaintenanceRe =
		/weekly[- ]?deep[- ]?maintenance|deep maintenance/i;
	const weeklyPersonaProposalsRe =
		/weekly[- ]?persona[- ]?proposals|persona proposals/i;
	...
	if (cfg.goalStewardship.enabled) {
		...
		try {
			if (!existsSync(cronStorePath)) {
				...
				if (entries.length === 0) {
					log(
						`${WARN} Cron store has no jobs — cannot confirm a heartbeat-shaped message will be delivered.`,
					);
				} else if (withText === 0) {
					log(
					...
					log(
						`${OK} Cron jobs with heartbeat-matching message: ${h.matchingJobIds.join(", ")}`,
					);
			...
			log(
				`${WARN} Cannot read or parse cron store for heartbeat check: ${e instanceof Error ? e.message : String(e)}`,
			);
			capturePluginError(e instanceof Error ? e : new Error(String(e)), {
				subsystem: "cli",
	...
	function formatJobStatus(
		...
		log(
			`${indent}${statusIcon} ${label.padEnd(30)} ${statusText}${statusDetails}`,
		);
		...
		if (job.state?.lastError && job.state.lastStatus === "error") {
			...
			log(
				`${indent}   └─ error: ${errorPreview}${job.state.lastError.length > 100 ? "..." : ""}`,
			);
		}
	}
	...
	if (existsSync(cronStorePath)) {
		try {
			...
			capturePluginError(e as Error, {
				subsystem: "cli",
				operation: "runVerifyForCli:read-job-state",
			});
	...
	if (existsSync(defaultConfigPath)) {
		try {
			...
			capturePluginError(e as Error, {
				subsystem: "cli",
				operation: "runVerifyForCli:read-default-config-jobs",
			});
			// Continue with incomplete data
		}
	}
	...
	const jobsToDisplay = [
		...
		},
		{
			...
			docsPath: "docs/REFLECTION.md § Scheduled Job",
		},
		{
			key: "weekly-extract-procedures",
			description: "procedural memory",
			docsPath: "docs/PROCEDURAL-MEMORY.md",
		},
		{
			key: "self-correction-analysis",
			description: "self-correction",
			docsPath: "docs/SELF-CORRECTION-PIPELINE.md",
		},
		{
			key: "weekly-deep-maintenance",
			description: "deep maintenance",
			docsPath: null,
		},
		{
			key: "monthly-consolidation",
			description: "monthly consolidation",
			docsPath: null,
		},
		{
			key: "weekly-persona-proposals",
			description: "persona proposals",
			docsPath: null,
		},
	];
	...
	const unknownJobs = Array.from(allJobs.entries()).filter(
	...
	try {
		if (existsSync(defaultConfigPath) && existsSync(cronStorePath)) {
			...
			const agentPrimary =
				readEffectiveAgentChatPrimaryFromOpenclawJsonRoot(root);
			if (agentPrimary) {
				...
				const llm = cfg.llm as
					| { _source?: string; default?: string[] }
					| undefined;
				if (
					llm &&
					llm._source !== "gateway" &&
		...
		capturePluginError(e as Error, {
			subsystem: "cli",
			operation: "runVerifyForCli:cron-model-alignment",
		});
	}
	...
	if (opts.logFile && existsSync(opts.logFile)) {
		try {
			...
			const lines = content
				.split("\n")
				.filter((l) =>
					/memory-hybrid|prune|auto-classify|periodic|failed/.test(l),
				);
			...
			capturePluginError(e as Error, {
				subsystem: "cli",
				operation: "runVerifyForCli:read-log-file",
			});
		}
	...
	if (opts.reconcile) {
		...
		if (!sqliteOk || !lanceOk || !vectorDb.isLanceDbAvailable()) {
			log(
				`${FAIL} Reconciliation skipped — both SQLite and LanceDB must be healthy to reconcile.`,
			);
			allOk = false;
		} else {
			try {
				...
				if (vectorOrphans.length === 0 && sqliteOrphans.length === 0) {
					...
					if (vectorOrphans.length > 0) {
						log(
							`${FAIL} Vector orphans (in LanceDB but not SQLite): ${vectorOrphans.length}`,
						);
						vectorOrphans.slice(0, 10).forEach((id) => log(`  - ${id}`));
						if (vectorOrphans.length > 10)
							log(`  … and ${vectorOrphans.length - 10} more`);
						if (opts.fix) {
							let deleted = 0;
							let failed = 0;
							...
							log(
								`  → Deleted ${deleted} orphan vector(s) from LanceDB${failed > 0 ? ` (${failed} failed)` : ""}.`,
							);
						} else {
							log(
						...
						issues.push(
							`${vectorOrphans.length} orphan vector(s) in LanceDB with no matching SQLite fact`,
						);
					}
					if (sqliteOrphans.length > 0) {
						const WARN = noEmoji ? "[WARN]" : "⚠️";
						log(
							`${WARN} SQLite orphans (facts in SQLite with no vector): ${sqliteOrphans.length}`,
						);
				...
				capturePluginError(e as Error, {
					subsystem: "cli",
					operation: "runVerifyForCli:reconcile",
				});
			}
		}
	}
	...
	if (opts.fix) {
		...
		if (lanceBindingsFailed) {
			try {
				...
				for (const pkg of pkgs) {
					const r = spawnSync("npm", ["rebuild", pkg], {
						cwd: extDir,
						shell: true,
					});
					if (r.status === 0) {
						...
						log(
							`Rebuild ${pkg} failed (exit ${r.status}). Run manually: cd ${extDir} && npm rebuild ${pkg}`,
						);
					}
				}
			} catch (e) {
				capturePluginError(e as Error, {
					subsystem: "cli",
					operation: "runVerifyForCli:rebuild-modules",
				});
			}
		}
		...
		if (existsSync(defaultConfigPath)) {
			try {
				...
				let changed = false;
				if (!fixConfig.plugins || typeof fixConfig.plugins !== "object")
					fixConfig.plugins = {};
				...
				if (!cfgFix.embedding || typeof cfgFix.embedding !== "object")
					cfgFix.embedding = {};
				...
				if (placeholder) {
					...
					changed = true;
					applied.push(
						"Set embedding.apiKey and model (use your key or ${OPENAI_API_KEY} in config)",
					);
				}
				...
				try {
					...
					if (
						typeof cfg.nightlyCycle?.schedule === "string" &&
						cfg.nightlyCycle.schedule.trim().length > 0
					) {
						scheduleOverrides["hybrid-mem:nightly-dream-cycle"] =
							cfg.nightlyCycle.schedule;
					}
					if (
						typeof cfg.sensorSweep?.schedule === "string" &&
						cfg.sensorSweep.schedule.trim().length > 0
					) {
						scheduleOverrides["hybrid-mem:sensor-sweep"] =
							cfg.sensorSweep.schedule;
					}
					const { added, normalized } = ensureMaintenanceCronJobs(
						openclawDir,
						getCronModelConfig(cfg),
						{
					...
					capturePluginError(e as Error, {
						subsystem: "cli",
						operation: "runVerifyForCli:add-cron-jobs",
					});
				}
				...
				if (changed) {
					writeFileSync(
						defaultConfigPath,
						JSON.stringify(fixConfig, null, 2),
						"utf-8",
					);
				}
				if (applied.length > 0) {
					...
					if (changed)
						log(
							`Config written: ${defaultConfigPath}. Restart the gateway and run verify again.`,
						);
				}
			} catch (e) {
				...
				capturePluginError(e as Error, {
					subsystem: "cli",
					operation: "runVerifyForCli:apply-fixes",
				});
				const snippet = {
					embedding: {
						apiKey: "<set your key or use ${OPENAI_API_KEY}>",
				...
				log(
					`Minimal config snippet to merge into plugins.entries["${PLUGIN_ID}"].config:`,
				);
	...
	if (opts.reconcile) {
		...
		try {
			...
			log(
				`${lanceRowsOk ? OK : PAUSE} LanceDB row count: ${vCount} (lanceAvailable=${lanceRowsOk})`,
			);
