...
function validateAndWriteConfig(
	...
	try {
		...
		capturePluginError(e as Error, {
			subsystem: "cli",
			operation: `runConfigSetForCli:write-${operation}`,
		});
...
export function runConfigViewForCli(
	...
	const rawEnabled = (key: string, parsedVal: boolean) => {
		if (
			rawCfg[key] &&
			typeof rawCfg[key] === "object" &&
			"enabled" in (rawCfg[key] as Record<string, unknown>)
		) {
	...
	log(
		`  Reflection (patterns/rules): ${on(rawEnabled("reflection", cfg.reflection.enabled))}`,
	);
	log(`  Persona proposals: ${on(cfg.personaProposals.enabled)}`);
	log(
		`  Self-correction: ${on(rawEnabled("selfCorrection", !!cfg.selfCorrection))}`,
	);
	log(
		`  Self-extension (tool proposals): ${on(cfg.selfExtension?.enabled ?? false)}`,
	);
	log(
		`  Crystallization (skill proposals): ${on(cfg.crystallization?.enabled ?? false)}`,
	);
	log(
		`  Extraction (multi-pass): ${on(rawEnabled("extraction", !!cfg.extraction?.extractionPasses))}`,
	);
	log(
		`  Goal stewardship: ${on(rawEnabled("goalStewardship", cfg.goalStewardship?.enabled ?? false))}`,
	);
	{
		const at = cfg.activeTask;
		const atPath = resolvedActiveTaskFilePath(at);
		log(
			`  Active task: ${on(rawEnabled("activeTask", at.enabled))} — ledger: ${at.ledger} — file: ${atPath}`,
		);
	}
	log(`  Frustration detection: ${on(cfg.frustrationDetection.enabled)}`);
	log(`  Cross-agent learning: ${on(cfg.crossAgentLearning.enabled)}`);
	log(
		`  Tool effectiveness: ${on(rawEnabled("toolEffectiveness", cfg.toolEffectiveness.enabled))}`,
	);
	...
	log(
		`  Contextual variants (index-time): ${on(cfg.contextualVariants.enabled)}`,
	);
	log(
		`  Error reporting: ${on(rawEnabled("errorReporting", cfg.errorReporting?.enabled ?? false))}`,
	);
	log(
		`  Cost tracking: ${on(rawEnabled("costTracking", cfg.costTracking?.enabled ?? false))}`,
	);
	log("");

	log(
		"LLM tiers (keep cheap models first; heavy only for quality-critical steps)",
	);
	try {
		...
		const fmt = (arr: string[]) =>
			arr.length === 0
				...
				: arr.length === 1
					? arr[0]
					: `${arr[0]} (+${arr.length - 1} more)`;
		log(`  nano (HyDE, classify, summarize): ${fmt(nano)}`);
		log(
			`  default (maintenance, dream cycle if nightlyCycle.model unset): ${fmt(def)}`,
		);
		log(`  heavy (distill, self-correction, hard tasks): ${fmt(heavy)}`);
		if (cfg.nightlyCycle?.model?.trim()) {
			log(
				`  nightlyCycle.model (overrides dream / MEMORY_INDEX LLM): ${cfg.nightlyCycle.model.trim()}`,
			);
		}
	...
	log(
		`  Retrieval directives: ${on(cfg.autoRecall.retrievalDirectives?.enabled ?? false)}`,
	);
	...
	const maxAutoEntities =
		typeof el?.maxAutoEntities === "number" && el.maxAutoEntities > 0
			? el.maxAutoEntities
			: 500;
	const entitySrc =
	...
	log(
		"Example (goals): openclaw hybrid-mem config-set goalStewardship enabled",
	);
	log(
		"Example (LLM tier lists as JSON): openclaw hybrid-mem config-set llm.nano '[\"azure-foundry/gpt-4.1-nano\"]'",
	);
	log("Help for a key: openclaw hybrid-mem help config-set <key>");
	log(
		"Detail: openclaw hybrid-mem goals config   |   openclaw hybrid-mem active-tasks config",
	);
}
...
export function runConfigSetHelpForCli(
	...
	if (!k)
		return {
			ok: false,
			error: "Key is required (e.g. autoCapture, credentials.enabled)",
		};
	...
	try {
		...
		if (existsSync(pluginPath)) {
			...
			if (hint?.help) {
				desc =
					hint.help.length > MAX_DESC_LEN
						? `${hint.help.slice(0, MAX_DESC_LEN - 3)}...`
						: hint.help;
			} else if (hint?.label) {
				desc = hint.label;
			}
		...
		capturePluginError(err as Error, {
			subsystem: "cli",
			operation: "runConfigSetHelpForCli:read-hints",
		});
	}
...
export function runConfigModeForCli(
	...
	if (!valid.includes(mode as ConfigMode)) {
		return {
			ok: false,
			error: `Invalid mode: ${mode}. Use one of: ${valid.join(", ")}`,
		};
	}
	const openclawDir = join(homedir(), ".openclaw");
	const configPath = join(openclawDir, "openclaw.json");
	...
	const writeResult = validateAndWriteConfig(
		out.config,
		out.root,
		configPath,
		"mode",
...
export function runConfigSetForCli(
	...
	if (!key.trim())
		return {
			ok: false,
			error:
				"Key is required (e.g. nightlyCycle, extraction, credentials, errorReporting.botName, store.fuzzyDedupe)",
		};
	const k = key.trim();
	const openclawDir = join(homedir(), ".openclaw");
	const configPath = join(openclawDir, "openclaw.json");
	...
	for (const { key, prop } of objectToggles) {
		if (k === key && !k.includes(".")) {
			if (
				!enableValues.includes(valueLower) &&
				!disableValues.includes(valueLower)
			) {
				return {
					ok: false,
	...
	if (k === "verbosity") {
		...
		if (!validVerbosity.includes(value)) {
			return {
				ok: false,
				error: `Invalid verbosity: "${value}". Use one of: ${validVerbosity.join(", ")}`,
			};
		}
	...
	if (enumKeys[k]) {
		...
		if (!enumKeys[k].includes(normalized)) {
			return {
				ok: false,
				error: `Invalid ${k}: "${value}". Use one of: ${enumKeys[k].join(", ")}`,
			};
		}
