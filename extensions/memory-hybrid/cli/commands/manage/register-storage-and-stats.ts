...
function entryMatchesHybridSearchFilters(
	...
	if (opts.entity != null && opts.entity !== "" && entry.entity !== opts.entity)
		return false;
	if (opts.key != null && opts.key !== "" && entry.key !== opts.key)
		return false;
	if (opts.source != null && opts.source !== "" && entry.source !== opts.source)
		return false;
	if (opts.tier != null && opts.tier !== "" && entry.tier !== opts.tier)
		return false;
	return true;
}

export function registerManageStorageAndStats(
	mem: Chainable,
	b: ManageBindings,
): void {
	const {
	...
	mem
		.command("compact")
		.description(
			"Run tier compaction: completed tasks -> COLD, inactive preferences -> WARM, active blockers -> HOT",
		)
		.action(
			withExit(async () => {
				let counts;
				...
				console.log(
					`Tier compaction: hot=${counts.hot} warm=${counts.warm} cold=${counts.cold}`,
				);
			}),
		);
	...
	mem
		...
		.option(
			...
			"Remove versions older than this many days (default: 7)",
			"7",
		)
		.action(
	...
	mem
		...
		.option(
			"--efficiency",
			"Show tier/source breakdown, estimated tokens, and token-savings note",
		)
		...
		.action(
			withExit(async (opts?: { efficiency?: boolean; brief?: boolean }) => {
				...
				if (useRich) {
					...
					const { reflectionPatternsCount, reflectionRulesCount } =
						factsDb.statsReflection();
					...
					console.log("");
					const proceduresNote =
						procedures === 0 ? " (run extract-procedures to populate)" : "";
					console.log(
					...
					console.log(
						`Reflection (patterns/rules): ${reflectionPatternsCount}/${reflectionRulesCount}`,
					);
					...
					console.log(
						`Categories in memory: ${uniqueInMemory.length} [${uniqueInMemory.slice(0, 3).join(", ")}...]`,
					);
					console.log("");
					console.log(
					...
					console.log("");
					if (timestamps.distill)
						console.log(`Last distill: ${timestamps.distill}`);
					if (timestamps.reflect)
						console.log(`Last reflect: ${timestamps.reflect}`);
					if (timestamps.compact)
						console.log(`Last compact: ${timestamps.compact}`);
					if (timestamps.distill || timestamps.reflect || timestamps.compact)
						console.log("");
					if (sizes.sqliteBytes != null)
						console.log(
							`SQLite size: ${(sizes.sqliteBytes / 1024 / 1024).toFixed(2)} MB`,
						);
					if (sizes.lanceBytes != null)
						console.log(
							`LanceDB size: ${(sizes.lanceBytes / 1024 / 1024).toFixed(2)} MB`,
						);
					if (sizes.sqliteBytes != null || sizes.lanceBytes != null)
						console.log("");
				} else if (efficiency) {
					...
					console.log("");
					console.log(
						"Note: Tiering and scoping can significantly reduce token usage in LLM context.",
					);
				} else {
					console.log(`Total facts (SQLite): ${sqlCount}`);
					console.log(`Total vectors (LanceDB): ${lanceCount}`);
	...
	mem
		...
		.action(
			withExit(async () => {
				...
				console.log(
					`Pruned ${pruned} expired facts. Before: ${before}, After: ${after}`,
				);
			}),
		);
			...
			withExit(
				async (opts?: {
					...
					const batchSize = Math.max(
						...
						Math.min(
							500,
							Number.parseInt(String(opts?.batchSize ?? "50"), 10) || 50,
						),
					);
					const delayMsBetweenBatches = Math.max(
						...
						Math.min(
							120_000,
							Number.parseInt(String(opts?.delayMsBetweenBatches ?? "0"), 10) ||
								0,
						),
					);
					...
					console.log(
						"Re-index: re-embedding all facts (this may take a while)...",
					);
						...
						onProgress: (completed, total) => {
							if (
								...
								process.stdout.write(
									`  ${completed}/${total} facts embedded...\r`,
								);
							}
						},
						logger: {
	...
	mem
		.command("test")
		.description(
			"Run memory diagnostics (structured + semantic + hybrid + auto-recall)",
		)
		.action(
			withExit(async () => {
				...
				console.log(
					`Structured search: ${icon(result.structured.ok)} (${result.structured.count} result(s))`,
				);
				console.log(
					`Semantic search: ${icon(result.semantic.ok)} (${result.semantic.count} result(s))`,
				);
				console.log(
					`Hybrid search: ${icon(result.hybrid.ok)} (${result.hybrid.count} result(s))`,
				);
				console.log(
					`Auto-recall: ${icon(result.autoRecall.ok)} (${result.autoRecall.count} candidate(s))`,
				);
			}),
		);
				...
				if (!name) {
					...
					try {
						...
						if (catalog === emb.dimensions) {
							console.log(
								`Catalog dimensions for '${emb.model}': ${catalog} (matches config)`,
							);
						} else {
							console.log(
	...
	mem
		...
		.description(
			"Report token usage per injected context source and recommendations",
		)
		.action(
			withExit(async () => {
				...
				console.log(
					`Workspace files: ${audit.workspaceFiles.totalTokens} tokens`,
				);
					...
					try {
						...
						const scopeFilter: ScopeFilter | undefined = opts?.scope
							? (() => {
									...
									if (opts.scope === "user")
										filter.userId = opts.scopeTarget || null;
									else if (opts.scope === "agent")
										filter.agentId = opts.scopeTarget || null;
									else if (opts.scope === "session")
										filter.sessionId = opts.scopeTarget || null;
									return filter;
								})()
							: undefined;
						...
						if (
							...
							combined = combined.filter((r) =>
								entryMatchesHybridSearchFilters(r.entry, opts),
							);
						}
	...
	mem
		.command("forget <id>")
		.description(
			"Remove a memory by ID (from SQLite and LanceDB). ID can be full UUID or a short hex prefix.",
		)
			...
			withExit(async (id: string, opts?: { yes?: boolean }) => {
				try {
					...
					if (!opts?.yes) {
						if (fact) {
							console.log(
								`About to remove: ${fact.text.slice(0, 80)}${fact.text.length > 80 ? "…" : ""}`,
							);
						} else {
							console.log(
								`Memory not found in SQLite (may still exist in LanceDB): ${resolvedId}`,
							);
						}
					...
					const note =
						resolvedId !== id ? ` (resolved from prefix "${id}")` : "";
					console.log(
						`Forgotten${note}. SQLite: ${sqlDeleted}, LanceDB: ${lanceDeleted}`,
					);
				} catch (err) {
					capturePluginError(
						err instanceof Error ? err : new Error(String(err)),
				...
				async (opts?: {
					...
					try {
						...
						if (
							...
							console.error(
								"error: --limit must be an integer from 1 to 50000",
							);
							process.exitCode = 1;
							return;
						}
						...
						for (const f of facts) {
							console.log(
								`  [${f.id}] ${f.text} (tier=${f.tier}, category=${f.category ?? "none"})`,
							);
						}
					} catch (err) {
						capturePluginError(
		...
		.action(
			withExit(async (id: string) => {
				...
				if (item.type === "proposal") {
					console.log(
						`Proposal ${id}. Use: openclaw hybrid-mem proposals show ${id} (--diff, --json)`,
					);
					return;
				}
	...
	mem
		...
		.option(
			"--type <t>",
			"Table alias or physical name (required unless --list-types)",
		)
		.option("--limit <n>", "Max rows (1–5000, default 20)", "20")
		.option(
			"--order <dir>",
			"Sort: last = newest first, first = oldest first",
			"last",
		)
		...
		.action(
			withExit(
				async (opts?: {
					...
					if (!typeRaw) {
						console.error(
							"error: --type is required (or use --list-types for valid names)",
						);
						process.exitCode = 1;
						return;
					}
					...
					try {
						...
						console.log(
							`Table ${result.table} (${result.rows.length} row(s), order=${order}):`,
						);
