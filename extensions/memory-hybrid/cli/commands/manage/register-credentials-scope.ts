...
export function registerManageCredentialsAndScope(
	mem: Chainable,
	b: ManageBindings,
): void {
	const {
	...
	const credentials = mem
		.command("credentials")
		.description("Manage credentials (vaulted)");
	credentials
		.command("migrate-to-vault")
		.description(
			"Migrate credentials from plaintext to vaulted storage (one-time)",
		)
		.action(
			withExit(async () => {
				let res;
				...
				console.log(
					`Migrated ${res.migrated} credentials (${res.skipped} skipped).`,
				);
			}),
		);
			...
			withExit(
				async (opts: {
					...
					if (opts.showValue) {
						...
						console.log(
							"value: *** (use --show-value to reveal, or --value-only to pipe)",
						);
					}
	...
	credentials
		.command("audit")
		.description(
			"Audit vault: flag suspicious entries (natural language, long service names, duplicates)",
		)
		...
		.action(
			withExit(async (opts?: { json?: boolean }) => {
				...
				if (opts?.json) {
					console.log(
						JSON.stringify(
							{ total: audit.total, entries: audit.entries },
							null,
							2,
						),
					);
				...
				console.log(
					`Audit: ${audit.total} total, ${suspicious.length} suspicious.`,
				);
	...
	credentials
		.command("prune")
		.description(
			"Remove suspicious credential entries (default: dry-run; use --yes to apply)",
		)
		.option("--dry-run", "Only list what would be removed (default)")
		.option("--yes", "Actually remove flagged entries")
		.option(
			"--only-flags <reasons>",
			"Comma-separated flags to prune (e.g. natural_language,service_too_long)",
		)
		.action(
			withExit(
				async (opts?: {
					...
					if (res.removed === 0) {
						console.log(
							res.dryRun
								? "No suspicious entries to prune (dry-run)."
								: "No entries removed.",
						);
						return;
					}
					if (res.dryRun) {
						console.log(
							`Would remove ${res.removed} entries (run with --yes to apply):`,
						);
	...
	const scope = mem
		.command("scope")
		.description("Manage memory scopes (global, user, agent, session)");
	scope
		.command("list")
	...
	scope
		...
		.action(
			withExit(async (opts: { scope: string; scopeTarget?: string }) => {
				...
				if (opts.scope === "user")
					scopeFilter.userId = opts.scopeTarget || null;
				else if (opts.scope === "agent")
					scopeFilter.agentId = opts.scopeTarget || null;
				else if (opts.scope === "session")
					scopeFilter.sessionId = opts.scopeTarget || null;
	...
	scope
		...
		.option(
			"--threshold-days <n>",
			"Minimum age in days for a session fact to be promoted (default: 7)",
			"7",
		)
		.option(
			"--min-importance <n>",
		...
		.action(
			withExit(
				async (opts: {
					...
					const candidates = factsDb.findSessionFactsForPromotion(
						thresholdDays,
						minImportance,
					);
					...
					if (opts.dryRun) {
						console.log(
							`Would promote ${candidates.length} facts from session to global scope (dry-run):`,
						);
					...
					console.log(
						`Promoted ${promoted} facts from session to global scope.`,
					);
				},
			),
		);
