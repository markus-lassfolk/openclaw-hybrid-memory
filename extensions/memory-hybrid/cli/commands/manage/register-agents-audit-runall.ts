...
export function registerManageAgentsAuditRunall(
	mem: Chainable,
	b: ManageBindings,
): void {
	const {
	...
	const agentsCmd = mem
		.command("agents")
		.description("Multi-agent health (Issue #789)");
	agentsCmd
		.command("health")
		...
		.action(
			withExit(async (opts?: { agent?: string }) => {
				...
				const views = mergeAgentHealthDashboard(
					forge,
					agentHealthStore.listAll(),
				);
		...
		.action(
			withExit(async (opts?: { agent?: string; hours?: string }) => {
				...
				const hours = Math.max(
					1,
					Math.min(720, Number.parseInt(String(opts?.hours ?? "24"), 10) || 24),
				);
	...
	mem
		.command("audit")
		.description(
			"Cross-agent audit trail (Issue #790): query logged memory operations",
		)
		...
		.action(
			withExit(
				async (opts?: {
					...
					if (!auditStore) {
						console.error(
							"Audit store is not available (e.g. in-memory tests or missing DB path).",
						);
						process.exitCode = 1;
						return;
					}
					const hours = Math.max(
						...
						Math.min(
							720,
							Number.parseInt(String(opts?.hours ?? "24"), 10) || 24,
						),
					);
					const sinceMs = Date.now() - hours * 3600 * 1000;
					const outcome =
					...
					if (fmt === "summary") {
						...
						console.log(
							`  success=${byOutcome.success} partial=${byOutcome.partial} failed=${byOutcome.failed}`,
						);
						for (const [a, c] of Object.entries(byAgent).sort(
							(x, y) => y[1] - x[1],
						)) {
							console.log(`  ${a}: ${c}`);
						}
					...
					if (fmt === "timeline") {
						...
						for (const k of keys) {
							console.log(
								`${k}  ${"█".repeat(Math.min(40, byHour.get(k) ?? 0))} (${byHour.get(k)})`,
							);
						}
						return;
					}
					for (const r of rows) {
						...
						console.log(
							`${ts} ${r.agentId} ${r.action} ${r.outcome}${tgt}${dur}${err}`,
						);
					}
					if (rows.length === 0) {
		...
		.action(
			withExit(
				async (
					...
					const memoryDir = resolvedSqlitePath
						? dirname(resolvedSqlitePath)
						: null;
					const backfillDonePath = memoryDir
						? join(memoryDir, BACKFILL_DECAY_MARKER)
						: null;
					...
					const steps: { name: string; run: () => Promise<void> }[] = [
							...
							? [
									{
										...
										run: async () => {
											const r = await runDistill(
												{ dryRun: false, days: 3, verbose },
												sink,
											);
											log(
												`Distill: ${r.stored} stored from ${r.sessionsScanned} sessions.`,
											);
										},
									},
								]
							...
							? [
									{
										...
										run: async () => {
											const r = await runExtractDaily(
												{ days: 7, dryRun: false, verbose },
												sink,
											);
							...
							? [
									{
										...
										run: async () => {
											const r = await runExtractDirectives({
												days: 7,
												verbose,
												dryRun: false,
											});
											log(
												`Extract-directives: ${r.sessionsScanned} sessions scanned.`,
											);
										},
									},
								]
							...
							? [
									{
										...
										run: async () => {
											const r = await runExtractReinforcement({
												days: 7,
												verbose,
												dryRun: false,
											});
											log(
												`Extract-reinforcement: ${r.sessionsScanned} sessions scanned.`,
											);
										},
									},
								]
							...
							? [
									{
										...
										run: async () => {
											const r = await runExtractImplicitFeedback({
												days: 3,
												verbose,
												dryRun: false,
											});
							...
							? [
									{
										...
										run: async () => {
											const r = await runGenerateAutoSkills({
												dryRun: false,
												verbose,
											});
						...
						},
						{
							name: "reflect-rules",
							run: async () => {
								const r = await runReflectionRules({
									dryRun: false,
									model: reflectionConfig.model,
									verbose,
								});
						...
						},
						{
							name: "reflect-meta",
							run: async () => {
								const r = await runReflectionMeta({
									dryRun: false,
									model: reflectionConfig.model,
									verbose,
								});
							...
							? [
									{
										...
										run: async () => {
											...
											log(
												`Reflect-identity: ${r.insightsStored} insights stored.`,
											);
										},
									},
								]
							...
							? [
									{
										...
										run: async () => {
											const r = await runGenerateProposals({
												dryRun: false,
												verbose,
											});
								...
								if (langPath && existsSync(langPath)) {
									try {
										...
										if (ageDays < 7) {
											if (verbose)
												log(
													`Build-languages: skipped (updated ${ageDays.toFixed(1)} days ago).`,
												);
											return;
										}
									} catch (err) {
										if (verbose)
											log(
												`Build-languages: could not read mtime (${err}); running anyway.`,
											);
									}
								}
								const r = await runBuildLanguageKeywords({ dryRun: false });
								if (r.ok)
									log(`Build-languages: ${r.languagesAdded} languages added.`);
