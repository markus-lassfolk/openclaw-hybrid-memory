...
export function registerManageCorrectionsAndPipeline(
	mem: Chainable,
	b: ManageBindings,
): void {
	const {
	...
	const corrections = mem
		.command("corrections")
		.description("Manage self-correction reports");
	corrections
		.command("list")
	...
	corrections
		...
		.action(
			withExit(async (opts?: { workspace?: string }) => {
				...
				const { applied, error } = await listCommands.correctionsApproveAll({
					workspace: opts?.workspace,
				});
				if (error) {
	...
	mem
		...
		.action(
			withExit(async (opts?: { workspace?: string }) => {
				...
				if (!listCommands) {
					console.log(
						"Review feature not available (personaProposals disabled or no workspace).",
					);
					return;
				}
				const proposals = listCommands.listProposals
					? await listCommands.listProposals({ status: "pending" })
					: [];
				...
				console.log("");
				console.log(
					"To approve/reject proposals: hybrid-mem proposals approve <id> | reject <id>",
				);
				console.log(
					"To approve all corrections: hybrid-mem corrections approve-all",
				);
				console.log("");
				console.log("Proposals:");
				for (const p of proposals) {
					console.log(
						`  [${p.id}] ${p.title} (target=${p.targetFile}, confidence=${p.confidence.toFixed(2)})`,
					);
				}
				console.log("");
				if (reportPath) {
	...
	mem
		.command("store <text>")
		.description(
			"Store a fact (with optional category, entity, key-value, sourceDate, tags, supersedes, scope)",
		)
			...
			withExit(
				async (
					...
					if (res.outcome === "duplicate") {
						...
						console.log(
							`Credential stored: ${res.service} (${res.type}), id=${res.id}`,
						);
					} else if (res.outcome === "credential_skipped_duplicate") {
						console.log(
							`Credential already in vault (skipped): ${res.service} (${res.type})`,
						);
						...
						console.log(
							"Credential vault error — could not write to secure vault (skipped).",
						);
					} else if (res.outcome === "credential_db_error") {
						console.log(
							"Credential pointer error — vault entry written but pointer storage failed (skipped).",
						);
						...
						console.log(
							`Updated fact ${res.id} (superseded ${res.supersededId}): ${res.reason}`,
						);
	...
	mem
		.command("config")
		.description(
			"Show current configuration and feature toggles (use config-set to change)",
		)
		.action(
			withExit(async () => {
				try {
	...
	mem
		...
		.description(
			"Set memory mode (local, minimal, enhanced, complete). Writes memory/.config if needed.",
		)
		.action(
	...
	mem
		...
		.option(
			"--include-credentials",
			"Include credentials in export (default: exclude)",
		)
		.option("--sources <s...>", "Filter by source (comma-separated)")
		.option(
			"--mode <m>",
			"Export mode: replace (overwrite) or additive (merge). Default: replace.",
			"replace",
		)
		.action(
			withExit(
	...
	mem
		...
		.description(
			"Find duplicate or near-duplicate facts using vector similarity",
		)
		.option(
			"--threshold <n>",
		...
		.option(
			"--include-structured",
			"Include structured facts (kv, credentials) in search",
		)
			...
			withExit(
				async (opts?: {
					...
					for (const p of res.pairs) {
						console.log(
							`  [${p.idA}] <-> [${p.idB}] (score=${p.score.toFixed(3)})`,
						);
	...
	mem
		.command("consolidate")
		.description(
			"Consolidate duplicate facts: cluster by vector similarity, merge via LLM, delete originals",
		)
		.option(
			"--threshold <n>",
			"Similarity threshold (0-1, default 0.85)",
		...
		.option(
			"--include-structured",
			"Include structured facts (kv, credentials) in consolidation",
		)
		.option(
			"--dry-run",
			"Show what would be consolidated without consolidating",
		)
		.option("--limit <n>", "Max clusters to process (default 10)", "10")
		.option(
			"--model <m>",
			"LLM model for merging (default: default tier from config)",
		)
		.action(
			withExit(
				async (opts?: {
					...
					const model =
						opts?.model ??
						getDefaultCronModel(getCronModelConfig(ctx.cfg), "default");
					let res;
					try {
						res = await runConsolidate({
							threshold,
							includeStructured,
							dryRun,
							limit,
	...
	mem
		.command("reflect")
		.description(
			"Run reflection: analyze recent facts, extract patterns, store in memory",
		)
		.option(
			"--window <n>",
			"Days to look back (default from config)",
			reflectionConfig.defaultWindow.toString(),
		)
		.option("--dry-run", "Show what would be stored without storing")
		.option(
			"--model <m>",
			"LLM model (default from config)",
			...
			withExit(
				async (
					...
					const window = opts?.window
						? Number.parseInt(opts.window, 10)
						: reflectionConfig.defaultWindow;
					const dryRun = !!opts?.dryRun;
	...
	mem
		...
		.description(
			"Run reflection (rules): extract high-level rules from patterns",
		)
		.option("--dry-run", "Show what would be stored without storing")
		.option(
			"--model <m>",
	...
	mem
		...
		.description(
			"Run reflection (meta-patterns): extract meta-patterns from existing patterns",
		)
		.option("--dry-run", "Show what would be stored without storing")
		.option(
			"--model <m>",
	...
	if (runReflectIdentity) {
		mem
			...
			.description(
				"Run identity reflection: synthesize persona-level insights from reflection outputs",
			)
			.option(
				"--window <n>",
				"Days to look back (default from config)",
			...
			.action(
				withExit(
					async (
						...
						const model = opts?.model ?? reflectionConfig.model;
						const verbose = !!opts?.verbose || readHybridMemVerbose(cmd);
						const res = await runReflectIdentity({
							dryRun,
							model,
							verbose,
							window,
		...
		mem
			...
			.action(
				withExit(async () => {
					...
					if (res.skipped) {
						console.log(
							"Dream cycle skipped (nightlyCycle.enabled = false in config).",
						);
					} else {
						...
						console.log(
							`  Events consolidated: ${res.eventsConsolidated} → ${res.factsCreated} facts`,
						);
					...
					if (
						!res.skipped &&
						runExtractImplicitFeedback &&
						cfg.implicitFeedback?.enabled !== false
					) {
						try {
					...
					if (
						!res.skipped &&
						cfg.closedLoop?.enabled !== false &&
						cfg.closedLoop?.runInNightlyCycle !== false
					) {
						try {
							const clReport = runClosedLoopAnalysis(
								factsDb,
								cfg.closedLoop ?? { enabled: true },
							);
							console.log(
					...
					if (
						...
						try {
							...
							if (pruned > 0)
								console.log(`Cost log: pruned ${pruned} old entries.`);
						} catch (err) {
							capturePluginError(
								err instanceof Error ? err : new Error(String(err)),
								{
	...
	mem
		...
		.description(
			"Resolve unresolved contradictions (auto-resolve obvious cases, report ambiguous pairs)",
		)
		.action(
			withExit(async () => {
				let res;
				...
				if (res.ambiguous.length > 0) {
					...
					for (const a of res.ambiguous.slice(0, 10)) {
						console.log(
							`  - ${a.factIdNew} ↔ ${a.factIdOld} (${a.contradictionId})`,
						);
					}
	...
	mem
		...
		.option(
			"--dry-run",
			"Show what would be reclassified without reclassifying",
		)
		.option("--limit <n>", "Max facts to classify (default 100)", "100")
		.option("--model <m>", "LLM model (default from config)")
		.action(
			withExit(
				async (opts?: { dryRun?: boolean; limit?: string; model?: string }) => {
					const dryRun = !!opts?.dryRun;
					const limit = Number.parseInt(opts?.limit ?? "100", 10);
	...
	mem
		...
		.option(
			"--verbose",
			"List candidate fact ids (dry-run) or enriched fact ids and mentions (after run)",
		)
		.action(
			withExit(
				async (
					...
					if (dryRun) {
						console.log(
							`Entity enrichment (dry-run): ${res.pending} facts pending (no API calls).`,
						);
						if (
							verbose &&
							res.pendingFactIds &&
							res.pendingFactIds.length > 0
						) {
						...
						if (verbose && res.enrichedFacts && res.enrichedFacts.length > 0) {
							...
							for (const ef of res.enrichedFacts) {
								const parts = ef.mentions
									.map((m) => `${m.label}:${JSON.stringify(m.surfaceText)}`)
									.join(", ");
	...
	mem
		...
		.option(
			"--output <path>",
			"Output path for incidents JSON (default: memory/.self-correction-incidents.json)",
		)
		.action(
	...
	mem
		...
		.description(
			"Analyze extracted incidents and auto-remediate (memory store, TOOLS.md); report to memory/reports",
		)
		.option(
			"--extract-path <path>",
			"Path to incidents JSON (default: memory/.self-correction-incidents.json)",
		)
		.option("--workspace <w>", "Workspace path (for TOOLS.md)")
			...
			withExit(
				async (opts?: {
					...
					if (res.toolsSuggestions && res.toolsSuggestions.length > 0) {
						console.log(
							`TOOLS.md suggestions (${res.toolsSuggestions.length}):`,
						);
	...
	mem
		...
		.description(
			"Generalise agent-scoped lessons into global patterns (Issue #263 — Phase 2)",
		)
		.action(
			withExit(async () => {
				if (!runCrossAgentLearning) {
					console.error(
						"cross-agent-learning is not available in this context.",
					);
					process.exitCode = 1;
					return;
				}
				if (!cfg.crossAgentLearning?.enabled) {
					console.log(
						"Cross-agent learning is disabled (crossAgentLearning.enabled = false).",
					);
					return;
				}
				let res;
	...
	mem
		.command("cost-report")
		.description(
			"Show LLM token usage and estimated cost breakdown by feature (Issue #270)",
		)
		.option("--days <n>", "Days of history to include (default: 7)", "7")
		.option("--model", "Show breakdown by model instead of feature")
		.option(
			"--feature <name>",
			"Filter to a specific feature (e.g. auto-classify)",
		)
		.option("--csv", "Output as CSV")
		.option(
			"--format <format>",
			"Output format: pretty (default, emoji+%) or compact (terse)",
			"pretty",
		)
		.option(
			"--modes",
			"Show estimated $/month cost ranges for each config mode (local/minimal/enhanced/complete)",
		)
		.action(
			withExit(
				async (opts?: {
	...
	mem
		...
		.description(
			"Compute and display tool effectiveness scores from workflow traces (Issue #263 — Phase 3)",
		)
			...
			withExit(
				async (opts?: { verbose?: boolean }, cmd?: CommanderOptsParent) => {
					if (!runToolEffectiveness) {
						console.error(
							"tool-effectiveness is not available in this context.",
						);
						process.exitCode = 1;
						return;
					}
	...
	mem
		...
		.option(
			"--days <n>",
			"Days of sessions to analyze (omit for auto: 30 on first run, 3 thereafter)",
		)
		.option(
			"--model <m>",
			"LLM model (e.g. gemini-2.0-flash for 1M context)",
			"",
		)
			...
			withExit(
				async (opts?: {
					...
					if (!runAnalyzeFeedbackPhrases) {
						console.error(
							"analyze-feedback-phrases is not available in this context.",
						);
						process.exitCode = 1;
						return;
					}
					...
					let res;
					try {
						res = await runAnalyzeFeedbackPhrases({
							days,
							model,
							outputPath,
							learn,
					...
					if (res.reinforcement.length > 0) {
						...
						if (res.reinforcement.length > 15)
							console.log(`  ... and ${res.reinforcement.length - 15} more`);
					}
					...
					if (res.correction.length > 0) {
						...
						if (res.correction.length > 15)
							console.log(`  ... and ${res.correction.length - 15} more`);
					}
					if (res.learned) {
						console.log(
