	...
	procedureCmd
		...
		.action(
			withExit(async (id: string) => {
				...
				console.log(
					`  Last Failed:   ${proc.lastFailed ? new Date(proc.lastFailed * 1000).toISOString() : "never"}`,
				);
				...
				if (failures.length > 0) {
					...
					for (const f of failures.slice(0, 10)) {
						...
						console.log(
							`    [${when}] v${f.versionNumber}${step}: ${f.context ?? "(no context)"}`,
						);
					}
				} else {
		...
		.action(
			withExit(async (opts?: { dryRun?: boolean }) => {
				...
				try {
					...
					console.log(
						`  State: ${join(homedir(), ".openclaw", "state", "memory-backup-last.json")}`,
					);
					console.log("");
	...
	maintenance
		...
		.action(
			withExit(async (opts?: { json?: boolean }) => {
				...
				console.log(
					`Stale threshold (daily): ${cfg.maintenance?.cronReliability?.staleThresholdHours ?? 28}h`,
				);
				console.log("");
