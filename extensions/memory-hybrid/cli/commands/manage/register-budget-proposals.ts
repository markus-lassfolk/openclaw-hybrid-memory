...
export function registerManageBudgetAndProposals(
	mem: Chainable,
	b: ManageBindings,
): void {
	const { factsDb, cfg, listCommands, runGenerateProposals, ctx } = b;

	// ---- Token-budget tiered trimming (Issue #792) ----
	const budget = mem
		.command("budget")
		.description("Token budget status and tiered trimming simulation");
	budget
		.command("show")
		...
		.action(
			withExit(async (opts?: { budget?: string }) => {
				try {
					...
					const budgetVal = Number.parseInt(
						opts?.budget ?? String(DEFAULT_BUDGET),
						10,
					);
...
Preserved (P0 — never trimmed, ${result.preserved.length} fact(s)):`);
	...
	mem
		.command("preserve <id>")
		.description(
			"Force-preserve a fact from tiered trimming. Run without options to show current preserve status.",
		)
		.option(
			"--until <epoch>",
			"Preserve until epoch seconds, 'never' to clear, or shorthand like '1y' (default: 1y)",
		)
		.option("-t, --tag <tag>", "Add a preserve tag (can be repeated)")
		.action(
			withExit(
				async (id: string, opts?: { until?: string; tag?: string | null }) => {
					try {
						...
						const preview =
							fact.text.length > 80 ? `${fact.text.slice(0, 80)}…` : fact.text;
						console.log(`Preserved: "${preview}"`);
						const untilStr =
							final?.preserveUntil != null
								? new Date(final.preserveUntil! * 1000).toISOString()
								: "null";
						console.log(`  preserveUntil: ${untilStr}`);
						console.log(
							`  preserveTags:  ${(final?.preserveTags ?? []).join(", ") || "(none)"}`,
						);
						const tags = (fact.tags ?? []).map(String);
						if (tags.includes("edict")) {
							console.log(
								`  note: fact already has 'edict' tag — already P0 (never trimmed)`,
							);
						}
					} catch (err) {
						capturePluginError(
	...
	const proposals = mem
		.command("proposals")
		.description("Manage persona-driven proposals");
	...
	proposals
		...
		.option(
			"--status <s>",
			`Filter by status: ${proposalStatusValues.join(", ")}`,
		)
		.action(
			withExit(async (opts?: { status?: string }) => {
				if (!listCommands?.listProposals) {
					console.log(
						"Proposals feature not available (personaProposals disabled or no workspace).",
					);
					return;
				}
				...
				if (
					status != null &&
					status !== "" &&
					!proposalStatusValues.includes(
						status as (typeof proposalStatusValues)[number],
					)
				) {
					console.error(
						`error: --status requires one of: ${proposalStatusValues.join(", ")}`,
					);
					process.exitCode = 1;
					return;
				}
				const items = await listCommands.listProposals({
		...
		.action(
			withExit(
				async (
					...
					if (opts?.json) {
						console.log(
							JSON.stringify({ ...proposal, diff: diffText }, null, 2),
						);
						return;
					}
