...
import {
	formatGoalStewardshipConfigLines,
	workspaceRootForCli,
} from "./config-feature-summaries.js";
...
export function registerGoalCommands(
	mem: Chainable,
	ctx: { cfg: HybridMemoryConfig },
): void {
	const g = mem
		.command("goals")
		.description(
			"Goal stewardship: list and manage tracked goals (see docs/GOAL-STEWARDSHIP-DESIGN.md)",
		);

	g.command("config")
		.description(
			"Show goal stewardship settings from plugin config (same keys as openclaw.json)",
		)
		.action(() => {
			for (const line of formatGoalStewardshipConfigLines(
				ctx.cfg.goalStewardship,
			)) {
				console.log(line);
			}
		});
	...
	g.command("list")
		...
		.action(async (opts: { all?: boolean; json?: boolean }) => {
			...
			const rows = opts.all
				? goals
				: goals.filter(
						(x) => !["completed", "failed", "abandoned"].includes(x.status),
					);
			if (opts.json) {
	...
	g.command("status [idOrLabel]")
		...
		.option(
			"--json",
			"output raw JSON (overview object when no label, or single goal detail)",
		)
		.action(async (idOrLabel: string | undefined, opts: { json?: boolean }) => {
			...
			if (
				(goal.sameBlockerStreak ?? 0) > 0 ||
				(goal.circuitBreakerLastProgressAssessmentCount ?? 0) > 0
			) {
				console.log(
			...
			if (last10.length > 0) {
				...
				last10.forEach((h) =>
					console.log(
						`  ${h.timestamp}  ${h.actor.padEnd(8)}  ${h.action.padEnd(18)} "${h.detail.slice(0, 100)}"`,
					),
				);
			}
		});
	...
	g.command("audit")
		.description(
			"Structured audit snapshot (JSON) for operators and automation",
		)
		...
		.action(async (opts: { jsonl?: boolean }) => {
			...
			if (opts.jsonl) {
				...
				if (goals.length === 0)
					console.log(JSON.stringify({ ...base, goal: null }));
			} else {
		...
		.action(async () => {
			...
			if (active.length === 0) {
				...
				for (const a of active) {
					const dPct =
						a.maxDispatches > 0
							? Math.round((a.dispatchCount / a.maxDispatches) * 100)
							: 0;
					const aPct =
						a.maxAssessments > 0
							? Math.round((a.assessmentCount / a.maxAssessments) * 100)
							: 0;
					console.log(
	...
	g.command("reset-budget <idOrLabel>")
		...
		.action(async (idOrLabel: string) => {
			...
			await updateGoal(
				dir,
				goal.id,
				patch as Parameters<typeof updateGoal>[2],
				{
			...
			console.log(
				`  dispatches: 0/${goal.maxDispatches} (was ${prevDisp}/${goal.maxDispatches})`,
			);
			console.log(
	...
	g.command("stewardship-run")
		.description(
			"Run one deterministic goal health check (same as plugin watchdog)",
		)
		.action(async () => {
			...
			console.log(
				`Checked ${result.goalsChecked}, updated ${result.goalsUpdated}`,
			);
