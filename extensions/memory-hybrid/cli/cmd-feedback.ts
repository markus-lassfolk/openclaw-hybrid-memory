...
export async function runExtractImplicitFeedbackForCli(
	...
	for (const filePath of filePaths) {
		...
		if (
			!opts.dryRun &&
			implicitCfg.feedToReinforcement !== false &&
			signals.length > 0
		) {
			const minConf = implicitCfg.minConfidence ?? 0.5;
			const positiveSignals = signals.filter(
				(s) => s.polarity === "positive" && s.confidence >= minConf,
			);
			...
			for (const sig of positiveSignals) {
				try {
					const searchQuery =
						sig.context.agentMessage || sig.context.userMessage;
		...
		if (
			!opts.dryRun &&
			implicitCfg.feedToSelfCorrection !== false &&
			signals.length > 0
		) {
			const minConf = implicitCfg.minConfidence ?? 0.5;
			const negativeSignals = signals.filter(
				(s) => s.polarity === "negative" && s.confidence >= minConf,
			);
			for (const sig of negativeSignals) {
				try {
					...
					capturePluginError(
						...
						{
							operation:
								"runExtractImplicitFeedbackForCli:feed-self-correction",
							severity: "info",
							subsystem: "implicit-feedback",
						},
					);
					...
					try {
						...
						if (implicitCfg.trajectoryLLMAnalysis) {
							try {
								...
								const nanoPref = getLLMModelPreference(
								...
								const chatFn = async (opts: {
									...
									const userMessage = opts.messages.find(
										(m) => m.role === "user",
									);
								...
								const llmAnalysis = await analyzeTrajectoriesWithLLM(
									traj,
									prompt,
									chatFn,
								);
								if (llmAnalysis) {
									...
									traj.lessonsExtracted = [
										llmAnalysis.keyLesson,
										...llmAnalysis.patterns,
									];
								...
								capturePluginError(
									...
									{
										operation:
											"runExtractImplicitFeedbackForCli:llm-trajectory-analysis",
										severity: "info",
										subsystem: "implicit-feedback",
									},
								);
...
export async function runCrossAgentLearningForCli(
	...
	const result = await runCrossAgentLearning(
		factsDb,
		openai,
		caCfg,
		ctx.logger ?? {},
...
export async function runToolEffectivenessForCli(
	...
	const sqlitePath =
		cfg.sqlitePath ?? join(homedir(), ".openclaw", "memory", "memory.db");
	const workflowDbPath = sqlitePath.replace(/(\.[^.]+)?$/, "-workflows.db");
	const effectivenessDbPath = sqlitePath.replace(
		/(\.[^.]+)?$/,
		"-tool-effectiveness.db",
	);
	...
	try {
		...
		try {
			...
			const existing = rawDb
				.prepare(
					"SELECT id FROM facts WHERE key = ? AND superseded_at IS NULL LIMIT 1",
				)
				.get(monthlyKey);
			if (!existing) {
			...
			capturePluginError(
				mrErr instanceof Error ? mrErr : new Error(String(mrErr)),
				{
...
export function runCostReportForCli(
	...
	if (opts.modes) {
		...
		if (!compact) {
			...
			log(
				"   Based on typical usage with the default cheapest model (gpt-4.1-nano).",
			);
			log(
				"   Actual costs depend on your volume, model choices, and feature config.",
			);
			log("");
		} else {
	...
	if (opts.model) {
		...
		if (breakdown.length === 0) {
			if (compact) {
				...
				log(
					"   Data appears after your first LLM calls (~1 hour of typical use).",
				);
			}
			return;
		}
		if (opts.csv) {
			...
			for (const r of breakdown) {
				log(
					`${r.model},${r.calls},${r.inputTokens},${r.outputTokens},${r.estimatedCostUsd.toFixed(6)}`,
				);
			}
			return;
		}
		...
		if (!compact) {
			...
			log(
				`💰 Total: ${fmtCost(total.estimatedCostUsd)} across ${total.calls} calls`,
			);
			log("");
		} else {
			...
			log(
				[
					...
					...(compact
						? []
						: [
								pct(r.estimatedCostUsd, total.estimatedCostUsd).padStart(
									colW[5]!,
								),
							]),
				].join("  "),
		...
		if (report.features.length === 0) {
			if (compact) {
				...
				log(
					"   Costs will appear here after your first LLM calls (~1 hour of typical use).",
				);
			}
		...
		if (opts.csv) {
			log(
				"feature,calls,input_tokens,output_tokens,est_cost_usd,est_savings_usd,net_cost_usd",
			);
		...
		if (!compact) {
			...
			if (totalSavings > 0) {
				log(
					`💚 Automation savings: ${fmtCost(totalSavings)} (${savingsReport.total.countAvoided} ops avoided)`,
				);
		...
		for (const r of report.features) {
			...
			if (hasSavings) {
				parts.push(
					(savings > 0 ? `-$${savings.toFixed(4)}` : "").padStart(colW[5]!),
				);
				parts.push(fmtCost(net).padStart(colW[6]!));
			}
			if (!compact) {
				parts.push(
					pct(r.estimatedCostUsd, report.total.estimatedCostUsd).padStart(
						colW[hasSavings ? 7 : 5]!,
					),
				);
			}
	...
	log("");
	log(
		"ℹ️  Costs are estimates based on published model pricing. Actual costs may vary.",
	);
