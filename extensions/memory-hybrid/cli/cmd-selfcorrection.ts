...
export function runSelfCorrectionExtractForCli(
	...
	const filePaths =
		opts.filePaths ??
		gatherSessionFiles({ days: opts.days ?? 3 }).map(
			(f: { path: string; mtime: number }) => f.path,
		);
	...
	try {
		...
		if (opts.outputPath && result.incidents.length > 0) {
			try {
				...
				writeFileSync(
					opts.outputPath,
					JSON.stringify(result.incidents, null, 2),
					"utf-8",
				);
			} catch (e) {
				capturePluginError(e as Error, {
					subsystem: "cli",
					operation: "runSelfCorrectionExtractForCli:write-output",
				});
			}
		}
		return result;
		...
		capturePluginError(err as Error, {
			subsystem: "cli",
			operation: "runSelfCorrectionExtractForCli",
		});
		throw err;
	}
}
...
export async function runSelfCorrectionRunForCli(
	...
	const { factsDb, vectorDb, embeddings, openai, cfg, logger, proposalsDb } =
		ctx;
	...
	try {
		...
		if (opts.incidents && opts.incidents.length > 0) {
			...
			try {
				...
				capturePluginError(e as Error, {
					subsystem: "cli",
					operation: "runSelfCorrectionRunForCli:read-extract",
				});
				return {
					incidentsFound: 0,
					analysed: 0,
			...
			if (pfCfgSC.enabled) {
				...
				const allPaths = sessionFiles.map(
					(f: { path: string; mtime: number }) => f.path,
				);
		...
		const model =
			opts.model ??
			heavyPref[0] ??
			getDefaultCronModel(getCronModelConfig(cfg), "heavy");
		...
		let analysed: Array<{
			...
			remediationContent:
				| string
				| { text?: string; entity?: string; key?: string; tags?: string[] };
			repeated?: boolean;
		}> = [];
		const useSpawn =
			scCfg.analyzeViaSpawn && incidents.length > scCfg.spawnThreshold;
		try {
			let content: string;
			if (useSpawn) {
				...
				const promptPath = join(
					osTmp(),
					`self-correction-prompt-${Date.now()}.txt`,
				);
				writeFileSync(promptPath, prompt, "utf-8");
				const spawnModel =
					scCfg.spawnModel?.trim() ||
				...
				try {
					...
					capturePluginError(err as Error, {
						subsystem: "cli",
						operation: "runSelfCorrectionRunForCli:cleanup-tmp",
					});
				}
				content = (r.stdout ?? "") + (r.stderr ?? "");
				if (r.status !== 0)
					throw new Error(
						`sessions spawn exited ${r.status}: ${content.slice(0, 500)}`,
					);
			} else {
			...
			capturePluginError(e as Error, {
				subsystem: "cli",
				operation: "runSelfCorrectionRunForCli:llm-analysis",
			});
			return {
			...
			if (a.remediationType === "MEMORY_STORE") {
				...
				if (scCfg.semanticDedup || !opts.dryRun) {
					try {
						...
						logger.warn?.(
							`memory-hybrid: self-correction embed/semantic dedup failed: ${err}`,
						);
						capturePluginError(err as Error, {
							subsystem: "cli",
							operation: "runSelfCorrectionRunForCli:embed-dedup",
						});
						continue;
					}
				}
				...
				try {
					...
					logger.warn?.(
						`memory-hybrid: self-correction MEMORY_STORE failed: ${err}`,
					);
					capturePluginError(err as Error, {
						subsystem: "cli",
						operation: "runSelfCorrectionRunForCli:memory-store",
					});
				}
				...
				if (line.trim()) {
					...
					if (
						a.remediationType === "AGENTS_RULE" &&
						proposalsDb &&
						(scCfg as { agentsRuleToProposals?: boolean })
							.agentsRuleToProposals !== false &&
						!opts.dryRun
					) {
						try {
		...
		const shouldApplyTools =
			!opts.dryRun &&
			(scCfg.applyToolsByDefault !== false || opts.approve) &&
			!noApplyTools;
		if (toolsSuggestions.length > 0 && !opts.dryRun) {
			if (scCfg.autoRewriteTools && shouldApplyTools && existsSync(toolsPath)) {
				try {
					...
					const rewritePrompt = fillPrompt(
						loadPrompt("self-correction-rewrite-tools"),
						{
					...
					logger.warn?.(
						`memory-hybrid: self-correction TOOLS rewrite failed: ${err}`,
					);
					capturePluginError(err as Error, {
						subsystem: "cli",
						operation: "runSelfCorrectionRunForCli:tools-rewrite",
					});
				}
				...
				try {
					...
					capturePluginError(err as Error, {
						subsystem: "cli",
						operation: "runSelfCorrectionRunForCli:insert-tools",
					});
				}
			}
		}
		...
		const reportLines = [
			...
			...(autoFixed > 0
				? [
						"## Auto-applied",
						"",
						`- ${autoFixed} memory store(s) and/or TOOLS.md rule(s).`,
						"",
					]
				: []),
			...(toolsSuggestions.length > 0 &&
			toolsApplied === 0 &&
			!scCfg.autoRewriteTools
				? [
			...
			...(toolsApplied > 0
				? [
						"## TOOLS.md updated",
						"",
						`- ${toolsApplied} rule(s) inserted under section "${toolsSection}".`,
						"",
					]
				: []),
			...(proposals.length > 0
		...
		try {
			...
			capturePluginError(e as Error, {
				subsystem: "cli",
				operation: "runSelfCorrectionRunForCli:write-report",
			});
		}
		...
		return {
			...
			reportPath,
			toolsSuggestions:
				toolsSuggestions.length > 0 ? toolsSuggestions : undefined,
