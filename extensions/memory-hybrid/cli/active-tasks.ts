...
function requireFacts(ctx: ActiveTaskContext): {
	factsDb: FactsDB;
	vectorDb: VectorDB;
	embeddings: EmbeddingProvider;
} {
	if (!ctx.factsDb || !ctx.vectorDb || !ctx.embeddings) {
		throw new Error(
			"activeTask.ledger=facts requires factsDb, vectorDb, and embeddings in CLI context",
		);
	}
	return {
		factsDb: ctx.factsDb,
...
export async function runActiveTaskComplete(
	...
	if (!taskFile) {
		return {
			ok: false,
			error: `ACTIVE-TASKS.md not found at ${ctx.activeTaskFilePath}`,
		};
	}
...
export async function runActiveTaskAdd(
	...
	if (ctx.ledger === "facts") {
		...
		const wasExisting =
			active.some((t) => t.label === opts.label) ||
			completed.some((t) => t.label === opts.label);
		const existing =
			active.find((t) => t.label === opts.label) ??
			completed.find((t) => t.label === opts.label);
		const status: ActiveTaskStatus = (() => {
			if (
				opts.status &&
				ACTIVE_TASK_STATUSES.includes(opts.status as ActiveTaskStatus)
			) {
...
export function registerActiveTaskCommands(
	...
	const ledgerHint =
		ctx?.ledger === "facts" ? " (facts ledger: category:project)" : "";
	...
	const activeTasks = mem
		...
		.action(async () => {
			if (!ctx) {
				...
				console.log(
					"  Enable: openclaw hybrid-mem config-set activeTask enabled",
				);
				console.log(
					"  Inspect settings: openclaw hybrid-mem active-tasks config",
				);
				return;
			}
	...
	activeTasks
		.command("config")
		.description(
			"Show active-task settings from plugin config (activeTask.* in openclaw.json)",
		)
		.action(() => {
	...
	activeTasks
		.command("stale")
		.description(
			`Show tasks not updated in >${formatDuration(ctx.staleMinutes)}`,
		)
		.action(async () => {
	...
	activeTasks
		...
		.option(
			"--dry-run",
			"List tasks that would be reconciled without persisting changes",
		)
		.action(async (opts: { dryRun?: boolean }) => {
			if (ctx.ledger === "facts") {
				...
				if (opts.dryRun) {
					console.log(
						`Dry run — would reconcile ${result.reconciledLabels.length} task(s):`,
					);
					for (const l of result.reconciledLabels) console.log(`  - [${l}]`);
					return;
				}
				console.log(
					`✅ Reconciled ${result.reconciledLabels.length} task(s) in facts ledger:`,
				);
				for (const l of result.reconciledLabels) console.log(`  - [${l}]`);
				return;
			}

			const result = await reconcileActiveTaskInProgressSessions(
				ctx.activeTaskFilePath,
				ctx.staleMinutes,
				{
					flushOnComplete: ctx.flushOnComplete,
			...
			console.log(
				`✅ Reconciled ${result.reconciledLabels.length} task(s) (moved to Completed):`,
			);
			for (const l of result.reconciledLabels) console.log(`  - [${l}]`);
		});
	...
	activeTasks
		...
		.option(
			"--status <status>",
			"Task status (In progress|Waiting|Stalled|Failed|Done)",
		)
