...
export function inferTargetFile(content: string): string {
	...
	if (
		/\b(my (name|role)|agent (name|role|identity)|who (i am|you are))\b/.test(
			lower,
		)
	)
		return "IDENTITY.md";
	if (/\b(preference|style|workflow|working|setup|tooling)\b/.test(lower))
		return "USER.md";
	return "SOUL.md";
}
...
export async function runStoreForCli(
	...
	const extracted = extractStructuredFields(
		text,
		(opts.category ?? "other") as MemoryCategory,
	);
	...
	if (
		...
		credentialsDb &&
		isCredentialLike(text, entity, key, value)
	) {
		...
		if (parsed) {
			...
			try {
				...
				capturePluginError(err as Error, {
					subsystem: "cli",
					operation: "runStoreForCli:credential-vault-store",
				});
			...
			try {
				...
				try {
					...
					capturePluginError(err as Error, {
						subsystem: "cli",
						operation: "runStoreForCli:vector-store",
					});
				}
			} catch (err) {
				...
				capturePluginError(err as Error, {
					subsystem: "cli",
					operation: "runStoreForCli:credential-db-store",
				});
	...
	const scopeTarget =
		scope === "global" ? null : (opts.scopeTarget?.trim() ?? null);
	...
	if (cfg.store.classifyBeforeWrite) {
		...
		try {
			...
			capturePluginError(err as Error, {
				subsystem: "cli",
				operation: "runStoreForCli:embed",
			});
		}
		if (vector) {
			...
			if (similarFacts.length > 0) {
				try {
					...
					if (classification.action === "NOOP")
						return { outcome: "noop", reason: classification.reason ?? "" };
					...
					if (classification.action === "UPDATE" && classification.targetId) {
						...
						if (oldFact) {
							...
							try {
								...
								capturePluginError(err as Error, {
									subsystem: "cli",
									operation: "runStoreForCli:vector-store-update",
								});
							}
							return {
								outcome: "updated",
					...
					capturePluginError(err as Error, {
						subsystem: "cli",
						operation: "runStoreForCli:classify",
					});
				}
			}
		}
	...
	try {
		...
		try {
			...
			capturePluginError(err as Error, {
				subsystem: "cli",
				operation: "runStoreForCli:vector-store-final",
			});
		}
		return {
			outcome: "stored",
		...
		capturePluginError(err as Error, {
			subsystem: "cli",
			operation: "runStoreForCli:store",
		});
		throw err;
	}
}
