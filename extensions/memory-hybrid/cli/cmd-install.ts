...
export function resolveAgentWorkspaceRoot(
	...
	if (typeof ws === "string" && isUsableWorkspacePath(ws))
		return expandTilde(ws.trim());
	const agentBlock = config.agent as Record<string, unknown> | undefined;
	const legacyWs = agentBlock?.workspace;
	if (typeof legacyWs === "string" && isUsableWorkspacePath(legacyWs))
		return expandTilde(legacyWs.trim());
...
export function resolveOpenclawJsonPathForWorkspace(): string {
	const explicit =
		getEnv("OPENCLAW_CONFIG")?.trim() || getEnv("OPENCLAW_CONFIG_PATH")?.trim();
...
export function ensureHybridMemoryWorkspaceSkillIfMissing(opts: {
	...
	if (existsSync(destDir)) {
		return {
			path: dest,
			deployed: false,
			skippedReason: "destination_dir_exists",
		};
	}
	try {
		mkdirSync(join(workspaceRoot, "skills"), { recursive: true });
...
function getToolsMdManagedBlockRe(): RegExp {
	return new RegExp(
		`${escapeRegExp(TOOLS_MD_MANAGED_BEGIN)}[\\s\\S]*?${escapeRegExp(TOOLS_MD_MANAGED_END)}`,
	);
}

/** True if applying the managed block would modify `existing` (append, replace, or replace with different body). */
function toolsMdManagedBlockWouldChange(
	existing: string,
	newBlock: string,
): boolean {
...
export function applyHybridMemoryToolsMd(opts: {
	...
	const snippetPath = join(
		opts.pluginRootDir,
		"workspace-snippets",
		"TOOLS-hybrid-memory-body.md",
	);
	if (!existsSync(snippetPath)) {
		return {
			path: toolsPath,
			error: `Bundled TOOLS snippet missing at ${snippetPath}`,
			updated: false,
		};
	}
	...
	if (opts.dryRun) {
		...
		if (!wouldChange && existsSync(toolsPath)) {
			try {
				...
				wouldChange =
					!cur.includes(TOOLS_MD_MANAGED_BEGIN) ||
					toolsMdManagedBlockWouldChange(cur, block);
			} catch {
				wouldChange = true;
			}
		}
...
const LEGACY_JOB_MATCHERS: Record<
	string,
	(j: Record<string, unknown>) => boolean
> = {
	...
	[`${PLUGIN_JOB_ID_PREFIX}weekly-reflection`]: (j) =>
		/weekly-reflection|memory reflection|pattern synthesis/i.test(
			String(j.name ?? ""),
		),
	[`${PLUGIN_JOB_ID_PREFIX}weekly-extract-procedures`]: (j) =>
		/extract-procedures|weekly-extract-procedures|procedural memory/i.test(
			String(j.name ?? ""),
		),
	...
	[`${PLUGIN_JOB_ID_PREFIX}monthly-consolidation`]: (j) =>
		/monthly-consolidation/i.test(String(j.name ?? "")),
	[`${PLUGIN_JOB_ID_PREFIX}nightly-dream-cycle`]: (j) =>
		/nightly-dream-cycle|dream.cycle/i.test(String(j.name ?? "")),
};
...
export function ensureMaintenanceCronJobs(
	...
	const agentPrimary =
		readAgentsPrimaryModelFromOpenclawJsonPath(openclawConfigPath);
	...
	for (const def of MAINTENANCE_CRON_JOBS) {
		...
		const existing = jobsArr.find(
			(j) => j && (j.pluginJobId === id || LEGACY_JOB_MATCHERS[id]?.(j)),
		);
		...
		if (!existing) {
			const job = resolveCronJob(def, pluginConfig, agentPrimary) as Record<
				string,
				unknown
			>;
			...
			if (normalizeExisting) {
				if (typeof existing.schedule === "string") {
					existing.schedule = {
						kind: "cron",
						expr: scheduleExpr ?? existing.schedule,
					};
					jobsChanged = true;
				...
				if (
					!existing.id &&
					typeof existing.pluginJobId === "string" &&
					existing.pluginJobId.length > 0
				) {
				...
				if (agentPrimary?.trim()) {
					const tier = (def.modelTier ?? "default") as
						| "nano"
						| "default"
						| "heavy";
 ...
 */
export async function runResetAuthBackoffForCli(
	ctx: HandlerContext,
): Promise<void> {
...
export function getPluginConfigFromFile(
	...
	| { config: Record<string, unknown>; root: Record<string, unknown> }
	| { error: string } {
	if (!existsSync(configPath))
		return { error: `Config not found: ${configPath}` };
	let root: Record<string, unknown>;
	try {
		root = JSON.parse(readFileSync(configPath, "utf-8")) as Record<
			string,
			unknown
		>;
	} catch (e) {
		capturePluginError(e as Error, {
			subsystem: "cli",
			operation: "getPluginConfigFromFile:read",
		});
 ...
 */
export function buildInstallDefaults(
...
export function runInstallForCli(opts: { dryRun: boolean }): InstallCliResult {
	...
	if (existsSync(configPath)) {
		try {
			...
			capturePluginError(e as Error, {
				subsystem: "cli",
				operation: "runInstallForCli:read-config",
			});
		...
		(
			(
				(config.plugins as Record<string, unknown>).entries as Record<
					...
					unknown
				>
			)[PLUGIN_ID] as Record<string, unknown>
		)?.config &&
		(
			(
				(
	...
	try {
		...
		try {
			...
			const dreamCycleRaw = pluginCfg?.nightlyCycle as
				| Record<string, unknown>
				| undefined;
			const dreamCycleSchedule =
				typeof dreamCycleRaw?.schedule === "string" &&
				(dreamCycleRaw.schedule as string).trim().length > 0
					? (dreamCycleRaw.schedule as string).trim()
					: undefined;
			const sensorSweepRaw = pluginCfg?.sensorSweep as
				| Record<string, unknown>
				| undefined;
			const sensorSweepSchedule =
				typeof sensorSweepRaw?.schedule === "string" &&
				(sensorSweepRaw.schedule as string).trim().length > 0
					? (sensorSweepRaw.schedule as string).trim()
					: undefined;
			const installScheduleOverrides: Record<string, string> = {};
			if (dreamCycleSchedule)
				installScheduleOverrides[`${PLUGIN_JOB_ID_PREFIX}nightly-dream-cycle`] =
					dreamCycleSchedule;
			if (sensorSweepSchedule)
				installScheduleOverrides[`${PLUGIN_JOB_ID_PREFIX}sensor-sweep`] =
					sensorSweepSchedule;
			ensureMaintenanceCronJobs(openclawDir, pluginConfig, {
				normalizeExisting: false,
				reEnableDisabled: false,
				scheduleOverrides:
					Object.keys(installScheduleOverrides).length > 0
						? installScheduleOverrides
						: undefined,
				featureGates: {
					"sensorSweep.enabled":
						(sensorSweepRaw?.enabled as boolean | undefined) === true,
					"nightlyCycle.enabled":
						(dreamCycleRaw?.enabled as boolean | undefined) === true,
				},
			});
		} catch (err) {
			capturePluginError(err as Error, {
				subsystem: "cli",
				operation: "runInstallForCli:cron-setup",
			});
		...
		capturePluginError(err as Error, {
			subsystem: "cli",
			operation: "runInstallForCli:write-config",
		});
...
export function runUninstallForCli(
	...
	if (!opts.leaveConfig && existsSync(configPath)) {
		try {
			...
			capturePluginError(e as Error, {
				subsystem: "cli",
				operation: "runUninstallForCli:update-config",
			});
	...
	if (opts.cleanAll) {
		if (existsSync(resolvedSqlitePath)) {
			try {
				...
				capturePluginError(err as Error, {
					subsystem: "cli",
					operation: "runUninstallForCli:remove-sqlite",
				});
			}
		}
		if (existsSync(resolvedLancePath)) {
			try {
				...
				capturePluginError(err as Error, {
					subsystem: "cli",
					operation: "runUninstallForCli:remove-lance",
				});
			}
		}
	}
...
export async function runUpgradeForCli(
	...
	try {
		...
		capturePluginError(e as Error, {
			subsystem: "cli",
			operation: "runUpgradeForCli:remove-dir",
		});
		return {
			ok: false,
	...
	try {
		...
		capturePluginError(err as Error, {
			subsystem: "cli",
			operation: "runUpgradeForCli:read-version",
		});
	}
	...
	try {
		...
		if (
			...
			scheduleOverrides[`${PLUGIN_JOB_ID_PREFIX}nightly-dream-cycle`] =
		...
		if (
			...
			scheduleOverrides[`${PLUGIN_JOB_ID_PREFIX}sensor-sweep`] =
		...
		capturePluginError(err as Error, {
			subsystem: "cli",
			operation: "runUpgradeForCli:ensure-cron-jobs",
		});
	...
	try {
		...
		if (existsSync(cfgPath)) {
			mergedConfig = JSON.parse(readFileSync(cfgPath, "utf-8")) as Record<
				string,
				unknown
			>;
		}
		...
		capturePluginError(err as Error, {
			subsystem: "cli",
			operation: "runUpgradeForCli:read-config-for-skill",
		});
	}
	...
	if (skillAfterUpgrade.error) {
		logger?.warn?.(
			`memory-hybrid: could not refresh workspace skill: ${skillAfterUpgrade.error}`,
		);
	} else {
		logger?.info?.(
			`memory-hybrid: workspace skill updated at ${skillAfterUpgrade.path}`,
		);
	}
	...
	if (toolsAfterUpgrade.error) {
		logger?.warn?.(
			`memory-hybrid: could not refresh TOOLS.md block: ${toolsAfterUpgrade.error}`,
		);
	} else if (toolsAfterUpgrade.updated) {
		logger?.info?.(
			`memory-hybrid: TOOLS.md hybrid block updated at ${toolsAfterUpgrade.path}`,
		);
	}
	return {
		ok: true,
