...
function formatExpires(proposal: {
...
export function parseSuggestedChange(suggestedChange: string): {
	...
	if (replaceMatch) {
		const remainderMatch = firstLine.match(
			/^\s*replace(?:\s+the)?\s+(?:entire|whole)?\s*file\b\s*:?\s*(.*)$/i,
		);
		const remainderRaw = remainderMatch?.[1] ? remainderMatch[1].trim() : "";
		const remainder =
			remainderRaw && !/^[.:]$/.test(remainderRaw) ? remainderRaw : "";
		let content = [remainder, ...lines.slice(1)].join("\n");
		content = content.replace(
			/^\s*(with|with the following|with the following content)\s*:\s*\n?/i,
			"",
		);
 ...
 */
export function capProposalConfidence(
	confidence: number,
	targetFile: string,
...
function isGitRepo(dirOrFilePath: string): boolean {
	let dir: string;
	try {
		dir = statSync(dirOrFilePath).isDirectory()
			? dirOrFilePath
...
function commitProposalChange(
	...
	if (repoRoot.status !== 0 || !repoRoot.stdout.trim()) {
		return {
			ok: false,
			error: `Failed to resolve git repo root: ${repoRoot.stderr || repoRoot.stdout}`,
		};
	}
	const cwd = repoRoot.stdout.trim();
	const relPath = relative(cwd, targetPath);
	const add = spawnSync("git", ["add", "--", relPath], {
		cwd,
		encoding: "utf-8",
	});
	if (add.status !== 0) {
	...
	const commit = spawnSync("git", ["commit", "-m", message, "--", relPath], {
		cwd,
		encoding: "utf-8",
	});
	if (commit.status !== 0) {
 ...
 */
function registerProposalsCli(
	program: Chainable,
	ctx: ProposalsCliContext,
): void {
	...
	proposals
		...
		.description(
			"Show full proposal content (observation, suggested change, optional diff)",
		)
		...
		.action((proposalId: string, opts?: { json?: boolean; diff?: boolean }) => {
			...
			if (includeDiff && existsSync(targetPath)) {
				try {
					...
					const proposed = buildAppliedContent(
						current,
						proposal,
			...
			const evidenceCount = Array.isArray(proposal.evidenceSessions)
				? proposal.evidenceSessions.length
				: 0;
			...
			console.log(
				`Created: ${created} (expires in ${formatExpires(proposal)})`,
			);
	...
	proposals
		...
		.description(
			"Approve or reject a persona proposal (action: approve|reject)",
		)
		...
		.action(
			async (
				...
				if (proposal.status !== "pending") {
					console.error(
						`Proposal ${proposalId} is already ${proposal.status}. Cannot review again.`,
					);
					process.exit(1);
				}
				...
				if (action === "approve") {
					...
					if (applyResult.ok) {
						console.log(
							`Applied to ${applyResult.targetFile}. Backup: ${applyResult.backupPath}`,
						);
					} else {
						console.error(
	...
	proposals
		...
		.description(
			"Apply an approved persona proposal to its target identity file",
		)
...
> {
	...
	if (
		!ctx.cfg.personaProposals.allowedFiles.includes(
			proposal.targetFile as IdentityFileType,
		)
	) {
		return {
			ok: false,
	...
	if (
		proposal.targetFile.includes("..") ||
		proposal.targetFile.includes("/") ||
		proposal.targetFile.includes("\\")
	) {
		return {
			ok: false,
			error: `Invalid target file path: ${proposal.targetFile}. Path traversal detected.`,
		};
	}
	const targetPath = resolveProposalTarget(proposal.targetFile);
	if (!existsSync(targetPath)) {
		return {
			ok: false,
			error: `Target file ${proposal.targetFile} not found at ${targetPath}`,
		};
	}
	...
	if (DANGEROUS_PATTERNS.test(proposal.suggestedChange)) {
		return {
			ok: false,
			error: `Proposal ${proposalId} contains potentially dangerous content and cannot be applied.`,
		};
	}
	try {
		...
		if (
			...
			proposal.targetHash !== currentSnapshot.hash
		) {
			return {
				ok: false,
		...
		if (!applied.content.trim()) {
			return {
				ok: false,
				error: `Proposal ${proposalId} does not contain replacement content to apply.`,
			};
		}
		...
		if (isGitRepo(targetPath)) {
			const commitResult = commitProposalChange(
				targetPath,
				proposalId,
				proposal.targetFile,
			);
			if (!commitResult.ok) {
				...
				if (repoRoot.status === 0 && repoRoot.stdout.trim()) {
					const cwd = repoRoot.stdout.trim();
					const relPath = relative(cwd, targetPath);
					spawnSync("git", ["reset", "HEAD", "--", relPath], {
						cwd,
						encoding: "utf-8",
					});
				}
