	...
	function walk(dir: string, rel = "memory"): void {
		...
		for (const e of entries) {
			...
			if (e.isDirectory()) {
				...
				out.push({ path: full, label: relPath });
		}
	}
	walk(memoryDir);
	return out;
...
function extractBackfillFact(line: string): {
	...
	if (decisionMatch) {
		...
		if (ruleMatch) {
			...
			value =
				lower.includes("never") || lower.includes("aldrig")
					? "never"
					: "always";
			category = "preference";
		} else {
			...
			const possessiveMatchSv = t.match(
			...
			if (possessiveMatch) {
				...
				if (preferMatch) {
					...
					const templateResult = tryExtractionFromTemplates(
						getExtractionTemplates(),
						t,
					);
...
export async function runBackfillForCli(
	...
	for (const { path: filePath, label } of files) {
		try {
			...
			capturePluginError(err as Error, {
				subsystem: "cli",
				operation: "runBackfillForCli:read-file",
				filePath,
			});
		}
	}
	if (opts.dryRun) {
		sink.log(
			`Would process ${allCandidates.length} facts from ${files.length} files under ${workspaceRoot}`,
		);
		return {
			stored: 0,
			skipped: 0,
	...
	for (const fact of allCandidates) {
		...
		try {
			...
			try {
				...
				sink.warn(
					`memory-hybrid: backfill vector store failed for "${fact.text.slice(0, 50)}...": ${err}`,
				);
				capturePluginError(err as Error, {
					subsystem: "cli",
					operation: "runBackfillForCli:vector-store",
				});
			}
			stored++;
		} catch (err) {
			capturePluginError(err as Error, {
				subsystem: "cli",
				operation: "runBackfillForCli:store-fact",
			});
		}
		processed++;
	}
...
export async function runAnalyzeFeedbackPhrasesForCli(
	...
	for (const { path: fp } of sessionFiles) {
		try {
			...
			capturePluginError(err as Error, {
				subsystem: "cli",
				operation: "runAnalyzeFeedbackPhrasesForCli:read-session",
			});
		}
	}
	...
	if (unmatched.length > 0) {
		...
		const nanoModel =
			nanoPref[0] ?? getDefaultCronModel(getCronModelConfig(cfg), "nano");
		const labels: string[] = [];
		for (let i = 0; i < unmatched.length; i += SENTIMENT_BATCH_SIZE) {
			...
			const truncated = batch.map((t) =>
				t.slice(0, SENTIMENT_MSG_MAX_CHARS).replace(/\n/g, " "),
			);
			...
			try {
				...
				capturePluginError(err as Error, {
					subsystem: "cli",
					operation: "runAnalyzeFeedbackPhrasesForCli:sentiment",
				});
	...
	const truncatedBlock =
		userMessagesBlock.length > maxChars
			? `${userMessagesBlock.slice(0, maxChars)}\n[truncated...]`
			: userMessagesBlock;
	const prompt = fillPrompt(loadPrompt("analyze-feedback-phrases"), {
		user_messages: truncatedBlock,
	});
	const cronCfg = getCronModelConfig(cfg);
	const defaultPref = getLLMModelPreference(cronCfg, "default");
	const model =
		opts.model ?? defaultPref[0] ?? getDefaultCronModel(cronCfg, "default");
	const { spawn } = await import("node:child_process");
	const { tmpdir: osTmp } = await import("node:os");
	const promptPath = join(
		osTmp(),
		`analyze-feedback-phrases-${Date.now()}.txt`,
	);
	...
	try {
		...
		const r = await new Promise<{
			...
			const child = spawn("openclaw", spawnArgs, {
		...
		try {
			...
			try {
				parsedOutput = JSON.parse(
					trimmedContent.slice(braceStart, braceEnd + 1),
				);
			} catch (e) {
				return {
					reinforcement: [],
		...
		if (parsedOutput !== null && typeof parsedOutput === "object") {
			const obj = parsedOutput as {
				reinforcement?: unknown;
				correction?: unknown;
			};
			if (Array.isArray(obj.reinforcement)) {
				reinforcement = obj.reinforcement.filter(
					(s) => typeof s === "string" && s.trim(),
				) as string[];
			}
			if (Array.isArray(obj.correction)) {
				correction = obj.correction.filter(
					(s) => typeof s === "string" && s.trim(),
				) as string[];
			}
		}
		if (opts.outputPath) {
			try {
				...
				writeFileSync(
					opts.outputPath,
					JSON.stringify(
						{ reinforcement, correction, sessionsScanned: sessionFiles.length },
						null,
						2,
					),
					"utf-8",
				...
				capturePluginError(e as Error, {
					subsystem: "cli",
					operation: "runAnalyzeFeedbackPhrasesForCli:write-output",
				});
			}
		}
		let learned = false;
		if (opts.learn) {
			const merged = {
				reinforcement: [
					...new Set([...existing.reinforcement, ...reinforcement]),
				],
...
export async function runIngestFilesForCli(
	...
	const workspaceRoot =
		opts.workspace ?? getEnv("OPENCLAW_WORKSPACE") ?? process.cwd();
	...
	if (files.length === 0) {
		sink.log(
			`No markdown files found for patterns: ${patterns.join(", ")} under ${workspaceRoot}`,
		);
		return {
			stored: 0,
			skipped: 0,
	...
	const model =
		ingestPref[0] ?? cfg.distill?.defaultModel ?? "gemini-3-pro-preview";
	...
	for (const fp of files) {
		try {
			...
			const relPath = fp.startsWith(workspaceRoot)
				? fp.slice(workspaceRoot.length).replace(/^\//, "")
				: basename(fp);
			...
			for (let c = 0; c < chunks.length; c++) {
				...
				if (
					...
					estimateTokens(currentBatch) + blockTokens > batchTokenLimit
				) {
			...
			capturePluginError(err as Error, {
				subsystem: "cli",
				operation: "runIngestFilesForCli:read-file",
				filePath: fp,
			});
		}
	}
		...
		try {
			...
			for (const line of lines) {
				...
				try {
					...
					capturePluginError(err as Error, {
						subsystem: "cli",
						operation: "runIngestFilesForCli:parse-json",
					});
				}
			}
		} catch (err) {
			sink.warn(
				`memory-hybrid: ingest-files LLM batch ${b + 1} failed: ${err}`,
			);
			capturePluginError(err as Error, {
				subsystem: "cli",
				operation: "runIngestFilesForCli:llm-batch",
			});
		}
	}
	...
	if (opts.dryRun) {
		sink.log(
			`Would extract ${allFacts.length} facts from ${files.length} files`,
		);
		return {
			stored: 0,
			skipped: 0,
	...
	for (const fact of allFacts) {
		...
		try {
			...
			const entry = factsDb.store({
				text: fact.text,
				category: (isValidCategory(fact.category)
					? fact.category
					: "technical") as MemoryCategory,
			...
			try {
				...
				sink.warn(
					`memory-hybrid: ingest-files vector store failed for "${fact.text.slice(0, 40)}...": ${err}`,
				);
				capturePluginError(err as Error, {
					subsystem: "cli",
					operation: "runIngestFilesForCli:vector-store",
				});
			}
			stored++;
		} catch (err) {
			sink.warn(
				`memory-hybrid: ingest-files store failed for "${fact.text.slice(0, 40)}...": ${err}`,
			);
			capturePluginError(err as Error, {
				subsystem: "cli",
				operation: "runIngestFilesForCli:store-fact",
			});
		}
	}
	return {
