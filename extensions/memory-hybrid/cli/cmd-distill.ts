			...
			for (const f of readdirSync(sessionsDir, { withFileTypes: true })) {
				...
				try {
					...
					if (stat.mtimeMs > cutoffMs)
						out.push({ path: fp, mtime: stat.mtimeMs });
				} catch (err) {
					capturePluginError(err as Error, {
						subsystem: "cli",
						operation: "gatherSessionFiles:stat",
						filePath: fp,
					});
				}
			}
		...
		try {
			...
			if (!line) {
				...
				if (Number.isNaN(lastRun.getTime())) {
					...
					startDate =
						lastRunDate < threeDaysAgoStr ? lastRunDate : threeDaysAgoStr;
			...
			capturePluginError(e as Error, {
				subsystem: "cli",
				operation: "runDistillWindowForCli",
			});
			mode = "full";
			const start = new Date(now);
			start.setDate(start.getDate() - FULL_DISTILL_MAX_DAYS);
 ...
 */
export function runRecordDistillForCli(
	ctx: HandlerContext,
): RecordDistillResult {
	const { resolvedSqlitePath } = ctx;
	const memoryDir = dirname(resolvedSqlitePath);
	...
	try {
		...
		capturePluginError(err as Error, {
			subsystem: "cli",
			operation: "runRecordDistillForCli",
		});
		throw err;
	}
}
...
export async function runDistillForCli(
	...
	const { factsDb, vectorDb, embeddings, openai, cfg, credentialsDb, logger } =
		ctx;
	...
	try {
		...
		let filesToProcess =
			maxSessions > 0 ? sessionFiles.slice(0, maxSessions) : sessionFiles;
		...
		if (pfCfg.enabled && filesToProcess.length > 0) {
			...
			if (!pfResult.ollamaUnavailable) {
				...
				sink.log(
					"memory-hybrid: distill pre-filter: Ollama unavailable — processing all sessions",
				);
			}
		}
			...
			try {
				...
				const validChunks = chunks.filter((chunk, idx) => {
					...
					if (chunkTokens > safeLimit) {
						sink.warn(
							`memory-hybrid: distill: chunk ${idx + 1} too large (${chunkTokens} tokens), skipping`,
						);
						return false;
					}
					return true;
			...
			try {
				...
				for (const line of lines) {
					...
					try {
						...
						const value =
							typeof obj.value === "string"
								...
								: entity && key
									...
									: "";
						const source_date =
							typeof obj.source_date === "string" ? obj.source_date : null;
						...
						capturePluginError(err as Error, {
							subsystem: "cli",
							operation: "runDistillForCli:parse-json",
						});
					}
				}
			} catch (err) {
				...
				capturePluginError(err as Error, {
					subsystem: "cli",
					operation: "runDistillForCli:llm-batch",
				});
			}
		}
		progress.done();
		if (opts.dryRun) {
			sink.log(
				`Would extract ${allFacts.length} facts from ${filesToProcess.length} sessions`,
			);
			return {
		...
		for (const fact of allFacts) {
				...
				if (parsed) {
					if (!opts.dryRun) {
						...
						try {
							...
							stored++;
							if (opts.verbose)
								sink.log(`  stored credential: ${parsed.service}`);
						} catch (err) {
							if (storedInVault) {
								try {
									...
									if (opts.verbose)
										sink.log(
											`  failed to clean up orphaned credential for ${parsed.service}: ${cleanupErr}`,
										);
									capturePluginError(cleanupErr as Error, {
										subsystem: "cli",
										operation:
											"runDistillForCli:credential-compensating-delete",
									});
								}
							}
							capturePluginError(err as Error, {
								subsystem: "cli",
								operation: "runDistillForCli:credential-store",
							});
						}
					}
					continue;
			...
			try {
				...
				const entry = factsDb.store({
					...
					source: "distillation",
					sourceDate: sourceDateSec(fact.source_date),
					tags: fact.tags?.length
						? fact.tags
						: extractTags(fact.text, fact.entity ?? undefined),
				});
				try {
					await vectorDb.store({
						text: fact.text,
					...
					sink.warn(
						`memory-hybrid: distill vector store failed for "${fact.text.slice(0, 40)}...": ${err}`,
					);
					capturePluginError(err as Error, {
						subsystem: "cli",
						operation: "runDistillForCli:vector-store",
					});
				}
				stored++;
				if (opts.verbose)
					sink.log(`  stored: [${fact.category}] ${fact.text.slice(0, 60)}...`);
			} catch (err) {
				sink.warn(
					`memory-hybrid: distill store failed for "${fact.text.slice(0, 40)}...": ${err}`,
				);
				capturePluginError(err as Error, {
					subsystem: "cli",
					operation: "runDistillForCli:store-fact",
				});
			}
		}
		try {
			...
			capturePluginError(err as Error, {
				subsystem: "cli",
				operation: "runDistillForCli:record-timestamp",
			});
		}
		if (!opts.dryRun) {
