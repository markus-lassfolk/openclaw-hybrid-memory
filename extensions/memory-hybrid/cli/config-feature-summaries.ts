...
export function resolvedActiveTaskFilePath(at: ActiveTaskConfig): string {
	const fp =
		typeof at.filePath === "string" && at.filePath.trim().length > 0
			? at.filePath.trim()
			: "ACTIVE-TASKS.md";
...
export function formatGoalStewardshipConfigLines(
	gs: GoalStewardshipConfig,
): string[] {
	const lines: string[] = [];
	lines.push(`Goal stewardship — ${gs.enabled ? "enabled" : "disabled"}`);
	lines.push(`  goalsDir: ${gs.goalsDir}`);
	lines.push(
		`  model: ${gs.model?.trim() ? gs.model.trim() : "(default from LLM tiers)"}`,
	);
	lines.push(
		`  heartbeatStewardship: ${gs.heartbeatStewardship ? "on" : "off"}`,
	);
	...
	lines.push(
		`  llmTriageOnHeartbeat: ${gs.llmTriageOnHeartbeat ? "on" : "off"}`,
	);
	lines.push(
		`  multiGoal: max ${gs.multiGoalMaxGoals} goals, ${gs.multiGoalMaxChars} chars cap`,
	);
	lines.push(
	...
	if (gs.circuitBreaker.enabled) {
		lines.push(
			`    sameBlockerRepeatLimit: ${gs.circuitBreaker.sameBlockerRepeatLimit}`,
		);
		lines.push(
			`    maxAssessmentsWithoutProgress: ${gs.circuitBreaker.maxAssessmentsWithoutProgress}`,
		);
	}
	lines.push(
	...
	lines.push(
		`  allowPrVerification: ${gs.allowPrVerification ? "on" : "off"} (GitHub pr_merged; off by default)`,
	);
	lines.push(
	...
	lines.push("");
	lines.push(
		"Toggle: openclaw hybrid-mem config-set goalStewardship enabled|disabled",
	);
	lines.push(
		"Help:   openclaw hybrid-mem help config-set goalStewardship.enabled",
	);
	return lines;
}
...
export function formatActiveTaskConfigLines(at: ActiveTaskConfig): string[] {
	...
	lines.push(
		`  flushOnComplete: ${at.flushOnComplete !== false ? "on" : "off"}`,
	);
	...
	lines.push("");
	lines.push(
		"Naming: JSON config uses camelCase `activeTask.*`; CLI command is `active-tasks` (plural).",
	);
	lines.push(
		"The working-memory file default is ACTIVE-TASKS.md (see filePath above).",
	);
	lines.push(
		"Toggle: openclaw hybrid-mem config-set activeTask enabled|disabled",
	);
	lines.push(
		"Ledger: openclaw hybrid-mem config-set activeTask.ledger markdown|facts",
	);
