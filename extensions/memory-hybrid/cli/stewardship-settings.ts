...
function requestAlreadyApplied(path: string, requestId: string): boolean {
  ...
  return readFileSync(path, "utf8").split("\n").some((line) => {
    try { return (JSON.parse(line) as { requestId?: string; result?: string }).requestId === requestId && JSON.parse(line).result === "applied"; }
    catch { return false; }
  });
}
...
export function runStewardshipSettingsForCli(request: StewardshipSettingsRequest): StewardshipSettingsResult {
  ...
  if (!request.reason.trim() || request.reason.trim().length > 500) return { ok: false, error: "A reason of 1-500 characters is required." };
  ...
  catch { return { ok: false, error: "Another stewardship settings update is in progress; retry with the same request id." }; }
  try {
    ...
    const globalLimits = typeof stewardship.globalLimits === "object" && stewardship.globalLimits !== null
    ...
    const oldValue = typeof oldRaw === "number" && Number.isFinite(oldRaw) ? Math.floor(oldRaw) : field === "maxActiveGoals" ? 5 : 6;
    globalLimits[field] = value;
    stewardship.globalLimits = globalLimits;
    const nextConfig = { ...out.config, goalStewardship: stewardship };
    try { hybridConfigSchema.parse(nextConfig); }
    catch (err) { return { ok: false, error: `Invalid stewardship setting: ${String(err)}` }; }
    if (request.dryRun) return { ok: true, changed: oldValue !== value, dryRun: true, oldValue, newValue: value, auditPath: aPath };
    ...
    const event = { timestamp: new Date().toISOString(), actor: request.actor.trim(), reason: request.reason.trim(), requestId: request.requestId ?? null, key: `goalStewardship.${request.key}`, oldValue, newValue: value, result: "applied" };
    writeAudit(aPath, event);
    try { writeFileSync(getRestartPendingPath(), "", { encoding: "utf8", mode: 0o600 }); } catch { /* advisory marker */ }
