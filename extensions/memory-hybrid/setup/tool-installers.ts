...
import {
	...
	resolveActiveTaskPathForTools,
} from "../tools/task-hygiene-tools.js";
import {
	type PluginContext as UtilityToolsContext,
...
type ProvenanceInstallerContext = Pick<
	ToolsContext,
	"factsDb" | "eventLog" | "provenanceService" | "cfg"
>;
...
type CrystallizationInstallerContext = Pick<
	ToolsContext,
	"crystallizationStore" | "workflowStore" | "cfg"
>;
type SelfExtensionInstallerContext = Pick<
	ToolsContext,
	"toolProposalStore" | "workflowStore" | "cfg"
>;
...
function selectMemoryCoreToolsContext(ctx: ToolsContext): MemoryToolsContext {
	...
	return {
		...
		walWrite: (operation, data, logger, supersedeTargetId) =>
			walWrite(wal, operation, data, logger, supersedeTargetId),
...
function installMemoryCoreTools(
...
function selectUtilityToolsContext(
	ctx: ToolsContext,
	api: ClawdbotPluginApi,
): UtilityInstallerContext {
	const {
		factsDb,
		vectorDb,
		embeddings,
	...
	return {
		...
		walWrite: (operation, data) =>
			ctx.walWrite(wal, operation, data, api.logger),
		walRemove: (id) => ctx.walRemove(wal, id, api.logger),
	};
}

function installUtilityTools(
	ctx: UtilityInstallerContext,
	api: ClawdbotPluginApi,
): void {
	registerUtilityTools(
		ctx.toolContext,
...
function installProvenanceTools(
	ctx: ProvenanceInstallerContext,
	api: ClawdbotPluginApi,
): void {
...
function installCredentialTools(
	ctx: CredentialInstallerContext,
	api: ClawdbotPluginApi,
): void {
...
type PersonaInstallerContext = PersonaToolsContext &
	Pick<ToolsContext, "timers">;
...
function installPersonaTools(
	ctx: PersonaInstallerContext,
	api: ClawdbotPluginApi,
): void {
		...
		() => {
			try {
				if (proposalsDb?.isOpen()) {
					...
					if (pruned > 0) {
						api.logger.info(
							`memory-hybrid: pruned ${pruned} expired proposal(s)`,
						);
					}
				}
			} catch (err) {
...
function installVerificationTools(
	ctx: VerificationInstallerContext,
	api: ClawdbotPluginApi,
): void {
...
function installIssueTools(
	ctx: IssueInstallerContext,
	api: ClawdbotPluginApi,
): void {
	if (ctx.issueStore) {
...
function selectWorkflowToolsContext({
	workflowStore,
}: ToolsContext): WorkflowInstallerContext {
	return { workflowStore };
}

function installWorkflowTools(
	ctx: WorkflowInstallerContext,
	api: ClawdbotPluginApi,
): void {
...
function installCrystallizationTools(
	ctx: CrystallizationInstallerContext,
	api: ClawdbotPluginApi,
): void {
...
function installSelfExtensionTools(
	ctx: SelfExtensionInstallerContext,
	api: ClawdbotPluginApi,
): void {
...
function selectApitapToolsContext({
	apitapStore,
	cfg,
}: ToolsContext): ApitapInstallerContext {
	return { apitapStore, cfg };
}

function installApitapTools(
	ctx: ApitapInstallerContext,
	api: ClawdbotPluginApi,
): void {
	if (ctx.apitapStore) {
...
function selectDashboardRoutesContext({
	cfg,
}: ToolsContext): DashboardRoutesContext {
	return { cfg };
}

function installDashboardRoutes(
	{ cfg }: DashboardRoutesContext,
	api: ClawdbotPluginApi,
): void {
...
function installPublicApiRoutes(
	ctx: PublicApiRoutesContext,
	api: ClawdbotPluginApi,
): void {
...
function selectGoalToolsContext(ctx: ToolsContext): GoalToolsContext {
	const workspaceRoot =
		...
		pathJoin(homedir(), ".openclaw", "workspace");
	const goalsDir = resolveGoalsDir(
		workspaceRoot,
		ctx.cfg.goalStewardship.goalsDir,
	);
	const resolvedActiveTaskPath = resolveActiveTaskPathForTools(
		ctx.cfg,
		workspaceRoot,
	);
	return {
