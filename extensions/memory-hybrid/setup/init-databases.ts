...
export function initializeDatabases(
  ...
  // Only initialize EventLog when at least one feature that writes to it is enabled
  // to avoid creating event-log.db for users who have no episodic features configured.
  const eventLogEnabled = cfg.nightlyCycle?.enabled || cfg.graph?.autoSupersede;
