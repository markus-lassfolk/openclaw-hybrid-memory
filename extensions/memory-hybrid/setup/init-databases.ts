 ...
 */
const ROUTABLE_BUILTIN_PROVIDERS = new Set([
	"google",
	"openai",
	"anthropic",
	"ollama",
...
function extractGatewayConfig(cfg: HybridMemoryConfig): {
	...
	const gatewayToken =
		gatewayAuthResolved ?? process.env.OPENCLAW_GATEWAY_TOKEN;
...
function inferFeatureLabel(
	...
	if (
		content.includes("memory classifier") ||
		content.includes("categorize each fact")
	)
	...
	if (
		content.includes("hypothetical document") ||
		content.includes("hypothetical answer")
	)
	...
	if (
		content.includes("positive-reinforcement analyst") ||
		content.includes("positive reinforcement analyst")
	)
	...
	if (content.includes("implicit") && content.includes("feedback"))
		return "implicit-feedback";
	if (content.includes("discover how this specific user expresses"))
		return "implicit-feedback";
	...
	if (
		content.includes("frustration") &&
		(content.includes("detect") || content.includes("analys"))
	)
		return "frustration-detection";

	// cross-agent-generalize.txt: "identify which of these lessons are general enough"
	if (
		content.includes("cross-agent") ||
		content.includes("lessons are general enough")
	)
		return "cross-agent-learning";

	// tool effectiveness
	if (
		content.includes("tool effectiveness") ||
		content.includes("tool scoring")
	)
	...
	if (content.includes("extracting facts, preferences, decisions"))
		return "distill";

	// language keywords
	if (content.includes("language") && content.includes("keyword"))
		return "language-keywords";

	// consolidate.txt: "You are a memory consolidator"
	if (
		content.includes("memory consolidator") ||
		content.includes("merge the following facts")
	)
	...
	if (content.includes("continuous") && content.includes("verification"))
		return "continuous-verification";
...
function buildMultiProviderOpenAI(
	...
	function hasApiKeyForProvider(prefix: string): boolean {
		const prov = cfg.llm?.providers as
		...
		if (prefix === "openai") {
			const k =
				resolveApiKey(prov?.openai?.apiKey) ??
				resolveApiKey(cfg.embedding?.apiKey);
			return Boolean(k && k.length >= 10);
		}
		if (prefix === "anthropic") {
			const claude = (cfg as Record<string, unknown>).claude as
				| { apiKey?: string }
				| undefined;
			const k =
				resolveApiKey(prov?.anthropic?.apiKey) ?? resolveApiKey(claude?.apiKey);
			return Boolean(k && k.length >= 10);
		}
	...
	function resolveClient(model: string): {
		...
		if (prefix === "google") {
			const apiKey =
				resolveApiKey(providerCfg?.apiKey ?? cfg.distill?.apiKey) ??
		...
		const envFallbackKey =
			process.env[`${prefix.toUpperCase()}_API_KEY`]?.trim();
		if (envFallbackKey) {
			const baseURL = providerCfg?.baseURL;
	...
	const isReasoningModel = (bare: string) =>
		/^o[0-9]+(?:-[a-z]+)?$/.test(bare.toLowerCase());
	const requiresMaxCompletionTokens = (bare: string) =>
		isReasoningModel(bare) || /^gpt-5/i.test(bare);
	...
	function remapMaxTokensForOpenAI(
		...
		let result = body;
		if (
			requiresMaxCompletionTokens(bareModel) &&
		...
		if (isReasoningModel(bareModel)) {
			...
			if (temperature !== undefined || top_p !== undefined) {
				api.logger.debug?.(
					`memory-hybrid: stripped temperature/top_p for reasoning model ${bareModel}`,
				);
			}
			result = rest;
		}
				...
				return {
					completions: {
						create(
							...
							if (costTracker) {
								const feature = inferFeatureLabel(
								...
								void (Promise.resolve(promise) as Promise<unknown>).then(
									...
									() => {
										try {
											...
											const reqMessages = Array.isArray(
												(body as unknown as Record<string, unknown>).messages,
											)
												? ((body as unknown as Record<string, unknown>)
														.messages as unknown[])
												: [];
											const estimatedInputTokens = Math.ceil(
												JSON.stringify(reqMessages).length / 4,
											);
											costTracker.record({
												feature,
												model: normalizedModel,
...
export function initializeDatabases(
	...
	if (
		...
		for (const [name, gw] of Object.entries(gwProviders)) {
			...
			const pluginHasKey =
				typeof prov[normalizedName]?.apiKey === "string" &&
	...
	if (
		...
		const agentModel = (api.config as Record<string, unknown>)?.agents as
			| Record<string, unknown>
			| undefined;
		const agentDefaults = agentModel?.defaults as
			| Record<string, unknown>
			| undefined;
		const modelCfg = agentDefaults?.model as
			| Record<string, unknown>
			| undefined;
		const primary =
			typeof modelCfg?.primary === "string" ? modelCfg.primary : undefined;
		...
		const pluginProviders = Object.fromEntries(
			Object.entries((cfg.llm?.providers ?? {}) as Record<string, unknown>).map(
				([k, v]) => [k.toLowerCase(), v],
			),
		);
		...
		const authOrder = cfg.auth?.order
			? Object.fromEntries(
					Object.entries(cfg.auth.order).map(([k, v]) => [k.toLowerCase(), v]),
				)
			: undefined;
		const canRoute = (m: string): boolean => {
			...
			if (
				...
				return true;
			if (
				ROUTABLE_BUILTIN_PROVIDERS.has(prefix) ||
				Object.hasOwn(pluginProviders, prefix)
			)
				return true;
		...
		if (gatewayModels.length > 0) {
			...
			const nano = uniqueModels.filter(
				(m) => isNanoModel(m) && !isHeavyModel(m),
			);
			const heavy = uniqueModels.filter(
				(m) => isHeavyModel(m) && !isNanoModel(m),
			);
			const light = uniqueModels.filter(
				(m) => isLightModel(m) && !isNanoModel(m) && !isHeavyModel(m),
			);
			const medium = uniqueModels.filter(
				(m) => !isNanoModel(m) && !isLightModel(m) && !isHeavyModel(m),
			);

			// default tier: agent order (primary then fallbacks) so reflection/general match what you set in openclaw.json
			const defaultIsHeavyOnly =
				uniqueModels.length > 0 && uniqueModels.every((m) => isHeavyModel(m));
			...
			cfg.llm = {
				...(cfg.llm?.localAutoStart !== undefined
					? { localAutoStart: cfg.llm.localAutoStart }
					: {}),
				...(cfg.llm?.providers !== undefined
					? { providers: cfg.llm.providers }
					: {}),
				...(cfg.llm?.fallbackToDefault !== undefined
					? { fallbackToDefault: cfg.llm.fallbackToDefault }
					: {}),
				...(cfg.llm?.fallbackModel !== undefined
	...
	const hasAnthropicModel = (list: string[]) =>
		list.some((m) => m.startsWith("anthropic/") || m.startsWith("claude-"));
	if (
		!prov.anthropic &&
		(hasAnthropicModel(defaultList) || hasAnthropicModel(heavyList))
	) {
		const envKey =
	...
	if (cfg.llm && mergedProviderNames.length > 0) {
		const defaultList = Array.isArray(cfg.llm.default)
			? [...cfg.llm.default]
			: [];
		const heavyList = Array.isArray(cfg.llm.heavy) ? [...cfg.llm.heavy] : [];
		...
		for (const name of mergedProviderNames) {
			if (hasModelFrom(defaultList, name) && hasModelFrom(heavyList, name))
				continue;
			...
			if (
				gwProviders &&
				typeof (gwProviders as Record<string, unknown>)[originalName] ===
					"object"
			) {
				const gw = (gwProviders as Record<string, unknown>)[
					originalName
				...
				const NON_CHAT_ID_RE =
					/\bembed|whisper|\btts\b|dall-e|transcri|gpt-image|image-gen/i;
				const isChatEntry = (entry: unknown): boolean => {
					...
					if (typeof entry === "string")
						return !NON_CHAT_ID_RE.test(entry.toLowerCase());
					return false;
				};
				...
				if (!defaultModel) {
					const gwModel =
						typeof gw.defaultModel === "string"
							? gw.defaultModel
							: typeof gw.model === "string"
								? gw.model
								: null;
					if (gwModel?.trim() && isChatEntry(gwModel))
						defaultModel = `${name}/${gwModel.trim()}`;
				}
			}
			...
			const heavyModel =
				name === "anthropic" ? "anthropic/claude-opus-4-6" : defaultModel;
	...
	if (cfg.llm?.localAutoStart) {
		...
		const hasOllamaModels = allModels.some(
			(m) => m.split("/")[0]?.toLowerCase() === "ollama",
		);
		if (hasOllamaModels) {
			void (async () => {
				try {
					const ollamaBase =
						(
							cfg.llm?.providers as
								...
								| undefined
						)?.["ollama"]?.baseURL?.replace(/\/v1\/?$/, "") ??
						OLLAMA_DEFAULT_BASE_URL;
					const running = await probeOllamaEndpoint(ollamaBase);
					if (!running) {
						api.logger.info(
							"memory-hybrid: Ollama is not running — attempting auto-start (llm.localAutoStart: true)",
						);
						...
						child.on("error", (err) => {
							api.logger.warn(
								`memory-hybrid: Ollama spawn error: ${err.message}`,
							);
						});
						child.unref();
	...
	if (cfg.wal.enabled) {
		const walPath =
			cfg.wal.walPath || join(dirname(resolvedSqlitePath), "memory.wal");
	...
	if (cfg.personaProposals.enabled) {
		...
		api.logger.info(
			`memory-hybrid: persona proposals enabled (${proposalsPath})`,
		);
	}
	...
	if (
		cfg.nightlyCycle.enabled ||
		cfg.graph?.autoSupersede ||
		cfg.passiveObserver.enabled
	) {
	...
	if (cfg.aliases?.enabled) {
		...
		api.logger.info(
			`memory-hybrid: retrieval aliases enabled (${aliasPath}, ${aliasLancePath})`,
		);
	}
	...
	const workflowStorePath = join(
		dirname(resolvedSqlitePath),
		"workflow-traces.db",
	);
	const workflowStore = new WorkflowStore(workflowStorePath);
	api.logger.info(
		`memory-hybrid: workflow store initialized (${workflowStorePath})`,
	);

	// Initialize CrystallizationStore — always created; proposals gated by cfg.crystallization.enabled (Issue #208)
	const crystallizationStorePath = join(
		dirname(resolvedSqlitePath),
		"crystallization-proposals.db",
	);
	const crystallizationStore = new CrystallizationStore(
		crystallizationStorePath,
	);
	api.logger.info(
		`memory-hybrid: crystallization store initialized (${crystallizationStorePath})`,
	);

	// Initialize ToolProposalStore — always created; proposals gated by cfg.selfExtension.enabled (Issue #210)
	const toolProposalStorePath = join(
		dirname(resolvedSqlitePath),
		"tool-proposals.db",
	);
	const toolProposalStore = new ToolProposalStore(toolProposalStorePath);
	api.logger.info(
		`memory-hybrid: tool proposal store initialized (${toolProposalStorePath})`,
	);
	...
	if (cfg.provenance.enabled) {
		...
		api.logger.info(
			`memory-hybrid: provenance tracing enabled (${provenancePath})`,
		);
	}
	...
	if (existsSync(discoveredPath)) {
		try {
			...
			if (Array.isArray(loaded) && loaded.length > 0) {
				...
				api.logger.info(
					`memory-hybrid: loaded ${loaded.length} discovered categories`,
				);
			}
		} catch (err) {
			api.logger.warn(
				`memory-hybrid: failed to load discovered categories: ${err}`,
			);
	...
	try {
		...
		if (
			!cfg.embedding.autoMigrate &&
			(!previousEmbeddingMeta || embeddingConfigChanged)
		) {
			factsDb.setEmbeddingMeta(
				currentEmbeddingMeta.provider,
				currentEmbeddingMeta.model,
			);
		} else if (
			cfg.embedding.autoMigrate &&
			!previousEmbeddingMeta &&
		...
		api.logger.warn(
			`memory-hybrid: failed to read embedding metadata (non-fatal): ${err}`,
		);
	}
	...
	const initialized = (async () => {
		try {
			...
			const effectiveProvider =
				embeddings.activeProvider ?? cfg.embedding.provider;
			api.logger.info(
		...
		if (cfg.credentials.enabled && credentialsDb) {
			...
			try {
				...
				if (err.code === "EEXIST") {
					...
					api.logger.warn(
						`memory-hybrid: failed to create migration flag (skipping migration): ${err}`,
					);
				}
			}
			if (shouldMigrate) {
				try {
					...
					if (result.migrated > 0) {
						api.logger.info(
							`memory-hybrid: migrated ${result.migrated} credential(s) from memory into vault`,
						);
					}
		...
		api.logger.warn(
			`memory-hybrid: async initialization encountered an error: ${err}`,
		);
	});
	...
	if (cfg.embedding.autoMigrate && embeddingConfigChanged) {
		void (async () => {
			try {
				...
				api.logger.warn(
					`memory-hybrid: autoMigrate embedding run failed: ${err}`,
				);
			}
		})();
	}
	...
	if (
		(cfg.vector.autoRepair || embeddingConfigChanged) &&
		!(cfg.embedding.autoMigrate && embeddingConfigChanged)
	) {
		void (async () => {
			const reembedProgressPath = join(
				dirname(resolvedSqlitePath),
				".reembed-progress.json",
			);
			try {
				...
				let needsReembedding =
					embeddingConfigChanged ||
					(cfg.vector.autoRepair && vectorDb.wasRepaired);
				let completedIds = new Set<string>();

				if (!needsReembedding && existsSync(reembedProgressPath)) {
					try {
						const progress = JSON.parse(
							readFileSync(reembedProgressPath, "utf-8"),
						) as {
							completedIds: string[];
							total: number;
						};
...
function resolveEmbeddingRegistryModels(
	...
	if (
		Array.isArray(embedding.multiModels) &&
		embedding.multiModels.length > 0
	) {
