...
interface DatabaseContext {
	...
	identityReflectionStore:
		| import("../backends/identity-reflection-store.js").IdentityReflectionStore
		| null;
	personaStateStore:
		| import("../backends/persona-state-store.js").PersonaStateStore
		| null;
...
export function initializeDatabases(
	...
	const { factsDb, edictStore, vectorDb, embeddings, embeddingRegistry } =
		installCoreBootstrapServices({
			cfg,
			api,
			resolvedSqlitePath,
	...
	if (
		...
		const envKey =
			normalizeResolvedSecretValue(getEnv("ANTHROPIC_API_KEY")) ?? "";
		if (envKey.length >= 10) {
	...
	api.logger.info(
		`memory-hybrid: narratives store initialized (${narrativesPath})`,
	);
	...
	const initialized = (async () => {
		...
		try {
			...
			const logEmbFailure =
				quota403 || is429OrWrapped(asErr) ? api.logger.warn : api.logger.error;
			const embTag =
				quota403 || is429OrWrapped(asErr)
					? "[embedding-quota]"
					: "[embedding-init]";
			logEmbFailure(
