...
export function registerLifecycleHooks(
	...
	api.on("llm_input", (ev: unknown) => {
		...
		if (!event?.historyMessages || !Array.isArray(event.historyMessages))
			return;
		let sanitized = sanitizeMessagesForClaude(
			event.historyMessages as MessageLike[],
		);
	...
	try {
		api.on("before_compaction", async (event: unknown) => {
			...
			try {
				...
				api.logger.debug?.(
					`memory-hybrid: failed to read AGENTS.md for pre-compaction: ${err}`,
				);
			}
			...
			try {
				...
				const pinnedRecallThreshold =
					ctx.cfg.autoRecall?.progressivePinnedRecallCount ?? 3;

				const pinnedFacts = hotFacts.filter(
					(x) =>
						x.entry.decayClass === "permanent" ||
						(x.entry.recallCount ?? 0) >= pinnedRecallThreshold,
				);
				...
				api.logger.debug?.(
					`memory-hybrid: failed to fetch pinned facts for pre-compaction: ${err}`,
				);
			}
		...
		api.logger.debug?.(
			`memory-hybrid: before_compaction hook not available (${err})`,
		);
	}
	...
	try {
		api.on(
			...
			async (
				event: unknown,
			): Promise<undefined | { prependContext: string }> => {
				const ev = event as {
					messageCount?: number;
					tokenCount?: number;
				...
				try {
					...
					api.logger.debug?.(
						`memory-hybrid: after_compaction — SQLite health OK, ${factCount} facts in store`,
					);
				} catch (dbErr) {
					api.logger.warn?.(
					...
					if (ctx.cfg.autoRecall.enabled) {
						...
						if (
							...
							if (recallBlock) {
								...
								api.logger.debug?.(
									"memory-hybrid: after_compaction — injecting post-compaction recall snippet (#957)",
								);
							}
						}
					}
						...
						try {
							...
							if (openIssues.length > 0) {
								...
								for (const issue of openIssues) {
									summaryLines.push(
										`- [${issue.severity}] ${issue.title} (${issue.status})`,
									);
								}
							}
						} catch {
		...
		api.logger.debug?.(
			`memory-hybrid: after_compaction hook not available (${err})`,
		);
	}
