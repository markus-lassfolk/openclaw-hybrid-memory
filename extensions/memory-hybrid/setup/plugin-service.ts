	...
	return {
		...
		start: async () => {
					...
					if (spawnError) {
						...
						api.logger.warn(
							`memory-hybrid: documents.enabled but required Python package(s) not installed: ${pkgs}. ` +
								`Run: ${cfg.documents.pythonPath} -m pip install ${missing.join(
									" ",
								)}  (see extensions/memory-hybrid/scripts/requirements.txt)`,
						);
					}
				}
			}
			...
			const versionCheckCachePath =
				resolvedSqlitePath === ":memory:"
					? null
					: join(dirname(resolvedSqlitePath), ".latest-plugin-version.json");
			const errorReportingActive =
				cfg.errorReporting.enabled && cfg.errorReporting.consent;
			let cachedVersionCheck = versionCheckCachePath
				? readVersionCheckCache(versionCheckCachePath)
				: null;
			api.logger.info(
			...
			try {
				...
				if (skillOutcome.deployed) {
					api.logger.info(
						`memory-hybrid: deployed bundled Agent Skill to workspace: ${skillOutcome.path}`,
					);
			...
			if (
				...
				isPluginOutdated(
					...
					cachedVersionCheck.latestVersion,
				) &&
				isVersionCheckCacheFresh(
					cachedVersionCheck,
					cfg.errorReporting.updateNudge.cacheTtlHours,
				)
			) {
				setErrorReporterMuted(
					true,
					`outdated-plugin:${cachedVersionCheck.latestVersion}`,
				);
				...
				if (
					...
					nextCachedVersionCheck.lastNudgedAt !==
						cachedVersionCheck.lastNudgedAt
				) {
					try {
						writeVersionCheckCache(
							versionCheckCachePath,
							nextCachedVersionCheck,
						);
					} catch (err) {
						api.logger.debug?.(
							`memory-hybrid: failed to update nudge cache: ${
			...
			try {
				...
				api.logger.warn(
					`memory-hybrid: error reporter initialization failed: ${err}`,
				);
				capturePluginError(
					err instanceof Error ? err : new Error(String(err)),
					{
			...
			versionCheckPromise = (async () => {
				...
				try {
					...
					if (
						...
						setErrorReporterMuted(
							true,
							`outdated-plugin:${latestPublished.latestVersion}`,
						);
					...
					if (versionCheckCachePath) {
						try {
							...
							api.logger.debug?.(
								`memory-hybrid: failed to write version check cache: ${
									err instanceof Error ? err.message : String(err)
								}`,
							);
						}
					...
					api.logger.debug?.(
						`memory-hybrid: latest-version check failed: ${
							err instanceof Error ? err.message : String(err)
						}`,
					);
					if (
			...
			if (wal) {
				...
				if (pendingEntries.length > 0) {
					api.logger.info(
						`memory-hybrid: WAL recovery starting — found ${pendingEntries.length} pending operation(s)`,
					);
					let recovered = 0;
					let failed = 0;
					...
					for (const entry of pendingEntries) {
						try {
							...
							api.logger.warn(
								`memory-hybrid: WAL recovery failed for entry ${entry.id}: ${err}`,
							);
							capturePluginError(
								err instanceof Error ? err : new Error(String(err)),
								{
					...
					try {
						...
						if (pruned > 0) {
							api.logger.info(
								`memory-hybrid: WAL pruned ${pruned} stale entries`,
							);
						}
					} catch (err) {
					...
					try {
						...
						if (compacted > 0) {
							api.logger.info(
								`memory-hybrid: WAL compacted ${compacted} stale entries (oversized file, issue #903)`,
							);
						}
					} catch (err) {
						api.logger.warn(
							`memory-hybrid: WAL compactIfOversized failed: ${err}`,
						);
						capturePluginError(
							err instanceof Error ? err : new Error(String(err)),
							{
			...
			try {
				...
				api.logger.warn?.(
					`memory-hybrid: cron guard sync failed (non-fatal): ${err}`,
				);
			}
			...
			if (cfg.dashboard.enabled) {
				try {
					...
					api.logger.info(
						`memory-hybrid: dashboard started on http://127.0.0.1:${dashboardServer.port}`,
					);
				} catch (err) {
					api.logger.warn(
						`memory-hybrid: dashboard server failed to start (non-fatal): ${err}`,
					);
					capturePluginError(
						err instanceof Error ? err : new Error(String(err)),
						{
					...
					if (
						...
						api.logger.info(
							`memory-hybrid: periodic prune — ${hardPruned} expired, ${softPruned} decayed, ${edictsPruned} edicts pruned${
								auditPruned > 0 ? `, ${auditPruned} audit rows` : ""
							}${
								healthPruned > 0 ? `, ${healthPruned} agent-health rows` : ""
							}`,
						);
					}
				} catch (err) {
			...
			if (cfg.autoClassify.enabled) {
				...
				timers.classifyStartupTimeout.value = setTimeout(async () => {
					try {
						...
						api.logger.warn(
							`memory-hybrid: startup auto-classify failed: ${err}`,
						);
						capturePluginError(
							err instanceof Error ? err : new Error(String(err)),
							{
				...
				timers.classifyTimer.value = setInterval(async () => {
					try {
						...
						api.logger.warn(
							`memory-hybrid: daily auto-classify failed: ${err}`,
						);
						capturePluginError(
							err instanceof Error ? err : new Error(String(err)),
							{
			...
			if (cfg.languageKeywords.autoBuild) {
					...
					try {
						...
						const result = await runBuildLanguageKeywords(
							...
							dirname(resolvedSqlitePath),
							{
								model:
									cfg.autoClassify.model ??
									getDefaultCronModel(getCronModelConfig(cfg), "default"),
								dryRun: false,
							},
						);
						if (result.ok && result.languagesAdded > 0) {
							api.logger.info(
								`memory-hybrid: language keywords updated (${result.topLanguages.join(
									", ",
								)}, +${result.languagesAdded} languages)`,
							);
						} else if (result.ok) {
							api.logger.info(
								`memory-hybrid: language keywords build done (${result.topLanguages.join(
									", ",
								)})`,
							);
						} else {
							api.logger.warn(
								`memory-hybrid: language keywords build failed: ${result.error}`,
							);
						}
					} catch (err) {
						api.logger.warn(
							`memory-hybrid: language keywords build failed: ${err}`,
						);
						capturePluginError(
							err instanceof Error ? err : new Error(String(err)),
							{
				...
				if (langFilePath && !existsSync(langFilePath)) {
					api.logger.info(
						"memory-hybrid: no language keywords file; building from memory samples in 3s…",
					);
				...
				const weeklyMs =
					cfg.languageKeywords.weeklyIntervalDays * 24 * 60 * 60 * 1000;
			...
			if (cfg.passiveObserver.enabled) {
				...
				const runObserver = async () => {
					try {
						...
						if (
							result.factsStored > 0 ||
							result.factsExtracted > 0 ||
							result.factsReinforced > 0
						) {
							api.logger.info(
						...
						api.logger.warn(
							`memory-hybrid: passive-observer run failed: ${err}`,
						);
						capturePluginError(
							err instanceof Error ? err : new Error(String(err)),
							{
			...
			const watchdogRun = async () => {
				try {
					await runTaskQueueWatchdog(
						{ repoDir: getEnv("OPENCLAW_WORKSPACE") ?? process.cwd() },
						api.logger,
					);
				} catch (err) {
					api.logger.warn?.(
						`memory-hybrid: task-queue-watchdog failed (non-fatal): ${err}`,
					);
					capturePluginError(
						err instanceof Error ? err : new Error(String(err)),
						{
				...
				if (cfg.activeTask.enabled) {
					try {
						...
						if (cfg.activeTask.ledger === "facts") {
							const r = await reconcileActiveTaskInProgressSessionsFacts(
								factsDb,
								vectorDb,
								embeddings,
								staleMinutes,
							...
							const r = await reconcileActiveTaskInProgressSessions(
								activeTaskFilePath,
								staleMinutes,
								{
									flushOnComplete: cfg.activeTask.flushOnComplete !== false,
						...
						api.logger.warn?.(
							`memory-hybrid: active-task session reconcile failed (non-fatal): ${reconcileErr}`,
						);
						capturePluginError(
				...
				if (
					cfg.goalStewardship.enabled &&
					cfg.goalStewardship.watchdogHealthCheck
				) {
					try {
						const workspaceRootGs =
							getEnv("OPENCLAW_WORKSPACE") ??
							join(homedir(), ".openclaw", "workspace");
						const goalsDir = resolveGoalsDir(
							workspaceRootGs,
							cfg.goalStewardship.goalsDir,
						);
						...
						api.logger.warn?.(
							`memory-hybrid: goal health check failed (non-fatal): ${ghErr}`,
						);
						capturePluginError(
							ghErr instanceof Error ? ghErr : new Error(String(ghErr)),
							{
								subsystem: "plugin-service",
					...
					try {
						...
						const runCli = async (args: string[]): Promise<boolean> => {
							return new Promise((resolve) => {
								...
								child.on("error", (err) => {
									api.logger.warn?.(
										`memory-hybrid: post-upgrade ${args[0]} error: ${err.message}`,
									);
									resolve(false);
								});
							});
						...
						if (langPath && !existsSync(langPath))
							await runCli(["build-languages"]);
						await runCli(["self-correction-run"]);
						if (cfg.reflection.enabled) {
							await runCli([
								"reflect",
								"--window",
								String(cfg.reflection.defaultWindow),
							]);
						...
						api.logger.warn?.(
							`memory-hybrid: post-upgrade pipeline error: ${e}`,
						);
		...
		stop: async () => {
			...
			if (observerRunPromise) {
				...
				if (!completed) {
					api.logger.warn(
						"memory-hybrid: passive-observer shutdown timed out; closing databases anyway",
					);
				}
			}
			if (watchdogRunPromise) {
				...
				if (!completed) {
					api.logger.warn(
						"memory-hybrid: task-queue-watchdog shutdown timed out; continuing shutdown anyway",
					);
				}
			}
			if (versionCheckPromise) {
				...
				if (!completed) {
					api.logger.warn(
						"memory-hybrid: version-check shutdown timed out; continuing shutdown anyway",
					);
				}
			}
			if (ctx.pythonBridge) {
