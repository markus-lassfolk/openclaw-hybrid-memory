...
const fullDefaults = {
		...
		defaults: {
			...
			compaction: {
				...
				memoryFlush: {
					...
					// NOTE: OpenClaw core consumes this (agents.compaction.memoryFlush.flushEveryCompaction).
	...
	jobs: [
		...
		},
		{
			...
			channel: "system",
			message:
				"Run memory reflection: analyze facts from the last 14 days, extract behavioral patterns, store as pattern-category facts. Use memory_reflect tool.",
			isolated: true,
			model: "gemini",
		},
		{
			...
			channel: "system",
			message:
				"Run nightly self-correction analysis: openclaw hybrid-mem self-correction-run. Uses last 3 days of sessions; multi-language correction detection from .language-keywords.json (run build-languages first for non-English). Report: workspace memory/reports/self-correction-YYYY-MM-DD.md.",
			isolated: true,
			model: "sonnet",
		},
	],
...
console.log(
	"Applied: memory-hybrid slot, full plugin config (all features), memorySearch, compaction prompts, bootstrap limits, pruning, autoClassify, nightly-memory-sweep job.",
);
console.log("\nNext steps:");
console.log(
	'  1. Set plugins.entries["memory-hybrid"].config.embedding.apiKey to your OpenAI key (replace YOUR_OPENAI_API_KEY).',
);
console.log(
	"  2. Copy extensions/memory-hybrid/ to your OpenClaw extensions dir and run npm install there.",
);
console.log(
	"  3. Start the gateway, then run: openclaw hybrid-mem verify [--fix]",
);
